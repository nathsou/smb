...
void InitScreen(void) {
  ...
  lda_abs(OperMode);
  if (zero_flag) { NextSubtask(); return; } // if mode still 0, do not load
...
void DisplayIntermediate(void) {
  ...
  if (zero_flag) { NoInter(); return; } // if in title screen mode, skip this
  cmp_imm(GameOverModeValue); // are we in game over mode?
  if (zero_flag) { GameOverInter(); return; } // if so, proceed to display game over screen
  lda_abs(AltEntranceControl); // otherwise check for mode of alternate entry
  if (!zero_flag) { NoInter(); return; } // and branch if found
  ...
  if (!zero_flag) { NoInter(); return; } // and jump to specific task, otherwise
...
void DrawTitleScreen(void) {
  ...
  if (!zero_flag) { IncModeTask_B(); return; } // if not, exit
...
void ClearBuffersDrawIcon(void) {
  ...
  if (!zero_flag) { IncModeTask_B(); return; } // if not title screen mode, leave
...
void RunGameOver(void) {
  ...
  if (!zero_flag) { TerminateGame(); return; }
  lda_abs(ScreenTimer); // if not pressed, wait for
  if (!zero_flag) { return; } // screen timer to expire
  TerminateGame();
}
...
void TerminateGame(void) {
  ...
  if (!carry_flag) { ContinueGame(); return; } // going, and do so if possible
...
IconDataRead:
    ...
    if (zero_flag) { return; } // if set to 1-player game, we're done
...
void BridgeCollapse(void) {
  ...
  if (zero_flag) { RemoveBridge(); return; }
  ...
  if (!carry_flag) { MoveD_Bowser(); return; }
...
void UpdateScreen(void) {
  ...
  lda_indy(0x0);
  if (!zero_flag) { WriteBufferToScreen(); return; } // if byte is zero we have no further updates to make here
  InitScroll();
}
...
GetScoreDiff:
    ...
    if (!carry_flag) { return; } // check to see if borrow is still set, if so, no new high score
...
DoAPTasks:
    ...
    if (!zero_flag) { return; } // render attribute table yet
...
void IntroPipe(void) {
  ...
  RenderSidewaysPipe();
  if (carry_flag) { return; } // if carry flag set, not time to draw vertical pipe part
...
EmptyChkLoop:
    ...
    if (zero_flag) { return; } // if zero, leave
    inx();
...
void ChkLrgObjFixedLength(void) {
  ...
  if (!neg_flag) { return; } // if counter not set, load it, otherwise leave alone
...
void VerticalPipeEntry(void) {
  ...
  if (!zero_flag) { ChgAreaPipe(); return; } // if set, branch to use mode 0
  iny();
  lda_abs(AreaType); // check for castle level type
  cmp_imm(0x3);
  if (!zero_flag) { ChgAreaPipe(); return; } // if not castle type level, use mode 1
  iny();
...
void PlayerDeath(void) {
  lda_abs(TimerControl); // check master timer control
  cmp_imm(0xf0); // for specific moment in time
  if (carry_flag) { return; } // branch to leave if before that point
  PlayerCtrlRoutine(); return; // otherwise run player control routine
}
...
void PlayerFireFlower(void) {
  ...
  if (zero_flag) { ResetPalFireFlower(); return; } // branch if at moment, not before or after
  lda_zp(FrameCounter); // get frame counter
  lsr_acc();
...
RdyNextA:
    ...
    if (!zero_flag) { return; } // beyond last valid task number, branch to leave
    inc_abs(LevelNumber); // increment level number used for game logic
    lda_abs(LevelNumber);
    cmp_imm(0x3); // check to see if we have yet reached level -4
    if (!zero_flag) { NextArea(); return; } // and skip this last part here if not
    ...
    if (!carry_flag) { NextArea(); return; } // at least this number of coins, leave flag clear
...
void BubbleCheck(void) {
  ...
  if (!zero_flag) { MoveBubl(); return; } // branch to move air bubble
...
void ProcessCannons(void) {
  lda_abs(AreaType); // get area type
  if (zero_flag) { return; } // if water type area, branch to leave
  ldx_imm(0x2);
...
void BumpBlock(void) {
  ...
  BlockBumpedChk(); // do a sub to check which block player bumped head on
  if (!carry_flag) { return; } // if no match was found, branch to leave
...
BumpChkLoop:
    ...
    if (zero_flag) { return; } // metatile found in block buffer, branch if so
...
void MoveD_EnemyVertically(void) {
  ...
  if (!zero_flag) { ContVMove(); return; } // and use, otherwise set different movement amount, continue on
...
void MoveDropPlatform(void) {
  ...
  if (!zero_flag) { SetMdMax(); return; } // skip ahead of other value set here
...
void SmallBBox(void) {
  ...
  if (!zero_flag) { SetBBox(); return; } // unconditional branch
  // --------------------------------
...
void InitLakitu(void) {
  ...
  if (!zero_flag) { KillLakitu(); return; } // the frenzy buffer, and branch to kill lakitu if so
...
void InitBowserFlame(void) {
  ...
  cmp_imm(Bowser);
  if (zero_flag) { SpawnFromMouth(); return; } // branch if found
...
void RunBowser(void) {
  ...
  if (zero_flag) { BowserControl(); return; }
  ...
  if (!carry_flag) { MoveD_Bowser(); return; } // otherwise proceed to KillAllEnemies
...
void RunStarFlagObj(void) {
  ...
  if (carry_flag) { StarFlagExit(); return; }
  switch (a) {
...
void AwardGameTimerPoints(void) {
  ...
  ora_abs(GameTimerDisplay + 2);
  if (zero_flag) { IncrementSFTask1(); return; } // if no time left on game timer at all, branch to next task
  lda_zp(FrameCounter);
...
SetoffF:
    ...
    if (zero_flag) { DrawFlagSetTimer(); return; } // if no fireworks left to go off, skip this part
    if (neg_flag) { DrawFlagSetTimer(); return; } // if no fireworks set to go off, skip this part
    lda_imm(Fireworks);
...
void DelayToAreaEnd(void) {
  ...
  if (!zero_flag) { return; } // not yet expired, branch to leave
  lda_abs(EventMusicBuffer); // if event music buffer empty,
  if (zero_flag) { IncrementSFTask2(); return; } // branch to increment task
  // --------------------------------
...
ChkJH:
    ...
    if (!zero_flag) { MoveHammerBroXDir(); return; } // if hammer bro a little offscreen, skip to movement code
...
HammerBroJumpCode:
    ...
    if (zero_flag) { MoveHammerBroXDir(); return; } // if set, branch ahead to moving code
    ...
    if (neg_flag) { SetHJ(); return; } // if on the bottom half of the screen, use current speed
    ...
    if (!carry_flag) { SetHJ(); return; } // if above the middle of the screen, use current speed and $01
    dec_zp(0x0); // otherwise return value to $00
    lda_absx(PseudoRandomBitReg + 1); // get part of LSFR, mask out all but LSB
    and_imm(0x1);
    if (!zero_flag) { SetHJ(); return; } // if d0 of LSFR set, branch and use current speed and $00
...
void MoveFlyGreenPTroopa(void) {
  ...
  if (!zero_flag) { return; } // branch to leave if set to move up/down every fourth frame
  lda_zp(FrameCounter);
...
void XMovingPlatform(void) {
  ...
  if (neg_flag) { return; } // branch ahead to leave
...
void DropPlatform(void) {
  ...
  if (neg_flag) { return; } // occurred, just leave without moving anything
...
void RightPlatform(void) {
  ...
  if (neg_flag) { return; } // and platform, branch ahead, leave speed unaltered
  lda_imm(0x10);
...
void ProcessBowserHalf(void) {
  ...
  lda_zpx(Enemy_State);
  if (!zero_flag) { return; } // if either enemy object not in normal state, branch to leave
  lda_imm(0xa);
...
GetHRp:
    ...
    if (!carry_flag) { return; } // bottom of the screen, we're done, branch to leave
    lda_zp(0x0);
...
void MoveLiftPlatforms(void) {
  ...
  if (!zero_flag) { return; } // and branch to leave
  lda_absx(Enemy_YMF_Dummy);
...
void ChkSmallPlatCollision(void) {
  ...
  if (zero_flag) { return; } // if none found, leave player position alone
...
GoombaPoints:
    ...
    if (!zero_flag) { EnemySmackScore(); return; } // branch if not found
...
void InjurePlayer(void) {
  ...
  if (!zero_flag) { ExInjColRoutines(); return; } // at zero, and branch to leave if so
  ForceInjury(); // <fallthrough>
}

void ForceInjury(void) {
  ldx_abs(PlayerStatus); // check player's status
  if (zero_flag) { KillPlayer(); return; } // branch if small
...
void KillPlayer(void) {
  ...
  if (!zero_flag) { SetKRout(); return; } // branch to set player's state and other things
...
void ChkEnemyFaceRight(void) {
  ...
  cmp_imm(0x1);
  if (!zero_flag) { LInj(); return; } // if not, branch
...
void ECLoop(void) {
  ...
  if (zero_flag) { ReadyNextEnemy(); return; } // branch if flag not set
  lda_zpx(Enemy_ID);
  cmp_imm(0x15); // check for enemy object => $15
  if (carry_flag) { ReadyNextEnemy(); return; } // branch if true
  cmp_imm(Lakitu);
  if (zero_flag) { ReadyNextEnemy(); return; } // branch if enemy object is lakitu
  cmp_imm(PiranhaPlant);
  if (zero_flag) { ReadyNextEnemy(); return; } // branch if enemy object is piranha plant
  lda_absx(EnemyOffscrBitsMasked);
  if (!zero_flag) { ReadyNextEnemy(); return; } // branch if masked offscreen bits set
  ...
  if (!zero_flag) { ReadyNextEnemy(); return; } // already set, and move onto next enemy slot if set
...
void ReadyNextEnemy(void) {
  ...
  dex();
  if (!neg_flag) { ECLoop(); return; } // loop until all enemy slots have been checked
...
void LargePlatformCollision(void) {
  ...
  lda_abs(TimerControl); // check master timer control
  if (!zero_flag) { ExLPC(); return; } // if set, branch to leave
  lda_zpx(Enemy_State); // if d7 set in object state,
  if (neg_flag) { ExLPC(); return; } // branch to leave
  lda_zpx(Enemy_ID);
  cmp_imm(0x24); // check enemy object identifier for
  if (!zero_flag) { ChkForPlayerC_LargeP(); return; } // balance platform, branch if not found
  lda_zpx(Enemy_State);
...
void ChkForPlayerC_LargeP(void) {
  ...
  if (carry_flag) { ExLPC(); return; } // or offscreen, branch to leave if true
  txa();
  ...
  tax();
  if (!carry_flag) { ExLPC(); return; } // if no collision, branch to leave
...
void ChkInvisibleMTiles(void) {
  ...
  if (zero_flag) { return; } // branch to leave if found
...
void ChkForLandJumpSpring(void) {
  ...
  if (!carry_flag) { return; } // if carry not set, jumpspring not found, therefore leave
  lda_imm(0x70);
...
void ChkToStunEnemies(void) {
  ...
  if (!carry_flag) { SetStun(); return; }
  cmp_imm(0x11); // if the enemy object identifier is equal to the values
  if (carry_flag) { SetStun(); return; } // $09, $0e, $0f or $10, it will be modified, and not
  ...
  if (!carry_flag) { SetStun(); return; } // are only necessary if branching from $d7a1
...
void HammerBroBGColl(void) {
  ...
  if (zero_flag) { NoUnderHammerBro(); return; }
  cmp_imm(0x23); // check for blank metatile $23 and branch if not found
  if (!zero_flag) { UnderHammerBro(); return; }
...
void UnderHammerBro(void) {
  ...
  if (!zero_flag) { NoUnderHammerBro(); return; } // branch if not expired
  lda_zpx(Enemy_State);
...
void GetFireballBoundBox(void) {
  ...
  if (!zero_flag) { FBallB(); return; } // unconditional branch
...
CMBits:
    ...
    if (!zero_flag) { MoveBoundBoxOffscreen(); return; } // if anything set here, branch
...
void LargePlatformBoundBox(void) {
  ...
  if (carry_flag) { MoveBoundBoxOffscreen(); return; } // box offscreen, otherwise start getting coordinates
...
ChkFlagOffscreen:
    ...
    if (zero_flag) { return; } // if none of these bits set, branch to leave
    // -------------------------------------------------------------------------------------
...
void JCoinGfxHandler(void) {
  ...
  if (carry_flag) { DrawFloateyNumber_Coin(); return; } // branch to draw floatey number
...
void ChkLeftCo(void) {
  ...
  if (zero_flag) { return; } // if not set, branch, otherwise move sprites offscreen
...
void DrawExplosion_Fireball(void) {
  ...
  if (carry_flag) { KillFireBall(); return; } // branch if so, otherwise continue to draw explosion
...
void DrawPlayerLoop(void) {
  ...
  if (!zero_flag) { DrawPlayerLoop(); return; } // do this until all rows are drawn
}
...
void GetGfxOffsetAdder(void) {
  lda_abs(PlayerSize); // get player's size
  if (zero_flag) { return; } // if player big, use current offset as-is
  tya(); // for big player
...
GorSLog:
    lda_abs(PlayerSize); // get player's size
    if (!zero_flag) { ShrinkPlayer(); return; } // if player small, skip ahead to next part
...
YLdBData:
    ...
    ldx_zp(0x4); // reobtain position in buffer
    cmp_imm(0x0);
    if (!zero_flag) { return; } // if bits not zero, branch to leave
...
void DividePDiff(void) {
  ...
  if (carry_flag) { return; } // if pixel difference >= preset value, branch
...
void Dump_Freq_Regs(void) {
  ...
  if (zero_flag) { return; } // if zero, then do not load
...
void SetFreq_Squ2(void) {
  ...
  if (!zero_flag) { Dump_Freq_Regs(); return; } // unconditional branch
...
void ContinueSndJump(void) {
  ...
  ldy_imm(0xf6);
  if (!zero_flag) { DmpJpFPS(); return; } // unconditional branch
  
N2Prt:
    cmp_imm(0x20); // check for third part
    if (!zero_flag) { DecJpFPS(); return; }
...
void DmpJpFPS(void) {
  Dump_Squ1_Regs();
  if (!zero_flag) { DecJpFPS(); return; } // unconditional branch outta here
...
void PlayFireballThrow(void) {
  ...
  if (!zero_flag) { Fthrow(); return; } // unconditional branch
...
void DecJpFPS(void) {
  if (!zero_flag) { BranchToDecLength1(); return; } // unconditional branch
...
void BranchToDecLength1(void) {
  if (!zero_flag) { DecrementSfx1Length(); return; } // unconditional branch (regardless of how we got here)
...
void PlaySmackEnemy(void) {
  ...
  PlaySqu1Sfx();
  if (!zero_flag) { DecrementSfx1Length(); return; } // unconditional branch
...
void PlayCoinGrab(void) {
  ...
  if (!zero_flag) { CGrab_TTickRegL(); return; }
...
N2Tone:
    if (!zero_flag) { DecrementSfx2Length(); return; }
    PlayBlast();
}
...
void SBlasJ(void) {
  if (!zero_flag) { BlstSJp(); return; } // unconditional branch to load rest of reg contents
...
void ContinuePowerUpGrab(void) {
  ...
  lsr_acc(); // divide by 2
  if (carry_flag) { DecrementSfx2Length(); return; } // alter frequency every other frame
  tay();
...
void EL_LRegs(void) {
  if (!zero_flag) { LoadSqu2Regs(); return; } // this is an unconditional branch outta here
...
DivLLoop:
    lsr_acc();
    if (carry_flag) { JumpToDecLength2(); return; } // if any bits set here, branch to dec the length
    dex();
...
void ContinueBrickShatter(void) {
  ...
  if (!carry_flag) { DecrementSfx3Length(); return; }
  tay();
...
NoStopSfx:
    ...
    if (!zero_flag) { FindEventMusicHeader(); return; }
    ldx_imm(0x8); // load offset to be added to length byte of header
    write_byte(NoteLengthTblAdder, x);
    if (!zero_flag) { FindEventMusicHeader(); return; } // unconditional branch
...
void HandleAreaMusicLoopB(void) {
  ...
  cpy_imm(0x32);
  if (!zero_flag) { LoadHeader(); return; } // branch ahead with alternate offset
  ldy_imm(0x11);
  if (!zero_flag) { GMLoopB(); return; } // unconditional branch
...
void FindEventMusicHeader(void) {
  ...
  if (!carry_flag) { FindEventMusicHeader(); return; }
