...
ColdBoot:
    jsr(InitializeMemory, 0); // clear memory using pointer in Y
    ...
    jsr(WritePPUReg1, 3);
    return; //  EndlessLoop: jmp EndlessLoop ; endless loop, need I say more?
    // -------------------------------------------------------------------------------------
...
RunDemo:
    jsr(GameCoreRoutine, 16); // run game engine
...
AutoPlayer:
    jsr(RelativePlayerPosition, 21); // get player's relative coordinates
...
PlayerInter:
    jsr(DrawPlayer_Intermediate, 34); // put player in appropriate place for
...
TaskLoop:
    jsr(AreaParserTaskHandler, 38); // render column set of current area
...
WriteBlankMT:
    jsr(PutBlockMetatile, 42); // do a sub to write blank metatile to vram buffer
    lda_imm(0x6);
...
RendBBuf:
    jsr(ProcessAreaData, 69); // do the area data loading routine now
    lda_abs(BlockBufferColumnPos);
...
RdyDecode:
    jsr(DecodeAreaData, 71); // do sub and do not turn on flag
    goto ChkLength;
...
NextAObj:
    jsr(IncAreaObjOffset, 72); // increment buffer offset and move on
...
ExitEmptyChk:
    goto rts; // if all values nonzero, carry flag is set
    // --------------------------------
...
ClrPlrPal:
    jsr(ResetPalStar, 143); // do sub to clear player's palette bits in attributes
...
RunParser:
    jsr(AreaParserTaskHandler, 144); // update the name table with more level graphics
...
IntroEntr:
    jsr(EnterSidePipe, 147); // execute sub to move player to the right
...
SizeChk:
    jsr(PlayerMovementSubs, 150); // run movement subroutines
...
PlayerSubs:
    jsr(ScrollHandler, 151); // move the screen if necessary
...
ProcMove:
    jsr(PlayerPhysicsSub, 166); // run sub related to jumping and swimming
...
GndMove:
    jsr(ImposeFriction, 168); // do a sub to impose friction on player's walk/run
...
JSMove:
    jsr(MovePlayerHorizontally, 172); // do a sub to move player horizontally
...
FPGfx:
    jsr(GetEnemyOffscreenBits, 190); // get offscreen information
...
DrawJSpr:
    jsr(RelativeEnemyPosition, 194); // get jumpspring's relative coordinates
...
BBFly:
    jsr(MoveEnemyHorizontally, 207); // do sub to move bullet bill horizontally
  
RunBBSubs:
    jsr(GetEnemyOffscreenBits, 208); // get offscreen information
...
KillBB:
    jsr(EraseEnemyObject, 212); // kill bullet bill and leave
    goto rts;
    // -------------------------------------------------------------------------------------
...
RunAllH:
    jsr(PlayerHammerCollision, 215); // handle collisions
  
RunHSubs:
    jsr(GetMiscOffscreenBits, 216); // get offscreen information
...
RunJCSubs:
    jsr(RelativeMiscPosition, 225); // get relative coordinates
...
ShroomM:
    jsr(MoveNormalEnemy, 234); // do sub to make mushrooms move
...
RunPUSubs:
    jsr(RelativeEnemyPosition, 236); // get coordinates relative to screen
...
Unbreak:
    jsr(BumpBlock, 246); // execute code for unbreakable brick or question block
...
InitBlock_XY_Pos:
    ...
    goto rts; // vertical high byte of block object and leave
    // --------------------------------
...
MatchBump:
    goto rts; // note carry is set if found match
    // --------------------------------
...
DoLpBack:
    jsr(ExecGameLoopback, 270); // if player is not in right place, loop back
...
NoInitCode:
    goto rts; // this executed when enemy object has no init code
    // --------------------------------
...
SetSpSpd:
    jsr(SmallBBox, 281); // set bounding box control, init attributes, lose contents of A
    ldy_imm(0x2);
...
FallE:
    jsr(MoveD_EnemyVertically, 327); // do a sub here to move enemy downwards
    ldy_imm(0x0);
...
FlyCC:
    jsr(MoveEnemyHorizontally, 346); // move cheep-cheep horizontally based on speed and force
...
KillLoop:
    jsr(EraseEnemyObject, 354); // branch to kill enemy objects
...
SetGfxF:
    jsr(RelativeEnemyPosition, 364); // get new relative coordinates
...
SetupExpl:
    jsr(RelativeEnemyPosition, 366); // get relative coordinates of explosion
...
ELPGive:
    jsr(DigitsMathRoutine, 369); // award 50 points per game timer interval
...
PlatUp:
    jsr(MovePlatformUp, 374); // do a sub to move upwards
    goto DoOtherPlatform; // jump ahead to remaining code
  
PlatSt:
    jsr(StopPlatforms, 375); // do a sub to stop movement
    goto DoOtherPlatform; // jump ahead to remaining code
  
PlatDn:
    jsr(MovePlatformDown, 376); // do a sub to move downwards
...
YMDown:
    jsr(MovePlatformDown, 387); // start slowing ascent/moving downwards
...
TooFar:
    jsr(EraseEnemyObject, 399); // erase object if necessary
...
StnE:
    jsr(ChkToStunEnemies, 404); // do yet another sub
    lda_zpx(Enemy_State);
...
KSPts:
    jsr(SetupFloateyNumber, 415); // set values for floatey number now
...
LInj:
    jsr(EnemyTurnAround, 424); // turn the enemy around, if necessary
...
YesEC:
    jsr(ProcEnemyCollisions, 428); // react according to the nature of collision
...
SideC:
    jsr(ImpedePlayerMove, 444); // deal with horizontal collision
...
ContChk:
    jsr(ChkInvisibleMTiles, 454); // do sub to check for hidden coin or 1-up blocks
...
LandPlyr:
    jsr(ChkForLandJumpSpring, 455); // do sub to check for jumpspring metatiles and deal with it
    lda_imm(0xf0);
...
ContSChk:
    jsr(CheckForCoinMTiles, 462); // check to see if player touched coin
...
ExCInvT:
    goto rts; // leave with zero flag set if either found
    // --------------------------------
...
YesIn:
    jsr(ChkUnderEnemy, 471); // if enemy object < $07, or = $12 or $2e, do this sub
...
FBallB:
    jsr(BoundingBoxCore, 492); // get bounding box coordinates
...
BBChk_E:
    jsr(BlockBufferCollision, 495); // do collision detection subroutine for sprite object
...
TopSP:
    jsr(DumpThreeSpr, 531); // dump vertical coordinate into Y coordinates
    pla(); // pull from stack
    clc();
...
RelWOfs:
    jsr(GetObjRelativePosition, 549); // get the coordinates
...
NoDecEnv1:
    jsr(LoadEnvelopeData, 584); // do a load of envelope data to replace default
...
NoDecEnv2:
    jsr(LoadEnvelopeData, 589); // do a load of envelope data
...
rts:
  switch (pop_jsr()) {
    ...
    case 465: goto jsr_ret_465;
    case 466: goto jsr_ret_466;
    case 467: goto jsr_ret_467;
    case 468: goto jsr_ret_468;
    case 469: goto jsr_ret_469;
    case 470: goto jsr_ret_470;
    case 471: goto jsr_ret_471;
    case 472: goto jsr_ret_472;
    case 473: goto jsr_ret_473;
    case 474: goto jsr_ret_474;
    case 475: goto jsr_ret_475;
    case 476: goto jsr_ret_476;
    case 477: goto jsr_ret_477;
    case 478: goto jsr_ret_478;
    case 479: goto jsr_ret_479;
    case 480: goto jsr_ret_480;
    case 481: goto jsr_ret_481;
    case 482: goto jsr_ret_482;
    case 483: goto jsr_ret_483;
    case 484: goto jsr_ret_484;
    case 485: goto jsr_ret_485;
    case 486: goto jsr_ret_486;
    case 487: goto jsr_ret_487;
    case 488: goto jsr_ret_488;
    case 489: goto jsr_ret_489;
    case 490: goto jsr_ret_490;
    case 491: goto jsr_ret_491;
    case 492: goto jsr_ret_492;
    case 493: goto jsr_ret_493;
    case 494: goto jsr_ret_494;
    case 495: goto jsr_ret_495;
    case 496: goto jsr_ret_496;
    case 497: goto jsr_ret_497;
    case 498: goto jsr_ret_498;
    case 499: goto jsr_ret_499;
    case 500: goto jsr_ret_500;
    case 501: goto jsr_ret_501;
    case 502: goto jsr_ret_502;
    case 503: goto jsr_ret_503;
    case 504: goto jsr_ret_504;
    case 505: goto jsr_ret_505;
    case 506: goto jsr_ret_506;
    case 507: goto jsr_ret_507;
    case 508: goto jsr_ret_508;
    case 509: goto jsr_ret_509;
    case 510: goto jsr_ret_510;
    case 511: goto jsr_ret_511;
    case 512: goto jsr_ret_512;
    case 513: goto jsr_ret_513;
    case 514: goto jsr_ret_514;
    case 515: goto jsr_ret_515;
    case 516: goto jsr_ret_516;
    case 517: goto jsr_ret_517;
    case 518: goto jsr_ret_518;
    case 519: goto jsr_ret_519;
    case 520: goto jsr_ret_520;
    case 521: goto jsr_ret_521;
    case 522: goto jsr_ret_522;
    case 523: goto jsr_ret_523;
    case 524: goto jsr_ret_524;
    case 525: goto jsr_ret_525;
    case 526: goto jsr_ret_526;
    case 527: goto jsr_ret_527;
    case 528: goto jsr_ret_528;
    case 529: goto jsr_ret_529;
    case 530: goto jsr_ret_530;
    case 531: goto jsr_ret_531;
    case 532: goto jsr_ret_532;
    case 533: goto jsr_ret_533;
    case 534: goto jsr_ret_534;
    case 535: goto jsr_ret_535;
    case 536: goto jsr_ret_536;
    case 537: goto jsr_ret_537;
    case 538: goto jsr_ret_538;
    case 539: goto jsr_ret_539;
    case 540: goto jsr_ret_540;
    case 541: goto jsr_ret_541;
    case 542: goto jsr_ret_542;
    case 543: goto jsr_ret_543;
    case 544: goto jsr_ret_544;
    case 545: goto jsr_ret_545;
    case 546: goto jsr_ret_546;
    case 547: goto jsr_ret_547;
    case 548: goto jsr_ret_548;
    case 549: goto jsr_ret_549;
    case 550: goto jsr_ret_550;
    case 551: goto jsr_ret_551;
    case 552: goto jsr_ret_552;
    case 553: goto jsr_ret_553;
    case 554: goto jsr_ret_554;
    case 555: goto jsr_ret_555;
    case 556: goto jsr_ret_556;
    case 557: goto jsr_ret_557;
    case 558: goto jsr_ret_558;
    case 559: goto jsr_ret_559;
    case 560: goto jsr_ret_560;
    case 561: goto jsr_ret_561;
    case 562: goto jsr_ret_562;
    case 563: goto jsr_ret_563;
    case 564: goto jsr_ret_564;
    case 565: goto jsr_ret_565;
    case 566: goto jsr_ret_566;
    case 567: goto jsr_ret_567;
    case 568: goto jsr_ret_568;
    case 569: goto jsr_ret_569;
    case 570: goto jsr_ret_570;
    case 571: goto jsr_ret_571;
    case 572: goto jsr_ret_572;
    case 573: goto jsr_ret_573;
    case 574: goto jsr_ret_574;
    case 575: goto jsr_ret_575;
    case 576: goto jsr_ret_576;
    case 577: goto jsr_ret_577;
    case 578: goto jsr_ret_578;
    case 579: goto jsr_ret_579;
    case 580: goto jsr_ret_580;
    case 581: goto jsr_ret_581;
    case 582: goto jsr_ret_582;
    case 583: goto jsr_ret_583;
    case 584: goto jsr_ret_584;
    case 585: goto jsr_ret_585;
    case 586: goto jsr_ret_586;
    case 587: goto jsr_ret_587;
    case 588: goto jsr_ret_588;
    case 589: goto jsr_ret_589;
    case 590: goto jsr_ret_590;
    case 591: goto jsr_ret_591;
    case 592: goto jsr_ret_592;
  }
}
