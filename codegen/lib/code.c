...
void Start(void) {
  ...
  dynamic_ram_write(PPU_CTRL_REG1, a);
...
WBootCheck:
  ...
  dynamic_ram_write(OperMode, a); // reset primary mode of operation
  lda_imm(0xa5); // set warm boot flag
  dynamic_ram_write(WarmBootValidation, a);
  dynamic_ram_write(PseudoRandomBitReg, a); // set seed for pseudorandom register
  lda_imm(0b00001111);
  dynamic_ram_write(SND_MASTERCTRL_REG, a); // enable all sound channels except dmc
  lda_imm(0b00000110);
  dynamic_ram_write(PPU_CTRL_REG2, a); // turn off clipping for OAM and background
...
void NonMaskableInterrupt(void) {
  ...
  dynamic_ram_write(Mirror_PPU_CTRL_REG1, a);
  and_imm(0b01111110); // alter name table address to be $2800
  dynamic_ram_write(PPU_CTRL_REG1, a); // (essentially $2000) but save other bits
...
ScreenOff:
  dynamic_ram_write(Mirror_PPU_CTRL_REG2, a); // save bits for later but not in register at the moment
  and_imm(0b11100111); // disable screen for now
  dynamic_ram_write(PPU_CTRL_REG2, a);
  ldx_abs(PPU_STATUS); // reset flip-flop and reset scroll registers to zero
  lda_imm(0x0);
  InitScroll();
  dynamic_ram_write(PPU_SPR_ADDR, a); // reset spr-ram address register
  lda_imm(0x2); // perform spr-ram DMA access on $0200-$02ff
  dynamic_ram_write(SPR_DMA, a);
  ...
  dynamic_ram_write(0x0, a);
  lda_absx(VRAM_AddrTable_High);
  dynamic_ram_write(0x1, a);
...
InitBuffer:
  ...
  dynamic_ram_write(VRAM_Buffer1_Offset + x, a);
  dynamic_ram_write(VRAM_Buffer1 + x, a);
  dynamic_ram_write(VRAM_Buffer_AddrCtrl, a); // reinit address control to $0301
...
DecTimers:
  ...
  lda_imm(0x14);
  dynamic_ram_write(IntervalTimerControl, a); // if control for interval timers expired,
...
PauseSkip:
  ...
  dynamic_ram_write(0x0, a); // save here
...
SkipSprite0:
  ...
  dynamic_ram_write(PPU_SCROLL_REG, a);
...
ChkStart:
  ...
  dynamic_ram_write(GamePauseTimer, a);
  lda_abs(GamePauseStatus);
  tay();
  iny(); // set pause sfx queue for next pause mode
  dynamic_ram_write(PauseSoundQueue, y);
...
SetPause:
  dynamic_ram_write(GamePauseStatus, a);
  // -------------------------------------------------------------------------------------
...
void SpriteShuffler(void) {
  ...
  dynamic_ram_write(0x0, a); // sprite #10
...
ShuffleLoop:
  ...
  if (carry_flag) {
    ...
    // StrSprOffset:
    dynamic_ram_write(SprDataOffset + x, a); // store new offset here or old one if branched to here
  }
  // NextSprOffset:
...
ChkSelect:
  ...
  dynamic_ram_write(SelectTimer, a); // set controller bits here if running demo
...
SelectBLogic:
  ...
  dynamic_ram_write(SelectTimer, a);
  ...
  dynamic_ram_write(NumberOfPlayers, a);
  DrawMushroomIcon();
  goto NullJoypad;
...
IncWorldSel:
  ...
  dynamic_ram_write(WorldSelectNumber, a); // store as current world select number
  GoContinue();
  
UpdateShroom:
  lda_absx(WSelectBufferTemplate); // write template for world select in vram buffer
  dynamic_ram_write(VRAM_Buffer1 - 1 + x, a); // do this until all bytes are written
  inx();
  cpx_imm(0x6);
  ...
  dynamic_ram_write(VRAM_Buffer1 + 3, y); // null terminator
  
NullJoypad:
  lda_imm(0x0); // clear joypad bits for player 1
  dynamic_ram_write(SavedJoypad1Bits, a);
...
ResetTitle:
  ...
  dynamic_ram_write(OperMode, a); // sprite 0 check and disable
  dynamic_ram_write(OperMode_Task, a); // screen output
  dynamic_ram_write(Sprite0HitDetectFlag, a);
...
StartWorld1:
  ...
  dynamic_ram_write(PrimaryHardMode, a); // hard mode must be on as well
  lda_imm(0x0);
  dynamic_ram_write(OperMode_Task, a); // set game mode here, and clear demo timer
  dynamic_ram_write(DemoTimer, a);
  ldx_imm(0x17);
  lda_imm(0x0);
  
InitScores:
  dynamic_ram_write(ScoreAndCoinDisplay + x, a); // clear player scores and coin displays
  dex();
...
void VictoryMode(void) {
  ...
  if (!zero_flag) {
    ldx_imm(0x0);
    dynamic_ram_write(ObjectOffset, x); // otherwise reset enemy object offset
...
void SetupIntermediate(void) {
  ...
  dynamic_ram_write(PlayerStatus, a); // and player status to not fiery
  lda_imm(0x2); // this is the ONLY time background color control
  dynamic_ram_write(BackgroundColorCtrl, a); // is set to less than 4
  GetPlayerColors();
  pla(); // we only execute this routine for
  dynamic_ram_write(PlayerStatus, a); // the intermediate lives display
  pla(); // and once we're done, we return bg
  dynamic_ram_write(BackgroundColorCtrl, a); // color ctrl and player status from stack
...
void SetVRAMAddr_A(void) {
  dynamic_ram_write(VRAM_Buffer_AddrCtrl, x); // store offset into buffer control
  NextSubtask();
}
...
void GetBackgroundColor(void) {
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(VRAM_Buffer_AddrCtrl, a); // note that if set to 5-7, $0301 will not be read
  }
  // NoBGColor:
...
ClrGetLoop:
  ...
  dynamic_ram_write(VRAM_Buffer1 + 3 + x, a); // in the buffer
  iny();
  inx();
  dec_zp(0x0);
  ...
  dynamic_ram_write(VRAM_Buffer1 + 3 + x, a);
  lda_imm(0x3f); // set for sprite palette address
  dynamic_ram_write(VRAM_Buffer1 + x, a); // save to buffer
  lda_imm(0x10);
  dynamic_ram_write(VRAM_Buffer1 + 1 + x, a);
  lda_imm(0x4); // write length byte to buffer
  dynamic_ram_write(VRAM_Buffer1 + 2 + x, a);
  lda_imm(0x0); // now the null terminator
  dynamic_ram_write(VRAM_Buffer1 + 7 + x, a);
...
void SetVRAMOffset(void) {
  dynamic_ram_write(VRAM_Buffer1_Offset, a); // store as new vram buffer offset
  // -------------------------------------------------------------------------------------
}
...
void SetVRAMAddr_B(void) {
  dynamic_ram_write(VRAM_Buffer_AddrCtrl, a);
  NoAltPal();
}
...
void DisplayTimeUp(void) {
  ...
  if (!zero_flag) {
    lda_imm(0x0);
    dynamic_ram_write(GameTimerExpiredFlag, a); // reset timer expiration flag
...
void GameOverInter(void) {
  ...
  dynamic_ram_write(ScreenTimer, a);
...
void NoInter(void) {
  ...
  dynamic_ram_write(ScreenRoutineTask, a);
  // -------------------------------------------------------------------------------------
}
...
TaskLoop:
  ...
  dynamic_ram_write(VRAM_Buffer_AddrCtrl, a); // on next NMI
  // -------------------------------------------------------------------------------------
...
void DrawTitleScreen(void) {
  ...
  dynamic_ram_write(PPU_ADDRESS, a); // the vram address register
  lda_imm(LOW_BYTE(TitleScreenDataOffset));
  dynamic_ram_write(PPU_ADDRESS, a);
  lda_imm(0x3); // put address $0300 into
  dynamic_ram_write(0x1, a); // the indirect at $00
  ldy_imm(0x0);
  dynamic_ram_write(0x0, y);
...
TScrClear:
  dynamic_ram_write(VRAM_Buffer1 - 1 + x, a);
  dynamic_ram_write(VRAM_Buffer1 - 1 + 0x100 + x, a);
  dex();
...
ClrSndLoop:
  dynamic_ram_write(SoundMemory + y, a); // clear out memory used
...
ClrTimersLoop:
  dynamic_ram_write(Timers + x, a); // clear out memory between
  dex(); // $0780 and $07a1
...
StartPage:
  dynamic_ram_write(ScreenLeft_PageLoc, a); // set as value here
  dynamic_ram_write(CurrentPageLoc, a); // also set as current page
  dynamic_ram_write(BackloadingFlag, a); // set flag here if halfway page or saved entry page number found
...
SetInitNTHigh:
  dynamic_ram_write(CurrentNTAddr_High, y); // store name table address
  ldy_imm(0x80);
  dynamic_ram_write(CurrentNTAddr_Low, y);
  asl_acc(); // store LSB of page number in high nybble
  asl_acc(); // of block buffer column position
  asl_acc();
  asl_acc();
  dynamic_ram_write(BlockBufferColumnPos, a);
  ...
  dynamic_ram_write(ColumnSets, a); // 12 column sets = 24 metatile columns = 1 1/2 screens
...
CheckHalfway:
  ...
  dynamic_ram_write(PlayerEntranceCtrl, a);
  
DoneInitArea:
  lda_imm(Silence); // silence music
  dynamic_ram_write(AreaMusicQueue, a);
  lda_imm(0x1); // disable screen output
  dynamic_ram_write(DisableScreenFlag, a);
...
void PrimaryGameSetup(void) {
  lda_imm(0x1);
  dynamic_ram_write(FetchNewGameTimerFlag, a); // set flag to load game timer from header
  dynamic_ram_write(PlayerSize, a); // set player's size to small
  lda_imm(0x2);
  dynamic_ram_write(NumberofLives, a); // give each player three lives
  dynamic_ram_write(OffScr_NumberofLives, a);
  SecondaryGameSetup(); // <fallthrough>
}

void SecondaryGameSetup(void) {
  lda_imm(0x0);
  dynamic_ram_write(DisableScreenFlag, a); // enable screen output
  tay();
  
ClearVRLoop:
  dynamic_ram_write(VRAM_Buffer1 - 1 + y, a); // clear buffer at $0300-$03ff
  iny();
  if (!zero_flag) { goto ClearVRLoop; }
  dynamic_ram_write(GameTimerExpiredFlag, a); // clear game timer exp flag
  dynamic_ram_write(DisableIntermediate, a); // clear skip lives display flag
  dynamic_ram_write(BackloadingFlag, a); // clear value here
  lda_imm(0xff);
  dynamic_ram_write(BalPlatformAlignment, a); // initialize balance platform assignment flag
  ...
  dynamic_ram_write(SprShuffleAmt + 2, a);
  lda_imm(0x48);
  dynamic_ram_write(SprShuffleAmt + 1, a);
  lda_imm(0x58);
  dynamic_ram_write(SprShuffleAmt, a);
  ldx_imm(0xe); // load default OAM offsets into $06e4-$06f2
  
ShufAmtLoop:
  lda_absx(DefaultSprOffsets);
  dynamic_ram_write(SprDataOffset + x, a);
...
ISpr0Loop:
  ...
  dynamic_ram_write(Sprite_Data + y, a);
  dey();
...
void SetupGameOver(void) {
  ...
  dynamic_ram_write(ScreenRoutineTask, a); // and game over modes
  dynamic_ram_write(Sprite0HitDetectFlag, a); // disable sprite 0 check
  lda_imm(GameOverMusic);
  dynamic_ram_write(EventMusicQueue, a); // put game over music in secondary queue
...
void TerminateGame(void) {
  lda_imm(Silence); // silence music
  dynamic_ram_write(EventMusicQueue, a);
  ...
  dynamic_ram_write(ContinueWorld, a); // player into secret continue function variable
  lda_imm(0x0);
  asl_acc(); // residual ASL instruction
  dynamic_ram_write(OperMode_Task, a); // reset all modes to title screen and
  dynamic_ram_write(ScreenTimer, a); // leave
  dynamic_ram_write(OperMode, a);
}

void ContinueGame(void) {
  LoadAreaPointer(); // update level pointer with
  lda_imm(0x1); // actual world and area numbers, then
  dynamic_ram_write(PlayerSize, a); // reset player's size, status, and
  inc_abs(FetchNewGameTimerFlag); // set game timer flag to reload
  lda_imm(0x0); // game timer from header
  dynamic_ram_write(TimerControl, a); // also set flag for timers to count again
  dynamic_ram_write(PlayerStatus, a);
  dynamic_ram_write(GameEngineSubroutine, a); // reset task for game core
  dynamic_ram_write(OperMode_Task, a); // set modes and leave
  lda_imm(0x1); // if in game over mode, switch back to
  dynamic_ram_write(OperMode, a); // game mode, because game is still on
}
...
void GameCoreRoutine(void) {
  ...
  dynamic_ram_write(SavedJoypadBits, a); // as the master controller bits
...
ProcELoop:
  dynamic_ram_write(ObjectOffset, x); // put incremented offset in X as enemy object offset
  ...
  ldx_imm(0x1);
  dynamic_ram_write(ObjectOffset, x); // set offset for second
  BlockObjectsCore(); // process second block object
  dex();
  dynamic_ram_write(ObjectOffset, x); // set offset for first
...
SaveAB:
  ...
  dynamic_ram_write(PreviousA_B_Buttons, a); // into temp variable to be used on next frame
  lda_imm(0x0);
  dynamic_ram_write(Left_Right_Buttons, a); // nullify left and right buttons temp variable
...
void UpdScrollVar(void) {
  ...
  dynamic_ram_write(ScrollThirtyTwo, a); // and store
  lda_imm(0x0); // reset vram buffer offset used in conjunction with
  dynamic_ram_write(VRAM_Buffer2_Offset, a); // level graphics buffer at $0341-$035f
...
BigKTS:
  ...
  dynamic_ram_write(Sprite_Tilenumber + 24 + y, a); // to animate player's feet when swimming
}
...
void PlayerGfxProcessing(void) {
  dynamic_ram_write(PlayerGfxOffset, a); // store offset to graphics table here
  lda_imm(0x4);
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(FireballThrowingTimer, y); // initialize fireball throw timer
    // if animation frame timer => fireball throw timer skip to end
    if (!carry_flag) {
      dynamic_ram_write(FireballThrowingTimer, a); // otherwise store animation timer into fireball throw timer
      ldy_imm(0x7); // load offset for throwing
      lda_absy(PlayerGfxTblOffsets); // get offset to graphics table
      dynamic_ram_write(PlayerGfxOffset, a); // store it for use later
  ...
  lsr_acc();
  lsr_acc();
  dynamic_ram_write(0x0, a); // store here
...
SprInitLoop:
  dynamic_ram_write(Sprite_Y_Position + y, a); // write 248 into OAM data's Y coordinate
...
void GoContinue(void) {
  dynamic_ram_write(WorldNumber, a); // start both players at the first area
  dynamic_ram_write(OffScr_WorldNumber, a); // of the previously saved world number
  ldx_imm(0x0); // note that on power-up using this function
  dynamic_ram_write(AreaNumber, x); // will make no difference
  dynamic_ram_write(OffScr_AreaNumber, x);
  // -------------------------------------------------------------------------------------
}
...
IconDataRead:
  ...
  dynamic_ram_write(VRAM_Buffer1 - 1 + y, a); // 1-player game
  dey();
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(VRAM_Buffer1 + 3, a);
    lda_imm(0xce); // then load shroom icon tile in 2-player position
    dynamic_ram_write(VRAM_Buffer1 + 5, a);
    // -------------------------------------------------------------------------------------
  }
}
...
void DemoEngine(void) {
  ...
  dynamic_ram_write(DemoActionTimer, a); // store as current timer
...
void SetupVictoryMode(void) {
  ...
  dynamic_ram_write(DestinationPageLoc, x); // store here
  lda_imm(EndOfCastleMusic);
  dynamic_ram_write(EventMusicQueue, a); // play win castle music
...
void PlayerVictoryWalk(void) {
  ...
  dynamic_ram_write(VictoryWalkControl, y);
...
DontWalk:
  ...
  adc_imm(0x80);
  dynamic_ram_write(ScrollFractional, a); // save fractional movement amount
...
EvalForMusic:
  ...
  dynamic_ram_write(EventMusicQueue, a); // otherwise load victory music first (world 8 only)
...
void PlayerEndWorld(void) {
  ...
  lda_imm(0x0);
  dynamic_ram_write(AreaNumber, a); // otherwise initialize area number used as offset
  dynamic_ram_write(LevelNumber, a); // and level number control to start at area 1
  dynamic_ram_write(OperMode_Task, a); // initialize secondary mode of operation
  ...
  dynamic_ram_write(OperMode, a); // set mode of operation to game mode
...
EndChkBButton:
  ...
  dynamic_ram_write(WorldSelectEnableFlag, a);
  lda_imm(0xff); // remove onscreen player's lives
  dynamic_ram_write(NumberofLives, a);
...
void BridgeCollapse(void) {
  ...
  if (zero_flag) {
    dynamic_ram_write(ObjectOffset, x); // store as enemy offset here
...
void RemoveBridge(void) {
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(Square2SoundQueue, a); // queue while at the same time loading the brick
    lda_imm(Sfx_BrickShatter); // shatter sound into the noise sfx queue thus
    dynamic_ram_write(NoiseSoundQueue, a); // producing the unique sound of the bridge collapsing
    ...
    if (zero_flag) {
      ...
      lda_imm(0b01000000);
      dynamic_ram_write(Enemy_State + x, a); // set bowser's state to one of defeated states (d6 set)
      lda_imm(Sfx_BowserFall);
      dynamic_ram_write(Square2SoundQueue, a); // play bowser defeat sound
    }
  }
  // NoBFall:
...
void FloateyNumbersRoutine(void) {
  ...
  dynamic_ram_write(FloateyNum_Control + x, a); // it in range
  
ChkNumTimer:
  tay(); // use as Y
  lda_absx(FloateyNum_Timer); // check value here
  if (!zero_flag) { goto DecNumTimer; } // if nonzero, branch ahead
  dynamic_ram_write(FloateyNum_Control + x, a); // initialize floatey number control and leave
  return;
...
DecNumTimer:
  ...
  dynamic_ram_write(Square2SoundQueue, a); // and play the 1-up sound
...
LoadNumTiles:
  ...
  dynamic_ram_write(DigitModifier + x, a); // store as amount to add to the digit
...
FloateyPart:
  ...
  sbc_imm(0x1);
  dynamic_ram_write(FloateyNum_Y_Pos + x, a); // otherwise subtract one and store as new
...
SetupNumSpr:
  ...
  dynamic_ram_write(Sprite_X_Position + y, a); // store into X coordinate of left sprite
  carry_flag = false;
  adc_imm(0x8); // add eight pixels and store into X
  dynamic_ram_write(Sprite_X_Position + 4 + y, a); // coordinate of right sprite
  lda_imm(0x2);
  dynamic_ram_write(Sprite_Attributes + y, a); // set palette control in attribute bytes
  dynamic_ram_write(Sprite_Attributes + 4 + y, a); // of left and right sprites
  ...
  dynamic_ram_write(Sprite_Tilenumber + y, a); // display first half of number of points
  lda_absx(FloateyNumTileData + 1);
  dynamic_ram_write(Sprite_Tilenumber + 4 + y, a); // display the second half
...
void OutputInter(void) {
  ...
  ResetScreenTimer();
  lda_imm(0x0);
  dynamic_ram_write(DisableScreenFlag, a); // reenable screen output
}
...
GameTextLoop:
  ...
  if (!zero_flag) {
    dynamic_ram_write(VRAM_Buffer1 + y, a); // otherwise write data to buffer
  ...
  dynamic_ram_write(VRAM_Buffer1 + y, a);
  ...
  if (!carry_flag) {
    ...
    if (zero_flag) {
      ...
      if (carry_flag) {
        ...
        dynamic_ram_write(VRAM_Buffer1 + 7, y); // the number of lives exceeds 19
      }
      // PutLives:
      dynamic_ram_write(VRAM_Buffer1 + 8, a);
      ldy_abs(WorldNumber); // write world and level numbers (incremented for display)
      iny(); // to the buffer in the spaces surrounding the dash
      dynamic_ram_write(VRAM_Buffer1 + 19, y);
      ldy_abs(LevelNumber);
      iny();
      dynamic_ram_write(VRAM_Buffer1 + 21, y); // we're done here
      return;
    }
    // CheckPlayerName:
    lda_abs(NumberOfPlayers); // check number of players
...
NameLoop:
        ...
        dynamic_ram_write(VRAM_Buffer1 + 3 + y, a);
        dey();
...
WarpNumLoop:
  ...
  dynamic_ram_write(VRAM_Buffer1 + 27 + y, a); // placeholders from earlier
  inx();
...
void RenderAreaGraphics(void) {
  ...
  dynamic_ram_write(VRAM_Buffer2 + 1 + y, a);
  lda_abs(CurrentNTAddr_High);
  dynamic_ram_write(VRAM_Buffer2 + y, a);
  lda_imm(0x9a); // store length byte of 26 here with d7 set
  dynamic_ram_write(VRAM_Buffer2 + 2 + y, a); // to increment by 32 (in columns)
  lda_imm(0x0); // init attribute row
  dynamic_ram_write(0x4, a);
  tax();
  
DrawMTLoop:
  dynamic_ram_write(0x1, x); // store init value of 0 or incremented offset for buffer
  lda_absx(MetatileBuffer); // get first metatile number, and mask out all but 2 MSB
  and_imm(0b11000000);
  dynamic_ram_write(0x3, a); // store attribute table bits here
  ...
  lda_indy(0x6);
  dynamic_ram_write(VRAM_Buffer2 + 3 + x, a); // get first tile number (top left or top right) and store
  iny();
  lda_indy(0x6); // now get the second (bottom left or bottom right) and store
  dynamic_ram_write(VRAM_Buffer2 + 4 + x, a);
...
SetAttrib:
  ...
  dynamic_ram_write(AttributeBuffer + y, a); // the old, and store
  ...
  iny();
  lda_imm(0x0);
  dynamic_ram_write(VRAM_Buffer2 + y, a); // put null terminator at end of data for name table
  dynamic_ram_write(VRAM_Buffer2_Offset, y); // store new buffer offset
  ...
  dynamic_ram_write(CurrentNTAddr_Low, a); // just under the status bar
  ...
  dynamic_ram_write(CurrentNTAddr_High, a);
...
void RenderAttributeTables(void) {
  ...
  dynamic_ram_write(0x1, a); // attribute table in our temp address
  ldx_imm(0x0);
  ldy_abs(VRAM_Buffer2_Offset); // get buffer offset
  
AttribLoop:
  lda_zp(0x0);
  dynamic_ram_write(VRAM_Buffer2 + y, a); // store high byte of attribute table address
  lda_zp(0x1);
  carry_flag = false; // get low byte, add 8 because we want to start
  adc_imm(0x8); // below the status bar, and store
  dynamic_ram_write(VRAM_Buffer2 + 1 + y, a);
  dynamic_ram_write(0x1, a); // also store in temp again
  lda_absx(AttributeBuffer); // fetch current attribute table byte and store
  dynamic_ram_write(VRAM_Buffer2 + 3 + y, a); // in the buffer
  lda_imm(0x1);
  dynamic_ram_write(VRAM_Buffer2 + 2 + y, a); // store length of 1 in buffer
  lsr_acc();
  dynamic_ram_write(AttributeBuffer + x, a); // clear current byte in attribute buffer
  ...
  dynamic_ram_write(VRAM_Buffer2 + y, a); // put null terminator at the end
  dynamic_ram_write(VRAM_Buffer2_Offset, y); // store offset in case we want to do any more
  SetVRAMCtrl(); // <fallthrough>
}

void SetVRAMCtrl(void) {
  lda_imm(0x6);
  dynamic_ram_write(VRAM_Buffer_AddrCtrl, a); // set buffer to $0341 and leave
  // -------------------------------------------------------------------------------------
...
GetBlankPal:
  ...
  dynamic_ram_write(VRAM_Buffer1 + x, a); // store it in the vram buffer
  ...
  lda_imm(0x3);
  dynamic_ram_write(0x0, a); // set counter here
...
GetAreaPal:
  ...
  dynamic_ram_write(VRAM_Buffer1 + 3 + x, a); // store it to overwrite blank palette in vram buffer
  iny();
  inx();
  ...
  dynamic_ram_write(VRAM_Buffer1 + 4 + x, a); // get and store current color in second slot of palette
  ...
  lda_imm(0x0);
  dynamic_ram_write(ColorRotateOffset, a); // otherwise, init to keep it in range
  // -------------------------------------------------------------------------------------
...
void RemoveCoin_Axe(void) {
  ...
  lda_imm(0x6);
  dynamic_ram_write(VRAM_Buffer_AddrCtrl, a); // set vram address controller to $0341 and leave
}
...
void PutBlockMetatile(void) {
  dynamic_ram_write(0x0, x); // store control bit from SprDataOffset_Ctrl
  dynamic_ram_write(0x1, y); // store vram buffer offset for next byte
  asl_acc();
  ...
  // SaveHAdder:
  dynamic_ram_write(0x3, y); // save high byte here
  and_imm(0xf); // mask out high nybble of block buffer pointer
  asl_acc(); // multiply by 2 to get appropriate name table low byte
  dynamic_ram_write(0x4, a); // and then store it here
  lda_imm(0x0);
  dynamic_ram_write(0x5, a); // initialize temp high byte
  ...
  dynamic_ram_write(0x4, a); // and store here
  ...
  dynamic_ram_write(0x5, a); // store here
...
void RemBridge(void) {
  ...
  dynamic_ram_write(VRAM_Buffer1 + 2 + y, a); // tile numbers into first spot
  lda_absx(BlockGfxData + 1);
  dynamic_ram_write(VRAM_Buffer1 + 3 + y, a);
  lda_absx(BlockGfxData + 2); // write bottom left and bottom
  dynamic_ram_write(VRAM_Buffer1 + 7 + y, a); // right tiles numbers into
  lda_absx(BlockGfxData + 3); // second spot
  dynamic_ram_write(VRAM_Buffer1 + 8 + y, a);
  lda_zp(0x4);
  dynamic_ram_write(VRAM_Buffer1 + y, a); // write low byte of name table
  carry_flag = false; // into first slot as read
  adc_imm(0x20); // add 32 bytes to value
  dynamic_ram_write(VRAM_Buffer1 + 5 + y, a); // write low byte of name table
  lda_zp(0x5); // plus 32 bytes into second slot
  dynamic_ram_write(VRAM_Buffer1 - 1 + y, a); // write high byte of name
  dynamic_ram_write(VRAM_Buffer1 + 4 + y, a); // table address to both slots
  lda_imm(0x2);
  dynamic_ram_write(VRAM_Buffer1 + 1 + y, a); // put length of 2 in
  dynamic_ram_write(VRAM_Buffer1 + 6 + y, a); // both slots
  lda_imm(0x0);
  dynamic_ram_write(VRAM_Buffer1 + 9 + y, a); // put null terminator at end
...
InitNTLoop:
  dynamic_ram_write(PPU_DATA, a); // count out exactly 768 tiles
  dey();
  ...
  txa();
  dynamic_ram_write(VRAM_Buffer1_Offset, a); // init vram buffer 1 offset
  dynamic_ram_write(VRAM_Buffer1, a); // init vram buffer 1
  
InitATLoop:
  dynamic_ram_write(PPU_DATA, a);
  dey();
  if (!zero_flag) { goto InitATLoop; }
  dynamic_ram_write(HorizontalScroll, a); // reset scroll variables
  dynamic_ram_write(VerticalScroll, a);
...
void ReadJoypads(void) {
  ...
  dynamic_ram_write(JOYPAD_PORT, a);
  lsr_acc();
...
PortLoop:
  ...
  dynamic_ram_write(0x0, a); // check d1 and d0 of port output
  ...
  dynamic_ram_write(SavedJoypadBits + x, a); // save controller status here always
  pha();
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(SavedJoypadBits + x, a); // or start bits and leave
    return;
  }
  // Save8Bits:
  pla();
  dynamic_ram_write(JoypadBitMask + x, a); // save with all bits in another place and leave
  // -------------------------------------------------------------------------------------
  // $00 - vram buffer address table low
  // $01 - vram buffer address table high
}
...
void WriteBufferToScreen(void) {
  dynamic_ram_write(PPU_ADDRESS, a); // store high byte of vram address
  iny();
  lda_indy(0x0); // load next byte (second)
  dynamic_ram_write(PPU_ADDRESS, a); // store low byte of vram address
  iny();
...
OutputToVRAM:
  ...
  dynamic_ram_write(0x0, a); // to allow this routine to read another set of updates
  lda_imm(0x0);
  adc_zp(0x1);
  dynamic_ram_write(0x1, a);
  ...
  lda_imm(0x0);
  dynamic_ram_write(PPU_ADDRESS, a);
  dynamic_ram_write(PPU_ADDRESS, a); // then reinitializes it for some reason
  dynamic_ram_write(PPU_ADDRESS, a);
...
void InitScroll(void) {
  dynamic_ram_write(PPU_SCROLL_REG, a); // store contents of A into scroll registers
  dynamic_ram_write(PPU_SCROLL_REG, a); // and end whatever subroutine led us here
  // -------------------------------------------------------------------------------------
}

void WritePPUReg1(void) {
  dynamic_ram_write(PPU_CTRL_REG1, a); // write contents of A to PPU register 1
  dynamic_ram_write(Mirror_PPU_CTRL_REG1, a); // and its mirror
  // -------------------------------------------------------------------------------------
...
void PrintStatusBarNumbers(void) {
  dynamic_ram_write(0x0, a); // store player-specific offset
...
void OutputNumbers(void) {
  ...
  if (!carry_flag) {
    ...
    dynamic_ram_write(VRAM_Buffer1 + 1 + x, a); // we're printing to the buffer
    lda_absy(StatusBarData + 1);
    dynamic_ram_write(VRAM_Buffer1 + 2 + x, a);
    dynamic_ram_write(0x3, a); // save length byte in counter
    dynamic_ram_write(0x2, x); // and buffer pointer elsewhere for now
...
DigitPLoop:
    ...
    inx();
    inx();
    dynamic_ram_write(VRAM_Buffer1_Offset, x); // store it in case we want to use it again
    // -------------------------------------------------------------------------------------
  }
}
...
StoreNewD:
  dynamic_ram_write(DisplayDigits + y, a); // store as new score or game timer digit
...
EraseMLoop:
  dynamic_ram_write(DigitModifier - 1 + x, a); // initialize the digit amounts to increment
  dex();
...
CopyScore:
    ...
    dynamic_ram_write(TopScoreDisplay + y, a);
    inx();
    iny();
...
StoreMusic:
  ...
  dynamic_ram_write(AreaMusicQueue, a); // store in queue and leave
  // -------------------------------------------------------------------------------------
}

void Entrance_GameTimerSetup(void) {
  lda_abs(ScreenLeft_PageLoc); // set current page for area objects
  dynamic_ram_write(Player_PageLoc, a); // as page location for player
  lda_imm(0x28); // store value here
  dynamic_ram_write(VerticalForceDown, a); // for fractional movement downwards if necessary
  lda_imm(0x1); // set high byte of player position and
  dynamic_ram_write(PlayerFacingDir, a); // set facing direction so that player faces right
  dynamic_ram_write(Player_Y_HighPos, a);
  lda_imm(0x0); // set player state to on the ground by default
  dynamic_ram_write(Player_State, a);
  dec_abs(Player_CollisionBits); // initialize player's collision bits
  ldy_imm(0x0); // initialize halfway page
  dynamic_ram_write(HalfwayPage, y);
  ...
  // ChkStPos:
  dynamic_ram_write(SwimmingFlag, y);
  ...
  dynamic_ram_write(Player_X_Position, a); // and vertical positions for the player, using
  lda_absx(PlayerStarting_Y_Pos); // AltEntranceControl as offset for horizontal and either $0710
  dynamic_ram_write(Player_Y_Position, a); // or value that overwrote $0710 as offset for vertical
  lda_absx(PlayerBGPriorityData);
  dynamic_ram_write(Player_SprAttrib, a); // set player sprite attributes using offset in X
  ...
  if (!zero_flag) {
    ...
    if (!zero_flag) {
      ...
      dynamic_ram_write(GameTimerDisplay, a); // use value of game timer control for first digit of game timer
      lda_imm(0x1);
      dynamic_ram_write(GameTimerDisplay + 2, a); // set last digit of game timer to 1
      lsr_acc();
      dynamic_ram_write(GameTimerDisplay + 1, a); // set second digit of game timer
      dynamic_ram_write(FetchNewGameTimerFlag, a); // clear flag for game timer reset
      dynamic_ram_write(StarInvincibleTimer, a); // clear star mario timer
    }
  }
  // ChkOverR:
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(Block_Y_Position, a);
  ...
  dynamic_ram_write(GameEngineSubroutine, a); // on the next frame of game engine
  // -------------------------------------------------------------------------------------
...
void PlayerLoseLife(void) {
  ...
  if (neg_flag) {
    lda_imm(0x0);
    dynamic_ram_write(OperMode_Task, a); // initialize mode task,
    lda_imm(GameOverModeValue); // switch to game over mode
    dynamic_ram_write(OperMode, a); // and leave
    return;
  }
  // StillInGame:
  ...
  // SetHalfway:
  dynamic_ram_write(HalfwayPage, a); // store as halfway page for player
...
void TransposePlayers(void) {
  ...
  dynamic_ram_write(CurrentPlayer, a);
  ldx_imm(0x6);
...
TransLoop:
  ...
  dynamic_ram_write(OnscreenPlayerInfo + x, a);
  pla();
  dynamic_ram_write(OffscreenPlayerInfo + x, a);
  dex();
...
void DoNothing1(void) {
  ...
  dynamic_ram_write(0x6c9, a); // not used anywhere in the program
...
void AreaParserTaskHandler(void) {
  ...
  if (zero_flag) {
    ldy_imm(0x8);
    dynamic_ram_write(AreaParserTaskNum, y); // otherwise, set eight by default
  }
  // DoAPTasks:
  dey();
  tya();
...
void IncrementColumnPos(void) {
  ...
  if (zero_flag) {
    dynamic_ram_write(CurrentColumnPos, a); // if no bits left set, wrap back to zero (0-f)
  ...
  dynamic_ram_write(BlockBufferColumnPos, a); // and save
  // -------------------------------------------------------------------------------------
...
ClrMTBuf:
  dynamic_ram_write(MetatileBuffer + x, a); // clear out metatile buffer
  dex();
...
ThirdP:
    ...
    if (!zero_flag) {
      ...
      dynamic_ram_write(0x0, a); // save low nybble
...
SceLoop1:
      ...
      dynamic_ram_write(MetatileBuffer + y, a); // store into buffer from offset of (msb / 16)
      inx();
      iny();
...
SceLoop2:
    ...
    if (!zero_flag) {
      dynamic_ram_write(MetatileBuffer + x, a);
    }
    // NoFore:
    iny();
    inx();
...
StoreMT:
  dynamic_ram_write(0x7, a); // store value here
...
TerrBChk:
  ...
  if (!zero_flag) {
    lda_zp(0x7);
    dynamic_ram_write(MetatileBuffer + x, a); // load terrain type metatile number and store into buffer here
  }
  // NextTBit:
...
ProcADLoop:
  dynamic_ram_write(ObjectOffset, x);
  lda_imm(0x0); // reset flag
  dynamic_ram_write(BehindAreaParserFlag, a);
...
Chk1Row13:
  ...
  dynamic_ram_write(AreaObjectPageLoc, a);
...
void IncAreaObjOffset(void) {
  ...
  dynamic_ram_write(AreaObjectPageSel, a);
}
...
ChkRow14:
  dynamic_ram_write(0x7, x); // store whatever value we just loaded here
...
ChkSRows:
  ...
  lda_imm(0x16);
  dynamic_ram_write(0x7, a); // otherwise set offset of 24 for small object
  lda_indy(AreaData); // reload second byte of level object
  and_imm(0b00001111); // mask out higher nybble and jump
  goto NormObj;
  
LrgObj:
  dynamic_ram_write(0x0, a); // store value here (branch for large objects)
...
NormObj:
  dynamic_ram_write(0x0, a); // store value here (branch for small objects and rows 13 and 14)
...
InitRear:
  ...
  dynamic_ram_write(BackloadingFlag, a); // behind-renderer flags and leave
  dynamic_ram_write(BehindAreaParserFlag, a);
  dynamic_ram_write(ObjectOffset, a);
...
void StrAObj(void) {
  ...
  dynamic_ram_write(AreaObjOffsetBuffer + x, a);
...
void AlterAreaAttributes(void) {
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(TerrainControl, a); // new terrain height type bits
    pla();
    ...
    lsr_acc();
    lsr_acc();
    dynamic_ram_write(BackgroundScenery, a); // then leave
    return;
  }
  // Alter2:
  pla();
  ...
  if (carry_flag) {
    dynamic_ram_write(BackgroundColorCtrl, a);
    lda_imm(0x0);
  }
  // SetFore:
  dynamic_ram_write(ForegroundScenery, a); // otherwise set new foreground scenery bits
  // --------------------------------
}
...
void ScrollLockObject_Warp(void) {
  ...
  // WarpNum:
  txa();
  dynamic_ram_write(WarpZoneControl, a); // store number here to be used by warp zone routine
...
void ScrollLockObject(void) {
  ...
  eor_imm(0b00000001);
  dynamic_ram_write(ScrollLock, a);
  // --------------------------------
...
FreCompLoop:
  ...
  // ExitAFrenzy:
  dynamic_ram_write(EnemyFrenzyQueue, a); // store enemy into frenzy queue
  // --------------------------------
...
void TreeLedge(void) {
  ...
  if (!zero_flag) {
    if (neg_flag) {
      tya();
      dynamic_ram_write(AreaObjectLength + x, a); // store lower nybble into buffer flag as length of ledge
    ...
    dynamic_ram_write(MetatileBuffer + x, a); // note that this is also used if ledge position is
...
void MushroomLedge(void) {
  ...
  dynamic_ram_write(0x6, y); // store length here for now
  if (carry_flag) {
    lda_absx(AreaObjectLength); // divide length by 2 and store elsewhere
    lsr_acc();
    dynamic_ram_write(MushroomLedgeHalfLen + x, a);
  ...
  dynamic_ram_write(0x6, a); // was stored originally
  ldx_zp(0x7);
  lda_imm(0x1a);
  dynamic_ram_write(MetatileBuffer + x, a); // render middle of mushroom
  cpy_zp(0x6); // are we smack dab in the center?
  if (zero_flag) {
    inx();
    lda_imm(0x4f);
    dynamic_ram_write(MetatileBuffer + x, a); // render stem top of mushroom underneath the middle
    lda_imm(0x50);
...
void PulleyRopeObject(void) {
  ...
  dynamic_ram_write(MetatileBuffer, a); // render at the top of the screen
  // --------------------------------
...
void CastleObject(void) {
  ...
  dynamic_ram_write(0x7, y); // if starting row is above $0a, game will crash!!!
  ldy_imm(0x4);
  ...
  lda_imm(0xb);
  dynamic_ram_write(0x6, a); // load upper limit of number of rows to print
...
NotTall:
  ...
  pla();
  dynamic_ram_write(Enemy_X_Position + x, a); // then write horizontal coordinate for star flag
  lda_abs(CurrentPageLoc);
  dynamic_ram_write(Enemy_PageLoc + x, a); // set page location for star flag
  lda_imm(0x1);
  dynamic_ram_write(Enemy_Y_HighPos + x, a); // set vertical high byte
  dynamic_ram_write(Enemy_Flag + x, a); // set flag for buffer
  lda_imm(0x90);
  dynamic_ram_write(Enemy_Y_Position + x, a); // set vertical coordinate
  lda_imm(StarFlagObject); // set star flag value in buffer itself
  dynamic_ram_write(Enemy_ID + x, a);
  return;
  
PlayerStop:
  ldy_imm(0x52); // put brick at floor to stop player at end of level
  dynamic_ram_write(MetatileBuffer + 10, y); // this is only done if we're on the second column
  // --------------------------------
}
...
void WaterPipe(void) {
  ...
  ldx_zp(0x7); // get row
  lda_imm(0x6b);
  dynamic_ram_write(MetatileBuffer + x, a); // draw something here and below it
  lda_imm(0x6c);
  dynamic_ram_write(MetatileBuffer + 1 + x, a);
  // --------------------------------
...
VPipeSectLoop:
    ...
    dynamic_ram_write(MetatileBuffer + x, a); // because otherwise it will look like exit pipe
    dex();
    if (!neg_flag) { goto VPipeSectLoop; }
    lda_absy(VerticalPipeData); // draw the end of the vertical pipe part
    dynamic_ram_write(MetatileBuffer + 7, a);
  }
}
...
void RenderSidewaysPipe(void) {
  ...
  dynamic_ram_write(MetatileBuffer + x, a); // note that the pipe parts are stored
...
void VerticalPipe(void) {
  ...
  if (!zero_flag) {
    ...
    if (!zero_flag) {
      ...
      if (!carry_flag) {
        ...
        dynamic_ram_write(Enemy_X_Position + x, a); // store as enemy's horizontal coordinate
        lda_abs(CurrentPageLoc); // add carry to current page number
        adc_imm(0x0);
        dynamic_ram_write(Enemy_PageLoc + x, a); // store as enemy's page coordinate
        lda_imm(0x1);
        dynamic_ram_write(Enemy_Y_HighPos + x, a);
        dynamic_ram_write(Enemy_Flag + x, a); // activate enemy flag
        GetAreaObjYPosition(); // get piranha plant's vertical coordinate and store here
        dynamic_ram_write(Enemy_Y_Position + x, a);
  ...
  dynamic_ram_write(MetatileBuffer + x, a); // render the top of the pipe
  inx();
...
void Hole_Water(void) {
  ...
  dynamic_ram_write(MetatileBuffer + 10, a);
  ldx_imm(0xb);
...
void FlagpoleObject(void) {
  ...
  dynamic_ram_write(MetatileBuffer, a);
  ...
  dynamic_ram_write(Enemy_X_Position + 5, a); // coordinate for the flag
  lda_abs(CurrentPageLoc);
  sbc_imm(0x0); // subtract borrow from page location and use as
  dynamic_ram_write(Enemy_PageLoc + 5, a); // page location for the flag
  lda_imm(0x30);
  dynamic_ram_write(Enemy_Y_Position + 5, a); // set vertical coordinate for flag
  lda_imm(0xb0);
  dynamic_ram_write(FlagpoleFNum_Y_Pos, a); // set initial vertical coordinate for flagpole's floatey number
  lda_imm(FlagpoleFlagObject);
  dynamic_ram_write(Enemy_ID + 5, a); // set flag identifier, note that identifier and coordinates
...
void BulletBillCannon(void) {
  ...
  dynamic_ram_write(Cannon_Y_Position + x, a); // and store it here
  lda_abs(CurrentPageLoc);
  dynamic_ram_write(Cannon_PageLoc + x, a); // store page number for cannon here
  GetAreaObjXPosition(); // get proper horizontal coordinate for cannon
  dynamic_ram_write(Cannon_X_Position + x, a); // and store it here
  inx();
  ...
  // StrCOffset:
  dynamic_ram_write(Cannon_Offset, x); // save new offset and leave
  // --------------------------------
}
...
void StaircaseObject(void) {
  ...
  if (carry_flag) {
    ...
    dynamic_ram_write(StaircaseControl, a); // of the staircase
  }
  // NextStair:
...
void Jumpspring(void) {
  ...
  dynamic_ram_write(Enemy_X_Position + x, a); // and store
  lda_abs(CurrentPageLoc); // store page location of jumpspring
  dynamic_ram_write(Enemy_PageLoc + x, a);
  GetAreaObjYPosition(); // get vertical coordinate for jumpspring
  dynamic_ram_write(Enemy_Y_Position + x, a); // and store
  dynamic_ram_write(Jumpspring_FixedYPos + x, a); // store as permanent coordinate here
  lda_imm(JumpspringObject);
  dynamic_ram_write(Enemy_ID + x, a); // write jumpspring object to enemy object buffer
  ldy_imm(0x1);
  dynamic_ram_write(Enemy_Y_HighPos + x, y); // store vertical high byte
...
void Hidden1UpBlock(void) {
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(Hidden1UpFlag, a);
...
void Hole_Empty(void) {
  ...
  if (carry_flag) {
    ...
    if (zero_flag) {
      ...
      dynamic_ram_write(Whirlpool_LeftExtent + x, a); // store as left extent of whirlpool
      lda_abs(CurrentPageLoc); // get page location of where we're at
      sbc_imm(0x0); // subtract borrow
      dynamic_ram_write(Whirlpool_PageLoc + x, a); // save as page location of whirlpool
      iny();
      ...
      dynamic_ram_write(Whirlpool_Length + x, a); // save size of whirlpool here
      inx();
      ...
      // StrWOffset:
      dynamic_ram_write(Whirlpool_Offset, x); // save new offset here
    }
  }
  // NoWhirlP:
...
void RenderUnderPart(void) {
  dynamic_ram_write(AreaObjectHeight, y); // store vertical length to render
...
DrawThisRow:
  dynamic_ram_write(MetatileBuffer + x, a); // render contents of A from routine that called this
...
void KillEnemies(void) {
  dynamic_ram_write(0x0, a); // store identifier here
  lda_imm(0x0);
...
KillELoop:
  ...
  if (zero_flag) {
    dynamic_ram_write(Enemy_Flag + x, a); // if found, deactivate enemy object flag
  }
  // NoKillE:
...
void GetPipeHeight(void) {
  ...
  dynamic_ram_write(0x6, a); // vertical length, then load Y with
...
void ChkLrgObjFixedLength(void) {
  ...
  if (neg_flag) {
    ...
    dynamic_ram_write(AreaObjectLength + x, a);
...
void GetLrgObjAttrib(void) {
  ...
  and_imm(0b00001111);
  dynamic_ram_write(0x7, a); // save row location
  iny();
...
void GetBlockBufferAddr(void) {
  ...
  dynamic_ram_write(0x6, a); // store here and leave
  // -------------------------------------------------------------------------------------
  // unused space
  //       .db $ff, $ff
  // -------------------------------------------------------------------------------------
...
void LoadAreaPointer(void) {
  ...
  dynamic_ram_write(AreaPointer, a);
...
void GetAreaType(void) {
  ...
  dynamic_ram_write(AreaType, a); // save 2 MSB as area type
}
...
void GetAreaDataAddrs(void) {
  ...
  and_imm(0b00011111);
  dynamic_ram_write(AreaAddrsLOffset, a); // save as low offset
  ...
  dynamic_ram_write(EnemyDataLow, a);
  lda_absy(EnemyDataAddrHigh);
  dynamic_ram_write(EnemyDataHigh, a);
  ...
  dynamic_ram_write(AreaDataLow, a);
  lda_absy(AreaDataAddrHigh);
  dynamic_ram_write(AreaDataHigh, a);
  ...
  cmp_imm(0x4);
  if (carry_flag) {
    dynamic_ram_write(BackgroundColorCtrl, a); // if 4 or greater, save value here as bg color control
    lda_imm(0x0);
  }
  // StoreFore:
  dynamic_ram_write(ForegroundScenery, a); // if less, save value here as foreground scenery
  ...
  lsr_acc();
  lsr_acc();
  dynamic_ram_write(PlayerEntranceCtrl, a); // save value here as player entrance control
  ...
  rol_acc();
  rol_acc();
  dynamic_ram_write(GameTimerSetting, a); // save value here as game timer setting
  iny();
  lda_indy(AreaData); // load second byte of header
  pha(); // save to stack
  and_imm(0b00001111); // mask out all but lower nybble
  dynamic_ram_write(TerrainControl, a);
  ...
  lsr_acc();
  lsr_acc();
  dynamic_ram_write(BackgroundScenery, a); // save as background scenery
  pla();
  and_imm(0b11000000);
  carry_flag = false;
  rol_acc(); // rotate bits over to LSBs
  ...
  if (zero_flag) {
    dynamic_ram_write(CloudTypeOverride, a); // otherwise store value in other place
    lda_imm(0x0);
  }
  // StoreStyle:
  dynamic_ram_write(AreaStyle, a);
...
void ScrollHandler(void) {
  ...
  dynamic_ram_write(Player_X_Scroll, a); // save as new value here to impose force on scroll
...
void ScrollScreen(void) {
  tya();
  dynamic_ram_write(ScrollAmount, a); // save value here
  carry_flag = false;
  adc_abs(ScrollThirtyTwo); // add to value already set here
  dynamic_ram_write(ScrollThirtyTwo, a); // save as new value here
  tya();
  carry_flag = false;
  adc_abs(ScreenLeft_X_Pos); // add to left side coordinate
  dynamic_ram_write(ScreenLeft_X_Pos, a); // save as new left side coordinate
  dynamic_ram_write(HorizontalScroll, a); // save here also
  lda_abs(ScreenLeft_PageLoc);
  adc_imm(0x0); // add carry to page location for left
  dynamic_ram_write(ScreenLeft_PageLoc, a); // side of the screen
  and_imm(0x1); // get LSB of page location
  dynamic_ram_write(0x0, a); // save as temp variable for PPU register 1 mirror
  ...
  dynamic_ram_write(Mirror_PPU_CTRL_REG1, a); // mirror to be used to set name table later
  GetScreenPosition(); // figure out where the right side is
  lda_imm(0x8);
  dynamic_ram_write(ScrollIntervalTimer, a); // set scroll timer (residual, not used elsewhere)
  ChkPOffscr(); // skip this part
}

void InitScrlAmt(void) {
  lda_imm(0x0);
  dynamic_ram_write(ScrollAmount, a); // initialize value here
...
void ChkPOffscr(void) {
  ...
  dynamic_ram_write(0x0, a); // save them here
...
KeepOnscr:
  ...
  dynamic_ram_write(Player_X_Position, a); // store as player position to prevent movement further
  lda_absy(ScreenEdge_PageLoc); // get left or right page location based on offset
  sbc_imm(0x0); // subtract borrow
  dynamic_ram_write(Player_PageLoc, a); // save as player's page location
  ...
  lda_imm(0x0);
  dynamic_ram_write(Player_X_Speed, a); // otherwise nullify horizontal speed of player
  
InitPlatScrl:
  lda_imm(0x0); // nullify platform force imposed on scroll
  dynamic_ram_write(Platform_X_Scroll, a);
}
...
void GetScreenPosition(void) {
  ...
  dynamic_ram_write(ScreenRight_X_Pos, a); // store as coordinate of screen's right boundary
  lda_abs(ScreenLeft_PageLoc); // get page number where left boundary is
  adc_imm(0x0); // add carry from before
  dynamic_ram_write(ScreenRight_PageLoc, a); // store as page number where right boundary is
  // -------------------------------------------------------------------------------------
}
...
VineEntr:
  ...
  lda_imm(0x3);
  dynamic_ram_write(Player_State, a); // otherwise set player state to climbing
  iny(); // increment value in Y
  lda_imm(0x8); // set block in block buffer to cover hole, then
  dynamic_ram_write(Block_Buffer_1 + 0xb4, a); // use same value to force player to climb
  
OffVine:
  dynamic_ram_write(DisableCollisionDet, y); // set collision detection disable flag
...
PlayerRdy:
  ...
  dynamic_ram_write(GameEngineSubroutine, a);
  lda_imm(0x1); // set to face player to the right
  dynamic_ram_write(PlayerFacingDir, a);
  lsr_acc(); // init A
  dynamic_ram_write(AltEntranceControl, a); // init mode of entry
  dynamic_ram_write(DisableCollisionDet, a); // init collision detection disable flag
  dynamic_ram_write(JoypadOverride, a); // nullify controller override bits
  // -------------------------------------------------------------------------------------
  // $07 - used to hold upper limit of high byte when player falls down hole
}

void AutoControlPlayer(void) {
  dynamic_ram_write(SavedJoypadBits, a); // override controller bits with contents of A if executing here
...
SaveJoyp:
  ...
  lda_imm(0x0);
  dynamic_ram_write(Left_Right_Buttons, a); // if pressing down while on the ground,
  dynamic_ram_write(Up_Down_Buttons, a); // nullify directional bits
...
ChkMoveDir:
  dynamic_ram_write(Player_BoundBoxCtrl, y); // set contents of Y as player's bounding box size control
...
SetMoveDir:
  dynamic_ram_write(Player_MovingDir, a); // set moving direction
...
PlayerSubs:
  ...
  dynamic_ram_write(Player_SprAttrib, a); // background priority flag
...
PlayerHole:
  ...
  ldx_imm(0x1);
  dynamic_ram_write(ScrollLock, x); // set scroll lock
  ldy_imm(0x4);
  dynamic_ram_write(0x7, y); // set value here
...
HoleDie:
  ...
  iny();
  dynamic_ram_write(EventMusicQueue, y); // otherwise play death music
  dynamic_ram_write(DeathMusicLoaded, y); // and set value here
  
HoleBottom:
  ldy_imm(0x6);
  dynamic_ram_write(0x7, y); // change value here
...
ChkHoleX:
  ...
  dynamic_ram_write(GameEngineSubroutine, a); // on next frame
...
CloudExit:
  lda_imm(0x0);
  dynamic_ram_write(JoypadOverride, a); // clear controller override bits if any are set
...
void ChgAreaPipe(void) {
  dec_abs(ChangeAreaTimer); // decrement timer for change of area
  if (zero_flag) {
    dynamic_ram_write(AltEntranceControl, y); // when timer expires set mode of alternate entry
...
void ChgAreaMode(void) {
  inc_abs(DisableScreenFlag); // set flag to disable screen output
  lda_imm(0x0);
  dynamic_ram_write(OperMode_Task, a); // set secondary mode of operation
  dynamic_ram_write(Sprite0HitDetectFlag, a); // disable sprite 0 check
}
...
void InitChangeSize(void) {
  ...
  if (zero_flag) {
    dynamic_ram_write(PlayerAnimCtrl, y); // otherwise initialize player's animation frame control
    inc_abs(PlayerChangeSizeFlag); // set growing/shrinking flag
    lda_abs(PlayerSize);
    eor_imm(0x1); // invert player's size
    dynamic_ram_write(PlayerSize, a);
    // -------------------------------------------------------------------------------------
...
void DonePlayerTask(void) {
  lda_imm(0x0);
  dynamic_ram_write(TimerControl, a); // initialize master timer control to continue timers
  lda_imm(0x8);
  dynamic_ram_write(GameEngineSubroutine, a); // set player control routine to run next frame
}
...
void CyclePlayerPalette(void) {
  ...
  dynamic_ram_write(0x0, a); // store result here to use as palette bits
  ...
  dynamic_ram_write(Player_SprAttrib, a); // store as new player attributes
}
...
void FlagpoleSlide(void) {
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(Square1SoundQueue, a); // into square 1's sfx queue
    lda_imm(0x0);
    dynamic_ram_write(FlagpoleSoundQueue, a); // init flagpole sound queue
...
void PlayerEndLevel(void) {
  ...
  if (carry_flag) {
    ...
    if (!zero_flag) {
      ...
      dynamic_ram_write(EventMusicQueue, a); // load win level music in event music queue
      lda_imm(0x0);
      dynamic_ram_write(ScrollLock, a); // turn off scroll lock to skip this part later
    }
  }
  // ChkStop:
  ...
  if (!carry_flag) {
    ...
    dynamic_ram_write(Player_SprAttrib, a); // give illusion of being inside the castle
  }
  // RdyNextA:
...
void NextArea(void) {
  ...
  dynamic_ram_write(HalfwayPage, a); // reset halfway page to 0 (beginning)
  lda_imm(Silence);
  dynamic_ram_write(EventMusicQueue, a); // silence music and leave
  // -------------------------------------------------------------------------------------
}
...
void MovePlayerYAxis(void) {
  ...
  dynamic_ram_write(Player_Y_Position, a);
  // -------------------------------------------------------------------------------------
}

void EnterSidePipe(void) {
  lda_imm(0x8); // set player's horizontal speed
  dynamic_ram_write(Player_X_Speed, a);
  ...
  if (zero_flag) {
    dynamic_ram_write(Player_X_Speed, a); // if lower nybble = 0, set as horizontal speed
...
SetCrouch:
  dynamic_ram_write(CrouchingFlag, a); // store value in crouch flag
...
ProcMove:
  ...
  ldy_imm(0x18);
  dynamic_ram_write(ClimbSideTimer, y); // otherwise reset timer now
...
void OnGroundStateSub(void) {
  ...
  if (!zero_flag) {
    dynamic_ram_write(PlayerFacingDir, a); // otherwise set new facing direction
  }
  // GndMove:
  ...
  dynamic_ram_write(Player_X_Scroll, a); // set returned value as player's movement speed for scroll
  // --------------------------------
}

void FallingSub(void) {
  lda_abs(VerticalForceDown);
  dynamic_ram_write(VerticalForce, a); // dump vertical movement force for falling into main one
...
DumpFall:
  ...
  dynamic_ram_write(VerticalForce, a);
...
ProcSwim:
  ...
  lda_imm(0x18);
  dynamic_ram_write(VerticalForce, a); // otherwise set fractional
...
LRWater:
  ...
  dynamic_ram_write(PlayerFacingDir, a); // otherwise set facing direction accordingly
  LRAir(); // <fallthrough>
}
...
void LRAir(void) {
  ...
  dynamic_ram_write(Player_X_Scroll, a); // set player's speed here, to be used for scroll later
  lda_zp(GameEngineSubroutine);
...
void ClimbingSub(void) {
  ...
  dynamic_ram_write(Player_YMF_Dummy, a);
  ...
  // MoveOnVine:
  dynamic_ram_write(0x0, y); // store adder here
  adc_zp(Player_Y_Position); // add carry to player's vertical position
  dynamic_ram_write(Player_Y_Position, a); // and store to move player up or down
  lda_zp(Player_Y_HighPos);
  adc_zp(0x0); // add carry to player's page location
  dynamic_ram_write(Player_Y_HighPos, a); // and store
  ...
  if (!zero_flag) {
    ...
    if (zero_flag) {
      ldy_imm(0x18);
      dynamic_ram_write(ClimbSideTimer, y); // otherwise set timer now
      ...
      dynamic_ram_write(Player_X_Position, a);
      ...
      dynamic_ram_write(Player_PageLoc, a);
      lda_zp(Left_Right_Buttons); // get left/right controller bits again
      eor_imm(0b00000011); // invert them and store them while player
      dynamic_ram_write(PlayerFacingDir, a); // is on vine to face player in opposite direction
    }
    // ExitCSub:
    return; // then leave
  }
  // InitCSTimer:
  dynamic_ram_write(ClimbSideTimer, a); // initialize timer here
  // -------------------------------------------------------------------------------------
...
InitJS:
  ...
  dynamic_ram_write(JumpSwimTimer, a);
  ldy_imm(0x0); // initialize vertical force and dummy variable
  dynamic_ram_write(Player_YMF_Dummy, y);
  dynamic_ram_write(Player_Y_MoveForce, y);
  lda_zp(Player_Y_HighPos); // get vertical high and low bytes of jump origin
  dynamic_ram_write(JumpOrigin_Y_HighPos, a); // and store them next to each other here
  lda_zp(Player_Y_Position);
  dynamic_ram_write(JumpOrigin_Y_Position, a);
...
ChkWtr:
  ...
  dynamic_ram_write(DiffToHaltJump, a);
...
GetYPhy:
  ...
  dynamic_ram_write(VerticalForce, a); // data here
  lda_absy(FallMForceData);
  dynamic_ram_write(VerticalForceDown, a);
  lda_absy(InitMForceData);
  dynamic_ram_write(Player_Y_MoveForce, a);
  lda_absy(PlayerYSpdData);
  dynamic_ram_write(Player_Y_Speed, a);
  lda_abs(SwimmingFlag); // if swimming flag disabled, branch
  if (zero_flag) { goto PJumpSnd; }
  lda_imm(Sfx_EnemyStomp); // load swim/goomba stomp sound into
  dynamic_ram_write(Square1SoundQueue, a); // square 1's sfx queue
  lda_zp(Player_Y_Position);
  ...
  dynamic_ram_write(Player_Y_Speed, a); // and jump to something else to keep player
...
SJumpSnd:
  dynamic_ram_write(Square1SoundQueue, a); // store appropriate jump sound in square 1 sfx queue
  
X_Physics:
  ldy_imm(0x0);
  dynamic_ram_write(0x0, y); // init value here
...
SetRTmr:
  ...
  dynamic_ram_write(RunningTimer, a);
  
GetXPhy:
  lda_absy(MaxLeftXSpdData); // get maximum speed to the left
  dynamic_ram_write(MaximumLeftSpeed, a);
...
GetXPhy2:
  ...
  dynamic_ram_write(MaximumRightSpeed, a);
  ldy_zp(0x0); // get other value in memory
  lda_absy(FrictionData); // get value using value in memory as offset
  dynamic_ram_write(FrictionAdderLow, a);
  lda_imm(0x0);
  dynamic_ram_write(FrictionAdderHigh, a); // init something here
...
SetRunSpd:
  dynamic_ram_write(RunningSpeed, a); // store zero or running speed here
  goto SetAnimSpd;
...
ProcSkid:
  ...
  lda_zp(PlayerFacingDir);
  dynamic_ram_write(Player_MovingDir, a); // otherwise use facing direction to set moving direction
  lda_imm(0x0);
  dynamic_ram_write(Player_X_Speed, a); // nullify player's horizontal speed
  dynamic_ram_write(Player_X_MoveForce, a); // and dummy variable for player
  
SetAnimSpd:
  lda_absy(PlayerAnimTmrData); // get animation timer setting using Y as offset
  dynamic_ram_write(PlayerAnimTimerSet, a);
  // -------------------------------------------------------------------------------------
}
...
LeftFrict:
  ...
  dynamic_ram_write(Player_X_MoveForce, a); // store here
  lda_zp(Player_X_Speed);
  adc_abs(FrictionAdderHigh); // add value plus carry to horizontal speed
  dynamic_ram_write(Player_X_Speed, a); // set as new horizontal speed
  ...
  dynamic_ram_write(Player_X_Speed, a); // thus slowing the player's left movement down
...
RghtFrict:
  ...
  dynamic_ram_write(Player_X_Speed, a); // thus slowing the player's right movement down
...
SetAbsSpd:
  dynamic_ram_write(Player_XSpeedAbsolute, a); // store walking/running speed here and leave
  // -------------------------------------------------------------------------------------
          ...
          if (zero_flag) {
            ...
            if (zero_flag) {
              ...
              if (!zero_flag) {
                ...
                dynamic_ram_write(Square1SoundQueue, a);
                lda_imm(0x2); // load state
                dynamic_ram_write(Fireball_State + x, a);
                ldy_abs(PlayerAnimTimerSet); // copy animation frame timer setting
                dynamic_ram_write(FireballThrowingTimer, y); // into fireball throwing timer
                dey();
                dynamic_ram_write(PlayerAnimTimer, y); // decrement and store in player's animation timer
...
BublLoop:
    dynamic_ram_write(ObjectOffset, x); // store offset
...
void FireballObjCore(void) {
  dynamic_ram_write(ObjectOffset, x); // store offset as current object
  ...
  if (!carry_flag) {
    ...
    if (!zero_flag) {
      ...
      if (!zero_flag) {
        ...
        dynamic_ram_write(Fireball_X_Speed + x, a);
        lda_imm(0x4); // set vertical speed of fireball
        dynamic_ram_write(Fireball_Y_Speed + x, a);
        lda_imm(0x7);
        dynamic_ram_write(Fireball_BoundBoxCtrl + x, a); // set bounding box size control for fireball
...
void BubbleCheck(void) {
  ...
  and_imm(0x1);
  dynamic_ram_write(0x7, a); // store pseudorandom bit here
...
void SetupBubble(void) {
  ...
  dynamic_ram_write(Bubble_X_Position + x, a); // save as horizontal position for airbubble
  lda_zp(Player_PageLoc);
  adc_imm(0x0); // add carry to player's page location
  dynamic_ram_write(Bubble_PageLoc + x, a); // save as page location for airbubble
  lda_zp(Player_Y_Position);
  carry_flag = false; // add eight pixels to player's vertical position
  adc_imm(0x8);
  dynamic_ram_write(Bubble_Y_Position + x, a); // save as vertical position for air bubble
  lda_imm(0x1);
  dynamic_ram_write(Bubble_Y_HighPos + x, a); // set vertical high byte for air bubble
  ldy_zp(0x7); // get pseudorandom bit, use as offset
  lda_absy(BubbleTimerData); // get data for air bubble timer
  dynamic_ram_write(AirBubbleTimer, a); // set air bubble timer
...
void MoveBubl(void) {
  ...
  dynamic_ram_write(Bubble_YMF_Dummy + x, a); // save dummy variable
  ...
  // Y_Bubl:
  dynamic_ram_write(Bubble_Y_Position + x, a); // store as new vertical coordinate for air bubble
}
...
void RunGameTimer(void) {
  ...
  dynamic_ram_write(EventMusicQueue, a); // otherwise load time running out music
  
ResGTCtrl:
  lda_imm(0x18); // reset game timer control
  dynamic_ram_write(GameTimerCtrlTimer, a);
  ldy_imm(0x23); // set offset for last digit
  lda_imm(0xff); // set value to decrement game timer digit
  dynamic_ram_write(DigitModifier + 5, a);
...
TimeUpOn:
  dynamic_ram_write(PlayerStatus, a); // init player status (note A will always be zero here)
...
void WarpZoneObject(void) {
  ...
  dynamic_ram_write(ScrollLock, a); // otherwise nullify scroll lock flag
...
void ProcessWhirlpools(void) {
  ...
  dynamic_ram_write(Whirlpool_Flag, a); // otherwise initialize whirlpool flag
...
WhLoop:
  ...
  dynamic_ram_write(0x2, a); // store result as right extent here
  lda_absy(Whirlpool_PageLoc); // get page location
  if (zero_flag) { goto NextWh; } // if none or page 0, branch to get next data
  adc_imm(0x0); // add carry
  dynamic_ram_write(0x1, a); // store result as page location of right extent here
  lda_zp(Player_X_Position); // get player's horizontal position
  carry_flag = true;
...
WhirlpoolActivate:
  ...
  dynamic_ram_write(0x1, a); // save as center of whirlpool
  lda_absy(Whirlpool_PageLoc); // get page location
  adc_imm(0x0); // add carry
  dynamic_ram_write(0x0, a); // save as page location of whirlpool center
  lda_zp(FrameCounter); // get frame counter
  ...
  dynamic_ram_write(Player_X_Position, a); // set player's new horizontal coordinate
  lda_zp(Player_PageLoc);
  sbc_imm(0x0); // subtract borrow
...
SetPWh:
  dynamic_ram_write(Player_PageLoc, a); // set player's new page location
  
WhPull:
  lda_imm(0x10);
  dynamic_ram_write(0x0, a); // set vertical movement force
  lda_imm(0x1);
  dynamic_ram_write(Whirlpool_Flag, a); // set whirlpool flag to be used later
  dynamic_ram_write(0x2, a); // also set maximum vertical speed
  lsr_acc();
...
void ImposeGravity(void) {
  ...
  dynamic_ram_write(SprObject_YMF_Dummy + x, a);
...
AlterYP:
  ...
  lda_zp(0x2);
  dynamic_ram_write(SprObject_Y_Speed + x, a); // keep vertical speed within maximum value
  lda_imm(0x0);
  dynamic_ram_write(SprObject_Y_MoveForce + x, a); // clear fractional
...
ChkUpM:
  ...
  tay();
  iny();
  dynamic_ram_write(0x7, y); // store two's compliment here
  lda_absx(SprObject_Y_MoveForce);
  ...
  dynamic_ram_write(SprObject_Y_MoveForce + x, a); // thus it effectively undoes add we did earlier
...
void FlagpoleRoutine(void) {
  ...
  dynamic_ram_write(ObjectOffset, x); // to special use slot
  lda_zpx(Enemy_ID);
...
GiveFPScr:
  ...
  dynamic_ram_write(DigitModifier + x, a); // store in digit modifier
  AddToScore(); // do sub to award player points depending on height of collision
  lda_imm(0x5);
  dynamic_ram_write(GameEngineSubroutine, a); // set to run end-of-level subroutine on next frame
...
PosJSpr:
  ...
  dynamic_ram_write(Enemy_Y_Position + x, a); // store as new vertical position
  ...
  lda_imm(0xf4);
  dynamic_ram_write(JumpspringForce, a); // otherwise write new jumpspring force here
...
BounceJS:
  ...
  dynamic_ram_write(Player_Y_Speed, a); // store jumpspring force as player's new vertical speed
  lda_imm(0x0);
  dynamic_ram_write(JumpspringAnimCtrl, a); // initialize jumpspring frame control
...
DrawJSpr:
  ...
  lda_imm(0x4);
  dynamic_ram_write(JumpspringTimer, a); // otherwise initialize jumpspring timer
...
void Setup_Vine(void) {
  ...
  dynamic_ram_write(Enemy_ID + x, a); // store in buffer
  lda_imm(0x1);
  dynamic_ram_write(Enemy_Flag + x, a); // set flag for enemy object buffer
  lda_zpy(Block_PageLoc);
  dynamic_ram_write(Enemy_PageLoc + x, a); // copy page location from previous object
  lda_zpy(Block_X_Position);
  dynamic_ram_write(Enemy_X_Position + x, a); // copy horizontal coordinate from previous object
  lda_zpy(Block_Y_Position);
  dynamic_ram_write(Enemy_Y_Position + x, a); // copy vertical coordinate from previous object
  ldy_abs(VineFlagOffset); // load vine flag/offset to next available vine slot
  // if set at all, don't bother to store vertical
  if (zero_flag) {
    dynamic_ram_write(VineStart_Y_Position, a); // otherwise store vertical coordinate here
  }
  // NextVO:
  txa(); // store object offset to next available vine slot
  dynamic_ram_write(VineObjOffset + y, a); // using vine flag as offset
  inc_abs(VineFlagOffset); // increment vine flag offset
  lda_imm(Sfx_GrowVine);
  dynamic_ram_write(Square2SoundQueue, a); // load vine grow sound
  // -------------------------------------------------------------------------------------
  ...
  if (zero_flag) {
    ...
    if (!zero_flag) {
      ...
      if (carry_flag) {
        ...
        dynamic_ram_write(Enemy_Y_Position + 5, a); // one pixel every frame it's time
...
KillVine:
        ...
        dynamic_ram_write(VineFlagOffset, a); // initialize vine flag/offset
        dynamic_ram_write(VineHeight, a); // initialize vine height
      }
      // WrCMTile:
...
ThreeSChk:
    dynamic_ram_write(ObjectOffset, x); // start at third enemy slot
      ...
      if (!carry_flag) {
        ...
        if (!zero_flag) {
          ...
          if (!zero_flag) {
            ...
            dynamic_ram_write(Cannon_Timer + y, a); // to count timer down
          ...
          if (zero_flag) {
            ...
            dynamic_ram_write(Cannon_Timer + y, a); // first, reset cannon timer
            lda_absy(Cannon_PageLoc); // get page location of cannon
            dynamic_ram_write(Enemy_PageLoc + x, a); // save as page location of bullet bill
            lda_absy(Cannon_X_Position); // get horizontal coordinate of cannon
            dynamic_ram_write(Enemy_X_Position + x, a); // save as horizontal coordinate of bullet bill
            ...
            dynamic_ram_write(Enemy_Y_Position + x, a); // save as vertical coordinate of bullet bill
            lda_imm(0x1);
            dynamic_ram_write(Enemy_Y_HighPos + x, a); // set vertical high byte of bullet bill
            dynamic_ram_write(Enemy_Flag + x, a); // set buffer flag
            lsr_acc(); // shift right once to init A
            dynamic_ram_write(Enemy_State + x, a); // then initialize enemy's state
            lda_imm(0x9);
            dynamic_ram_write(Enemy_BoundBoxCtrl + x, a); // set bounding box size control for bullet bill
            lda_imm(BulletBill_CannonVar);
            dynamic_ram_write(Enemy_ID + x, a); // load identifier for bullet bill (cannon variant)
...
SetupBB:
  dynamic_ram_write(Enemy_MovingDir + x, y); // set bullet bill's moving direction
  dey(); // decrement to use as offset
  lda_absy(BulletBillXSpdData); // get horizontal speed based on moving direction
  dynamic_ram_write(Enemy_X_Speed + x, a); // and store it
  ...
  lda_imm(0x1);
  dynamic_ram_write(Enemy_State + x, a); // otherwise set bullet bill's state
  lda_imm(0xa);
  dynamic_ram_write(EnemyFrameTimer + x, a); // set enemy frame timer
  lda_imm(Sfx_Blast);
  dynamic_ram_write(Square2SoundQueue, a); // play fireworks/gunfire sound
...
void EnemyGfxHandler(void) {
  ...
  dynamic_ram_write(0x5, a); // relative to screen
  ldy_absx(Enemy_SprDataOffset);
  dynamic_ram_write(0xeb, y); // get sprite data offset
  lda_imm(0x0);
  dynamic_ram_write(VerticalFlipFlag, a); // initialize vertical flip flag by default
  lda_zpx(Enemy_MovingDir);
  dynamic_ram_write(0x3, a); // get enemy object moving direction
  lda_absx(Enemy_SprAttrib);
  dynamic_ram_write(0x4, a); // get enemy object sprite attributes
  lda_zpx(Enemy_ID);
...
SBBAt:
  dynamic_ram_write(0x4, a); // set new sprite attributes
  ldy_imm(0x0); // nullify saved enemy state both in Y and in
  dynamic_ram_write(0xed, y); // memory location here
...
CheckForPodoboo:
  dynamic_ram_write(0xef, a); // store saved enemy object value here
  dynamic_ram_write(0xec, y); // and Y here (enemy state -2 MSB if not changed)
...
GmbaAnim:
  ...
  dynamic_ram_write(0x3, a); // leave alone otherwise
...
FlipBowserOver:
  dynamic_ram_write(VerticalFlipFlag, x); // set vertical flip flag to nonzero
...
CheckForSpiny:
  ...
  lda_imm(0x2);
  dynamic_ram_write(0x3, a); // set enemy direction to reverse sprites horizontally
  lda_imm(0x5);
  dynamic_ram_write(0xec, a); // set enemy state
...
CheckDefeatedState:
  ...
  ldy_imm(0x1);
  dynamic_ram_write(VerticalFlipFlag, y); // set vertical flip flag
  dey();
  dynamic_ram_write(0xec, y); // init saved value here
...
FlipEnemyVertically:
  ...
  dynamic_ram_write(Sprite_Tilenumber + x, a); // with first or second row tiles
  lda_absy(Sprite_Tilenumber + 20);
  dynamic_ram_write(Sprite_Tilenumber + 4 + x, a);
  pla(); // pull first or second row tiles from stack
  dynamic_ram_write(Sprite_Tilenumber + 20 + y, a); // and save in third row
  pla();
  dynamic_ram_write(Sprite_Tilenumber + 16 + y, a);
...
ESRtnr:
  ...
  dynamic_ram_write(Sprite_Attributes + 20 + y, a); // note that palette bits were already set earlier
...
MirrorEnemyGfx:
  ...
  and_imm(0b10100011);
  dynamic_ram_write(Sprite_Attributes + y, a); // save vertical flip, priority, and palette bits
  dynamic_ram_write(Sprite_Attributes + 8 + y, a); // in left sprite column of enemy object OAM data
  dynamic_ram_write(Sprite_Attributes + 16 + y, a);
...
EggExc:
  dynamic_ram_write(Sprite_Attributes + 4 + y, a); // set bits of right sprite column
  dynamic_ram_write(Sprite_Attributes + 12 + y, a); // of enemy object sprite data
  dynamic_ram_write(Sprite_Attributes + 20 + y, a);
  ...
  ora_imm(0b10000000);
  dynamic_ram_write(Sprite_Attributes + 8 + y, a); // store bits with vertical flip in
  dynamic_ram_write(Sprite_Attributes + 16 + y, a); // second and third row left sprites
  ora_imm(0b01000000);
  dynamic_ram_write(Sprite_Attributes + 12 + y, a); // store with horizontal and vertical flip in
  dynamic_ram_write(Sprite_Attributes + 20 + y, a); // second and third row right sprites
...
CheckToMirrorLakitu:
  ...
  dynamic_ram_write(Sprite_Attributes + 12 + y, a); // otherwise set same for second row right sprite
  and_imm(0b10000001);
  dynamic_ram_write(Sprite_Attributes + 8 + y, a); // preserve vertical flip and palette bits for left sprite
...
NVFLak:
  ...
  and_imm(0b10000001);
  dynamic_ram_write(Sprite_Attributes + y, a); // save vertical flip and palette bits
  ...
  dynamic_ram_write(Sprite_Attributes + 4 + y, a); // note that vertical flip is left as-is
...
CheckToMirrorJSpring:
  ...
  lda_imm(0x82);
  dynamic_ram_write(Sprite_Attributes + 8 + y, a); // set vertical flip and palette bits of
  dynamic_ram_write(Sprite_Attributes + 16 + y, a); // second and third row left sprites
  ora_imm(0b01000000);
  dynamic_ram_write(Sprite_Attributes + 12 + y, a); // set, in addition to those, horizontal flip
  dynamic_ram_write(Sprite_Attributes + 20 + y, a); // for second and third row right sprites
...
void SpawnHammerObj(void) {
  ...
  if (zero_flag) {
    ...
    if (zero_flag) {
      ...
      txa();
      dynamic_ram_write(HammerEnemyOffset + y, a); // save here
      lda_imm(0x90);
      dynamic_ram_write(Misc_State + y, a); // save hammer's state here
      lda_imm(0x7);
      dynamic_ram_write(Misc_BoundBoxCtrl + y, a); // set something else entirely, here
...
void ProcHammerObj(void) {
  ...
  lda_imm(0x10);
  dynamic_ram_write(0x0, a); // set downward movement force
  lda_imm(0xf);
  dynamic_ram_write(0x1, a); // set upward movement force (not used)
  lda_imm(0x4);
  dynamic_ram_write(0x2, a); // set maximum vertical speed
...
SetHSpd:
  lda_imm(0xfe);
  dynamic_ram_write(Misc_Y_Speed + x, a); // set hammer's vertical speed
  lda_zpy(Enemy_State); // get enemy object state
  and_imm(0b11110111); // mask out d3
  dynamic_ram_write(Enemy_State + y, a); // store new state
  ...
  dynamic_ram_write(Misc_X_Speed + x, a); // set hammer's horizontal speed
...
SetHPos:
  ...
  dynamic_ram_write(Misc_X_Position + x, a); // store as hammer's horizontal position
  lda_zpy(Enemy_PageLoc); // get enemy's page location
  adc_imm(0x0); // add carry
  dynamic_ram_write(Misc_PageLoc + x, a); // store as hammer's page location
  lda_zpy(Enemy_Y_Position); // get enemy's vertical position
  carry_flag = true;
  sbc_imm(0xa); // move position 10 pixels upward
  dynamic_ram_write(Misc_Y_Position + x, a); // store as hammer's vertical position
  lda_imm(0x1);
  dynamic_ram_write(Misc_Y_HighPos + x, a); // set hammer's vertical high byte
...
void CoinBlock(void) {
  ...
  dynamic_ram_write(Misc_PageLoc + y, a); // store as page location of misc object
  lda_zpx(Block_X_Position); // get horizontal coordinate of block object
  ora_imm(0x5); // add 5 pixels
  dynamic_ram_write(Misc_X_Position + y, a); // store as horizontal coordinate of misc object
  lda_zpx(Block_Y_Position); // get vertical coordinate of block object
  sbc_imm(0x10); // subtract 16 pixels
  dynamic_ram_write(Misc_Y_Position + y, a); // store as vertical coordinate of misc object
...
void SetupJumpCoin(void) {
  ...
  dynamic_ram_write(Misc_PageLoc + y, a); // and save as page location for misc object
  ...
  dynamic_ram_write(Misc_X_Position + y, a); // save as horizontal coordinate for misc object
  lda_zp(0x2); // get vertical high nybble offset from earlier
  adc_imm(0x20); // add 32 pixels for the status bar
  dynamic_ram_write(Misc_Y_Position + y, a); // store as vertical coordinate
  JCoinC(); // <fallthrough>
}

void JCoinC(void) {
  lda_imm(0xfb);
  dynamic_ram_write(Misc_Y_Speed + y, a); // set vertical speed
  lda_imm(0x1);
  dynamic_ram_write(Misc_Y_HighPos + y, a); // set vertical high byte
  dynamic_ram_write(Misc_State + y, a); // set state for misc object
  dynamic_ram_write(Square2SoundQueue, a); // load coin grab sound
  dynamic_ram_write(ObjectOffset, x); // store current control bit as misc object offset
...
FMiscLoop:
  ...
  // UseMiscS:
  dynamic_ram_write(JumpCoinMiscOffset, y); // store offset of misc object buffer here (residual)
  // -------------------------------------------------------------------------------------
}
...
MiscLoop:
  dynamic_ram_write(ObjectOffset, x); // store misc object offset here
...
ProcJumpCoin:
  ...
  dynamic_ram_write(Misc_X_Position + x, a); // store as new horizontal coordinate
  lda_zpx(Misc_PageLoc); // get page location
  adc_imm(0x0); // add carry
  dynamic_ram_write(Misc_PageLoc + x, a); // store as new page location
  lda_zpx(Misc_State);
  cmp_imm(0x30); // check state of object for preset value
  if (!zero_flag) { goto RunJCSubs; } // if not yet reached, branch to subroutines
  lda_imm(0x0);
  dynamic_ram_write(Misc_State + x, a); // otherwise nullify object state
...
JCoinRun:
  ...
  dynamic_ram_write(0x1, a); // as upward movement amount (apparently residual)
...
void GiveOneCoin(void) {
  ...
  dynamic_ram_write(DigitModifier + 5, a); // to the current player's coin tally
  ...
  if (zero_flag) {
    lda_imm(0x0);
    dynamic_ram_write(CoinTally, a); // otherwise, reinitialize coin amount
    inc_abs(NumberofLives); // give the player an extra life
    lda_imm(Sfx_ExtraLife);
    dynamic_ram_write(Square2SoundQueue, a); // play 1-up sound
  }
  // CoinPoints:
  lda_imm(0x2); // set digit modifier to award
  dynamic_ram_write(DigitModifier + 4, a); // 200 points to the player
...
void UpdateNumber(void) {
  ...
  if (zero_flag) {
    lda_imm(0x24);
    dynamic_ram_write(VRAM_Buffer1 - 6 + y, a);
  }
  // NoZSup:
...
void SetupPowerUp(void) {
  ...
  dynamic_ram_write(Enemy_ID + 5, a); // special use slot of enemy object buffer
  lda_zpx(Block_PageLoc); // store page location of block object
  dynamic_ram_write(Enemy_PageLoc + 5, a); // as page location of power-up object
  lda_zpx(Block_X_Position); // store horizontal coordinate of block object
  dynamic_ram_write(Enemy_X_Position + 5, a); // as horizontal coordinate of power-up object
  lda_imm(0x1);
  dynamic_ram_write(Enemy_Y_HighPos + 5, a); // set vertical high byte of power-up object
  lda_zpx(Block_Y_Position); // get vertical coordinate of block object
  carry_flag = true;
  sbc_imm(0x8); // subtract 8 pixels
  dynamic_ram_write(Enemy_Y_Position + 5, a); // and use as vertical coordinate of power-up object
  PwrUpJmp(); // <fallthrough>
}

void PwrUpJmp(void) {
  lda_imm(0x1); // this is a residual jump point in enemy object jump table
  dynamic_ram_write(Enemy_State + 5, a); // set power-up object's state
  dynamic_ram_write(Enemy_Flag + 5, a); // set buffer flag
  lda_imm(0x3);
  dynamic_ram_write(Enemy_BoundBoxCtrl + 5, a); // set bounding box size control for power-up object
  lda_zp(PowerUpType);
  ...
  if (!carry_flag) {
    ...
    // StrType:
    dynamic_ram_write(PowerUpType, a); // store type here
  }
  // PutBehind:
  lda_imm(0b00100000);
  dynamic_ram_write(Enemy_SprAttrib + 5, a); // set background priority bit
  lda_imm(Sfx_GrowPowerUp);
  dynamic_ram_write(Square2SoundQueue, a); // load power-up reveal sound and leave
  // -------------------------------------------------------------------------------------
}
...
GrowThePowerUp:
  ...
  lda_imm(0x10);
  dynamic_ram_write(Enemy_X_Speed + x, a); // otherwise set horizontal speed
  lda_imm(0b10000000);
  dynamic_ram_write(Enemy_State + 5, a); // and then set d7 in power-up object's state
  asl_acc(); // shift once to init A
  dynamic_ram_write(Enemy_SprAttrib + 5, a); // initialize background priority bit set here
  rol_acc(); // rotate A to set right moving direction
  dynamic_ram_write(Enemy_MovingDir + x, a); // set moving direction
...
DBlockSte:
  dynamic_ram_write(Block_State + x, a); // store into block object buffer
  ...
  lda_zp(0x2); // get vertical high nybble offset used in block buffer routine
  dynamic_ram_write(Block_Orig_YPos + x, a); // set as vertical coordinate for block object
  tay();
  lda_zp(0x6); // get low byte of block buffer address used in same routine
  dynamic_ram_write(Block_BBuf_Low + x, a); // save as offset here to be used later
  ...
  dynamic_ram_write(0x0, a); // store metatile here
  ldy_abs(PlayerSize); // check player's size
...
ChkBrick:
  ...
  dynamic_ram_write(Block_State + x, y); // note this applies to both player sizes
...
StartBTmr:
  ...
  lda_imm(0xb);
  dynamic_ram_write(BrickCoinTimer, a); // if not set, set brick coin timer
...
PutMTileB:
  dynamic_ram_write(Block_Metatile + x, a); // store whatever metatile be appropriate here
  ...
  lda_imm(0x10);
  dynamic_ram_write(BlockBounceTimer, a); // set block bounce timer
  pla(); // pull original metatile from stack
  dynamic_ram_write(0x5, a); // and save here
...
BigBP:
  ...
  dynamic_ram_write(Block_Y_Position + x, a); // save as vertical coordinate for block object
...
InvOBit:
  ...
  dynamic_ram_write(SprDataOffset_Ctrl, a);
  // --------------------------------
}
...
void InitBlock_XY_Pos(void) {
  ...
  dynamic_ram_write(Block_X_Position + x, a); // save as horizontal coordinate for block object
  lda_zp(Player_PageLoc);
  adc_imm(0x0); // add carry to page location of player
  dynamic_ram_write(Block_PageLoc + x, a); // save as page location of block object
  dynamic_ram_write(Block_PageLoc2 + x, a); // save elsewhere to be used later
  lda_zp(Player_Y_HighPos);
  dynamic_ram_write(Block_Y_HighPos + x, a); // save vertical high byte of player into
  // --------------------------------
}
...
void BumpBlock(void) {
  ...
  lda_imm(Sfx_Bump);
  dynamic_ram_write(Square1SoundQueue, a); // play bump sound
  lda_imm(0x0);
  dynamic_ram_write(Block_X_Speed + x, a); // initialize horizontal speed for block object
  dynamic_ram_write(Block_Y_MoveForce + x, a); // init fractional movement force
  dynamic_ram_write(Player_Y_Speed, a); // init player's vertical speed
  lda_imm(0xfe);
  dynamic_ram_write(Block_Y_Speed + x, a); // set vertical speed for block object
...
void ExtraLifeMushBlockSkip(void) {
  dynamic_ram_write(0x39, a); // store correct power-up type
  SetupPowerUp();
}
...
void BrickShatter(void) {
  ...
  dynamic_ram_write(Block_RepFlag + x, a); // set flag for block object to immediately replace metatile
  dynamic_ram_write(NoiseSoundQueue, a); // load brick shatter sound
  SpawnBrickChunks(); // create brick chunk objects
  lda_imm(0xfe);
  dynamic_ram_write(Player_Y_Speed, a); // set vertical speed for player
  lda_imm(0x5);
  dynamic_ram_write(DigitModifier + 5, a); // set digit modifier to give player 50 points
...
void CheckTopOfBlock(void) {
  ...
  dynamic_ram_write(0x2, a); // store as new vertical high nybble offset
  tay();
...
void SpawnBrickChunks(void) {
  ...
  dynamic_ram_write(Block_Orig_XPos + x, a); // as original horizontal coordinate here
  lda_imm(0xf0);
  dynamic_ram_write(Block_X_Speed + x, a); // set horizontal speed for brick chunk objects
  dynamic_ram_write(Block_X_Speed + 2 + x, a);
  lda_imm(0xfa);
  dynamic_ram_write(Block_Y_Speed + x, a); // set vertical speed for one
  lda_imm(0xfc);
  dynamic_ram_write(Block_Y_Speed + 2 + x, a); // set lower vertical speed for the other
  lda_imm(0x0);
  dynamic_ram_write(Block_Y_MoveForce + x, a); // init fractional movement force for both
  dynamic_ram_write(Block_Y_MoveForce + 2 + x, a);
  lda_zpx(Block_PageLoc);
  dynamic_ram_write(Block_PageLoc + 2 + x, a); // copy page location
  lda_zpx(Block_X_Position);
  dynamic_ram_write(Block_X_Position + 2 + x, a); // copy horizontal coordinate
  ...
  dynamic_ram_write(Block_Y_Position + 2 + x, a);
  lda_imm(0xfa);
  dynamic_ram_write(Block_Y_Speed + x, a); // set vertical speed...again??? (redundant)
  // -------------------------------------------------------------------------------------
}
...
void BlockObjectsCore(void) {
  ...
  dynamic_ram_write(Block_Y_Position + 2 + x, a); // otherwise set offscreen coordinate
...
BouncingBlockHandler:
  ...
  lda_imm(0x1);
  dynamic_ram_write(Block_RepFlag + x, a); // otherwise set flag to replace metatile
  
KillBlock:
  lda_imm(0x0); // if branched here, nullify object state
  
UpdSte:
  dynamic_ram_write(Block_State + x, a); // store contents of A in block object state
  // -------------------------------------------------------------------------------------
...
UpdateLoop:
  dynamic_ram_write(ObjectOffset, x); // set offset here
  ...
  if (zero_flag) {
    ...
    if (!zero_flag) {
      ...
      dynamic_ram_write(0x6, a); // store into block buffer address
      lda_imm(0x5);
      dynamic_ram_write(0x7, a); // set high byte of block buffer address
      lda_absx(Block_Orig_YPos); // get original vertical coordinate of block object
      dynamic_ram_write(0x2, a); // store here and use as offset to block buffer
      tay();
      ...
      lda_imm(0x0);
      dynamic_ram_write(Block_RepFlag + x, a); // clear block object flag
    }
  }
  // NextBUpd:
...
void MoveObjectHorizontally(void) {
  ...
  asl_acc();
  asl_acc();
  dynamic_ram_write(0x1, a); // store result here
  ...
  // SaveXSpd:
  dynamic_ram_write(0x0, a); // save result here
  ...
  // UseAdder:
  dynamic_ram_write(0x2, y); // save Y here
  lda_absx(SprObject_X_MoveForce); // get whatever number's here
  carry_flag = false;
  adc_zp(0x1); // add low nybble moved to high
  dynamic_ram_write(SprObject_X_MoveForce + x, a); // store result here
  lda_imm(0x0); // init A
  ...
  dynamic_ram_write(SprObject_X_Position + x, a); // plus $f0 if necessary) to object's horizontal position
  lda_zpx(SprObject_PageLoc);
  adc_zp(0x2); // add carry plus other saved value to the
  dynamic_ram_write(SprObject_PageLoc + x, a); // object's page location and save
  pla();
...
void MoveRedPTroopa(void) {
  ...
  lda_imm(0x3);
  dynamic_ram_write(0x0, a); // set downward movement amount here
  lda_imm(0x6);
  dynamic_ram_write(0x1, a); // set upward movement amount here
  lda_imm(0x2);
  dynamic_ram_write(0x2, a); // set maximum speed here
...
void SetXMoveAmt(void) {
  dynamic_ram_write(0x0, y); // set movement amount here
  inx(); // increment X for enemy offset
...
void MovePlatformUpSkip(void) {
  ...
  // SetDplSpd:
  dynamic_ram_write(0x0, a); // save downward movement amount here
...
ChkBowserF:
  ...
  if (!zero_flag) { return; }
  dynamic_ram_write(Enemy_Flag + x, a); // if second enemy flag not set, also clear first one
  // --------------------------------
  // loop command data
}
...
void ExecGameLoopback(void) {
  ...
  dynamic_ram_write(EnemyObjectPageSel, a); // area and enemy objects
  dynamic_ram_write(AreaObjectPageSel, a);
  dynamic_ram_write(EnemyDataOffset, a); // initialize enemy object data offset
  dynamic_ram_write(EnemyObjectPageLoc, a); // and enemy object page control
  lda_absy(AreaDataOfsLoopback); // adjust area object offset based on
  dynamic_ram_write(AreaDataOffset, a); // which loop command we encountered
}
...
InitMLp:
  ...
  dynamic_ram_write(MultiLoopPassCntr, a);
  dynamic_ram_write(MultiLoopCorrectCntr, a);
  
InitLCmd:
  lda_imm(0x0); // initialize loop command flag
  dynamic_ram_write(LoopCommand, a);
  // --------------------------------
  
ChkEnemyFrenzy:
  lda_abs(EnemyFrenzyQueue); // check for enemy object in frenzy queue
  if (zero_flag) { goto ProcessEnemyData; } // if not, skip this part
  dynamic_ram_write(Enemy_ID + x, a); // store as enemy object identifier here
  lda_imm(0x1);
  dynamic_ram_write(Enemy_Flag + x, a); // activate enemy object flag
  lda_imm(0x0);
  dynamic_ram_write(Enemy_State + x, a); // initialize state and frenzy queue
  dynamic_ram_write(EnemyFrenzyQueue, a);
...
CheckRightBounds:
  ...
  adc_imm(0x0);
  dynamic_ram_write(0x6, a); // store page location + carry
...
CheckPageCtrlRow:
  ...
  and_imm(0b00111111);
  dynamic_ram_write(EnemyObjectPageLoc, a); // store as page control for enemy object data
...
PositionEnemyObj:
  ...
  dynamic_ram_write(Enemy_PageLoc + x, a); // for enemy object
  lda_indy(EnemyData); // get first byte of enemy object
  and_imm(0b11110000);
  dynamic_ram_write(Enemy_X_Position + x, a); // store column position
...
StrID:
  dynamic_ram_write(Enemy_ID + x, a); // store enemy object number into buffer
  lda_imm(0x1);
  dynamic_ram_write(Enemy_Flag + x, a); // set flag for enemy in buffer
  InitEnemyObject();
...
StrFre:
  dynamic_ram_write(Enemy_ID + x, a); // store contents of frenzy buffer into enemy identifier value
  InitEnemyObject(); // <fallthrough>
}

void InitEnemyObject(void) {
  lda_imm(0x0); // initialize enemy state
  dynamic_ram_write(Enemy_State + x, a);
...
void ParseRow0e(void) {
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(AreaPointer, a); // to addresses for level and enemy object data
    iny();
    ...
    dynamic_ram_write(EntrancePage, a); // used upon entry to area, if area is entered
  }
  // NotUse:
  Inc3B();
}
...
void Inc2B(void) {
  ...
  dynamic_ram_write(EnemyObjectPageSel, a);
...
void CheckpointEnemyID(void) {
  ...
  if (!carry_flag) {
    ...
    dynamic_ram_write(Enemy_Y_Position + x, a); // enemy object's vertical coordinate ($00-$14 only)
    lda_imm(0x1);
    dynamic_ram_write(EnemyOffscrBitsMasked + x, a); // set offscreen masked bit
...
void InitPodoboo(void) {
  ...
  dynamic_ram_write(Enemy_Y_HighPos + x, a); // the bottom of the screen
  dynamic_ram_write(Enemy_Y_Position + x, a);
  lsr_acc();
  dynamic_ram_write(EnemyIntervalTimer + x, a); // set timer for enemy
  lsr_acc();
  dynamic_ram_write(Enemy_State + x, a); // initialize enemy state, then jump to use
...
void InitRetainerObj(void) {
  ...
  dynamic_ram_write(Enemy_Y_Position + x, a); // princess/mushroom retainer object
  // --------------------------------
}
...
void SetESpd(void) {
  dynamic_ram_write(Enemy_X_Speed + x, a); // store as speed for enemy object
...
void InitHammerBro(void) {
  ...
  dynamic_ram_write(HammerThrowingTimer + x, a); // apparently to time hammer throwing
  dynamic_ram_write(Enemy_X_Speed + x, a);
  ldy_abs(SecondaryHardMode); // get secondary hard mode flag
  lda_absy(HBroWalkingTimerData);
  dynamic_ram_write(EnemyIntervalTimer + x, a); // set value as delay for hammer bro to walk left
...
void InitBloober(void) {
  lda_imm(0x0); // initialize horizontal speed
  dynamic_ram_write(BlooperMoveSpeed + x, a);
...
void InitRedPTroopa(void) {
  ...
  dynamic_ram_write(RedPTroopaOrigXPos + x, a); // be used as original vertical coordinate
  ...
  dynamic_ram_write(RedPTroopaCenterYPos + x, a); // store as central vertical coordinate
...
void SetBBox(void) {
  dynamic_ram_write(Enemy_BoundBoxCtrl + x, a); // set bounding box control here
  lda_imm(0x2); // set moving direction for left
  dynamic_ram_write(Enemy_MovingDir + x, a);
  InitVStf(); // <fallthrough>
}

void InitVStf(void) {
  lda_imm(0x0); // initialize vertical speed
  dynamic_ram_write(Enemy_Y_Speed + x, a); // and movement force
  dynamic_ram_write(Enemy_Y_MoveForce + x, a);
  // --------------------------------
}
...
void InitBulletBill(void) {
  ...
  dynamic_ram_write(Enemy_BoundBoxCtrl + x, a);
  // --------------------------------
}
...
void InitCheepCheep(void) {
  ...
  dynamic_ram_write(CheepCheepMoveMFlag + x, a); // save as movement flag of some sort
  lda_zpx(Enemy_Y_Position);
  dynamic_ram_write(CheepCheepOrigYPos + x, a); // save original vertical coordinate here
  // --------------------------------
}
...
void SetupLakitu(void) {
  ...
  dynamic_ram_write(LakituReappearTimer, a);
...
void LakituAndSpinyHandler(void) {
  ...
  dynamic_ram_write(FrenzyEnemyTimer, a);
...
CreateSpiny:
  ...
  dynamic_ram_write(Enemy_PageLoc + x, a); // into the coordinates of the spiny we're going to create
  lda_zpy(Enemy_X_Position);
  dynamic_ram_write(Enemy_X_Position + x, a);
...
DifLoop:
  ...
  dynamic_ram_write(0x1 + x, a); // to $01-$03
  iny();
...
SetSpSpd:
  ...
  ldy_imm(0x2);
  dynamic_ram_write(Enemy_X_Speed + x, a); // set horizontal speed to zero because previous contents
  cmp_imm(0x0); // of A were lost...branch here will never be taken for
  if (neg_flag) { goto SpinyRte; } // the same reason
  dey();
  
SpinyRte:
  dynamic_ram_write(Enemy_MovingDir + x, y); // set moving direction to the right
  lda_imm(0xfd);
  dynamic_ram_write(Enemy_Y_Speed + x, a); // set vertical speed to move upwards
  lda_imm(0x1);
  dynamic_ram_write(Enemy_Flag + x, a); // enable enemy object by setting flag
  lda_imm(0x5);
  dynamic_ram_write(Enemy_State + x, a); // put spiny in egg state and leave
  // --------------------------------
}
...
void InitShortFirebar(void) {
  ...
  dynamic_ram_write(FirebarSpinState_Low + x, a);
  ...
  dynamic_ram_write(FirebarSpinSpeed + x, a);
  lda_absy(FirebarSpinDirData); // get spinning direction of firebar
  dynamic_ram_write(FirebarSpinDirection + x, a);
  lda_zpx(Enemy_Y_Position);
...
MaxCC:
  dynamic_ram_write(0x0, y); // store whatever pseudorandom bits are in Y
  ...
  dynamic_ram_write(0x0, a); // and store in two places
  dynamic_ram_write(0x1, a);
  lda_imm(0xfb); // set vertical speed for cheep-cheep
  dynamic_ram_write(Enemy_Y_Speed + x, a);
...
GSeed:
  ...
  dynamic_ram_write(0x0, a); // save it there
  lda_absx(PseudoRandomBitReg + 1);
  ...
  dynamic_ram_write(0x0, a); // third LSFR part
...
D2XPos1:
  ...
  dynamic_ram_write(Enemy_X_Position + x, a); // and save as enemy's horizontal position
  lda_zp(Player_PageLoc); // get player's page location
...
D2XPos2:
  ...
  dynamic_ram_write(Enemy_X_Position + x, a); // offset and save as enemy's horizontal position
  lda_zp(Player_PageLoc); // get player's page location
  sbc_imm(0x0); // subtract borrow
  
FinCCSt:
  dynamic_ram_write(Enemy_PageLoc + x, a); // save as enemy's page location
  lda_imm(0x1);
  dynamic_ram_write(Enemy_Flag + x, a); // set enemy's buffer flag
  dynamic_ram_write(Enemy_Y_HighPos + x, a); // set enemy's high vertical byte
  lda_imm(0xf8);
  dynamic_ram_write(Enemy_Y_Position + x, a); // put enemy below the screen, and we are done
  // --------------------------------
}

void InitBowser(void) {
  DuplicateEnemyObj(); // jump to create another bowser object
  dynamic_ram_write(BowserFront_Offset, x); // save offset of first here
  lda_imm(0x0);
  dynamic_ram_write(BowserBodyControls, a); // initialize bowser's body controls
  dynamic_ram_write(BridgeCollapseOffset, a); // and bridge collapse offset
  lda_zpx(Enemy_X_Position);
  dynamic_ram_write(BowserOrigXPos, a); // store original horizontal position here
  lda_imm(0xdf);
  dynamic_ram_write(BowserFireBreathTimer, a); // store something here
  dynamic_ram_write(Enemy_MovingDir + x, a); // and in moving direction
  lda_imm(0x20);
  dynamic_ram_write(BowserFeetCounter, a); // set bowser's feet timer and in enemy timer
  dynamic_ram_write(EnemyFrameTimer + x, a);
  lda_imm(0x5);
  dynamic_ram_write(BowserHitPoints, a); // give bowser 5 hit points
  lsr_acc();
  dynamic_ram_write(BowserMovementSpeed, a); // set default movement speed here
  // --------------------------------
}
...
void InitBowserFlame(void) {
  ...
  if (zero_flag) {
    dynamic_ram_write(Enemy_Y_MoveForce + x, a); // reset something here
    lda_zp(NoiseSoundQueue);
    ora_imm(Sfx_BowserFlame); // load bowser's flame sound into queue
    dynamic_ram_write(NoiseSoundQueue, a);
    ...
    // SetFrT:
    dynamic_ram_write(FrenzyEnemyTimer, a); // set timer accordingly
    lda_absx(PseudoRandomBitReg);
    and_imm(0b00000011); // get 2 LSB from first part of LSFR
    dynamic_ram_write(BowserFlamePRandomOfs + x, a); // set here
    tay(); // use as offset
...
void PutAtRightExtent(void) {
  dynamic_ram_write(Enemy_Y_Position + x, a); // set vertical position
...
void SpawnFromMouth(void) {
  ...
  dynamic_ram_write(Enemy_X_Position + x, a); // save as flame's horizontal position
  lda_zpy(Enemy_PageLoc);
  dynamic_ram_write(Enemy_PageLoc + x, a); // copy page location from bowser to flame
  lda_zpy(Enemy_Y_Position);
  carry_flag = false; // add 8 pixels to bowser's vertical position
  adc_imm(0x8);
  dynamic_ram_write(Enemy_Y_Position + x, a); // save as flame's vertical position
  lda_absx(PseudoRandomBitReg);
  and_imm(0b00000011); // get 2 LSB from first part of LSFR
  dynamic_ram_write(Enemy_YMF_Dummy + x, a); // save here
  tay(); // use as offset
  ...
  dynamic_ram_write(Enemy_Y_MoveForce + x, a); // to vertical movement force
  lda_imm(0x0);
  dynamic_ram_write(EnemyFrenzyBuffer, a); // clear enemy frenzy buffer
...
void FinishFlame(void) {
  ...
  dynamic_ram_write(Enemy_Y_HighPos + x, a); // enemy buffer flag
  dynamic_ram_write(Enemy_Flag + x, a);
  lsr_acc();
  dynamic_ram_write(Enemy_X_MoveForce + x, a); // initialize horizontal movement force, and
  dynamic_ram_write(Enemy_State + x, a); // enemy state
  // --------------------------------
}
...
StarFChk:
    ...
    dynamic_ram_write(0x0, a); // of the star flag object
    ...
    dynamic_ram_write(Enemy_X_Position + x, a); // store as the fireworks object horizontal coordinate
    lda_zp(0x0);
    adc_imm(0x0); // add carry and store as page location for
    dynamic_ram_write(Enemy_PageLoc + x, a); // the fireworks object
    lda_absy(FireworksYPosData); // get vertical position using same offset
    dynamic_ram_write(Enemy_Y_Position + x, a); // and store as vertical coordinate for fireworks object
    lda_imm(0x1);
    dynamic_ram_write(Enemy_Y_HighPos + x, a); // store in vertical high byte
    dynamic_ram_write(Enemy_Flag + x, a); // and activate enemy buffer flag
    lsr_acc();
    dynamic_ram_write(ExplosionGfxCounter + x, a); // initialize explosion counter
    lda_imm(0x8);
    dynamic_ram_write(ExplosionTimerCounter + x, a); // set explosion timing counter
    // --------------------------------
  }
}
...
Set17ID:
  dynamic_ram_write(Enemy_ID + x, a); // store whatever's in A as enemy identifier
  lda_abs(BitMFilter);
  ...
  dynamic_ram_write(BitMFilter, a);
...
FireBulletBill:
  ...
  dynamic_ram_write(Square2SoundQueue, a);
...
void HandleGroupEnemies(void) {
  ...
  // SnglID:
  dynamic_ram_write(0x1, y); // save enemy id here
  ...
  // CntGrp:
  dynamic_ram_write(NumberofGroupEnemies, y); // save number of enemies here
...
GSltLp:
  ...
  if (!carry_flag) {
    ...
    lda_zp(0x1);
    dynamic_ram_write(Enemy_ID + x, a); // store enemy object identifier
    lda_zp(0x2);
    dynamic_ram_write(Enemy_PageLoc + x, a); // store page location for enemy object
    lda_zp(0x3);
    dynamic_ram_write(Enemy_X_Position + x, a); // store x coordinate for enemy object
    carry_flag = false;
    ...
    dynamic_ram_write(Enemy_Y_HighPos + x, a); // put enemy within the screen vertically
    dynamic_ram_write(Enemy_Flag + x, a);
...
void InitPiranhaPlant(void) {
  ...
  dynamic_ram_write(PiranhaPlant_Y_Speed + x, a);
  lsr_acc();
  dynamic_ram_write(Enemy_State + x, a); // initialize enemy state and what would normally
  dynamic_ram_write(PiranhaPlant_MoveFlag + x, a); // be used as vertical speed, but not in this case
  lda_zpx(Enemy_Y_Position);
  dynamic_ram_write(PiranhaPlantDownYPos + x, a); // save original vertical coordinate here
  carry_flag = true;
  sbc_imm(0x18);
  dynamic_ram_write(PiranhaPlantUpYPos + x, a); // save original vertical coordinate - 24 pixels here
  lda_imm(0x9);
...
void InitEnemyFrenzy(void) {
  ...
  dynamic_ram_write(EnemyFrenzyBuffer, a); // save in enemy frenzy buffer
  carry_flag = true;
...
LakituChk:
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(Enemy_State + y, a);
  }
  // NextFSlot:
  dey(); // move onto the next slot
  if (!neg_flag) { goto LakituChk; } // do this until all slots are checked
  lda_imm(0x0);
  dynamic_ram_write(EnemyFrenzyBuffer, a); // empty enemy frenzy buffer
  dynamic_ram_write(Enemy_Flag + x, a); // disable enemy buffer flag for this object
  // --------------------------------
}
...
void SetBBox2(void) {
  dynamic_ram_write(Enemy_BoundBoxCtrl + x, a); // set bounding box control then leave
  // --------------------------------
}
...
void InitBalPlatform(void) {
  ...
  dynamic_ram_write(Enemy_State + x, a); // set platform alignment to object state here
  ...
  // SetBPA:
  dynamic_ram_write(BalPlatformAlignment, y); // store whatever value's in Y here
  lda_imm(0x0);
  dynamic_ram_write(Enemy_MovingDir + x, a); // init moving direction
  tay(); // init Y
...
void InitDropPlatform(void) {
  lda_imm(0xff);
  dynamic_ram_write(PlatformCollisionFlag + x, a); // set some value here
...
void InitHoriPlatform(void) {
  lda_imm(0x0);
  dynamic_ram_write(XMoveSecondaryCounter + x, a); // init one of the moving counters
...
void InitVertPlatform(void) {
  ...
  // SetYO:
  dynamic_ram_write(YPlatformTopYPos + x, a); // save as top vertical position
  tya();
  carry_flag = false; // load value from earlier, add number of pixels
  adc_zpx(Enemy_Y_Position); // to vertical position
  dynamic_ram_write(YPlatformCenterYPos + x, a); // save result as central vertical position
  // --------------------------------
...
void SPBBox(void) {
  ...
  // CasPBB:
  dynamic_ram_write(Enemy_BoundBoxCtrl + x, a); // set bounding box size control here and leave
  // --------------------------------
}
...
void CommonSmallLift(void) {
  ...
  lda_imm(0x4);
  dynamic_ram_write(Enemy_BoundBoxCtrl + x, a); // set bounding box control for small platforms
  // --------------------------------
}
...
FSLoop:
  ...
  dynamic_ram_write(DuplicateObj_Offset, y); // otherwise set offset here
  txa(); // transfer original enemy buffer offset
  ora_imm(0b10000000); // store with d7 set as flag in new enemy
  dynamic_ram_write(Enemy_Flag + y, a); // slot as well as enemy offset
  lda_zpx(Enemy_PageLoc);
  dynamic_ram_write(Enemy_PageLoc + y, a); // copy page location and horizontal coordinates
  lda_zpx(Enemy_X_Position); // from original enemy to new enemy
  dynamic_ram_write(Enemy_X_Position + y, a);
  lda_imm(0x1);
  dynamic_ram_write(Enemy_Flag + x, a); // set flag as normal for original enemy
  dynamic_ram_write(Enemy_Y_HighPos + y, a); // set high vertical byte for new enemy
  lda_zpx(Enemy_Y_Position);
  dynamic_ram_write(Enemy_Y_Position + y, a); // copy vertical coordinate from original to new
  // --------------------------------
}
...
void PosPlatform(void) {
  ...
  dynamic_ram_write(Enemy_X_Position + x, a); // store as new horizontal coordinate
  lda_zpx(Enemy_PageLoc);
  adc_absy(PlatPosDataHigh); // add or subtract page location depending on offset
  dynamic_ram_write(Enemy_PageLoc + x, a); // store as new page location
  // --------------------------------
}
...
void RunNormalEnemies(void) {
  ...
  dynamic_ram_write(Enemy_SprAttrib + x, a);
  GetEnemyOffscreenBits();
  RelativeEnemyPosition();
  EnemyGfxHandler();
  GetEnemyBoundBox();
...
KillLoop:
  ...
  dynamic_ram_write(EnemyFrenzyBuffer, a); // empty frenzy buffer
  ldx_zp(ObjectOffset); // get enemy object offset and leave
}
...
FeetTmr:
  ...
  lda_imm(0x20); // otherwise, reset timer
  dynamic_ram_write(BowserFeetCounter, a);
...
ResetMDr:
  ...
  dynamic_ram_write(Enemy_MovingDir + x, a); // sixteen frames
...
B_FaceP:
  ...
  lda_imm(0x1);
  dynamic_ram_write(Enemy_MovingDir + x, a); // set bowser to move and face to the right
  lda_imm(0x2);
  dynamic_ram_write(BowserMovementSpeed, a); // set movement speed
  lda_imm(0x20);
  dynamic_ram_write(EnemyFrameTimer + x, a); // set timer here
  dynamic_ram_write(BowserFireBreathTimer, a); // set timer used for bowser's flame
  lda_zpx(Enemy_X_Position);
...
GetPRCmp:
  ...
  dynamic_ram_write(MaxRangeFromOrigin, a); // and store here
...
CompDToO:
  ...
  dynamic_ram_write(BowserMovementSpeed, y); // otherwise change bowser's movement speed
...
SetHmrTmr:
  ...
  dynamic_ram_write(EnemyFrameTimer + x, a); // set for timer here
...
MakeBJump:
  ...
  lda_imm(0xfe);
  dynamic_ram_write(Enemy_Y_Speed + x, a); // set vertical speed to move bowser upwards
...
SpawnFBr:
  ...
  lda_imm(0x20);
  dynamic_ram_write(BowserFireBreathTimer, a); // set timer here
  lda_abs(BowserBodyControls);
  eor_imm(0b10000000); // invert bowser's mouth bit to open
  dynamic_ram_write(BowserBodyControls, a); // and close bowser's mouth
...
SetFBTmr:
  dynamic_ram_write(BowserFireBreathTimer, a); // set value as timer here
  lda_imm(BowserFlame); // put bowser's flame identifier
  dynamic_ram_write(EnemyFrenzyBuffer, a); // in enemy frenzy buffer
  // --------------------------------
...
void BowserGfxHandler(void) {
  ...
  dynamic_ram_write(Enemy_X_Position + y, a); // store A as bowser's rear horizontal coordinate
  lda_zpx(Enemy_Y_Position);
  ...
  dynamic_ram_write(Enemy_Y_Position + y, a); // for bowser's rear
  lda_zpx(Enemy_State);
  dynamic_ram_write(Enemy_State + y, a); // copy enemy state directly from front to rear
  lda_zpx(Enemy_MovingDir);
  dynamic_ram_write(Enemy_MovingDir + y, a); // copy moving direction also
  ...
  dynamic_ram_write(Enemy_ID + x, a); // store in bowser's rear object
  ProcessBowserHalf(); // do a sub here to process bowser's rear
  pla();
  dynamic_ram_write(ObjectOffset, a); // get original enemy object offset
  tax();
  lda_imm(0x0); // nullify bowser's front/rear graphics flag
  dynamic_ram_write(BowserGfxFlag, a);
}
...
void RunFireworks(void) {
  ...
  lda_imm(0x8);
  dynamic_ram_write(ExplosionTimerCounter + x, a); // reset counter
...
SetupExpl:
  ...
  dynamic_ram_write(Fireball_Rel_YPos, a); // from the enemy object to the fireball object
  lda_abs(Enemy_Rel_XPos); // first vertical, then horizontal
  dynamic_ram_write(Fireball_Rel_XPos, a);
...
FireworksSoundScore:
  ...
  dynamic_ram_write(DigitModifier + 4, a);
...
void RunStarFlagObj(void) {
  ...
  dynamic_ram_write(EnemyFrenzyBuffer, a);
...
void GameTimerFireworks(void) {
  ...
  // SetFWC:
  dynamic_ram_write(FireworksCounter, a); // set fireworks counter here
  dynamic_ram_write(Enemy_State + x, y); // set whatever state we have in star flag object
...
void AwardGameTimerPoints(void) {
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(Square2SoundQueue, a); // load timer tick sound
  }
  // NoTTick:
  ...
  dynamic_ram_write(DigitModifier + 5, a); // from the last digit of the game timer
  DigitsMathRoutine(); // subtract digit
  lda_imm(0x5); // set now to add 50 points
  dynamic_ram_write(DigitModifier + 5, a); // per game timer interval subtracted
...
void RaiseFlagSetoffFWorks(void) {
  ...
  lda_imm(Fireworks);
  dynamic_ram_write(EnemyFrenzyBuffer, a); // otherwise set fireworks object in frenzy queue
...
DSFLoop:
  ...
  dynamic_ram_write(Sprite_Y_Position + y, a); // store as Y coordinate
  lda_absx(StarFlagTileData); // get tile number
  dynamic_ram_write(Sprite_Tilenumber + y, a); // store as tile number
  lda_imm(0x22); // set palette and background priority bits
  dynamic_ram_write(Sprite_Attributes + y, a); // store as attributes
  lda_abs(Enemy_Rel_XPos); // get relative horizontal coordinate
  carry_flag = false;
  adc_absx(StarFlagXPosAdder); // add X coordinate adder data
  dynamic_ram_write(Sprite_X_Position + y, a); // store as X coordinate
  iny();
...
void DrawFlagSetTimer(void) {
  ...
  lda_imm(0x6);
  dynamic_ram_write(EnemyIntervalTimer + x, a); // set interval timer here
...
void MovePodoboo(void) {
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(Enemy_Y_MoveForce + x, a); // store as movement force
    and_imm(0b00001111); // mask out high nybble
    ora_imm(0x6); // set for at least six intervals
    dynamic_ram_write(EnemyIntervalTimer + x, a); // store as new enemy timer
    lda_imm(0xf9);
    dynamic_ram_write(Enemy_Y_Speed + x, a); // set vertical speed to move podoboo upwards
  }
  // PdbM:
...
void ProcHammerBro(void) {
  ...
  if (!zero_flag) {
    ...
    if (zero_flag) {
      ...
      dynamic_ram_write(HammerThrowingTimer + x, a); // set as new timer
  ...
  dynamic_ram_write(0x0, a); // save into temp variable for now
...
void SetHJ(void) {
  dynamic_ram_write(Enemy_Y_Speed + x, y); // set vertical speed for jumping
  ...
  dynamic_ram_write(EnemyFrameTimer + x, a); // save in enemy timer
  lda_absx(PseudoRandomBitReg + 1);
  ora_imm(0b11000000); // get contents of part of LSFR, set d7 and d6, then
  dynamic_ram_write(HammerBroJumpTimer + x, a); // store in jump timer
...
void MoveHammerBroXDir(void) {
  ...
  // Shimmy:
  dynamic_ram_write(Enemy_X_Speed + x, y); // store horizontal speed
  ...
  if (!neg_flag) {
    ...
    if (zero_flag) {
      lda_imm(0xf8);
      dynamic_ram_write(Enemy_X_Speed + x, a); // otherwise, make the hammer bro walk left towards player
    }
  }
  // SetShim:
  dynamic_ram_write(Enemy_MovingDir + x, y); // set moving direction
...
AddHS:
  ...
  dynamic_ram_write(Enemy_X_Speed + x, a); // save as horizontal speed temporarily
  MoveEnemyHorizontally(); // then do a sub to move horizontally
  pla();
  dynamic_ram_write(Enemy_X_Speed + x, a); // get old horizontal speed from stack and return to
...
ReviveStunned:
  ...
  dynamic_ram_write(Enemy_State + x, a); // otherwise initialize enemy state to normal
  lda_zp(FrameCounter);
  and_imm(0x1); // get d0 of frame counter
  tay(); // use as Y and increment for movement direction
  iny();
  dynamic_ram_write(Enemy_MovingDir + x, y); // store as pseudorandom movement direction
...
SetRSpd:
  ...
  dynamic_ram_write(Enemy_X_Speed + x, a); // and leave
}
...
void ProcMoveRedPTroopa(void) {
  ...
  dynamic_ram_write(Enemy_YMF_Dummy + x, a); // initialize something here
...
SBMDir:
  dynamic_ram_write(Enemy_MovingDir + x, y); // set moving direction of bloober, then continue on here
...
BlooberSwim:
  ...
  dynamic_ram_write(Enemy_Y_Position + x, a); // otherwise, set new vertical position, make bloober swim
...
SwimX:
  ...
  dynamic_ram_write(Enemy_X_Position + x, a); // store result as new horizontal coordinate
  lda_zpx(Enemy_PageLoc);
  adc_imm(0x0); // add carry to page location
  dynamic_ram_write(Enemy_PageLoc + x, a); // store as new page location and leave
  return;
...
void MoveBulletBill(void) {
  ...
  dynamic_ram_write(Enemy_X_Speed + x, a); // and move it accordingly (note: this bullet bill
...
CCSwim:
  dynamic_ram_write(0x3, a); // save enemy state in $03
  ...
  dynamic_ram_write(Enemy_X_MoveForce + x, a); // store as new horizontal force
  lda_zpx(Enemy_X_Position); // get horizontal coordinate
  sbc_imm(0x0); // subtract borrow (thus moving it slowly)
  dynamic_ram_write(Enemy_X_Position + x, a); // and save as new horizontal coordinate
  lda_zpx(Enemy_PageLoc);
  sbc_imm(0x0); // subtract borrow again, this time from the
  dynamic_ram_write(Enemy_PageLoc + x, a); // page location, then save
  lda_imm(0x20);
  dynamic_ram_write(0x2, a); // save new value here
  ...
  dynamic_ram_write(Enemy_YMF_Dummy + x, a); // and save dummy
  lda_zpx(Enemy_Y_Position); // get vertical coordinate
  adc_zp(0x3); // add carry to it plus enemy state to slowly move it downwards
  dynamic_ram_write(Enemy_Y_Position + x, a); // save as new vertical coordinate
...
ChkSwimYPos:
  dynamic_ram_write(Enemy_Y_HighPos + x, a); // save new page location here
...
YPDiff:
  ...
  tya();
  dynamic_ram_write(CheepCheepMoveMFlag + x, a); // otherwise change movement speed
  // --------------------------------
...
void MoveFlyingCheepCheep(void) {
  ...
  if (!zero_flag) {
    lda_imm(0x0);
    dynamic_ram_write(Enemy_SprAttrib + x, a); // otherwise clear sprite attributes
  ...
  dynamic_ram_write(Enemy_SprAttrib + x, a); // broken or residual code, value is overwritten before
  // --------------------------------
...
ChkLS:
  ...
  lda_imm(0x0);
  dynamic_ram_write(LakituMoveDirection + x, a); // otherwise initialize moving direction to move to left
  dynamic_ram_write(EnemyFrenzyBuffer, a); // initialize frenzy buffer
  lda_imm(0x10);
  if (!zero_flag) { goto SetLSpd; } // load horizontal speed and do unconditional branch
  
Fr12S:
  lda_imm(Spiny);
  dynamic_ram_write(EnemyFrenzyBuffer, a); // set spiny identifier in frenzy buffer
  ldy_imm(0x2);
  
LdLDa:
  lda_absy(LakituDiffAdj); // load values
  dynamic_ram_write(0x1 + y, a); // store in zero page
  dey();
  if (!neg_flag) { goto LdLDa; } // do this until all values are stired
  PlayerLakituDiff(); // execute sub to set speed and create spinys
  
SetLSpd:
  dynamic_ram_write(LakituMoveSpeed + x, a); // set movement speed returned from sub
  ...
  carry_flag = false;
  adc_imm(0x1);
  dynamic_ram_write(LakituMoveSpeed + x, a); // store as new moving speed
  iny(); // increment moving direction to left
  
SetLMov:
  dynamic_ram_write(Enemy_MovingDir + x, y); // store moving direction
...
void MovePiranhaPlant(void) {
  ...
  adc_imm(0x1);
  dynamic_ram_write(0x0, a); // save as new horizontal difference
...
ReversePlantSpeed:
  ...
  adc_imm(0x1);
  dynamic_ram_write(PiranhaPlant_Y_Speed + x, a); // save as new vertical speed
...
RiseFallPiranhaPlant:
  dynamic_ram_write(0x0, a); // save vertical coordinate here
  lda_zp(FrameCounter); // get frame counter
  lsr_acc();
  ...
  lda_imm(0x0);
  dynamic_ram_write(PiranhaPlant_MoveFlag + x, a); // otherwise clear movement flag
  lda_imm(0x40);
  dynamic_ram_write(EnemyFrameTimer + x, a); // set timer to delay piranha plant movement
  
PutinPipe:
  lda_imm(0b00100000); // set background priority bit in sprite
  dynamic_ram_write(Enemy_SprAttrib + x, a); // attributes to give illusion of being inside pipe
  // -------------------------------------------------------------------------------------
  // $07 - spinning speed
}
...
ChkForFall:
  ...
  dynamic_ram_write(Enemy_Y_Position + x, a); // of current platform and branch elsewhere
...
ChkOtherForFall:
  ...
  carry_flag = false;
  adc_imm(0x2); // otherwise add 2 pixels to vertical position
  dynamic_ram_write(Enemy_Y_Position + y, a); // of other platform and branch elsewhere
...
DoOtherPlatform:
  ...
  dynamic_ram_write(Enemy_Y_Position + y, a); // platform to move it in the opposite direction
...
DrawEraseRope:
  ...
  dynamic_ram_write(VRAM_Buffer1 + x, a); // first the high byte, then the low
  lda_zp(0x0);
  dynamic_ram_write(VRAM_Buffer1 + 1 + x, a);
  ...
  lda_imm(0xa2);
  dynamic_ram_write(VRAM_Buffer1 + 3 + x, a); // otherwise put tile numbers for left
...
EraseR1:
  ...
  dynamic_ram_write(VRAM_Buffer1 + 3 + x, a); // to erase rope
  dynamic_ram_write(VRAM_Buffer1 + 4 + x, a);
...
OtherRope:
  ...
  lda_zp(0x1); // write name table address to vram buffer
  dynamic_ram_write(VRAM_Buffer1 + 5 + x, a); // this time we're doing putting tiles for
  lda_zp(0x0); // the other platform
  dynamic_ram_write(VRAM_Buffer1 + 6 + x, a);
  lda_imm(0x2);
  dynamic_ram_write(VRAM_Buffer1 + 7 + x, a); // set length again for 2 bytes
  ...
  lda_imm(0xa2);
  dynamic_ram_write(VRAM_Buffer1 + 8 + x, a); // otherwise put tile numbers for left
  lda_imm(0xa3); // and right sides of rope in vram
  dynamic_ram_write(VRAM_Buffer1 + 9 + x, a); // transfer buffer
  goto EndRp; // jump to skip this part
  
EraseR2:
  lda_imm(0x24); // put blank tiles in vram buffer
  dynamic_ram_write(VRAM_Buffer1 + 8 + x, a); // to erase rope
  dynamic_ram_write(VRAM_Buffer1 + 9 + x, a);
  
EndRp:
  lda_imm(0x0); // put null terminator at the end
  dynamic_ram_write(VRAM_Buffer1 + 10 + x, a);
...
void StopPlatforms(void) {
  ...
  dynamic_ram_write(Enemy_Y_Speed + y, a); // for both platforms and leave
  dynamic_ram_write(Enemy_Y_MoveForce + y, a);
}
...
void PositionPlayerOnHPlat(void) {
  ...
  dynamic_ram_write(Player_X_Position, a); // player accordingly in horizontal position
  lda_zp(Player_PageLoc); // get player's page location
...
SetPVar:
  dynamic_ram_write(Player_PageLoc, a); // save result to player's page location
  dynamic_ram_write(Platform_X_Scroll, y); // put saved value from second sub here to be used later
...
void RightPlatform(void) {
  ...
  dynamic_ram_write(0x0, a); // store saved value here (residual code)
  lda_absx(PlatformCollisionFlag); // check collision flag, if no collision between player
  if (!neg_flag) {
    lda_imm(0x10);
    dynamic_ram_write(Enemy_X_Speed + x, a); // otherwise set new speed (gets moving if motionless)
...
void EraseEnemyObject(void) {
  ...
  dynamic_ram_write(Enemy_ID + x, a);
  dynamic_ram_write(Enemy_State + x, a);
  dynamic_ram_write(FloateyNum_Control + x, a);
  dynamic_ram_write(EnemyIntervalTimer + x, a);
  dynamic_ram_write(ShellChainCounter + x, a);
  dynamic_ram_write(Enemy_SprAttrib + x, a);
  dynamic_ram_write(EnemyFrameTimer + x, a);
  // -------------------------------------------------------------------------------------
}
...
void XMoveCntr_Platform(void) {
  dynamic_ram_write(0x1, a); // store value here
  lda_zp(FrameCounter);
...
void MoveWithXMCntrs(void) {
  ...
  if (zero_flag) {
    ...
    adc_imm(0x1);
    dynamic_ram_write(XMoveSecondaryCounter + x, a);
    ldy_imm(0x2); // load alternate value here
  }
  // XMRight:
  dynamic_ram_write(Enemy_MovingDir + x, y); // store as moving direction
  MoveEnemyHorizontally();
  dynamic_ram_write(0x0, a); // save value obtained from sub here
  pla(); // get secondary counter from stack
  dynamic_ram_write(XMoveSecondaryCounter + x, a); // and return to original place
  // --------------------------------
}
  ...
  if (zero_flag) {
    ...
    if (!carry_flag) {
      ...
      if (zero_flag) {
        ...
        adc_imm(0x1);
        dynamic_ram_write(Enemy_Y_MoveForce + x, a); // set movement force
        dynamic_ram_write(BlooperMoveSpeed + x, a); // set as movement speed
        cmp_imm(0x2);
    ...
    if (zero_flag) {
      ...
      if (zero_flag) {
        inc_zpx(BlooperMoveCounter); // otherwise increment movement counter
        lda_imm(0x2);
        dynamic_ram_write(EnemyIntervalTimer + x, a); // set enemy's timer
      }
    }
    // NoSSw:
    return; // leave
...
Floatdown:
  ...
  lda_imm(0x0);
  dynamic_ram_write(BlooperMoveCounter + x, a); // otherwise nullify movement counter
  // --------------------------------
}
...
void ProcFirebar(void) {
  ...
  dynamic_ram_write(FirebarSpinState_High + x, a); // and store as new high byte of spinstate
...
SkpFSte:
  ...
  dynamic_ram_write(FirebarSpinState_High + x, a);
  
SetupGFB:
  dynamic_ram_write(0xef, a); // save high byte of spinning thing, modified or otherwise
  ...
  ldy_absx(Enemy_SprDataOffset); // get OAM data offset
  lda_abs(Enemy_Rel_YPos); // get relative vertical coordinate
  dynamic_ram_write(Sprite_Y_Position + y, a); // store as Y in OAM data
  dynamic_ram_write(0x7, a); // also save here
  lda_abs(Enemy_Rel_XPos); // get relative horizontal coordinate
  dynamic_ram_write(Sprite_X_Position + y, a); // store as X in OAM data
  dynamic_ram_write(0x6, a); // also save here
  lda_imm(0x1);
  dynamic_ram_write(0x0, a); // set $01 value here (not necessary)
...
SetMFbar:
  dynamic_ram_write(0xed, y); // store maximum value for length of firebars
  lda_imm(0x0);
  dynamic_ram_write(0x0, a); // initialize counter here
...
DrawFbar:
  ...
  dynamic_ram_write(0x6, a); // using long firebar offset, then store as new one here
...
AddHA:
  ...
  dynamic_ram_write(Sprite_X_Position + y, a); // store as X coordinate here
  dynamic_ram_write(0x6, a); // store here for now, note offset is saved in Y still
...
SetVFbr:
  dynamic_ram_write(Sprite_Y_Position + y, a); // store as Y coordinate here
  dynamic_ram_write(0x7, a); // also store here for now
...
void FirebarCollision(void) {
  ...
  dynamic_ram_write(0x5, a); // otherwise initialize counter
  ldy_zp(Player_Y_HighPos);
...
ChkVFBD:
  ...
  dynamic_ram_write(0x4, a); // store here
...
SetSDir:
  dynamic_ram_write(Enemy_MovingDir, x); // store movement direction here
  ldx_imm(0x0);
  lda_zp(0x0); // save value written to $00 to stack
  pha();
  InjurePlayer(); // perform sub to hurt or kill player
  pla();
  dynamic_ram_write(0x0, a); // get value of $00 from stack
...
void GetFirebarPosition(void) {
  ...
  // GetHAdder:
  dynamic_ram_write(0x1, a); // store result, modified or not, here
  ...
  // GetVAdder:
  dynamic_ram_write(0x2, a); // store result here
  ldy_zp(0x0);
...
SetLMovD:
  ...
  dynamic_ram_write(LakituMoveDirection + x, a); // difference between enemy and player if necessary
...
ChkPSpeed:
  ...
  lsr_acc();
  dynamic_ram_write(0x0, a); // store as new value
...
void ProcessBowserHalf(void) {
  ...
  if (zero_flag) {
    lda_imm(0xa);
    dynamic_ram_write(Enemy_BoundBoxCtrl + x, a); // set bounding box size control
    GetEnemyBoundBox(); // get bounding box coordinates
...
void SetFlameTimer(void) {
  ...
  dynamic_ram_write(BowserFlameTimerCtrl, a);
...
SFlmX:
  dynamic_ram_write(0x0, a); // store value here
  lda_absx(Enemy_X_MoveForce);
  carry_flag = true; // subtract value from movement force
  sbc_zp(0x0);
  dynamic_ram_write(Enemy_X_MoveForce + x, a); // save new value
  lda_zpx(Enemy_X_Position);
  sbc_imm(0x1); // subtract one from horizontal position to move
  dynamic_ram_write(Enemy_X_Position + x, a); // to the left
  lda_zpx(Enemy_PageLoc);
  sbc_imm(0x0); // subtract borrow from page location
  dynamic_ram_write(Enemy_PageLoc + x, a);
  ...
  dynamic_ram_write(Enemy_Y_Position + x, a); // as new vertical coordinate
...
SetGfxF:
  ...
  dynamic_ram_write(0x0, a); // write first tile number
...
FlmeAt:
  dynamic_ram_write(0x1, y); // set bowser's flame sprite attributes here
  ldy_absx(Enemy_SprDataOffset); // get OAM data offset
  ldx_imm(0x0);
  
DrawFlameLoop:
  lda_abs(Enemy_Rel_YPos); // get Y relative coordinate of current enemy object
  dynamic_ram_write(Sprite_Y_Position + y, a); // write into Y coordinate of OAM data
  lda_zp(0x0);
  dynamic_ram_write(Sprite_Tilenumber + y, a); // write current tile number into OAM data
  inc_zp(0x0); // increment tile number to draw more bowser's flame
  lda_zp(0x1);
  dynamic_ram_write(Sprite_Attributes + y, a); // write saved attributes into OAM data
  lda_abs(Enemy_Rel_XPos);
  dynamic_ram_write(Sprite_X_Position + y, a); // write X relative coordinate of current enemy object
  carry_flag = false;
  adc_imm(0x8);
  dynamic_ram_write(Enemy_Rel_XPos, a); // then add eight to it and store
  iny();
  iny();
  iny();
  ...
  dynamic_ram_write(Sprite_Y_Position + 12 + y, a); // residual since flame is only made of three sprites
...
M3FOfs:
  ...
  dynamic_ram_write(Sprite_Y_Position + 8 + y, a);
...
M2FOfs:
  ...
  dynamic_ram_write(Sprite_Y_Position + 4 + y, a);
...
M1FOfs:
  ...
  lda_imm(0xf8);
  dynamic_ram_write(Sprite_Y_Position + y, a); // otherwise move first sprite offscreen
  // --------------------------------
}

void FirebarSpin(void) {
  dynamic_ram_write(0x7, a); // save spinning speed here
...
void SetupPlatformRope(void) {
  ...
  lda_zpy(Enemy_PageLoc);
  adc_imm(0x0); // add carry to page location
  dynamic_ram_write(0x2, a); // and save here
  ...
  lsr_acc();
  lsr_acc();
  dynamic_ram_write(0x0, a); // store result here as part of name table low byte
  ...
  dynamic_ram_write(0x1, a); // address, then store
  ...
  dynamic_ram_write(0x1, a); // the proper name table and the right place on it
  ...
  dynamic_ram_write(0x0, a); // save as name table low byte
  lda_zpy(Enemy_Y_Position);
...
void InitPlatformFall(void) {
  ...
  dynamic_ram_write(FloateyNum_X_Pos + x, a); // put floatey number coordinates where player is
  lda_zp(Player_Y_Position);
  dynamic_ram_write(FloateyNum_Y_Pos + x, a);
  lda_imm(0x1); // set moving direction as flag for
  dynamic_ram_write(Enemy_MovingDir + x, a); // falling platforms
...
void MoveLiftPlatforms(void) {
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(Enemy_YMF_Dummy + x, a);
    ...
    dynamic_ram_write(Enemy_Y_Position + x, a); // and then leave
  }
}
...
FireballEnemyCDLoop:
  dynamic_ram_write(0x1, x); // store enemy object offset here
  tya();
...
NotGoomba:
  ...
  lda_imm(0b10000000);
  dynamic_ram_write(Fireball_State + x, a); // set d7 in enemy state
...
HurtBowser:
  ...
  dynamic_ram_write(Enemy_X_Speed + x, a); // initialize horizontal speed
  dynamic_ram_write(EnemyFrenzyBuffer, a); // init enemy frenzy buffer
  lda_imm(0xfe);
  dynamic_ram_write(Enemy_Y_Speed + x, a); // set vertical speed to make defeated bowser jump a little
  ldy_abs(WorldNumber); // use world number as offset
  lda_absy(BowserIdentities); // get enemy identifier to replace bowser with
  dynamic_ram_write(Enemy_ID + x, a); // set as new enemy identifier
...
SetDBSte:
  dynamic_ram_write(Enemy_State + x, a); // set defeated enemy state
  lda_imm(Sfx_BowserFall);
  dynamic_ram_write(Square2SoundQueue, a); // load bowser defeat sound
  ldx_zp(0x1); // get enemy offset
...
void PlayerHammerCollision(void) {
  ...
  lda_imm(0x1);
  dynamic_ram_write(Misc_Collision_Flag + x, a); // otherwise set collision flag now
  lda_zpx(Misc_X_Speed);
  eor_imm(0xff); // get two's compliment of
  carry_flag = false; // hammer's horizontal speed
  adc_imm(0x1);
  dynamic_ram_write(Misc_X_Speed + x, a); // set to send hammer flying the opposite direction
...
ClHCol:
  ...
  dynamic_ram_write(Misc_Collision_Flag + x, a);
  // -------------------------------------------------------------------------------------
}
...
void HandlePowerUpCollision(void) {
  ...
  dynamic_ram_write(Square2SoundQueue, a); // play the power-up sound
  lda_zp(PowerUpType); // check power-up type
  cmp_imm(0x2);
  ...
  dynamic_ram_write(StarInvincibleTimer, a); // timer, and load the star mario music
...
SetFor1Up:
  ...
  dynamic_ram_write(FloateyNum_Control + x, a); // and then leave
  return;
...
void PlayerEnemyCollision(void) {
  ...
  dynamic_ram_write(Enemy_CollisionBits + x, a); // collision bit
...
HandlePECollisions:
  ...
  dynamic_ram_write(Enemy_CollisionBits + x, a);
...
void ForceInjury(void) {
  ...
  dynamic_ram_write(PlayerStatus, a); // otherwise set player's status to small
  lda_imm(0x8);
  dynamic_ram_write(InjuryTimer, a); // set injured invincibility timer
  asl_acc();
  dynamic_ram_write(Square1SoundQueue, a); // play pipedown/injury sound
...
void SetPRout(void) {
  dynamic_ram_write(GameEngineSubroutine, a); // load new value to run subroutine on next frame
  dynamic_ram_write(Player_State, y); // store new player state
  ldy_imm(0xff);
  dynamic_ram_write(TimerControl, y); // set master timer control flag to halt timers
  iny();
  dynamic_ram_write(ScrollAmount, y); // initialize scroll speed
...
void KillPlayer(void) {
  dynamic_ram_write(Player_X_Speed, x); // halt player's horizontal movement by initializing speed
  inx();
  dynamic_ram_write(EventMusicQueue, x); // set event music queue to death music
  lda_imm(0xfc);
  dynamic_ram_write(Player_Y_Speed, a); // set new vertical speed
...
EnemyStompedPts:
  ...
  pla();
  dynamic_ram_write(Enemy_MovingDir + x, a); // return enemy movement direction from stack
  lda_imm(0b00100000);
  dynamic_ram_write(Enemy_State + x, a); // set d5 in enemy state
  InitVStf(); // nullify vertical speed, physics-related thing,
  dynamic_ram_write(Enemy_X_Speed + x, a); // and horizontal speed
...
ChkForDemoteKoopa:
  ...
  dynamic_ram_write(Enemy_State + x, y);
  ...
  dynamic_ram_write(Enemy_X_Speed + x, a); // set appropriate moving speed based on direction
...
HandleStompedShellE:
  ...
  dynamic_ram_write(EnemyIntervalTimer + x, a); // set as enemy timer to revive stomped enemy
  
SBnce:
  lda_imm(0xfc); // set player's vertical speed for bounce
  dynamic_ram_write(Player_Y_Speed, a); // and then leave!!!
}
...
void EnemyFacePlayer(void) {
  ...
  // SFcRt:
  dynamic_ram_write(Enemy_MovingDir + x, y); // set moving direction here
  dey(); // then decrement to use as a proper offset
}

void SetupFloateyNumber(void) {
  dynamic_ram_write(FloateyNum_Control + x, a); // set number of points control for floatey numbers
  lda_imm(0x30);
  dynamic_ram_write(FloateyNum_Timer + x, a); // set timer for floatey numbers
  lda_zpx(Enemy_Y_Position);
  dynamic_ram_write(FloateyNum_Y_Pos + x, a); // set vertical coordinate
  lda_abs(Enemy_Rel_XPos);
  dynamic_ram_write(FloateyNum_X_Pos + x, a); // set horizontal coordinate and leave
  // -------------------------------------------------------------------------------------
...
void ECLoop(void) {
  dynamic_ram_write(0x1, x); // save enemy object buffer offset for second enemy here
  ...
  if (carry_flag) {
    ...
    if (zero_flag) {
      ...
      dynamic_ram_write(Enemy_CollisionBits + y, a);
    }
    // YesEC:
  ...
  dynamic_ram_write(Enemy_CollisionBits + y, a); // then move onto next enemy slot
...
void RXSpd(void) {
  ...
  tay();
  iny();
  dynamic_ram_write(Enemy_X_Speed + x, y); // store as new horizontal speed
  lda_zpx(Enemy_MovingDir);
  eor_imm(0b00000011); // invert moving direction and store, then leave
  dynamic_ram_write(Enemy_MovingDir + x, a); // thus effectively turning the enemy around
  // -------------------------------------------------------------------------------------
...
void LargePlatformCollision(void) {
  ...
  dynamic_ram_write(PlatformCollisionFlag + x, a);
  lda_abs(TimerControl); // check master timer control
...
void ChkForPlayerC_LargeP(void) {
  ...
  dynamic_ram_write(0x0, a); // temp variable for now
...
void SmallPlatformCollision(void) {
  ...
  dynamic_ram_write(PlatformCollisionFlag + x, a); // otherwise initialize collision flag
  ...
  lda_imm(0x2);
  dynamic_ram_write(0x0, a); // load counter here for 2 bounding boxes
...
MoveBoundBox:
  ...
  adc_imm(0x80);
  dynamic_ram_write(BoundingBox_UL_YPos + y, a);
  lda_absy(BoundingBox_DR_YPos);
  carry_flag = false;
  adc_imm(0x80);
  dynamic_ram_write(BoundingBox_DR_YPos + y, a);
...
void ProcLPlatCollisions(void) {
  ...
  dynamic_ram_write(Player_Y_Speed, a); // speed of player to kill jump
...
SetCollisionFlag:
  ldx_zp(ObjectOffset); // get enemy object buffer offset
  dynamic_ram_write(PlatformCollisionFlag + x, a); // save either bounding box counter or enemy offset here
  lda_imm(0x0);
  dynamic_ram_write(Player_State, a); // set player state to normal then leave
  return;
  
PlatformSideCollisions:
  lda_imm(0x1); // set value here to indicate possible horizontal
  dynamic_ram_write(0x0, a); // collision on left side of platform
...
void PositionPlayerOnVPlatSkip(void) {
  ...
  dynamic_ram_write(Player_Y_Position, a); // save as player's new vertical coordinate
  tya();
  sbc_imm(0x0); // subtract borrow and store as player's
  dynamic_ram_write(Player_Y_HighPos, a); // new vertical high byte
  lda_imm(0x0);
  dynamic_ram_write(Player_Y_Speed, a); // initialize vertical speed and low byte of force
  dynamic_ram_write(Player_Y_MoveForce, a); // and then leave
  // -------------------------------------------------------------------------------------
}
...
SetPSte:
  dynamic_ram_write(Player_State, a); // set whatever player state is appropriate
...
ChkOnScr:
  ...
  lda_imm(0xff);
  dynamic_ram_write(Player_CollisionBits, a); // initialize player's collision flag
  lda_zp(Player_Y_Position);
...
GBBAdr:
  ...
  dynamic_ram_write(0xeb, a); // store value here
...
SolidOrClimb:
  ...
  lda_imm(Sfx_Bump);
  dynamic_ram_write(Square1SoundQueue, a); // otherwise load bump sound
  
NYSpd:
  lda_imm(0x1); // set player's vertical speed to nullify
  dynamic_ram_write(Player_Y_Speed, a); // jump or swim
...
DoFootCheck:
  ...
  dynamic_ram_write(0x0, a); // save bottom right metatile here
  pla();
  dynamic_ram_write(0x1, a); // pull bottom left metatile and save here
...
ContChk:
  ...
  dynamic_ram_write(0x0, a); // use player's moving direction as temp variable
...
LandPlyr:
  ...
  dynamic_ram_write(Player_Y_Position, a); // and store as new vertical position to land player properly
  HandlePipeEntry(); // do sub to process potential pipe entry
  lda_imm(0x0);
  dynamic_ram_write(Player_Y_Speed, a); // initialize vertical speed and fractional
  dynamic_ram_write(Player_Y_MoveForce, a); // movement force to stop player's vertical movement
  dynamic_ram_write(StompChainCounter, a); // initialize enemy stomp counter
  
InitSteP:
  lda_imm(0x0);
  dynamic_ram_write(Player_State, a); // set player's state to normal
...
SideCheckLoop:
  ...
  dynamic_ram_write(0xeb, y); // store it
  lda_zp(Player_Y_Position);
...
PipeDwnS:
  ...
  dynamic_ram_write(Square1SoundQueue, y); // otherwise load pipedown/injury sound
  
PlyrPipe:
  ora_imm(0b00100000);
  dynamic_ram_write(Player_SprAttrib, a); // set background priority bit in player attributes
  lda_zp(Player_X_Position);
...
SetCATmr:
  ...
  dynamic_ram_write(ChangeAreaTimer, a);
...
ChkGERtn:
  ...
  lda_imm(0x2);
  dynamic_ram_write(GameEngineSubroutine, a); // otherwise set sideways pipe entry routine to run
  return; // and leave
  // --------------------------------
...
HandleAxeMetatile:
  lda_imm(0x0);
  dynamic_ram_write(OperMode_Task, a); // reset secondary mode
  lda_imm(0x2);
  dynamic_ram_write(OperMode, a); // set primary mode to autoctrl mode
  lda_imm(0x18);
  dynamic_ram_write(Player_X_Speed, a); // set horizontal speed and continue to erase axe metatile
  ErACM(); // <fallthrough>
}
...
NXSpd:
  ldy_imm(0x10);
  dynamic_ram_write(SideCollisionTimer, y); // set timer of some sort
  ldy_imm(0x0);
  dynamic_ram_write(Player_X_Speed, y); // nullify player's horizontal speed
...
PlatF:
  dynamic_ram_write(0x0, y); // store Y as high bits of horizontal adder
  carry_flag = false;
  adc_zp(Player_X_Position); // add contents of A to player's horizontal
  dynamic_ram_write(Player_X_Position, a); // position to move player left or right
  lda_zp(Player_PageLoc);
  adc_zp(0x0); // add high bits and carry to
  dynamic_ram_write(Player_PageLoc, a); // page location if necessary
  
ExIPM:
  txa(); // invert contents of X
  eor_imm(0xff);
  and_abs(Player_CollisionBits); // mask out bit that was set here
  dynamic_ram_write(Player_CollisionBits, a); // store to clear bit
  // --------------------------------
}
...
FlagpoleCollision:
  ...
  lda_imm(0x1);
  dynamic_ram_write(PlayerFacingDir, a); // set player's facing direction to right
  ...
  lda_imm(Silence);
  dynamic_ram_write(EventMusicQueue, a); // silence music
  lsr_acc();
  dynamic_ram_write(FlagpoleSoundQueue, a); // load flagpole sound into flagpole sound queue
  ldx_imm(0x4); // start at end of vertical coordinate data
  lda_zp(Player_Y_Position);
  dynamic_ram_write(FlagpoleCollisionYPos, a); // store player's vertical coordinate here to be used later
...
MtchF:
  dynamic_ram_write(FlagpoleScore, x); // store offset here to be used later
  
RunFR:
  lda_imm(0x4);
  dynamic_ram_write(GameEngineSubroutine, a); // set value to run flagpole slide routine
...
VineCollision:
  ...
  lda_imm(0x1);
  dynamic_ram_write(GameEngineSubroutine, a); // otherwise set to run autoclimb routine next frame
  
PutPlayerOnVine:
  lda_imm(0x3); // set player state to climbing
  dynamic_ram_write(Player_State, a);
  lda_imm(0x0); // nullify player's horizontal speed
  dynamic_ram_write(Player_X_Speed, a); // and fractional horizontal movement force
  dynamic_ram_write(Player_X_MoveForce, a);
  lda_zp(Player_X_Position); // get player's horizontal coordinate
  carry_flag = true;
  ...
  lda_imm(0x2);
  dynamic_ram_write(PlayerFacingDir, a); // otherwise force player to face left
...
SetVXPl:
  ...
  dynamic_ram_write(Player_X_Position, a); // store as player's horizontal coordinate
  ...
  dynamic_ram_write(Player_PageLoc, a); // store as player's page location
  // --------------------------------
}
...
void ChkForLandJumpSpring(void) {
  ...
  if (carry_flag) {
    lda_imm(0x70);
    dynamic_ram_write(VerticalForce, a); // otherwise set vertical movement force for player
    lda_imm(0xf9);
    dynamic_ram_write(JumpspringForce, a); // set default jumpspring force
    lda_imm(0x3);
    dynamic_ram_write(JumpspringTimer, a); // set jumpspring timer to be used later
    lsr_acc();
    dynamic_ram_write(JumpspringAnimCtrl, a); // set jumpspring animation control to start animating
  }
}
...
void HandlePipeEntry(void) {
  ...
  if (!zero_flag) { return; } // branch to leave if not found
  lda_imm(0x30);
  dynamic_ram_write(ChangeAreaTimer, a); // set timer for change of area
  lda_imm(0x3);
  dynamic_ram_write(GameEngineSubroutine, a); // set to run vertical pipe entry routine on next frame
  lda_imm(Sfx_PipeDown_Injury);
  dynamic_ram_write(Square1SoundQueue, a); // load pipedown/injury sound
  lda_imm(0b00100000);
  dynamic_ram_write(Player_SprAttrib, a); // set background priority bit in player's attributes
...
GetWNum:
  ...
  dynamic_ram_write(WorldNumber, y); // store as world number and offset
  ...
  dynamic_ram_write(AreaPointer, a); // store area offset here to be used to change areas
  lda_imm(Silence);
  dynamic_ram_write(EventMusicQueue, a); // silence music
  lda_imm(0x0);
  dynamic_ram_write(EntrancePage, a); // initialize starting page number
  dynamic_ram_write(AreaNumber, a); // initialize area number used for area address offset
  dynamic_ram_write(LevelNumber, a); // initialize level number used for world display
  dynamic_ram_write(AltEntranceControl, a); // initialize mode of entry
...
void CheckForCoinMTiles(void) {
  ...
  dynamic_ram_write(Square2SoundQueue, a); // load coin grab sound and leave
}
...
void ChkToStunEnemies(void) {
  ...
  dynamic_ram_write(Enemy_ID + x, a); // into green or red koopa troopa to demote them
  SetStun(); // <fallthrough>
}
...
void SetStun(void) {
  ...
  ora_imm(0b00000010);
  dynamic_ram_write(Enemy_State + x, a); // set d1 of enemy state
  dec_zpx(Enemy_Y_Position);
...
SetNotW:
  dynamic_ram_write(Enemy_Y_Speed + x, a); // set vertical speed now
  ldy_imm(0x1);
...
NoCDirF:
  ...
  dynamic_ram_write(Enemy_X_Speed + x, a); // and store, then leave
  // --------------------------------
...
SetForStn:
  dynamic_ram_write(EnemyIntervalTimer + x, a); // set timer here
  lda_imm(0x3); // set state here, apparently used to render
  dynamic_ram_write(Enemy_State + x, a); // upside-down koopas and buzzy beetles
...
ProcEnemyDirection:
  ...
  lda_imm(0x1);
  dynamic_ram_write(Enemy_MovingDir + x, a); // send enemy moving to the right by default
  lda_imm(0x8);
  dynamic_ram_write(Enemy_X_Speed + x, a); // set horizontal speed accordingly
  lda_zp(FrameCounter);
...
LandEnemyInitState:
  ...
  dynamic_ram_write(Enemy_State + x, a); // note this will also turn spiny's egg into spiny
  return;
...
SetD6Ste:
  dynamic_ram_write(Enemy_State + x, a); // set as new state
  DoEnemySideCheck();
  // --------------------------------
...
void DoEnemySideCheck(void) {
  ...
  if (carry_flag) {
    ...
    dynamic_ram_write(0xeb, a); // OAM data offset
...
void ChkForBump_HammerBroJ(void) {
  ...
  if (!zero_flag) {
    ...
    if (carry_flag) {
      ...
      dynamic_ram_write(Square1SoundQueue, a); // sound will never be played if branching from ChkForRedKoopa
    }
  }
  // NoBump:
  ...
  if (zero_flag) {
    lda_imm(0x0);
    dynamic_ram_write(0x0, a); // initialize value here for bitmask
    ...
    if (carry_flag) {
      ...
      if (!zero_flag) {
        ...
        if (!zero_flag) {
          ...
          lda_imm(0xfd);
          dynamic_ram_write(Enemy_Y_Speed + x, a); // make the paratroopa jump again
        }
      }
    }
  }
...
void PlayerEnemyDiff(void) {
  ...
  dynamic_ram_write(0x0, a); // and store here
  lda_zpx(Enemy_PageLoc);
...
void EnemyLanding(void) {
  ...
  dynamic_ram_write(Enemy_Y_Position + x, a); // neatly on whatever it's landing on
}
...
void UnderHammerBro(void) {
  ...
  dynamic_ram_write(Enemy_State + x, a); // and store
...
void FireballBGCollision(void) {
  ...
  lda_imm(0xfd);
  dynamic_ram_write(Fireball_Y_Speed + x, a); // otherwise set vertical speed to move upwards (give it bounce)
  lda_imm(0x1);
  dynamic_ram_write(FireballBouncingFlag + x, a); // set bouncing flag
  lda_zpx(Fireball_Y_Position);
  and_imm(0xf8); // modify vertical coordinate to land it properly
  dynamic_ram_write(Fireball_Y_Position + x, a); // store as new vertical coordinate
  return; // leave
  
ClearBounceFlag:
  lda_imm(0x0);
  dynamic_ram_write(FireballBouncingFlag + x, a); // clear bouncing flag by default
  return; // leave
  
InitFireballExplode:
  lda_imm(0x80);
  dynamic_ram_write(Fireball_State + x, a); // set exploding flag in fireball's state
  lda_imm(Sfx_Bump);
  dynamic_ram_write(Square1SoundQueue, a); // load bump sound
  // -------------------------------------------------------------------------------------
...
void GetMaskedOffScrBits(void) {
  ...
  dynamic_ram_write(0x1, a); // store here
  ...
  dynamic_ram_write(EnemyOffscrBitsMasked + x, a); // save masked offscreen bits here
...
void MoveBoundBoxOffscreen(void) {
  ...
  lda_imm(0xff);
  dynamic_ram_write(EnemyBoundingBoxCoord + y, a); // load value into four locations here and leave
  dynamic_ram_write(EnemyBoundingBoxCoord + 1 + y, a);
  dynamic_ram_write(EnemyBoundingBoxCoord + 2 + y, a);
  dynamic_ram_write(EnemyBoundingBoxCoord + 3 + y, a);
}
...
void CheckRightScreenBBox(void) {
  ...
  if (carry_flag) {
    ...
    if (!neg_flag) {
      ...
      if (!neg_flag) {
        dynamic_ram_write(BoundingBox_UL_XPos + y, a); // store offscreen value for left side
      }
      // SORte:
      dynamic_ram_write(BoundingBox_DR_XPos + y, a); // store offscreen value for right side
    }
    // NoOfs:
...
void BoundingBoxCore(void) {
  dynamic_ram_write(0x0, x); // save offset here
  lda_absy(SprObject_Rel_YPos); // store object coordinates relative to screen
  dynamic_ram_write(0x2, a); // vertically and horizontally, respectively
  ...
  dynamic_ram_write(BoundingBox_UL_Corner + y, a); // store here
  lda_zp(0x1);
  carry_flag = false;
  adc_absx(BoundBoxCtrlData + 2); // add the third number in the bounding box data to the
  dynamic_ram_write(BoundingBox_LR_Corner + y, a); // relative horizontal coordinate and store
  ...
  dynamic_ram_write(BoundingBox_UL_Corner + y, a);
  lda_zp(0x2);
  carry_flag = false;
  adc_absx(BoundBoxCtrlData + 2); // add the fourth number to the relative vertical coordinate
  dynamic_ram_write(BoundingBox_LR_Corner + y, a); // and store
...
void SprObjectCollisionCore(void) {
  dynamic_ram_write(0x6, y); // save contents of Y here
  lda_imm(0x1);
  dynamic_ram_write(0x7, a); // save value 1 here as counter, compare horizontal coordinates first
...
void BlockBufferCollision(void) {
  pha(); // save contents of A to stack
  dynamic_ram_write(0x4, y); // save contents of Y here
  ...
  dynamic_ram_write(0x3, a); // and store here
...
RetYC:
  ...
  dynamic_ram_write(0x4, a); // store masked out result here
...
void DrawVine(void) {
  dynamic_ram_write(0x0, y); // save offset here
  lda_abs(Enemy_Rel_YPos); // get relative vertical coordinate
  carry_flag = false;
  adc_absy(VineYPosAdder); // add value using offset in Y to get value
  ldx_absy(VineObjOffset); // get offset to vine
  ldy_absx(Enemy_SprDataOffset); // get sprite data offset
  dynamic_ram_write(0x2, y); // store sprite data offset here
  SixSpriteStacker(); // stack six sprites on top of each other vertically
  lda_abs(Enemy_Rel_XPos); // get relative horizontal coordinate
  dynamic_ram_write(Sprite_X_Position + y, a); // store in first, third and fifth sprites
  dynamic_ram_write(Sprite_X_Position + 8 + y, a);
  dynamic_ram_write(Sprite_X_Position + 16 + y, a);
  carry_flag = false;
  adc_imm(0x6); // add six pixels to second, fourth and sixth sprites
  dynamic_ram_write(Sprite_X_Position + 4 + y, a); // to give characteristic staggered vine shape to
  dynamic_ram_write(Sprite_X_Position + 12 + y, a); // our vertical stack of sprites
  dynamic_ram_write(Sprite_X_Position + 20 + y, a);
  lda_imm(0b00100001); // set bg priority and palette attribute bits
  dynamic_ram_write(Sprite_Attributes + y, a); // set in first, third and fifth sprites
  dynamic_ram_write(Sprite_Attributes + 8 + y, a);
  dynamic_ram_write(Sprite_Attributes + 16 + y, a);
  ora_imm(0b01000000); // additionally, set horizontal flip bit
  dynamic_ram_write(Sprite_Attributes + 4 + y, a); // for second, fourth and sixth sprites
  dynamic_ram_write(Sprite_Attributes + 12 + y, a);
  dynamic_ram_write(Sprite_Attributes + 20 + y, a);
  ldx_imm(0x5); // set tiles for six sprites
  
VineTL:
  lda_imm(0xe1); // set tile number for sprite
  dynamic_ram_write(Sprite_Tilenumber + y, a);
  ...
  if (zero_flag) {
    lda_imm(0xe0);
    dynamic_ram_write(Sprite_Tilenumber + y, a); // set other tile number for top of vine
  }
  // SkpVTop:
...
ChkFTop:
  ...
  if (carry_flag) {
    lda_imm(0xf8);
    dynamic_ram_write(Sprite_Y_Position + y, a); // otherwise move sprite offscreen
  }
  // NextVSp:
...
StkLp:
  dynamic_ram_write(Sprite_Data + y, a); // store X or Y coordinate into OAM data
  carry_flag = false;
  adc_imm(0x8); // add eight pixels
  iny();
...
RenderH:
  ...
  dynamic_ram_write(Sprite_Y_Position + y, a); // store as sprite Y coordinate for first sprite
  carry_flag = false;
  adc_absx(SecondSprYPos); // add second sprite vertical adder based on offset
  dynamic_ram_write(Sprite_Y_Position + 4 + y, a); // store as sprite Y coordinate for second sprite
  ...
  dynamic_ram_write(Sprite_X_Position + y, a); // store as sprite X coordinate for first sprite
  carry_flag = false;
  adc_absx(SecondSprXPos); // add second sprite horizontal adder based on offset
  dynamic_ram_write(Sprite_X_Position + 4 + y, a); // store as sprite X coordinate for second sprite
  lda_absx(FirstSprTilenum);
  dynamic_ram_write(Sprite_Tilenumber + y, a); // get and store tile number of first sprite
  lda_absx(SecondSprTilenum);
  dynamic_ram_write(Sprite_Tilenumber + 4 + y, a); // get and store tile number of second sprite
  lda_absx(HammerSprAttrib);
  dynamic_ram_write(Sprite_Attributes + y, a); // get and store attribute bytes for both
  dynamic_ram_write(Sprite_Attributes + 4 + y, a); // note in this case they use the same data
  ldx_zp(ObjectOffset); // get misc object offset
  ...
  lda_imm(0x0);
  dynamic_ram_write(Misc_State + x, a); // otherwise nullify misc object state
  lda_imm(0xf8);
...
void FlagpoleGfxHandler(void) {
  ...
  lda_abs(Enemy_Rel_XPos); // get relative horizontal coordinate
  dynamic_ram_write(Sprite_X_Position + y, a); // store as X coordinate for first sprite
  carry_flag = false;
  adc_imm(0x8); // add eight pixels and store
  dynamic_ram_write(Sprite_X_Position + 4 + y, a); // as X coordinate for second and third sprites
  dynamic_ram_write(Sprite_X_Position + 8 + y, a);
  carry_flag = false;
  adc_imm(0xc); // add twelve more pixels and
  dynamic_ram_write(0x5, a); // store here to be used later by floatey number
  lda_zpx(Enemy_Y_Position); // get vertical coordinate
  DumpTwoSpr(); // and do sub to dump into first and second sprites
  adc_imm(0x8); // add eight pixels
  dynamic_ram_write(Sprite_Y_Position + 8 + y, a); // and store into third sprite
  lda_abs(FlagpoleFNum_Y_Pos); // get vertical coordinate for floatey number
  dynamic_ram_write(0x2, a); // store it here
  lda_imm(0x1);
  dynamic_ram_write(0x3, a); // set value for flip which will not be used, and
  dynamic_ram_write(0x4, a); // attribute byte for floatey number
  dynamic_ram_write(Sprite_Attributes + y, a); // set attribute bytes for all three sprites
  dynamic_ram_write(Sprite_Attributes + 4 + y, a);
  dynamic_ram_write(Sprite_Attributes + 8 + y, a);
  lda_imm(0x7e);
  dynamic_ram_write(Sprite_Tilenumber + y, a); // put triangle shaped tile
  dynamic_ram_write(Sprite_Tilenumber + 8 + y, a); // into first and third sprites
  lda_imm(0x7f);
  dynamic_ram_write(Sprite_Tilenumber + 4 + y, a); // put skull tile into second sprite
...
void DumpSixSpr(void) {
  dynamic_ram_write(Sprite_Data + 20 + y, a); // dump A contents
  dynamic_ram_write(Sprite_Data + 16 + y, a); // into third row sprites
  DumpFourSpr(); // <fallthrough>
}

void DumpFourSpr(void) {
  dynamic_ram_write(Sprite_Data + 12 + y, a); // into second row sprites
  DumpThreeSpr(); // <fallthrough>
}

void DumpThreeSpr(void) {
  dynamic_ram_write(Sprite_Data + 8 + y, a);
  DumpTwoSpr(); // <fallthrough>
}

void DumpTwoSpr(void) {
  dynamic_ram_write(Sprite_Data + 4 + y, a); // and into first row sprites
  dynamic_ram_write(Sprite_Data + y, a);
  // -------------------------------------------------------------------------------------
}
...
void DrawLargePlatform(void) {
  ldy_absx(Enemy_SprDataOffset); // get OAM data offset
  dynamic_ram_write(0x2, y); // store here
...
SetLast2Platform:
  ldy_absx(Enemy_SprDataOffset); // get OAM data offset
  dynamic_ram_write(Sprite_Y_Position + 16 + y, a); // store vertical coordinate or offscreen
  dynamic_ram_write(Sprite_Y_Position + 20 + y, a); // coordinate into last two sprites as Y coordinate
...
SetPlatformTilenum:
  ...
  dynamic_ram_write(Sprite_Y_Position + y, a);
...
SChk4:
  ...
  dynamic_ram_write(Sprite_Y_Position + 12 + y, a);
...
SChk5:
  ...
  dynamic_ram_write(Sprite_Y_Position + 16 + y, a);
...
SChk6:
  ...
  lda_imm(0xf8);
  dynamic_ram_write(Sprite_Y_Position + 20 + y, a); // if d2 was set, move sixth sprite offscreen
...
void DrawFloateyNumber_Coin(void) {
  ...
  carry_flag = false;
  adc_imm(0x8); // add eight pixels
  dynamic_ram_write(Sprite_X_Position + 4 + y, a); // store as X coordinate for second sprite
  lda_imm(0x2);
  dynamic_ram_write(Sprite_Attributes + y, a); // store attribute byte in both sprites
  dynamic_ram_write(Sprite_Attributes + 4 + y, a);
  lda_imm(0xf7);
  dynamic_ram_write(Sprite_Tilenumber + y, a); // put tile numbers into both sprites
  lda_imm(0xfb); // that resemble "200"
  dynamic_ram_write(Sprite_Tilenumber + 4 + y, a);
}
...
void JCoinGfxHandler(void) {
  ...
  dynamic_ram_write(Sprite_Y_Position + y, a); // Y coordinate for first sprite
  carry_flag = false;
  adc_imm(0x8); // add eight pixels
  dynamic_ram_write(Sprite_Y_Position + 4 + y, a); // store as Y coordinate for second sprite
  lda_abs(Misc_Rel_XPos); // get relative horizontal coordinate
  dynamic_ram_write(Sprite_X_Position + y, a);
  dynamic_ram_write(Sprite_X_Position + 4 + y, a); // store as X coordinate for first and second sprites
  lda_zp(FrameCounter); // get frame counter
  ...
  lda_imm(0x2);
  dynamic_ram_write(Sprite_Attributes + y, a); // set attribute byte in first sprite
  lda_imm(0x82);
  dynamic_ram_write(Sprite_Attributes + 4 + y, a); // set attribute byte with vertical flip in second sprite
  ldx_zp(ObjectOffset); // get misc object offset
  // -------------------------------------------------------------------------------------
...
void DrawPowerUp(void) {
  ...
  dynamic_ram_write(0x4, a); // store attributes here
  txa();
  ...
  tax(); // use as X
  lda_imm(0x1);
  dynamic_ram_write(0x7, a); // set counter here to draw two rows of sprite object
  dynamic_ram_write(0x3, a); // init d1 of flip control
...
PUpDrawLoop:
  ...
  if (!zero_flag) {
    ...
    if (!zero_flag) {
      dynamic_ram_write(0x0, a); // store power-up type here now
      lda_zp(FrameCounter); // get frame counter
      ...
      dynamic_ram_write(Sprite_Attributes + y, a); // set as new palette bits for top left and
      dynamic_ram_write(Sprite_Attributes + 4 + y, a); // top right sprites for fire flower and star
      ldx_zp(0x0);
      dex(); // check power-up type for fire flower
      // if found, skip this part
      if (!zero_flag) {
        dynamic_ram_write(Sprite_Attributes + 8 + y, a); // otherwise set new palette bits  for bottom left
        dynamic_ram_write(Sprite_Attributes + 12 + y, a); // and bottom right sprites as well for star only
      }
      // FlipPUpRightSide:
      ...
      dynamic_ram_write(Sprite_Attributes + 12 + y, a); // note these are only done for fire flower and star power-ups
    }
  }
  // PUpOfs:
...
void MoveESprColOffscreen(void) {
  ...
  dynamic_ram_write(Sprite_Data + 16 + y, a); // move third row sprite in column offscreen
  // -------------------------------------------------------------------------------------
  // $00-$01 - tile numbers
...
void DrawBlock(void) {
  ...
  dynamic_ram_write(0x2, a); // store here
  lda_abs(Block_Rel_XPos); // get relative horizontal coordinate of block object
  dynamic_ram_write(0x5, a); // store here
  lda_imm(0x3);
  dynamic_ram_write(0x4, a); // set attribute byte here
  lsr_acc();
  dynamic_ram_write(0x3, a); // set horizontal flip bit here (will not be used)
...
DBlkLoop:
  ...
  dynamic_ram_write(0x0, a); // set here
  ...
  if (!zero_flag) {
    lda_imm(0x86);
    dynamic_ram_write(Sprite_Tilenumber + y, a); // otherwise remove brick tiles with lines
    dynamic_ram_write(Sprite_Tilenumber + 4 + y, a); // and replace then with lineless brick tiles
  }
  // ChkRep:
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(Sprite_Attributes + y, a); // store attribute byte as-is in first sprite
    ora_imm(0b01000000);
    dynamic_ram_write(Sprite_Attributes + 4 + y, a); // set horizontal flip bit for second sprite
    ora_imm(0b10000000);
    dynamic_ram_write(Sprite_Attributes + 12 + y, a); // set both flip bits for fourth sprite
    and_imm(0b10000011);
    dynamic_ram_write(Sprite_Attributes + 8 + y, a); // set vertical flip bit for third sprite
  }
  // BlkOffscr:
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(Sprite_Y_Position + 4 + y, a); // on the right side
    dynamic_ram_write(Sprite_Y_Position + 12 + y, a);
  }
  // PullOfsB:
...
void MoveColOffscreen(void) {
  lda_imm(0xf8); // move offscreen two OAMs
  dynamic_ram_write(Sprite_Y_Position + y, a); // on the left side (or two rows of enemy on either side
  dynamic_ram_write(Sprite_Y_Position + 8 + y, a); // if branched here from enemy graphics handler)
  // -------------------------------------------------------------------------------------
...
DChunks:
  ...
  dynamic_ram_write(Sprite_X_Position + y, a); // save into X coordinate of first sprite
  ...
  dynamic_ram_write(0x0, a); // store result as relative horizontal coordinate of original
  carry_flag = true;
  ...
  dynamic_ram_write(Sprite_X_Position + 4 + y, a); // save into X coordinate of second sprite
  lda_abs(Block_Rel_YPos + 1); // get second block object's relative vertical coordinate
  dynamic_ram_write(Sprite_Y_Position + 8 + y, a);
  dynamic_ram_write(Sprite_Y_Position + 12 + y, a); // dump into Y coordinates of third and fourth sprites
  lda_abs(Block_Rel_XPos + 1); // get second block object's relative horizontal coordinate
  dynamic_ram_write(Sprite_X_Position + 8 + y, a); // save into X coordinate of third sprite
  ...
  dynamic_ram_write(Sprite_X_Position + 12 + y, a); // save into X coordinate of fourth sprite
  lda_abs(Block_OffscreenBits); // get offscreen bits for block object
...
void DrawFireball(void) {
  ...
  dynamic_ram_write(Sprite_Y_Position + y, a); // store as sprite Y coordinate
  lda_abs(Fireball_Rel_XPos); // get relative horizontal coordinate
  dynamic_ram_write(Sprite_X_Position + y, a); // store as sprite X coordinate, then do shared code
...
void DrawFirebar(void) {
  ...
  dynamic_ram_write(Sprite_Tilenumber + y, a); // thus tile changes every four frames
  pla(); // get from stack
  ...
  // FireA:
  dynamic_ram_write(Sprite_Attributes + y, a); // store attribute byte and leave
  // -------------------------------------------------------------------------------------
}
...
void DrawExplosion_Fireworks(void) {
  ...
  dynamic_ram_write(Sprite_X_Position + y, a);
  dynamic_ram_write(Sprite_X_Position + 4 + y, a);
  carry_flag = false; // add eight pixels horizontally
  adc_imm(0x8); // for third and fourth sprites
  dynamic_ram_write(Sprite_X_Position + 8 + y, a);
  dynamic_ram_write(Sprite_X_Position + 12 + y, a);
  lda_imm(0x2); // set palette attributes for all sprites, but
  dynamic_ram_write(Sprite_Attributes + y, a); // set no flip at all for first sprite
  lda_imm(0x82);
  dynamic_ram_write(Sprite_Attributes + 4 + y, a); // set vertical flip for second sprite
  lda_imm(0x42);
  dynamic_ram_write(Sprite_Attributes + 8 + y, a); // set horizontal flip for third sprite
  lda_imm(0xc2);
  dynamic_ram_write(Sprite_Attributes + 12 + y, a); // set both flips for fourth sprite
}
...
void DrawSmallPlatform(void) {
  ...
  lda_abs(Enemy_Rel_XPos); // get relative horizontal coordinate
  dynamic_ram_write(Sprite_X_Position + y, a);
  dynamic_ram_write(Sprite_X_Position + 12 + y, a); // dump as X coordinate into first and fourth sprites
  carry_flag = false;
  adc_imm(0x8); // add eight pixels
  dynamic_ram_write(Sprite_X_Position + 4 + y, a); // dump into second and fifth sprites
  dynamic_ram_write(Sprite_X_Position + 16 + y, a);
  carry_flag = false;
  adc_imm(0x8); // add eight more pixels
  dynamic_ram_write(Sprite_X_Position + 8 + y, a); // dump into third and sixth sprites
  dynamic_ram_write(Sprite_X_Position + 20 + y, a);
  lda_zpx(Enemy_Y_Position); // get vertical coordinate
  tax();
  pha(); // save to stack
  ...
  // BotSP:
  dynamic_ram_write(Sprite_Y_Position + 12 + y, a); // dump vertical coordinate + 128 pixels
  dynamic_ram_write(Sprite_Y_Position + 16 + y, a); // into Y coordinates
  dynamic_ram_write(Sprite_Y_Position + 20 + y, a);
  lda_abs(Enemy_OffscreenBits); // get offscreen bits
  pha(); // save to stack
  and_imm(0b00001000); // check d3
  if (!zero_flag) {
    lda_imm(0xf8); // if d3 was set, move first and
    dynamic_ram_write(Sprite_Y_Position + y, a); // fourth sprites offscreen
    dynamic_ram_write(Sprite_Y_Position + 12 + y, a);
  }
  // SOfs:
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(Sprite_Y_Position + 4 + y, a); // fifth sprites offscreen
    dynamic_ram_write(Sprite_Y_Position + 16 + y, a);
  }
  // SOfs2:
  pla(); // get from stack
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(Sprite_Y_Position + 8 + y, a); // sixth sprites offscreen
    dynamic_ram_write(Sprite_Y_Position + 20 + y, a);
  }
  // ExSPl:
  ldx_zp(ObjectOffset); // get enemy object offset and leave
...
void DrawBubble(void) {
  ...
  dynamic_ram_write(Sprite_Y_Position + y, a); // store as Y coordinate here
  lda_imm(0x74);
  dynamic_ram_write(Sprite_Tilenumber + y, a); // put air bubble tile into OAM data
  lda_imm(0x2);
  dynamic_ram_write(Sprite_Attributes + y, a); // set attribute byte
  // -------------------------------------------------------------------------------------
...
PIntLoop:
  ...
  dynamic_ram_write(0x2 + x, a); // appears on world/lives display
  dex();
  ...
  ora_imm(0b01000000); // set horizontal flip bit for bottom-right sprite
  dynamic_ram_write(Sprite_Attributes + 32, a); // store and leave
  // -------------------------------------------------------------------------------------
...
void RenderPlayerSub(void) {
  dynamic_ram_write(0x7, a); // store number of rows of sprites to draw
  lda_abs(Player_Rel_XPos);
  dynamic_ram_write(Player_Pos_ForScroll, a); // store player's relative horizontal position
  dynamic_ram_write(0x5, a); // store it here also
  lda_abs(Player_Rel_YPos);
  dynamic_ram_write(0x2, a); // store player's vertical position
  lda_zp(PlayerFacingDir);
  dynamic_ram_write(0x3, a); // store player's facing direction
  lda_abs(Player_SprAttrib);
  dynamic_ram_write(0x4, a); // store player's sprite attributes
...
NonAnimatedActs:
  ...
  lda_imm(0x0);
  dynamic_ram_write(PlayerAnimCtrl, a); // initialize animation frame control
...
void AnimationControl(void) {
  dynamic_ram_write(0x0, a); // store upper extent here
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(PlayerAnimTimer, a); // and set timer accordingly
    ...
    // SetAnimC:
    dynamic_ram_write(PlayerAnimCtrl, a); // store as new animation frame control
  }
  // ExAnimC:
...
void HandleChangeSize(void) {
  ...
  if (zero_flag) {
    ...
    if (carry_flag) {
      ...
      dynamic_ram_write(PlayerChangeSizeFlag, y); // and animation frame control
    }
    // CSzNext:
    dynamic_ram_write(PlayerAnimCtrl, y); // store proper frame control
  }
  // GorSLog:
  lda_abs(PlayerSize); // get player's size
...
KilledAtt:
  ...
  dynamic_ram_write(Sprite_Attributes + 16 + y, a); // for third row sprites and save
  lda_absy(Sprite_Attributes + 20);
  and_imm(0b00111111);
  ora_imm(0b01000000); // set horizontal flip bit for second
  dynamic_ram_write(Sprite_Attributes + 20 + y, a); // sprite in the third row
  
C_S_IGAtt:
  lda_absy(Sprite_Attributes + 24);
  and_imm(0b00111111); // mask out horizontal and vertical flip bits
  dynamic_ram_write(Sprite_Attributes + 24 + y, a); // for fourth row sprites and save
  lda_absy(Sprite_Attributes + 28);
  and_imm(0b00111111);
  ora_imm(0b01000000); // set horizontal flip bit for second
  dynamic_ram_write(Sprite_Attributes + 28 + y, a); // sprite in the fourth row
  // -------------------------------------------------------------------------------------
...
void VariableObjOfsRelPos(void) {
  dynamic_ram_write(0x0, x); // store value to add to A here
  carry_flag = false;
...
void GetObjRelativePosition(void) {
  ...
  dynamic_ram_write(SprObject_Rel_YPos + y, a); // store here
  lda_zpx(SprObject_X_Position); // load horizontal coordinate
  carry_flag = true; // subtract left edge coordinate
  sbc_abs(ScreenLeft_X_Pos);
  dynamic_ram_write(SprObject_Rel_XPos + y, a); // store result here
  // -------------------------------------------------------------------------------------
...
void GetOffScreenBitsSet(void) {
  ...
  dynamic_ram_write(0x0, a); // store both here
  pla(); // get offscreen bits offset from stack
  tay();
  lda_zp(0x0); // get value here and store elsewhere
  dynamic_ram_write(SprObject_OffscrBits + y, a);
  ldx_zp(ObjectOffset);
}
...
void GetXOffscreenBits(void) {
  dynamic_ram_write(0x4, x); // save position in buffer to here
...
XOfsLoop:
  ...
  dynamic_ram_write(0x7, a); // store here
...
void DividePDiff(void) {
  dynamic_ram_write(0x5, a); // store current value in A here
...
void DrawSpriteObject(void) {
  ...
  dynamic_ram_write(Sprite_Tilenumber + 4 + y, a); // store first tile into second sprite
...
NoHFlip:
  dynamic_ram_write(Sprite_Tilenumber + y, a); // store first tile into first sprite
...
SetHFAt:
  ...
  dynamic_ram_write(Sprite_Attributes + y, a); // store sprite attributes
  dynamic_ram_write(Sprite_Attributes + 4 + y, a);
  lda_zp(0x2); // now the y coordinates
  dynamic_ram_write(Sprite_Y_Position + y, a); // note because they are
  dynamic_ram_write(Sprite_Y_Position + 4 + y, a); // side by side, they are the same
  lda_zp(0x5);
  dynamic_ram_write(Sprite_X_Position + y, a); // store x coordinate, then
...
void SoundEngine(void) {
  ...
  dynamic_ram_write(SND_MASTERCTRL_REG, a); // if so, disable sound and leave
  return;
  
SndOn:
  lda_imm(0xff);
  dynamic_ram_write(JOYPAD_PORT2, a); // disable irqs and set frame counter mode???
  lda_imm(0xf);
  dynamic_ram_write(SND_MASTERCTRL_REG, a); // enable first four channels
...
InPause:
  ...
  dynamic_ram_write(PauseSoundBuffer, a); // if queue full, store in buffer and activate
  dynamic_ram_write(PauseModeFlag, a); // pause mode to interrupt game sounds
  lda_imm(0x0); // disable sound and clear sfx buffers
  dynamic_ram_write(SND_MASTERCTRL_REG, a);
  dynamic_ram_write(Square1SoundBuffer, a);
  dynamic_ram_write(Square2SoundBuffer, a);
  dynamic_ram_write(NoiseSoundBuffer, a);
  lda_imm(0xf);
  dynamic_ram_write(SND_MASTERCTRL_REG, a); // enable sound again
  lda_imm(0x2a); // store length of sound in pause counter
  dynamic_ram_write(Squ1_SfxLenCounter, a);
...
DecPauC:
  ...
  dynamic_ram_write(SND_MASTERCTRL_REG, a); // not currently playing the pause sfx
  ...
  dynamic_ram_write(PauseModeFlag, a);
  
SkipPIn:
  lda_imm(0x0); // clear pause sfx buffer
  dynamic_ram_write(PauseSoundBuffer, a);
  if (zero_flag) { goto SkipSoundSubroutines; }
...
SkipSoundSubroutines:
  ...
  dynamic_ram_write(Square2SoundQueue, a);
  dynamic_ram_write(NoiseSoundQueue, a);
  dynamic_ram_write(PauseSoundQueue, a);
...
void Dump_Squ1_Regs(void) {
  ...
  dynamic_ram_write(SND_SQUARE1_REG, x);
}
...
void Dump_Sq2_Regs(void) {
  dynamic_ram_write(SND_SQUARE2_REG, x); // dump the contents of X and Y into square 2's control regs
...
void JumpRegContents(void) {
  ...
  dynamic_ram_write(Squ1_SfxLenCounter, a); // then continue on here
...
void Square1SfxHandler(void) {
  ...
  dynamic_ram_write(Square1SoundBuffer, y); // if found, put in buffer
...
ContinueSwimStomp:
  ...
  dynamic_ram_write(SND_SQUARE1_REG, a); // envelope
  cpy_imm(0x6);
...
SmTick:
  dynamic_ram_write(SND_SQUARE1_REG, a);
...
void StopSquare1Sfx(void) {
  ...
  dynamic_ram_write(0xf1, x); // and stop making the sfx
  ldx_imm(0xe);
  dynamic_ram_write(SND_MASTERCTRL_REG, x);
  ldx_imm(0xf);
  dynamic_ram_write(SND_MASTERCTRL_REG, x);
}
...
void CGrab_TTickRegL(void) {
  dynamic_ram_write(Squ2_SfxLenCounter, a);
...
void EmptySfx2Buffer(void) {
  ...
  dynamic_ram_write(Square2SoundBuffer, x);
...
void GrowItemRegs(void) {
  ...
  dynamic_ram_write(Sfx_SecondaryCounter, a);
...
void ContinueGrowItems(void) {
  ...
  if (!zero_flag) {
    ...
    dynamic_ram_write(SND_SQUARE2_REG, a);
...
void Square2SfxHandler(void) {
  ...
  dynamic_ram_write(Square2SoundBuffer, y); // if found, put in buffer and check for the following
...
void PlayBrickShatter(void) {
  ...
  dynamic_ram_write(Noise_SfxLenCounter, a);
...
void PlayNoiseSfx(void) {
  dynamic_ram_write(SND_NOISE_REG, a); // play the sfx
...
void DecrementSfx3Length(void) {
  ...
  if (zero_flag) {
    ...
    dynamic_ram_write(SND_NOISE_REG, a);
    lda_imm(0x0);
    dynamic_ram_write(NoiseSoundBuffer, a);
  }
}
...
void NoiseSfxHandler(void) {
  ...
  dynamic_ram_write(NoiseSoundBuffer, y); // if found, put in buffer
...
void LoadEventMusic(void) {
  dynamic_ram_write(EventMusicBuffer, a); // copy event music queue contents to buffer
  ...
  dynamic_ram_write(AreaMusicBuffer_Alt, x); // save current area music buffer to be re-obtained later
  ldy_imm(0x0);
  dynamic_ram_write(NoteLengthTblAdder, y); // default value for additional length byte offset
  dynamic_ram_write(AreaMusicBuffer, y); // clear area music buffer
  ...
  dynamic_ram_write(NoteLengthTblAdder, x);
  // unconditional branch
  if (!zero_flag) {
    FindEventMusicHeader();
    return;
...
void GMLoopB(void) {
  dynamic_ram_write(GroundMusicHeaderOfs, y);
  HandleAreaMusicLoopB(); // <fallthrough>
}

void HandleAreaMusicLoopB(void) {
  ldy_imm(0x0); // clear event music buffer
  dynamic_ram_write(EventMusicBuffer, y);
  dynamic_ram_write(AreaMusicBuffer, a); // copy area music queue contents to buffer
  ...
  dynamic_ram_write(MusicOffset_Square2, y); // residual instruction here
...
void LoadHeader(void) {
  ...
  dynamic_ram_write(NoteLenLookupTblOfs, a);
  lda_absy(MusicHeaderData + 1);
  dynamic_ram_write(MusicDataLow, a);
  lda_absy(MusicHeaderData + 2);
  dynamic_ram_write(MusicDataHigh, a);
  lda_absy(MusicHeaderData + 3);
  dynamic_ram_write(MusicOffset_Triangle, a);
  lda_absy(MusicHeaderData + 4);
  dynamic_ram_write(MusicOffset_Square1, a);
  lda_absy(MusicHeaderData + 5);
  dynamic_ram_write(MusicOffset_Noise, a);
  dynamic_ram_write(NoiseDataLoopbackOfs, a);
  lda_imm(0x1); // initialize music note counters
  dynamic_ram_write(Squ2_NoteLenCounter, a);
  dynamic_ram_write(Squ1_NoteLenCounter, a);
  dynamic_ram_write(Tri_NoteLenCounter, a);
  dynamic_ram_write(Noise_BeatLenCounter, a);
  lda_imm(0x0); // initialize music data offset for square 2
  dynamic_ram_write(MusicOffset_Square2, a);
  dynamic_ram_write(AltRegContentFlag, a); // initialize alternate control reg data used by square 1
...
NotTRO:
  ...
  dynamic_ram_write(AreaMusicBuffer, a); // control regs of square and triangle channels
  dynamic_ram_write(EventMusicBuffer, a);
  dynamic_ram_write(SND_TRIANGLE_REG, a);
  lda_imm(0x90);
  dynamic_ram_write(SND_SQUARE1_REG, a);
  dynamic_ram_write(SND_SQUARE2_REG, a);
  return;
...
Squ2LengthHandler:
  ...
  dynamic_ram_write(Squ2_NoteLenBuffer, a);
...
Rest:
  dynamic_ram_write(Squ2_EnvelopeDataCtrl, a); // save contents of A
...
NoDecEnv1:
  ...
  dynamic_ram_write(SND_SQUARE2_REG, a); // based on offset set by first load unless playing
...
FetchSqu1MusicData:
  ...
  lda_imm(0x83);
  dynamic_ram_write(SND_SQUARE1_REG, a); // store some data into control regs for square 1
  ...
  dynamic_ram_write(AltRegContentFlag, a);
  if (!zero_flag) { goto FetchSqu1MusicData; } // unconditional branch
  
Squ1NoteHandler:
  AlternateLengthHandler();
  dynamic_ram_write(Squ1_NoteLenCounter, a); // save contents of A in square 1 note counter
...
SkipCtrlL:
  dynamic_ram_write(Squ1_EnvelopeDataCtrl, a); // save envelope offset
  Dump_Squ1_Regs();
...
NoDecEnv2:
  ...
  dynamic_ram_write(SND_SQUARE1_REG, a); // based on offset set by first load
...
HandleTriangleMusic:
  ...
  dynamic_ram_write(Tri_NoteLenBuffer, a); // save contents of A
  lda_imm(0x1f);
  dynamic_ram_write(SND_TRIANGLE_REG, a); // load some default data for triangle control reg
...
TriNoteHandler:
  ...
  dynamic_ram_write(Tri_NoteLenCounter, x);
...
LoadTriCtrlReg:
  dynamic_ram_write(SND_TRIANGLE_REG, a); // save final contents of A into control reg for triangle
...
FetchNoiseBeatData:
  ...
  dynamic_ram_write(MusicOffset_Noise, a); // and loopback next time around
  if (!zero_flag) { goto FetchNoiseBeatData; } // unconditional branch
  
NoiseBeatHandler:
  AlternateLengthHandler();
  dynamic_ram_write(Noise_BeatLenCounter, a); // store length in noise beat counter
  txa();
...
PlayBeat:
  dynamic_ram_write(SND_NOISE_REG, a); // load beat data into noise regs
  dynamic_ram_write(SND_NOISE_REG + 2, x);
