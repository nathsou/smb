...
void Start(void) {
  ...
  cld();
  lda_imm_nf(0b00010000); // init PPU control register 1
  ppu_ctrl = a;
  ldx_imm_nf(0xff); // reset stack pointer
  sp = x;
...
VBlank2:
  ...
  ldy_imm_nf(ColdBootOffset); // load default cold boot pointer
  ldx_imm_nf(0x5); // this is where we check for a warm boot
  
WBootCheck:
  lda_absx_nf(TopScoreDisplay); // check each score digit in the top score
  cmp_imm(10); // to see if we have a valid digit
  // if not, give up and proceed with cold boot
  if (!carry_flag) {
    dex_fn();
    if (!neg_flag) { goto WBootCheck; }
    lda_abs_nf(WarmBootValidation); // second checkpoint, check to see if
  ...
  inc_abs_nf(DisableScreenFlag); // set flag to disable screen output
  lda_abs_nf(Mirror_PPU_CTRL_REG1);
...
void NonMaskableInterrupt(void) {
  lda_abs_nf(Mirror_PPU_CTRL_REG1); // disable NMIs in mirror reg
  and_imm_nf(0b01111111); // save all other bits
  ram[Mirror_PPU_CTRL_REG1] = a;
  and_imm_nf(0b01111110); // alter name table address to be $2800
  ppu_ctrl = a; // (essentially $2000) but save other bits
  lda_abs_nf(Mirror_PPU_CTRL_REG2); // disable OAM and background display by default
  and_imm_nf(0b11100110);
  ldy_abs_fz(DisableScreenFlag); // get screen disable flag
  if (!zero_flag) { goto ScreenOff; } // if set, used bits as-is
  lda_abs_nf(Mirror_PPU_CTRL_REG2); // otherwise reenable bits and save them
  ora_imm_nf(0b00011110);
  
ScreenOff:
  ram[Mirror_PPU_CTRL_REG2] = a; // save bits for later but not in register at the moment
  and_imm_nf(0b11100111); // disable screen for now
  ppu_mask = a;
  ldx_abs_nf(PPU_STATUS); // reset flip-flop and reset scroll registers to zero
  lda_imm(0x0);
  InitScroll();
  oam_addr = a; // reset spr-ram address register
  lda_imm_nf(0x2); // perform spr-ram DMA access on $0200-$02ff
  ppu_transfer_oam((uint16_t)(a << 8));
  ldx_abs_nf(VRAM_Buffer_AddrCtrl); // load control for pointer to buffer contents
  lda_absx_nf(VRAM_AddrTable_Low); // set indirect at $00 to pointer
  ram[0x0] = a;
  lda_absx(VRAM_AddrTable_High);
  ram[0x1] = a;
  UpdateScreen(); // update screen with buffer contents
  ldy_imm_nf(0x0);
  ldx_abs_nf(VRAM_Buffer_AddrCtrl); // check for usage of $0341
  cpx_imm_fcz(0x6);
  if (!zero_flag) { goto InitBuffer; }
  iny_nf(); // get offset based on usage
  
InitBuffer:
  ldx_absy_nf(VRAM_Buffer_Offset);
  lda_imm_nf(0x0); // clear buffer header at last location
  ...
  UpdateTopScore();
  lda_abs_nf(GamePauseStatus); // check for pause status
  lsr_acc_fc();
...
DecTimers:
  ldx_imm_nf(0x14); // load end offset for end of frame timers
  ...
  ldx_imm_nf(0x23); // interval timers will decrement along with frame timers
  
DecTimersLoop:
  lda_absx_fz(Timers); // check current timer
  if (zero_flag) { goto SkipExpTimer; } // if current timer expired, branch to skip,
  dec_absx_nf(Timers); // otherwise decrement the current timer
...
NoDecTimers:
  inc_zp_nf(FrameCounter); // increment frame counter
  
PauseSkip:
  ldx_imm_nf(0x0);
  ldy_imm_nf(0x7);
  lda_abs_nf(PseudoRandomBitReg); // get first memory location of LSFR bytes
  and_imm_nf(0b00000010); // mask out all but d1
  ram[0x0] = a; // save here
  lda_abs_nf(PseudoRandomBitReg + 1); // get second memory location
  and_imm_nf(0b00000010); // mask out all but d1
...
Sprite0Clr:
  lda_abs_nf(PPU_STATUS); // wait for sprite 0 flag to clear, which will
  and_imm_fz(0b01000000); // not happen until vblank has ended
  if (!zero_flag) { goto Sprite0Clr; }
  lda_abs_nf(GamePauseStatus); // if in pause mode, do not bother with sprites at all
  lsr_acc();
...
Sprite0Hit:
  lda_abs_nf(PPU_STATUS); // do sprite #0 hit detection
  and_imm_fz(0b01000000);
  if (zero_flag) { goto Sprite0Hit; }
  ldy_imm_nf(0x14); // small delay, to wait until we hit horizontal blank time
...
SkipSprite0:
  lda_abs_nf(HorizontalScroll); // set scroll registers from variables
  ppu_write_scroll(a);
  lda_abs_nf(VerticalScroll);
  ppu_write_scroll(a);
  lda_abs_nf(Mirror_PPU_CTRL_REG1); // load saved mirror of $2000
  pha();
  ppu_ctrl = a;
  lda_abs_nf(GamePauseStatus); // if in pause mode, do not perform operation mode stuff
  lsr_acc();
  if (carry_flag) { goto SkipMainOper; }
  OperModeExecutionTree(); // otherwise do one of many, many possible subroutines
  
SkipMainOper:
  lda_abs_nf(PPU_STATUS); // reset flip-flop
  pla_nf();
...
void PauseRoutine(void) {
  lda_abs_nf(OperMode); // are we in victory mode?
  ...
  lda_abs_nf(OperMode_Task); // if we are in game mode, are we running game engine?
  cmp_imm(0x3);
  if (!zero_flag) { return; } // if not, leave
...
ChkStart:
  lda_abs_nf(SavedJoypad1Bits); // check to see if start is pressed
  and_imm_fz(Start_Button); // on controller 1
  if (zero_flag) { goto ClrPauseTimer; }
  lda_abs_nf(GamePauseStatus); // check to see if timer flag is set
  ...
  lda_imm_nf(0x2b); // set pause timer
  ram[GamePauseTimer] = a;
  lda_abs_nf(GamePauseStatus);
  tay_nf();
  iny_nf(); // set pause sfx queue for next pause mode
  ram[PauseSoundQueue] = y;
  eor_imm_nf(0b00000001); // invert d0 and set d7
  ora_imm(0b10000000);
  if (!zero_flag) { goto SetPause; } // unconditional branch
  
ClrPauseTimer:
  lda_abs_nf(GamePauseStatus); // clear timer flag if timer is at zero and start button
...
void SpriteShuffler(void) {
  ldy_abs_nf(AreaType); // load level type, likely residual code
  lda_imm_nf(0x28); // load preset value which will put it at
  ram[0x0] = a; // sprite #10
  ldx_imm_nf(0xe); // start at the end of OAM data offsets
  
ShuffleLoop:
  lda_absx_nf(SprDataOffset); // check for offset value against
  cmp_zp_fc(0x0); // the preset value
  // if less, skip this part
  if (carry_flag) {
    ldy_abs_nf(SprShuffleAmtOffset); // get current offset to preset value we want to add
    carry_flag = false;
    adc_absy_fc(SprShuffleAmt); // get shuffle amount, add to current sprite offset
    // if not exceeded $ff, skip second add
    if (carry_flag) {
      carry_flag = false;
      adc_zp_nf(0x0); // otherwise add preset value $28 to offset
    }
    // StrSprOffset:
  ...
  ldx_abs_nf(SprShuffleAmtOffset); // load offset
  inx_nf();
  cpx_imm_fz(0x3); // check if offset + 1 goes to 3
  // if offset + 1 not 3, store
  if (zero_flag) {
    ldx_imm_nf(0x0); // otherwise, init to 0
  }
  // SetAmtOffset:
  ram[SprShuffleAmtOffset] = x;
  ldx_imm_nf(0x8); // load offsets for values and storage
  ldy_imm_nf(0x2);
  
SetMiscOffset:
  lda_absy_nf(SprDataOffset + 5); // load one of three OAM data offsets
  ram[Misc_SprDataOffset - 2 + x] = a; // store first one unmodified, but
  carry_flag = false; // add eight to the second and eight
  adc_imm_nf(0x8); // more to the third one
...
void GameMenuRoutine(void) {
  ldy_imm_nf(0x0);
  lda_abs_nf(SavedJoypad1Bits); // check to see if either player pressed
  ora_abs_nf(SavedJoypad2Bits); // only the start button (either joypad)
...
ChkWorldSel:
  ...
  if (!zero_flag) { goto NullJoypad; }
  iny_nf(); // if so, increment Y and execute same code as select
...
SelectBLogic:
  ...
  lda_abs_nf(NumberOfPlayers); // if no, must have been the select button, therefore
...
IncWorldSel:
  ldx_abs_nf(WorldSelectNumber); // increment world select number
  inx_nf();
  txa_nf();
...
UpdateShroom:
  lda_absx_nf(WSelectBufferTemplate); // write template for world select in vram buffer
  ram[VRAM_Buffer1 - 1 + x] = a; // do this until all bytes are written
  inx_nf();
  cpx_imm_fcn(0x6);
  if (neg_flag) { goto UpdateShroom; }
  ldy_abs_nf(WorldNumber); // get world number from variable and increment for
  iny_nf(); // proper display, and put in blank byte before
...
RunDemo:
  ...
  lda_zp_nf(GameEngineSubroutine); // check to see if we're running lose life routine
  cmp_imm(0x6);
  if (!zero_flag) { return; } // if not, do not do all the resetting below
  
ResetTitle:
  lda_imm_nf(0x0); // reset game modes, disable
...
StartWorld1:
  LoadAreaPointer();
  inc_abs_nf(Hidden1UpFlag); // set 1-up box flag for both players
  inc_abs_nf(OffScr_Hidden1UpFlag);
  inc_abs_nf(FetchNewGameTimerFlag); // set fetch new game timer flag
  inc_abs_nf(OperMode); // set next game mode
  lda_abs_nf(WorldSelectEnableFlag); // if world select flag is on, then primary
...
void SetupIntermediate(void) {
  lda_abs_nf(BackgroundColorCtrl); // save current background color control
  pha(); // and player status to stack
  lda_abs_nf(PlayerStatus);
  pha();
  lda_imm_nf(0x0); // set background color to black
  ...
  GetPlayerColors();
  pla_nf(); // we only execute this routine for
...
void GetAreaPalette(void) {
  ldy_abs_nf(AreaType); // select appropriate palette to load
...
void GetBackgroundColor(void) {
  ...
  if (!zero_flag) {
    lda_absy_nf(BGColorCtrl_Addr - 4); // put appropriate palette into vram
    ram[VRAM_Buffer_AddrCtrl] = a; // note that if set to 5-7, $0301 will not be read
  }
  // NoBGColor:
  inc_abs_nf(ScreenRoutineTask); // increment to next subtask and plod on through
  GetPlayerColors(); // <fallthrough>
}

void GetPlayerColors(void) {
  ldx_abs_nf(VRAM_Buffer1_Offset); // get current buffer offset
  ldy_imm_nf(0x0);
  lda_abs_fz(CurrentPlayer); // check which player is on the screen
  if (!zero_flag) {
    ldy_imm_nf(0x4); // load offset for luigi
  }
  // ChkFiery:
  lda_abs_nf(PlayerStatus); // check player status
  cmp_imm_fz(0x2);
  ...
  // StartClrGet:
  lda_imm_nf(0x3); // do four colors
  ram[0x0] = a;
  
ClrGetLoop:
  lda_absy_nf(PlayerColors); // fetch player colors and store them
  ram[VRAM_Buffer1 + 3 + x] = a; // in the buffer
  iny_nf();
  inx_nf();
  dec_zp_fn(0x0);
  if (!neg_flag) { goto ClrGetLoop; }
  ldx_abs_nf(VRAM_Buffer1_Offset); // load original offset from before
  ...
  if (zero_flag) {
    ldy_abs_nf(AreaType); // otherwise use area type bits from area offset as offset
  }
  // SetBGColor:
  lda_absy_nf(BackgroundColors); // to background color instead
  ram[VRAM_Buffer1 + 3 + x] = a;
  lda_imm_nf(0x3f); // set for sprite palette address
  ram[VRAM_Buffer1 + x] = a; // save to buffer
  lda_imm_nf(0x10);
  ram[VRAM_Buffer1 + 1 + x] = a;
  lda_imm_nf(0x4); // write length byte to buffer
  ram[VRAM_Buffer1 + 2 + x] = a;
  lda_imm_nf(0x0); // now the null terminator
  ram[VRAM_Buffer1 + 7 + x] = a;
  txa_nf(); // move the buffer pointer ahead 7 bytes
...
void GetAlternatePalette1(void) {
  lda_abs_nf(AreaStyle); // check for mushroom level style
  cmp_imm(0x1);
  if (!zero_flag) {
    NoAltPal();
    return;
...
void WriteBottomStatusLine(void) {
  ...
  ldx_abs_nf(VRAM_Buffer1_Offset);
  lda_imm_nf(0x20); // write address for world-area number on screen
  ram[VRAM_Buffer1 + x] = a;
  lda_imm_nf(0x73);
  ram[VRAM_Buffer1 + 1 + x] = a;
  lda_imm_nf(0x3); // write length for it
  ram[VRAM_Buffer1 + 2 + x] = a;
  ldy_abs_nf(WorldNumber); // first the world number
  iny_nf();
  tya_nf();
  ram[VRAM_Buffer1 + 3 + x] = a;
  lda_imm_nf(0x28); // next the dash
  ram[VRAM_Buffer1 + 4 + x] = a;
  ldy_abs_nf(LevelNumber); // next the level number
  iny_nf(); // increment for proper number display
  tya_nf();
  ram[VRAM_Buffer1 + 5 + x] = a;
  lda_imm_nf(0x0); // put null terminator on
  ram[VRAM_Buffer1 + 6 + x] = a;
  txa_nf(); // move the buffer offset up by 6 bytes
  carry_flag = false;
  adc_imm(0x6);
...
void DisplayIntermediate(void) {
  ...
  ldy_abs_nf(AreaType); // check if we are on castle level
...
void GameOverInter(void) {
  lda_imm_nf(0x12); // set screen timer
  ram[ScreenTimer] = a;
...
TaskLoop:
  ...
  if (neg_flag) {
    inc_abs_nf(ScreenRoutineTask); // if not, move on to the next task
  }
  // OutputCol:
...
void DrawTitleScreen(void) {
  ...
  lda_imm_nf(HIGH_BYTE(TitleScreenDataOffset)); // load address $1ec0 into
  ppu_write_address(a); // the vram address register
  lda_imm_nf(LOW_BYTE(TitleScreenDataOffset));
  ppu_write_address(a);
  lda_imm_nf(0x3); // put address $0300 into
  ram[0x1] = a; // the indirect at $00
  ldy_imm_nf(0x0);
  ram[0x0] = y;
  lda_abs_nf(PPU_DATA); // do one garbage read
  
OutputTScr:
  lda_abs_nf(PPU_DATA); // get title screen from chr-rom
  dynamic_ram_write(read_word(0x0) + y, a); // store 256 bytes into buffer
  iny_fz();
  // if not past 256 bytes, do not increment
  if (zero_flag) {
    inc_zp_nf(0x1); // otherwise increment high byte of indirect
  }
  // ChkHiByte:
  lda_zp_nf(0x1); // check high byte?
...
void ClearBuffersDrawIcon(void) {
  ...
  ldx_imm_nf(0x0); // otherwise, clear buffer space
...
StartPage:
  ...
  ldy_imm_nf(0x20); // if on odd numbered page, use $2480 as start of rendering
...
SetInitNTHigh:
  ...
  asl_acc_nf(); // store LSB of page number in high nybble
  asl_acc_nf(); // of block buffer column position
  asl_acc_nf();
  asl_acc_fc();
  ram[BlockBufferColumnPos] = a;
  dec_abs_nf(AreaObjectLength); // set area object lengths for all empty
  dec_abs_nf(AreaObjectLength + 1);
  dec_abs_nf(AreaObjectLength + 2);
  ...
  lda_abs_nf(WorldNumber); // otherwise check world number
  ...
  lda_abs_nf(LevelNumber); // otherwise, world 5, so check level number
  cmp_imm_fc(Level3); // if 1 or 2, do not set secondary hard mode flag
  if (!carry_flag) { goto CheckHalfway; }
  
SetSecHard:
  inc_abs_nf(SecondaryHardMode); // set secondary hard mode flag for areas 5-3 and beyond
  
CheckHalfway:
  lda_abs_fz(HalfwayPage);
  if (zero_flag) { goto DoneInitArea; }
  lda_imm_nf(0x2); // if halfway page set, overwrite start position from header
  ram[PlayerEntranceCtrl] = a;
  
DoneInitArea:
  lda_imm_nf(Silence); // silence music
  ram[AreaMusicQueue] = a;
  lda_imm_nf(0x1); // disable screen output
...
ClearVRLoop:
  ...
  lda_abs_nf(ScreenLeft_PageLoc); // get left side page location
  lsr_abs_fc(Mirror_PPU_CTRL_REG1); // shift LSB of ppu register #1 mirror out
  and_imm_nf(0x1); // mask out all but LSB of page location
  ...
  lda_imm_nf(0x38); // load sprite shuffle amounts to be used later
  ram[SprShuffleAmt + 2] = a;
  lda_imm_nf(0x48);
  ram[SprShuffleAmt + 1] = a;
  lda_imm_nf(0x58);
  ram[SprShuffleAmt] = a;
  ldx_imm_nf(0xe); // load default OAM offsets into $06e4-$06f2
  
ShufAmtLoop:
  lda_absx_nf(DefaultSprOffsets);
  ram[SprDataOffset + x] = a;
  dex_fn(); // do this until they're all set
  if (!neg_flag) { goto ShufAmtLoop; }
  ldy_imm_nf(0x3); // set up sprite #0
  
ISpr0Loop:
  lda_absy_nf(Sprite0Data);
  ram[Sprite_Data + y] = a;
  dey();
  if (!neg_flag) { goto ISpr0Loop; }
  DoNothing2(); // these jsrs doesn't do anything useful
  DoNothing1();
  inc_abs_nf(Sprite0HitDetectFlag); // set sprite #0 check flag
...
void SetupGameOver(void) {
  lda_imm_nf(0x0); // reset screen routine task control for title screen, game,
  ram[ScreenRoutineTask] = a; // and game over modes
  ram[Sprite0HitDetectFlag] = a; // disable sprite 0 check
  lda_imm_nf(GameOverMusic);
  ram[EventMusicQueue] = a; // put game over music in secondary queue
  inc_abs_nf(DisableScreenFlag); // disable screen output
  inc_abs(OperMode_Task); // set secondary mode to 1
  // -------------------------------------------------------------------------------------
}

void RunGameOver(void) {
  lda_imm_nf(0x0); // reenable screen
  ram[DisableScreenFlag] = a;
  lda_abs_nf(SavedJoypad1Bits); // check controller for start pressed
...
void TerminateGame(void) {
  ...
  lda_abs_nf(WorldNumber); // otherwise put world number of current
...
void ContinueGame(void) {
  ...
  lda_imm_nf(0x1); // actual world and area numbers, then
  ram[PlayerSize] = a; // reset player's size, status, and
  inc_abs_nf(FetchNewGameTimerFlag); // set game timer flag to reload
  lda_imm_nf(0x0); // game timer from header
...
void GameCoreRoutine(void) {
  ldx_abs_nf(CurrentPlayer); // get which player is on the screen
  ...
  lda_abs_nf(OperMode_Task); // check major task of operating mode
...
NoChgMus:
  ldy_abs_nf(StarInvincibleTimer); // get invincibility timer
  lda_zp_nf(FrameCounter); // get frame counter
  cpy_imm_fc(0x8); // if timer still above certain point,
  if (carry_flag) { goto CycleTwo; } // branch to cycle player's palette quickly
  lsr_acc_nf(); // otherwise, divide by 8 to cycle every eighth frame
  lsr_acc_nf();
...
SaveAB:
  lda_zp_nf(A_B_Buttons); // save current A and B button
...
void UpdScrollVar(void) {
  lda_abs_nf(VRAM_Buffer_AddrCtrl);
  ...
  lda_abs_nf(ScrollThirtyTwo); // get horizontal scroll in 0-31 or $00-$20 range
  cmp_imm(0x20); // check to see if exceeded $21
  if (neg_flag) { return; } // branch to leave if not
  lda_abs_nf(ScrollThirtyTwo);
...
CntPl:
  ...
  tax_nf(); // initialize X to zero
  ldy_abs_nf(Player_SprDataOffset); // get player sprite data offset
  lda_zp_nf(PlayerFacingDir); // get player's facing direction
  lsr_acc_fc();
  if (carry_flag) { goto SwimKT; } // if player facing to the right, use current offset
  iny_nf();
  iny_nf(); // otherwise move to next OAM data
  iny_nf();
  iny_nf();
...
SwimKT:
  ...
  lda_absy_nf(Sprite_Tilenumber + 24); // check tile number of seventh/eighth sprite
  ...
  inx_nf(); // otherwise increment X for second tile
...
void PlayerKilled(void) {
  ldy_imm_nf(0xe); // load offset for player killed
  lda_absy_nf(PlayerGfxTblOffsets); // get offset to graphics table
...
void PlayerGfxProcessing(void) {
  ...
  if (!zero_flag) {
    ldy_imm_nf(0x0); // set value to initialize by default
    lda_abs_nf(PlayerAnimTimer); // get animation frame timer
    ...
    if (!carry_flag) {
      ...
      ldy_imm_nf(0x7); // load offset for throwing
      lda_absy_nf(PlayerGfxTblOffsets); // get offset to graphics table
      ram[PlayerGfxOffset] = a; // store it for use later
      ldy_imm_nf(0x4); // set to update four sprite rows by default
      lda_zp_nf(Player_X_Speed);
      ...
      if (!zero_flag) {
        dey_nf(); // otherwise set to update only three sprite rows
      }
      // SUpdR:
  ...
  // PlayerOffscreenChk:
  lda_abs_nf(Player_OffscreenBits); // get player's offscreen bits
  lsr_acc_nf();
  lsr_acc_nf(); // move vertical bits to low nybble
  lsr_acc_nf();
  lsr_acc_nf();
  ram[0x0] = a; // store here
  ldx_imm_nf(0x3); // check all four rows of player sprites
  lda_abs_nf(Player_SprDataOffset); // get player's sprite data offset
  carry_flag = false;
  adc_imm_nf(0x18); // add 24 bytes to start at bottom row
  tay_nf(); // set as offset here
  
PROfsLoop:
  lda_imm_nf(0xf8); // load offscreen Y coordinate just in case
...
void MoveSpritesOffscreenSkip(void) {
  lda_imm_nf(0xf8); // off the screen
  
SprInitLoop:
  ram[Sprite_Y_Position + y] = a; // write 248 into OAM data's Y coordinate
  iny_nf(); // which will move it off the screen
  iny_nf();
  iny_nf();
  iny();
...
void DrawMushroomIcon(void) {
  ldy_imm_nf(0x7); // read eight bytes to be read by transfer routine
  
IconDataRead:
  lda_absy_nf(MushroomIconData); // note that the default position is set for a
  ram[VRAM_Buffer1 - 1 + y] = a; // 1-player game
  dey_fn();
  if (!neg_flag) { goto IconDataRead; }
  lda_abs(NumberOfPlayers); // check number of players
  if (!zero_flag) {
    lda_imm_nf(0x24); // otherwise, load blank tile in 1-player position
...
void DemoEngine(void) {
  ldx_abs_nf(DemoAction); // load current demo action
...
DoAction:
  lda_absx_nf(DemoActionData - 1); // get and perform action (current or next)
  ram[SavedJoypad1Bits] = a;
...
void SetupVictoryMode(void) {
  ldx_abs_nf(ScreenRight_PageLoc); // get page location of right side of screen
  inx_nf(); // increment to next page
...
void PlayerVictoryWalk(void) {
  ldy_imm_nf(0x0); // set value here to not walk player by default
  ram[VictoryWalkControl] = y;
  lda_zp_nf(Player_PageLoc); // get player's page location
  ...
  lda_zp_nf(Player_X_Position); // otherwise get player's horizontal position
  cmp_imm_fc(0x60); // compare with preset horizontal position
  if (carry_flag) { goto DontWalk; } // if still on other page, branch ahead
  
PerformWalk:
  inc_zp_nf(VictoryWalkControl); // otherwise increment value and Y
  iny_nf(); // note Y will be used to walk the player
  
DontWalk:
  tya(); // put contents of Y in A and
  AutoControlPlayer(); // use A to move player to the right or not
  lda_abs_nf(ScreenLeft_PageLoc); // check page location of left side of screen
  ...
  inc_zp_nf(VictoryWalkControl); // increment value to stay in this routine
...
void PrintVictoryMessages(void) {
  ...
  ldy_abs_nf(WorldNumber); // check world number
  cpy_imm_fz(World8);
  ...
  sbc_imm_nf(0x1); // otherwise subtract one
...
SecondPartMsg:
  iny_nf(); // increment Y to do world 8's message
  lda_abs_nf(WorldNumber);
  cmp_imm_fz(World8); // check world number
  if (zero_flag) { goto EvalForMusic; } // if at world 8, branch to next part
  dey_nf(); // otherwise decrement Y for world 1-7's message
...
void PlayerEndWorld(void) {
  ...
  inc_abs_nf(FetchNewGameTimerFlag); // set flag to load game timer from header
...
EndChkBButton:
  lda_abs_nf(SavedJoypad1Bits);
  ora_abs_nf(SavedJoypad2Bits); // check to see if B button was pressed on
  and_imm(B_Button); // either controller
  if (zero_flag) { return; } // branch to leave if not
  lda_imm_nf(0x1); // otherwise set world selection flag
...
void BridgeCollapse(void) {
  ldx_abs_nf(BowserFront_Offset); // get enemy offset for bowser
  lda_zpx_nf(Enemy_ID); // check enemy object identifier for bowser
  ...
  if (zero_flag) {
    ...
    if (!zero_flag) {
      lda_zpx_nf(Enemy_Y_Position); // check bowser's vertical coordinate
...
void RemoveBridge(void) {
  ...
  if (zero_flag) {
    ...
    lda_abs_nf(BowserBodyControls);
    eor_imm_nf(0x1); // invert bit to control bowser's feet
    ram[BowserBodyControls] = a;
    lda_imm_nf(0x22); // put high byte of name table address here for now
    ram[0x5] = a;
    ldy_abs_nf(BridgeCollapseOffset); // get bridge collapse offset here
    lda_absy_nf(BridgeCollapseData); // load low byte of name table address and store here
    ram[0x4] = a;
    ldy_abs_nf(VRAM_Buffer1_Offset); // increment vram buffer offset
    iny_nf();
    ...
    lda_imm_nf(Sfx_Blast); // load the fireworks/gunfire sound into the square 2 sfx
    ram[Square2SoundQueue] = a; // queue while at the same time loading the brick
    lda_imm_nf(Sfx_BrickShatter); // shatter sound into the noise sfx queue thus
    ram[NoiseSoundQueue] = a; // producing the unique sound of the bridge collapsing
    inc_abs_nf(BridgeCollapseOffset); // increment bridge collapse offset
    lda_abs_nf(BridgeCollapseOffset);
...
void FloateyNumbersRoutine(void) {
  ...
  lda_imm_nf(0xb); // otherwise set to $0b, thus keeping
...
DecNumTimer:
  dec_absx_nf(FloateyNum_Timer); // decrement value here
  ...
  inc_abs_nf(NumberofLives); // give player one extra life (1-up)
  lda_imm_nf(Sfx_ExtraLife);
  ram[Square2SoundQueue] = a; // and play the 1-up sound
  
LoadNumTiles:
  lda_absy_nf(ScoreUpdateData); // load point value here
  lsr_acc_nf(); // move high nybble to low
  lsr_acc_nf();
  lsr_acc_nf();
  lsr_acc_fc();
  tax_nf(); // use as X offset, essentially the digit
  lda_absy_nf(ScoreUpdateData); // load again and this time
...
ChkTallEnemy:
  ldy_absx_nf(Enemy_SprDataOffset); // get OAM data offset for enemy object
  lda_zpx_nf(Enemy_ID); // get enemy object identifier
  cmp_imm_fz(Spiny);
  ...
  lda_zpx_nf(Enemy_State);
  cmp_imm_fc(0x2); // if enemy state defeated or otherwise
  if (carry_flag) { goto FloateyPart; } // $02 or greater, branch beyond this part
  
GetAltOffset:
  ldx_abs_nf(SprDataOffset_Ctrl); // load some kind of control bit
  ldy_absx_nf(Alt_SprDataOffset); // get alternate OAM data offset
  ldx_zp_nf(ObjectOffset); // get enemy object offset again
  
FloateyPart:
  lda_absx_nf(FloateyNum_Y_Pos); // get vertical coordinate for
...
SetupNumSpr:
  ...
  lda_absx_nf(FloateyNum_Control);
  asl_acc_fc(); // multiply our floatey number control by 2
  tax_nf(); // and use as offset for look-up table
  lda_absx_nf(FloateyNumTileData);
  ram[Sprite_Tilenumber + y] = a; // display first half of number of points
  lda_absx_nf(FloateyNumTileData + 1);
...
void WriteGameText(void) {
  ...
  asl_acc_nf();
  tay_nf(); // multiply by 2 and use as offset
  ...
  if (carry_flag) {
    ...
    if (carry_flag) {
      ldy_imm_nf(0x8); // otherwise warp zone, therefore set offset
    }
    // Chk2Players:
    lda_abs_fz(NumberOfPlayers); // check for number of players
    // if there are two, use current offset to also print name
    if (zero_flag) {
      iny_nf(); // otherwise increment offset by one to not print name
    }
  }
  // LdGameText:
  ldx_absy_nf(GameTextOffsets); // get offset to message we want to print
  ldy_imm_nf(0x0);
  
GameTextLoop:
  lda_absx_nf(GameText); // load message data
  cmp_imm_fz(0xff); // check for terminator
  // branch to end text if found
  if (!zero_flag) {
    ram[VRAM_Buffer1 + y] = a; // otherwise write data to buffer
    inx_nf(); // and increment increment
    iny_fz();
    if (!zero_flag) { goto GameTextLoop; } // do this for 256 bytes if no terminator found
  }
  // EndGameText:
  lda_imm_nf(0x0); // put null terminator at end
  ram[VRAM_Buffer1 + y] = a;
  pla_nf(); // pull original text number from stack
  tax_nf();
  ...
  if (!carry_flag) {
    ...
    if (zero_flag) {
      lda_abs_nf(NumberofLives); // otherwise, check number of lives
      ...
      if (carry_flag) {
        ...
        ldy_imm_nf(0x9f); // next to the difference...strange things happen if
        ram[VRAM_Buffer1 + 7] = y; // the number of lives exceeds 19
      }
      // PutLives:
      ram[VRAM_Buffer1 + 8] = a;
      ldy_abs_nf(WorldNumber); // write world and level numbers (incremented for display)
      iny_nf(); // to the buffer in the spaces surrounding the dash
      ram[VRAM_Buffer1 + 19] = y;
      ldy_abs_nf(LevelNumber);
      iny();
    ...
    if (!zero_flag) {
      lda_abs_nf(CurrentPlayer); // load current player
      dex_fz(); // check to see if current message number is for time up
      if (zero_flag) {
        ldy_abs_nf(OperMode); // check for game over mode
        cpy_imm_fz(GameOverModeValue);
        if (!zero_flag) {
          eor_imm_nf(0b00000001); // if not, must be time up, invert d0 to do other player
        }
      }
      // ChkLuigi:
      lsr_acc();
...
NameLoop:
        lda_absy_nf(LuigiName); // otherwise, replace "MARIO" with "LUIGI"
  ...
  sbc_imm_nf(0x4); // subtract 4 and then shift to the left
  asl_acc_nf(); // twice to get proper warp zone number
  asl_acc_nf(); // offset
  tax_nf();
  ldy_imm_nf(0x0);
  
WarpNumLoop:
  lda_absx_nf(WarpZoneNumbers); // print warp zone numbers into the
  ram[VRAM_Buffer1 + 27 + y] = a; // placeholders from earlier
  inx_nf();
  iny_nf(); // put a number in every fourth space
  iny_nf();
  iny_nf();
  iny_nf();
  cpy_imm_fc(0xc);
...
void ResetScreenTimer(void) {
  lda_imm_nf(0x7); // reset timer again
  ram[ScreenTimer] = a;
  inc_abs(ScreenRoutineTask); // move onto next task
  // -------------------------------------------------------------------------------------
...
void RenderAreaGraphics(void) {
  lda_abs_nf(CurrentColumnPos); // store LSB of where we're at
  and_imm_nf(0x1);
  ram[0x5] = a;
  ldy_abs_nf(VRAM_Buffer2_Offset); // store vram buffer offset
  ram[0x0] = y;
  lda_abs_nf(CurrentNTAddr_Low); // get current name table address we're supposed to render
  ram[VRAM_Buffer2 + 1 + y] = a;
  lda_abs_nf(CurrentNTAddr_High);
  ram[VRAM_Buffer2 + y] = a;
  lda_imm_nf(0x9a); // store length byte of 26 here with d7 set
  ram[VRAM_Buffer2 + 2 + y] = a; // to increment by 32 (in columns)
  lda_imm_nf(0x0); // init attribute row
  ram[0x4] = a;
  tax_nf();
  
DrawMTLoop:
  ram[0x1] = x; // store init value of 0 or incremented offset for buffer
  lda_absx_nf(MetatileBuffer); // get first metatile number, and mask out all but 2 MSB
  and_imm_nf(0b11000000);
  ram[0x3] = a; // store attribute table bits here
  asl_acc_fc(); // note that metatile format is:
  rol_acc_fc(); // %xx000000 - attribute table bits,
  rol_acc_nf(); // %00xxxxxx - metatile number
  tay_nf(); // rotate bits to d1-d0 and use as offset here
  lda_absy_nf(MetatileGraphics_Low); // get address to graphics table from here
  ram[0x6] = a;
  lda_absy_nf(MetatileGraphics_High);
  ram[0x7] = a;
  lda_absx_nf(MetatileBuffer); // get metatile number again
  asl_acc_nf(); // multiply by 4 and use as tile offset
  asl_acc_nf();
  ram[0x2] = a;
  lda_abs_nf(AreaParserTaskNum); // get current task number for level processing and
  and_imm_nf(0b00000001); // mask out all but LSB, then invert LSB, multiply by 2
  eor_imm_nf(0b00000001); // to get the correct column position in the metatile,
  asl_acc_fc(); // then add to the tile offset so we can draw either side
  adc_zp_nf(0x2); // of the metatiles
  tay_nf();
  ldx_zp_nf(0x0); // use vram buffer offset from before as X
  lda_indy_nf(0x6);
  ram[VRAM_Buffer2 + 3 + x] = a; // get first tile number (top left or top right) and store
  iny_nf();
  lda_indy_nf(0x6); // now get the second (bottom left or bottom right) and store
  ram[VRAM_Buffer2 + 4 + x] = a;
  ldy_zp_nf(0x4); // get current attribute row
  ...
  lda_zp_nf(0x1); // get current row we're rendering
...
RightCheck:
  lda_zp_nf(0x1); // get LSB of current row we're rendering
  lsr_acc_fc(); // branch if set (clear = top right, set = bottom right)
  if (carry_flag) { goto NextMTRow; }
  lsr_zp_nf(0x3); // shift attribute bits 4 to the right
  lsr_zp_nf(0x3); // thus in d3-d2, for upper right square
  lsr_zp_nf(0x3);
  lsr_zp_nf(0x3);
  goto SetAttrib;
  
LLeft:
  lsr_zp_nf(0x3); // shift attribute bits 2 to the right
  lsr_zp_nf(0x3); // thus in d5-d4 for lower left square
  
NextMTRow:
  inc_zp_nf(0x4); // move onto next attribute row
  
SetAttrib:
  lda_absy_nf(AttributeBuffer); // get previously saved bits from before
  ora_zp_nf(0x3); // if any, and put new bits, if any, onto
  ram[AttributeBuffer + y] = a; // the old, and store
  inc_zp_nf(0x0); // increment vram buffer offset by 2
  inc_zp_nf(0x0);
  ldx_zp_nf(0x1); // get current gfx buffer row, and check for
  inx_nf(); // the bottom of the screen
  cpx_imm_fc(0xd);
  if (!carry_flag) { goto DrawMTLoop; } // if not there yet, loop back
  ldy_zp_nf(0x0); // get current vram buffer offset, increment by 3
  iny_nf(); // (for name table address and length bytes)
  iny_nf();
  iny_nf();
  lda_imm_nf(0x0);
  ram[VRAM_Buffer2 + y] = a; // put null terminator at end of data for name table
  ram[VRAM_Buffer2_Offset] = y; // store new buffer offset
  inc_abs_nf(CurrentNTAddr_Low); // increment name table address low
  lda_abs_nf(CurrentNTAddr_Low); // check current low byte
  and_imm(0b00011111); // if no wraparound, just skip this part
  if (!zero_flag) { goto ExitDrawM; }
  lda_imm_nf(0x80); // if wraparound occurs, make sure low byte stays
  ram[CurrentNTAddr_Low] = a; // just under the status bar
  lda_abs_nf(CurrentNTAddr_High); // and then invert d2 of the name table address high
...
void RenderAttributeTables(void) {
  lda_abs_nf(CurrentNTAddr_Low); // get low byte of next name table address
  and_imm_nf(0b00011111); // to be written to, mask out all but 5 LSB,
  carry_flag = true; // subtract four
  sbc_imm_fc(0x4);
  and_imm_nf(0b00011111); // mask out bits again and store
  ram[0x1] = a;
  lda_abs_nf(CurrentNTAddr_High); // get high byte and branch if borrow not set
  if (!carry_flag) {
    eor_imm_nf(0b00000100); // otherwise invert d2
  }
  // SetATHigh:
  and_imm_nf(0b00000100); // mask out all other bits
  ora_imm_nf(0x23); // add $2300 to the high byte and store
  ram[0x0] = a;
  lda_zp_nf(0x1); // get low byte - 4, divide by 4, add offset for
  lsr_acc_nf(); // attribute table and store
  lsr_acc_fc();
  adc_imm_nf(0xc0); // we should now have the appropriate block of
  ram[0x1] = a; // attribute table in our temp address
  ldx_imm_nf(0x0);
  ldy_abs_nf(VRAM_Buffer2_Offset); // get buffer offset
...
AttribLoop:
  ...
  adc_imm_nf(0x8); // below the status bar, and store
  ram[VRAM_Buffer2 + 1 + y] = a;
  ram[0x1] = a; // also store in temp again
  lda_absx_nf(AttributeBuffer); // fetch current attribute table byte and store
  ...
  iny_nf(); // increment buffer offset by 4 bytes
  iny_nf();
  iny_nf();
  iny_nf();
  inx_nf(); // increment attribute offset and check to see
...
void ColorRotation(void) {
  ...
  ldx_abs_nf(VRAM_Buffer1_Offset); // check vram buffer offset
  cpx_imm(0x31);
  if (carry_flag) { return; } // if offset over 48 bytes, branch to leave
  tay_nf(); // otherwise use frame counter's 3 LSB as offset here
  
GetBlankPal:
  lda_absy_nf(BlankPalette); // get blank palette for palette 3
  ram[VRAM_Buffer1 + x] = a; // store it in the vram buffer
  inx_nf(); // increment offsets
  iny_nf();
  cpy_imm_fc(0x8);
  if (!carry_flag) { goto GetBlankPal; } // do this until all bytes are copied
  ldx_abs_nf(VRAM_Buffer1_Offset); // get current vram buffer offset
  lda_imm_nf(0x3);
  ram[0x0] = a; // set counter here
  lda_abs_nf(AreaType); // get area type
  asl_acc_nf(); // multiply by 4 to get proper offset
  asl_acc_nf();
  tay_nf(); // save as offset here
  
GetAreaPal:
  lda_absy_nf(Palette3Data); // fetch palette to be written based on area type
  ...
  ldx_abs_nf(VRAM_Buffer1_Offset); // get current vram buffer offset
  ldy_abs_nf(ColorRotateOffset); // get color cycling offset
  lda_absy_nf(ColorRotatePalette);
  ram[VRAM_Buffer1 + 4 + x] = a; // get and store current color in second slot of palette
  lda_abs_nf(VRAM_Buffer1_Offset);
  carry_flag = false; // add seven bytes to vram buffer offset
  adc_imm_nf(0x7);
  ram[VRAM_Buffer1_Offset] = a;
  inc_abs_nf(ColorRotateOffset); // increment color cycling offset
  lda_abs_nf(ColorRotateOffset);
...
void RemoveCoin_Axe(void) {
  ldy_imm_nf(0x41); // set low byte so offset points to $0341
  lda_imm_nf(0x3); // load offset for default blank metatile
...
void ReplaceBlockMetatile(void) {
  ...
  inc_abs_nf(Block_ResidualCounter); // increment unused counter (residual code)
...
void WriteBlockMetatile(void) {
  ...
  if (!zero_flag) {
    ...
    if (!zero_flag) {
      ...
      if (!zero_flag) {
        iny_nf(); // increment offset for brick metatile w/o line
        cmp_imm_fcz(0x5d);
        ...
        if (!zero_flag) {
          ...
          if (!zero_flag) {
            iny_nf(); // if any other metatile, increment offset for empty block
          }
        }
      }
    }
  ...
  // UseBOffset:
  tya_nf(); // put Y in A
  ldy_abs_nf(VRAM_Buffer1_Offset); // get vram buffer offset
...
void MoveVOffset(void) {
  dey_nf(); // decrement vram buffer offset
  tya_nf(); // add 10 bytes to it
  carry_flag = false;
  adc_imm(10);
...
void PutBlockMetatile(void) {
  ...
  asl_acc_nf();
  asl_acc_nf(); // multiply A by four and use as X
  tax_nf();
  ldy_imm_nf(0x20); // load high byte for name table 0
  lda_zp_nf(0x6); // get low byte of block buffer pointer
  cmp_imm_fc(0xd0); // check to see if we're on odd-page block buffer
  // if not, use current high byte
  if (carry_flag) {
    ldy_imm_nf(0x24); // otherwise load high byte for name table 1
  }
  // SaveHAdder:
  ram[0x3] = y; // save high byte here
  and_imm_nf(0xf); // mask out high nybble of block buffer pointer
  asl_acc_nf(); // multiply by 2 to get appropriate name table low byte
  ram[0x4] = a; // and then store it here
  lda_imm_nf(0x0);
  ram[0x5] = a; // initialize temp high byte
  lda_zp_nf(0x2); // get vertical high nybble offset used in block buffer routine
  carry_flag = false;
  adc_imm_nf(0x20); // add 32 pixels for the status bar
  asl_acc_fc();
  rol_zp_nf(0x5); // shift and rotate d7 onto d0 and d6 into carry
  asl_acc_fc();
  ...
  lda_zp_nf(0x5); // get whatever was in d7 and d6 of vertical high nybble
  adc_imm_nf(0x0); // add carry
  carry_flag = false;
  adc_zp_nf(0x3); // then add high byte of name table
  ram[0x5] = a; // store here
  ldy_zp_nf(0x1); // get vram buffer offset to be used
  RemBridge(); // <fallthrough>
}

void RemBridge(void) {
  lda_absx_nf(BlockGfxData); // write top left and top right
  ram[VRAM_Buffer1 + 2 + y] = a; // tile numbers into first spot
  lda_absx_nf(BlockGfxData + 1);
  ram[VRAM_Buffer1 + 3 + y] = a;
  lda_absx_nf(BlockGfxData + 2); // write bottom left and bottom
  ram[VRAM_Buffer1 + 7 + y] = a; // right tiles numbers into
  lda_absx_nf(BlockGfxData + 3); // second spot
  ...
  lda_zp_nf(0x5); // plus 32 bytes into second slot
...
void InitializeNameTables(void) {
  lda_abs_nf(PPU_STATUS); // reset flip-flop
  lda_abs_nf(Mirror_PPU_CTRL_REG1); // load mirror of ppu reg $2000
  ora_imm_nf(0b00010000); // set sprites for first 4k and background for second 4k
...
void WriteNTAddr(void) {
  ...
  lda_imm_nf(0x0);
  ppu_write_address(a);
  ldx_imm_nf(0x4); // clear name table with blank tile #24
  ldy_imm_nf(0xc0);
  lda_imm_nf(0x24);
...
InitNTLoop:
  ...
  dex_fz();
  if (!zero_flag) { goto InitNTLoop; }
  ldy_imm_nf(64); // now to clear the attribute table (with zero this time)
  txa_nf();
...
void ReadJoypads(void) {
  lda_imm_nf(0x1); // reset and clear strobe of joypad ports
  write_joypad1(a);
  lsr_acc_fc();
...
PortLoop:
  ...
  lda_absx_nf(JOYPAD_PORT); // read current bit on joypad port
  ram[0x0] = a; // check d1 and d0 of port output
  lsr_acc_nf(); // this is necessary on the old
  ora_zp_nf(0x0); // famicom systems in japan
  lsr_acc_fc();
  pla_nf(); // read bits from stack
  ...
  pha();
  and_imm_nf(0b00110000); // check for select or start
...
void WriteBufferToScreen(void) {
  ...
  iny_nf();
  lda_indy_nf(0x0); // load next byte (second)
  ppu_write_address(a); // store low byte of vram address
  iny_nf();
  lda_indy_nf(0x0); // load next byte (third)
  asl_acc_fc(); // shift to left and save in stack
  pha();
  lda_abs_nf(Mirror_PPU_CTRL_REG1); // load mirror of $2000,
  ...
  pla_nf(); // pull from stack and shift to left again
  asl_acc_fc();
  // if d6 of third byte was clear, do not repeat byte
  if (carry_flag) {
    ora_imm_nf(0b00000010); // otherwise set d1 and increment Y
    iny_nf();
  }
  // GetLength:
  lsr_acc_nf(); // shift back to the right to get proper length
...
OutputToVRAM:
  ...
  if (!carry_flag) {
    iny_nf(); // otherwise increment Y to load next byte
  }
  // RepeatByte:
  lda_indy_nf(0x0); // load more data from buffer and write to vram
  ppu_write_data(a);
  dex_fz(); // done writing?
  ...
  adc_zp_fc(0x1);
  ram[0x1] = a;
  lda_imm_nf(0x3f); // sets vram address to $3f00
  ppu_write_address(a);
  lda_imm_nf(0x0);
  ppu_write_address(a);
...
void UpdateScreen(void) {
  ldx_abs_nf(PPU_STATUS); // reset flip-flop
  ldy_imm_nf(0x0); // load first byte from indirect as a pointer
  lda_indy(0x0);
...
void PrintStatusBarNumbers(void) {
  ...
  lda_zp_nf(0x0); // move high nybble to low
  lsr_acc_nf(); // and print to score display
  lsr_acc_nf();
  lsr_acc_nf();
  lsr_acc();
  OutputNumbers();
...
void OutputNumbers(void) {
  ...
  adc_imm_nf(0x1);
  and_imm_nf(0b00001111); // mask out high nybble
  cmp_imm(0x6);
  if (!carry_flag) {
    pha(); // save incremented value to stack for now and
    asl_acc_nf(); // shift to left and use as offset
    tay_nf();
    ldx_abs_nf(VRAM_Buffer1_Offset); // get current buffer pointer
    lda_imm_nf(0x20); // put at top of screen by default
    cpy_imm_fz(0x0); // are we writing top score on title screen?
    if (zero_flag) {
      lda_imm_nf(0x22); // if so, put further down on the screen
    }
    // SetupNums:
    ram[VRAM_Buffer1 + x] = a;
    lda_absy_nf(StatusBarData); // write low vram address and length of thing
    ram[VRAM_Buffer1 + 1 + x] = a; // we're printing to the buffer
    lda_absy_nf(StatusBarData + 1);
    ram[VRAM_Buffer1 + 2 + x] = a;
    ram[0x3] = a; // save length byte in counter
    ram[0x2] = x; // and buffer pointer elsewhere for now
    pla_nf(); // pull original incremented value from stack
    tax_nf();
    lda_absx_nf(StatusBarOffset); // load offset to value we want to write
    carry_flag = true;
    sbc_absy_fc(StatusBarData + 1); // subtract from length byte we read before
    tay_nf(); // use value as offset to display digits
    ldx_zp_nf(0x2);
    
DigitPLoop:
    lda_absy_nf(DisplayDigits); // write digits to the buffer
    ram[VRAM_Buffer1 + 3 + x] = a;
    inx_nf();
    iny_nf();
    ...
    lda_imm_nf(0x0); // put null terminator at end
    ram[VRAM_Buffer1 + 3 + x] = a;
    inx_nf(); // increment buffer pointer by 3
    inx_nf();
    inx();
...
StoreNewD:
  ...
  dey_nf(); // move onto next digits in score or game timer
  dex_fn(); // and digit amounts to increment
  if (!neg_flag) { goto AddModLoop; } // loop back if we're not done yet
  
EraseDMods:
  lda_imm_nf(0x0); // store zero here
  ldx_imm_nf(0x6); // start with the last digit
...
BorrowOne:
  dec_absx_nf(DigitModifier - 1); // decrement the previous digit, then put $09 in
  lda_imm_fz(0x9); // the game timer digit we're currently on to "borrow
...
CarryOne:
  ...
  inc_absx_nf(DigitModifier - 1); // preceding current digit to "carry the one" properly
...
void TopScoreCheck(void) {
  ldy_imm_nf(0x5); // start with the lowest digit
  carry_flag = true;
  
GetScoreDiff:
  lda_absx_nf(PlayerScoreDisplay); // subtract each player digit from each high score digit
  sbc_absy_fc(TopScoreDisplay); // from lowest to highest, if any top score digit exceeds
  dex_nf(); // any player digit, borrow will be set until a subsequent
  dey(); // subtraction clears it (player digit is higher than top)
  if (!neg_flag) { goto GetScoreDiff; }
  if (carry_flag) {
    inx_nf(); // increment X and Y once to the start of the score
    iny_nf();
    
CopyScore:
    lda_absx_nf(PlayerScoreDisplay); // store player's score digits into high score memory area
...
void InitializeMemory(void) {
  ldx_imm_nf(0x7); // set initial high byte to $0700-$07ff
  lda_imm_nf(0x0); // set initial low byte to start of page (at $00 of page)
  ram[0x6] = a;
...
void GetAreaMusic(void) {
  ...
  if (zero_flag) { return; }
  lda_abs_nf(AltEntranceControl); // check for specific alternate mode of entry
  ...
  ldy_imm_nf(0x5); // select music for pipe intro scene by default
  lda_abs_nf(PlayerEntranceCtrl); // check value from level header for certain values
  cmp_imm_fcz(0x6);
...
ChkAreaType:
  ldy_abs_nf(AreaType); // load area type as offset for music bit
  lda_abs_fz(CloudTypeOverride);
  if (zero_flag) { goto StoreMusic; } // check for cloud type override
  ldy_imm_nf(0x4); // select music for cloud type level if found
...
void Entrance_GameTimerSetup(void) {
  lda_abs_nf(ScreenLeft_PageLoc); // set current page for area objects
  ram[Player_PageLoc] = a; // as page location for player
  lda_imm_nf(0x28); // store value here
  ram[VerticalForceDown] = a; // for fractional movement downwards if necessary
  lda_imm_nf(0x1); // set high byte of player position and
  ram[PlayerFacingDir] = a; // set facing direction so that player faces right
  ram[Player_Y_HighPos] = a;
  lda_imm_nf(0x0); // set player state to on the ground by default
  ram[Player_State] = a;
  dec_abs_nf(Player_CollisionBits); // initialize player's collision bits
  ldy_imm_nf(0x0); // initialize halfway page
  ram[HalfwayPage] = y;
  ...
  // ChkStPos:
  ram[SwimmingFlag] = y;
  ldx_abs_nf(PlayerEntranceCtrl); // get starting position loaded from header
  ldy_abs_fz(AltEntranceControl); // check alternate mode of entry flag for 0 or 1
  if (!zero_flag) {
    cpy_imm_fcz(0x1);
    if (!zero_flag) {
      ldx_absy_nf(AltYPosOffset - 2); // if not 0 or 1, override $0710 with new offset in X
    }
  }
  // SetStPos:
  lda_absy_nf(PlayerStarting_X_Pos); // load appropriate horizontal position
  ram[Player_X_Position] = a; // and vertical positions for the player, using
  lda_absx_nf(PlayerStarting_Y_Pos); // AltEntranceControl as offset for horizontal and either $0710
  ...
  if (!zero_flag) {
    ...
    if (!zero_flag) {
      lda_absy_nf(GameTimerData); // if game timer is set and game timer flag is also set,
  ...
  if (!zero_flag) {
    lda_imm_nf(0x3); // set player state to climbing
    ram[Player_State] = a;
    ldx_imm(0x0); // set offset for first slot, for block object
    InitBlock_XY_Pos();
    lda_imm_nf(0xf0); // set vertical coordinate for block object
    ram[Block_Y_Position] = a;
    ldx_imm_nf(0x5); // set offset in X for last enemy object buffer slot
...
void PlayerLoseLife(void) {
  inc_abs_nf(DisableScreenFlag); // disable screen and sprite 0 check
  lda_imm_nf(0x0);
  ram[Sprite0HitDetectFlag] = a;
  lda_imm_nf(Silence); // silence music
  ram[EventMusicQueue] = a;
  ...
  // StillInGame:
  lda_abs_nf(WorldNumber); // multiply world number by 2 and use
  asl_acc_nf(); // as offset
  tax_nf();
  lda_abs_nf(LevelNumber); // if in area -3 or -4, increment
  ...
  // GetHalfway:
  ldy_absx_nf(HalfwayPageNybbles); // get halfway page number with offset
  lda_abs_nf(LevelNumber); // check area number's LSB
  lsr_acc_fc();
  tya_nf(); // if in area -2 or -4, use lower nybble
  if (!carry_flag) {
    lsr_acc_nf(); // move higher nybble to lower if area
    lsr_acc_nf(); // number is -1 or -3
    lsr_acc_nf();
    lsr_acc_nf();
  }
  // MaskHPNyb:
  and_imm_nf(0b00001111); // mask out all but lower nybble
...
void TransposePlayers(void) {
  ...
  lda_abs_nf(CurrentPlayer); // invert bit to update
  eor_imm_nf(0b00000001); // which player is on the screen
  ram[CurrentPlayer] = a;
  ldx_imm_nf(0x6);
  
TransLoop:
  lda_absx_nf(OnscreenPlayerInfo); // transpose the information
  pha(); // of the onscreen player
  lda_absx_nf(OffscreenPlayerInfo); // with that of the offscreen player
...
void IncrementColumnPos(void) {
  inc_abs_nf(CurrentColumnPos); // increment column where we're at
  lda_abs_nf(CurrentColumnPos);
  and_imm_fz(0b00001111); // mask out higher nybble
  if (zero_flag) {
    ram[CurrentColumnPos] = a; // if no bits left set, wrap back to zero (0-f)
    inc_abs_nf(CurrentPageLoc); // and increment page number where we're at
  }
  // NoColWrap:
  inc_abs_nf(BlockBufferColumnPos); // increment column offset where we're at
  lda_abs_nf(BlockBufferColumnPos);
...
ClrMTBuf:
  ...
  if (!zero_flag) {
    lda_abs_nf(CurrentPageLoc); // otherwise check for every third page
...
ThirdP:
    ...
    // RendBack:
    asl_acc_nf(); // move results to higher nybble
    asl_acc_nf();
    asl_acc_nf();
    asl_acc_fc();
    adc_absy_fc(BSceneDataOffsets - 1); // add to it offset loaded from here
    adc_abs_nf(CurrentColumnPos); // add to the result our current column position
    tax_nf();
    lda_absx_fz(BackSceneryData); // load data from sum of offsets
    // if zero, no scenery for that part
    if (!zero_flag) {
      pha();
      and_imm_nf(0xf); // save to stack and clear high nybble
      carry_flag = true;
      sbc_imm_nf(0x1); // subtract one (because low nybble is $01-$0c)
      ram[0x0] = a; // save low nybble
      asl_acc_fc(); // multiply by three (shift to left and add result to old one)
      adc_zp_nf(0x0); // note that since d7 was nulled, the carry flag is always clear
      tax_nf(); // save as offset for background scenery metatile data
      pla_nf(); // get high nybble from stack, move low
      lsr_acc_nf();
      lsr_acc_nf();
      lsr_acc_nf();
      lsr_acc_nf();
      tay_nf(); // use as second offset (used to determine height)
      lda_imm_nf(0x3); // use previously saved memory location for counter
      ram[0x0] = a;
      
SceLoop1:
      lda_absx_nf(BackSceneryMetatiles); // load metatile data from offset of (lsb - 1) * 3
  ...
  if (!zero_flag) {
    ldy_absx_nf(FSceneDataOffsets - 1); // load offset from location offset by header value, then
    ldx_imm_nf(0x0); // reinit X
...
SceLoop2:
  ...
  if (zero_flag) {
    lda_abs_nf(WorldNumber); // check world number, if not world number eight
    cmp_imm_fz(World8); // then skip this part
    if (zero_flag) {
      lda_imm_nf(0x62); // if set as water level and world number eight,
      goto StoreMT; // use castle wall metatile as terrain type
    }
  }
  // TerMTile:
  lda_absy_nf(TerrainMetatiles); // otherwise get appropriate metatile for area type
  ldy_abs_fz(CloudTypeOverride); // check for cloud type override
  // if not set, keep value otherwise
  if (!zero_flag) {
    lda_imm_nf(0x88); // use cloud block terrain
  }
  
StoreMT:
  ram[0x7] = a; // store value here
  ldx_imm_nf(0x0); // initialize X, use as metatile buffer offset
  lda_abs_nf(TerrainControl); // use yet another value from the header
  asl_acc_nf(); // multiply by 2 and use as yet another offset
  tay_nf();
  
TerrLoop:
  lda_absy_nf(TerrainRenderBits); // get one of the terrain rendering bit data
  ram[0x0] = a;
  iny_nf(); // increment Y and use as offset next time around
  ram[0x1] = y;
  lda_abs_fz(CloudTypeOverride); // skip if value here is zero
  if (!zero_flag) {
    cpx_imm_fz(0x0); // otherwise, check if we're doing the ceiling byte
    if (!zero_flag) {
      lda_zp_nf(0x0); // if not, mask out all but d3
      and_imm_nf(0b00001000);
      ram[0x0] = a;
    }
  }
  // NoCloud2:
  ldy_imm_nf(0x0); // start at beginning of bitmasks
  
TerrBChk:
  lda_absy_nf(Bitmasks); // load bitmask, then perform AND on contents of first byte
  bit_zp_fz(0x0);
  ...
  // NextTBit:
  inx_nf(); // continue until end of buffer
  cpx_imm(0xd);
  // if we're at the end, break out of this loop
  if (!zero_flag) {
    lda_abs_nf(AreaType); // check world type for underground area
    cmp_imm_fz(0x2);
    ...
    if (zero_flag) {
      ...
      if (zero_flag) {
        lda_imm_nf(0x54); // old terrain type with ground level terrain type
        ram[0x7] = a;
      }
    }
    // EndUChk:
    iny_nf(); // increment bitmasks offset in Y
    cpy_imm_fcz(0x8);
  ...
  ldx_imm_nf(0x0);
  ldy_imm_nf(0x0); // init index regs and start at beginning of smaller buffer
  
ChkMTLow:
  ram[0x0] = y;
  lda_absx_nf(MetatileBuffer); // load stored metatile number
  and_imm_nf(0b11000000); // mask out all but 2 MSB
  asl_acc_fc();
  rol_acc_fc(); // make %xx000000 into %000000xx
  rol_acc_nf();
  tay_nf(); // use as offset in Y
  lda_absx_nf(MetatileBuffer); // reload original unmasked value here
  cmp_absy_fc(BlockBuffLowBounds); // check for certain values depending on bits set
  // if equal or greater, branch
  if (!carry_flag) {
    lda_imm_nf(0x0); // if less, init value before storing
  }
  // StrBlock:
  ldy_zp_nf(0x0); // get offset for block buffer
  dynamic_ram_write(read_word(0x6) + y, a); // store value into block buffer
  tya_nf();
  carry_flag = false; // add 16 (move down one row) to offset
  adc_imm_nf(0x10);
  tay_nf();
  inx_nf(); // increment column value
  cpx_imm(0xd);
...
void ProcessAreaData(void) {
  ldx_imm_nf(0x2); // start at the end of area object buffer
  
ProcADLoop:
  ram[ObjectOffset] = x;
  lda_imm_nf(0x0); // reset flag
  ram[BehindAreaParserFlag] = a;
  ldy_abs_nf(AreaDataOffset); // get offset of area data pointer
  lda_indy_nf(AreaData); // get first byte of area object
  ...
  iny_nf();
  lda_indy_nf(AreaData); // get second byte of area object
  ...
  inc_abs_nf(AreaObjectPageSel); // if not already set, set it now
  inc_abs_nf(AreaObjectPageLoc); // and increment page location
  
Chk1Row13:
  dey_nf();
  lda_indy_nf(AreaData); // reread first byte of level object
  and_imm_nf(0xf); // mask out high nybble
  cmp_imm_fcz(0xd); // row 13?
  if (!zero_flag) { goto Chk1Row14; }
  iny_nf(); // if so, reread second byte of level object
  lda_indy_nf(AreaData);
  dey_nf(); // decrement to get ready to read first byte
  ...
  if (!zero_flag) { goto CheckRear; }
  iny_nf(); // if d6 not set, reread second byte
  lda_indy_nf(AreaData);
  and_imm_nf(0b00011111); // mask out all but 5 LSB and store in page control
...
CheckRear:
  lda_abs_nf(AreaObjectPageLoc); // check to see if current page of level object is
...
ChkLength:
  ldx_zp_nf(ObjectOffset); // get buffer offset
  ...
  dec_absx_nf(AreaObjectLength); // otherwise decrement length or get rid of it
...
void IncAreaObjOffset(void) {
  inc_abs_nf(AreaDataOffset); // increment offset of level pointer
  inc_abs_nf(AreaDataOffset);
...
void DecodeAreaData(void) {
  ...
  ldy_absx_nf(AreaObjOffsetBuffer); // if not, get offset from buffer
  
Chk1stB:
  ldx_imm_nf(0x10); // load offset of 16 for special row 15
  lda_indy_nf(AreaData); // get first byte of level object again
  cmp_imm(0xfd);
  if (zero_flag) { goto LeavePar; } // if end of level, leave this routine
  and_imm_nf(0xf); // otherwise, mask out low nybble
  cmp_imm_fz(0xf); // row 15?
  if (zero_flag) { goto ChkRow14; } // if so, keep the offset of 16
  ldx_imm_nf(0x8); // otherwise load offset of 8 for special row 12
  cmp_imm_fz(0xc); // row 12?
  if (zero_flag) { goto ChkRow14; } // if so, keep the offset value of 8
  ldx_imm_nf(0x0); // otherwise nullify value by default
  
ChkRow14:
  ram[0x7] = x; // store whatever value we just loaded here
  ldx_zp_nf(ObjectOffset); // get object offset again
  cmp_imm_fz(0xe); // row 14?
  if (!zero_flag) { goto ChkRow13; }
  lda_imm_nf(0x0); // if so, load offset with $00
  ram[0x7] = a;
  lda_imm_fz(0x2e); // and load A with another value
  if (!zero_flag) { goto NormObj; } // unconditional branch
  
ChkRow13:
  cmp_imm_fcz(0xd); // row 13?
  if (!zero_flag) { goto ChkSRows; }
  lda_imm_nf(0x22); // if so, load offset with 34
  ram[0x7] = a;
  iny_nf(); // get next byte
  lda_indy_nf(AreaData);
  ...
  lda_indy_nf(AreaData); // otherwise, get byte again
  and_imm_nf(0b01111111); // mask out d7
  ...
  inc_abs_nf(LoopCommand); // if loop command, set loop command flag
  
Mask2MSB:
  and_imm_nf(0b00111111); // mask out d7 and d6
  goto NormObj; // and jump
  
ChkSRows:
  cmp_imm_fc(0xc); // row 12-15?
  if (carry_flag) { goto SpecObj; }
  iny_nf(); // if not, get second byte of level object
  lda_indy_nf(AreaData);
  ...
  lda_indy_nf(AreaData); // reload second byte of level object
  and_imm_nf(0b00001111); // mask out higher nybble and jump
  goto NormObj;
...
LrgObj:
  ...
  lda_indy_nf(AreaData); // if not, reload second byte
  ...
  lda_imm_nf(0x0); // otherwise, nullify value for warp pipe
  ram[0x0] = a;
  
NotWPipe:
  lda_zp_nf(0x0); // get value and jump ahead
  goto MoveAOId;
  
SpecObj:
  iny_nf(); // branch here for rows 12-15
  lda_indy_nf(AreaData);
  and_imm_nf(0b01110000); // get next byte and mask out all but d6-d4
  
MoveAOId:
  lsr_acc_nf(); // move d6-d4 to lower nybble
  lsr_acc_nf();
  lsr_acc_nf();
  lsr_acc_nf();
...
NormObj:
  ...
  lda_abs_nf(AreaObjectPageLoc); // otherwise check to see if the object we've loaded is on the
  cmp_abs_fcz(CurrentPageLoc); // same page as the renderer, and if so, branch
  if (zero_flag) { goto InitRear; }
  ldy_abs_nf(AreaDataOffset); // if not, get old offset of level pointer
  lda_indy_nf(AreaData); // and reload first byte
  and_imm_nf(0b00001111);
  cmp_imm(0xe); // row 14?
...
void BackColC(void) {
  ldy_abs_nf(AreaDataOffset); // get first byte again
  lda_indy_nf(AreaData);
  and_imm_nf(0b11110000); // mask out low nybble and move high to low
  lsr_acc_nf();
  lsr_acc_nf();
  lsr_acc_nf();
  lsr_acc_nf();
...
void RunAObj(void) {
  lda_zp_nf(0x0); // get stored value and add offset to it
...
void AlterAreaAttributes(void) {
  ldy_absx_nf(AreaObjOffsetBuffer); // load offset for level object data saved in buffer
  iny_nf(); // load second byte
  lda_indy_nf(AreaData);
  ...
  if (zero_flag) {
    ...
    and_imm_nf(0b00001111); // mask out high nybble and store as
    ram[TerrainControl] = a; // new terrain height type bits
    pla_nf();
    and_imm_nf(0b00110000); // pull and mask out all but d5 and d4
    lsr_acc_nf(); // move bits to lower nybble and store
    lsr_acc_nf(); // as new background scenery bits
    lsr_acc_nf();
    lsr_acc();
  ...
  // Alter2:
  pla_nf();
  and_imm_nf(0b00000111); // mask out all but 3 LSB
...
void ScrollLockObject_Warp(void) {
  ldx_imm_nf(0x4); // load value of 4 for game text routine as default
  lda_abs_fz(WorldNumber); // warp zone (4-3-2), then check world number
  if (!zero_flag) {
    inx_nf(); // if world number > 1, increment for next warp zone (5)
    ldy_abs_nf(AreaType); // check area type
    dey_fz();
    // if ground area type, increment for last warp zone
    if (zero_flag) {
      inx_nf(); // (8-7-6) and move on
    }
  }
  // WarpNum:
  txa();
...
void ScrollLockObject(void) {
  lda_abs_nf(ScrollLock); // invert scroll lock to turn it on
  eor_imm(0b00000001);
  ram[ScrollLock] = a;
  // --------------------------------
...
void AreaFrenzy(void) {
  ldx_zp_nf(0x0); // use area object identifier bit as offset
  lda_absx_nf(FrenzyIDData - 8); // note that it starts at 8, thus weird address here
  ldy_imm_nf(0x5);
...
void TreeLedge(void) {
  ...
  if (!zero_flag) {
    ...
    // MidTreeL:
    ldx_zp_nf(0x7);
    lda_imm_nf(0x17); // render middle of tree ledge
...
void MushroomLedge(void) {
  ...
  if (carry_flag) {
    lda_absx_nf(AreaObjectLength); // divide length by 2 and store elsewhere
    lsr_acc_fc();
  ...
  // EndMushL:
  lda_imm_nf(0x1b); // if at the end, render end of mushroom
  ldy_absx_fz(AreaObjectLength);
  if (zero_flag) {
    NoUnder();
    return;
  }
  lda_absx_nf(MushroomLedgeHalfLen); // get divided length and store where length
...
void NoUnder(void) {
  ldx_zp_nf(0x7); // load row of ledge
...
void CastleObject(void) {
  ...
  ldy_absx_nf(AreaObjectLength); // use current length as offset for castle data
  ldx_zp_nf(0x7); // begin at starting row
  lda_imm_nf(0xb);
  ram[0x6] = a; // load upper limit of number of rows to print
  
CRendLoop:
  lda_absy_nf(CastleMetatiles); // load current byte using offset
  ram[MetatileBuffer + x] = a;
  inx_nf(); // store in buffer and increment buffer offset
  lda_zp_fz(0x6);
  if (zero_flag) { goto ChkCFloor; } // have we reached upper limit yet?
  iny_nf(); // if not, increment column-wise
  iny_nf(); // to byte in next row
  iny_nf();
  iny_nf();
  iny_nf();
  dec_zp_nf(0x6); // move closer to upper limit
  
ChkCFloor:
  cpx_imm_fcz(0xb); // have we reached the row just before floor?
  if (!zero_flag) { goto CRendLoop; } // if not, go back and do another row
  pla_nf();
  tax_nf(); // get obj buffer offset from before
  lda_abs(CurrentPageLoc);
  if (zero_flag) { return; } // if we're at page 0, we do not need to do anything else
  lda_absx_nf(AreaObjectLength); // check length
...
void WaterPipe(void) {
  ...
  ldy_absx_nf(AreaObjectLength); // get length (residual code, water pipe is 1 col thick)
  ldx_zp_nf(0x7); // get row
  lda_imm_nf(0x6b);
...
void IntroPipe(void) {
  ...
  RenderSidewaysPipe();
  if (!carry_flag) {
    ldx_imm_nf(0x6); // blank everything above the vertical pipe part
    
VPipeSectLoop:
    lda_imm_nf(0x0); // all the way to the top of the screen
...
void RenderSidewaysPipe(void) {
  dey_nf(); // decrement twice to make room for shaft at bottom
  dey_nf(); // and store here for now as vertical length
  ram[0x5] = y;
  ldy_absx_nf(AreaObjectLength); // get length left over and store here
  ram[0x6] = y;
  ldx_zp_nf(0x5); // get vertical length plus one, use as buffer offset
  inx_nf();
  lda_absy_nf(SidePipeShaftData); // check for value $00 based on horizontal offset
  cmp_imm_fcz(0x0);
  ...
  // DrawSidePart:
  ldy_zp_nf(0x6); // render side pipe part at the bottom
  lda_absy_nf(SidePipeTopPart);
...
void VerticalPipe(void) {
  ...
  if (!zero_flag) {
    ...
    iny_nf();
    iny_nf();
    iny_nf(); // add four if usage control bit was not set
  }
  // WarpPipe:
  tya_nf(); // save value in stack
  pha();
  lda_abs_nf(AreaNumber);
  ...
  if (!zero_flag) {
    ...
    if (!zero_flag) {
      ...
      if (!carry_flag) {
        ...
        lda_abs_nf(CurrentPageLoc); // add carry to current page number
        adc_imm_fc(0x0);
  ...
  // DrawPipe:
  pla_nf(); // get value saved earlier and use as Y
  tay_nf();
  ldx_zp_nf(0x7); // get buffer offset
  lda_absy_nf(VerticalPipeData); // draw the appropriate pipe with the Y we loaded earlier
  ram[MetatileBuffer + x] = a; // render the top of the pipe
  inx_nf();
  lda_absy_nf(VerticalPipeData + 2); // render the rest of the pipe
  ldy_zp_nf(0x6); // subtract one from length and render the part underneath
  dey();
  RenderUnderPart();
}
...
void Hole_Water(void) {
  ...
  lda_imm_nf(0x86); // render waves
  ram[MetatileBuffer + 10] = a;
  ldx_imm_nf(0xb);
  ldy_imm_nf(0x1); // now render the water underneath
  lda_imm(0x87);
  RenderUnderPart();
  // --------------------------------
}
...
void QuestionBlockRow_LowSkip(void) {
  ...
  ChkLrgObjLength(); // get low nybble and save as length
  pla_nf();
  tax_nf(); // render question boxes with coins
  lda_imm(0xc0);
  ram[MetatileBuffer + x] = a;
  // --------------------------------
}
...
void Bridge_LowSkip(void) {
  ...
  ChkLrgObjLength(); // get low nybble and save as length
  pla_nf();
  tax_nf(); // render bridge railing
  lda_imm_nf(0xb);
  ram[MetatileBuffer + x] = a;
  inx_nf();
  ldy_imm_nf(0x0); // now render the bridge itself
  lda_imm(0x63);
  RenderUnderPart();
  // --------------------------------
}
...
void FlagBalls_Residual(void) {
  ...
  ldx_imm_nf(0x2); // render flag balls on third row from top
...
void FlagpoleObject(void) {
  lda_imm_nf(0x24); // render flagpole ball on top
  ram[MetatileBuffer] = a;
  ldx_imm_nf(0x1); // now render the flagpole shaft
  ldy_imm_nf(0x8);
  lda_imm(0x25);
  RenderUnderPart();
...
void EndlessRope(void) {
  ldx_imm_nf(0x0); // render rope from the top to the bottom of screen
  ldy_imm(0xf);
  DrawRope();
}

void BalancePlatRope(void) {
  txa_nf(); // save object buffer offset for now
  pha();
  ldx_imm_nf(0x1); // blank out all from second row to the bottom
  ldy_imm_nf(0xf); // with blank used for balance platform rope
  lda_imm(0x44);
  RenderUnderPart();
  pla_nf(); // get back object buffer offset
  tax();
...
void RowOfCoins(void) {
  ldy_abs_nf(AreaType); // get area type
...
void AxeObj(void) {
  lda_imm_nf(0x8); // load bowser's palette into sprite portion of palette
  ram[VRAM_Buffer_AddrCtrl] = a;
  ChainObj(); // <fallthrough>
}

void ChainObj(void) {
  ldy_zp_nf(0x0); // get value loaded earlier from decoder
  ldx_absy_nf(C_ObjectRow - 2); // get appropriate row and metatile for object
...
void RowOfBricks(void) {
  ldy_abs_nf(AreaType); // load area type obtained from area offset pointer
  lda_abs_fz(CloudTypeOverride); // check for cloud type override
  if (!zero_flag) {
    ldy_imm_nf(0x4); // if cloud type, override area type
  }
  // DrawBricks:
...
void DrawRow(void) {
  ldx_zp_nf(0x7);
  ldy_imm_nf(0x0); // set vertical height of 1
  pla();
  RenderUnderPart(); // render object
}

void ColumnOfBricks(void) {
  ldy_abs_nf(AreaType); // load area type obtained from area offset
...
void BulletBillCannon(void) {
  ...
  ldx_zp_nf(0x7); // start at first row
  lda_imm_nf(0x64); // render bullet bill cannon
  ram[MetatileBuffer + x] = a;
  inx_nf();
  dey_fn(); // done yet?
  if (!neg_flag) {
    lda_imm_nf(0x65); // if not, render middle part
    ram[MetatileBuffer + x] = a;
    inx_nf();
    dey_fn(); // done yet?
    if (!neg_flag) {
...
void StaircaseObject(void) {
  ...
  if (carry_flag) {
    lda_imm_nf(0x9); // start past the end for the bottom
    ram[StaircaseControl] = a; // of the staircase
  }
  // NextStair:
  dec_abs_nf(StaircaseControl); // move onto next step (or first if starting)
  ldy_abs_nf(StaircaseControl);
  ldx_absy_nf(StaircaseRowData); // get starting row and height to render
  lda_absy_nf(StaircaseHeightData);
  tay_nf();
...
void Jumpspring(void) {
  ...
  lda_imm_nf(JumpspringObject);
  ram[Enemy_ID + x] = a; // write jumpspring object to enemy object buffer
  ldy_imm_nf(0x1);
  ram[Enemy_Y_HighPos + x] = y; // store vertical high byte
  inc_zpx_nf(Enemy_Flag); // set flag for enemy object buffer
  ldx_zp_nf(0x7);
  lda_imm_nf(0x67); // draw metatiles in two rows where jumpspring is
  ram[MetatileBuffer + x] = a;
  lda_imm(0x68);
  ram[MetatileBuffer + 1 + x] = a;
  // --------------------------------
...
void BrickWithItem(void) {
  ...
  ram[0x7] = y;
  lda_imm_nf(0x0); // load default adder for bricks with lines
  ldy_abs_nf(AreaType); // check level type for ground level
  dey_fz();
  // if ground type, do not start with 5
  if (!zero_flag) {
    lda_imm_nf(0x5); // otherwise use adder for bricks without lines
  }
  // BWithL:
  carry_flag = false; // add object ID to adder
  adc_zp_fc(0x7);
  tay_nf(); // use as offset for metatile
...
void Hole_Empty(void) {
  ...
  if (carry_flag) {
    ...
    if (zero_flag) {
      ...
      lda_abs_nf(CurrentPageLoc); // get page location of where we're at
      sbc_imm_nf(0x0); // subtract borrow
      ram[Whirlpool_PageLoc + x] = a; // save as page location of whirlpool
      iny_nf();
      iny_nf(); // increment length by 2
      tya_nf();
      asl_acc_nf(); // multiply by 16 to get size of whirlpool
      asl_acc_nf(); // note that whirlpool will always be
      asl_acc_nf(); // two blocks bigger than actual size of hole
      asl_acc_nf(); // and extend one block beyond each edge
      ...
      if (carry_flag) {
        ldx_imm_nf(0x0); // otherwise initialize it
      }
      // StrWOffset:
  ...
  // NoWhirlP:
  ldx_abs_nf(AreaType); // get appropriate metatile, then
  lda_absx_nf(HoleMetatiles); // render the hole proper
  ldx_imm_nf(0x8);
  ldy_imm_nf(0xf); // start at ninth row and go to bottom, run RenderUnderPart
  // --------------------------------
...
WaitOneRow:
  ...
  ldy_abs_nf(AreaObjectHeight); // decrement, and stop rendering if there is no more length
  dey();
...
void KillEnemies(void) {
  ...
  lda_imm_nf(0x0);
  ldx_imm_nf(0x4); // check for identifier in enemy object buffer
...
void GetPipeHeight(void) {
  ...
  GetLrgObjAttrib();
  tya_nf(); // get saved lower nybble as height
  and_imm_nf(0x7); // save only the three lower bits as
...
void GetAreaObjectID(void) {
  lda_zp_nf(0x0); // get value saved from area parser routine
  carry_flag = true;
...
void GetLrgObjAttrib(void) {
  ldy_absx_nf(AreaObjOffsetBuffer); // get offset saved from area obj decoding routine
  lda_indy_nf(AreaData); // get first byte of level object
  and_imm_nf(0b00001111);
  ram[0x7] = a; // save row location
  iny_nf();
  lda_indy_nf(AreaData); // get next byte, save lower nybble (length or height)
  and_imm_nf(0b00001111); // as Y, then leave
  tay();
  // --------------------------------
}

void GetAreaObjXPosition(void) {
  lda_abs_nf(CurrentColumnPos); // multiply current offset where we're at by 16
  asl_acc_nf(); // to obtain horizontal pixel coordinate
  asl_acc_nf();
  asl_acc_nf();
  asl_acc();
  // --------------------------------
...
void GetAreaObjYPosition(void) {
  lda_zp_nf(0x7); // multiply value by 16
  asl_acc_nf();
  asl_acc_nf(); // this will give us the proper vertical pixel coordinate
  asl_acc_nf();
  asl_acc_nf();
  carry_flag = false;
...
void GetBlockBufferAddr(void) {
  ...
  lsr_acc_nf();
  lsr_acc_nf();
  tay_nf(); // use nybble as pointer to high byte
  lda_absy_nf(BlockBufferAddr + 2); // of indirect here
  ram[0x7] = a;
  pla_nf();
  and_imm_nf(0b00001111); // pull from stack, mask out high nybble
  carry_flag = false;
...
void GetAreaType(void) {
  and_imm_nf(0b01100000); // mask out all but d6 and d5
  asl_acc_fc();
  rol_acc_fc();
  rol_acc_fc();
...
void FindAreaPointer(void) {
  ldy_abs_nf(WorldNumber); // load offset from world variable
  lda_absy_nf(WorldAddrOffsets);
...
void GetAreaDataAddrs(void) {
  ...
  GetAreaType();
  tay_nf();
  lda_abs_nf(AreaPointer); // mask out all but 5 LSB
  and_imm_nf(0b00011111);
  ram[AreaAddrsLOffset] = a; // save as low offset
  lda_absy_nf(EnemyAddrHOffsets); // load base value with 2 altered MSB,
  carry_flag = false; // then add base value to 5 LSB, result
  adc_abs_nf(AreaAddrsLOffset); // becomes offset for level data
  tay_nf();
  lda_absy_nf(EnemyDataAddrLow); // use offset to load pointer
  ram[EnemyDataLow] = a;
  lda_absy_nf(EnemyDataAddrHigh);
  ram[EnemyDataHigh] = a;
  ldy_abs_nf(AreaType); // use area type as offset
  lda_absy_nf(AreaDataHOffsets); // do the same thing but with different base value
  carry_flag = false;
  adc_abs_nf(AreaAddrsLOffset);
  tay_nf();
  lda_absy_nf(AreaDataAddrLow); // use this offset to load another pointer
  ram[AreaDataLow] = a;
  lda_absy_nf(AreaDataAddrHigh);
  ram[AreaDataHigh] = a;
  ldy_imm_nf(0x0); // load first byte of header
  lda_indy_nf(AreaData);
  pha(); // save it to the stack for now
  and_imm_nf(0b00000111); // save 3 LSB for foreground scenery or bg color control
  cmp_imm_fc(0x4);
  if (carry_flag) {
  ...
  pla_nf(); // pull byte from stack and push it back
  pha();
  and_imm_nf(0b00111000); // save player entrance control bits
  lsr_acc_nf(); // shift bits over to LSBs
  lsr_acc_nf();
  lsr_acc_nf();
  ram[PlayerEntranceCtrl] = a; // save value here as player entrance control
  pla_nf(); // pull byte again but do not push it back
  and_imm_nf(0b11000000); // save 2 MSB for game timer setting
  carry_flag = false;
  ...
  iny_nf();
  lda_indy_nf(AreaData); // load second byte of header
  pha(); // save to stack
  and_imm_nf(0b00001111); // mask out all but lower nybble
  ram[TerrainControl] = a;
  pla_nf(); // pull and push byte to copy it to A
  pha();
  and_imm_nf(0b00110000); // save 2 MSB for background scenery type
  lsr_acc_nf();
  lsr_acc_nf(); // shift bits to LSBs
  lsr_acc_nf();
  lsr_acc_nf();
  ...
  // StoreStyle:
  ram[AreaStyle] = a;
  lda_zp_nf(AreaDataLow); // increment area data address by 2 bytes
  carry_flag = false;
  adc_imm_fc(0x2);
  ram[AreaDataLow] = a;
  lda_zp_nf(AreaDataHigh);
  adc_imm(0x0);
  ram[AreaDataHigh] = a;
  // -------------------------------------------------------------------------------------
  // GAME LEVELS DATA
...
void ScrollHandler(void) {
  lda_abs_nf(Player_X_Scroll); // load value saved here
  carry_flag = false;
  ...
  lda_abs_nf(Player_Pos_ForScroll);
  ...
  ldy_abs_nf(Player_X_Scroll); // get value and decrement by one
  ...
  if (carry_flag) {
    dey_nf(); // otherwise decrement by one
  }
  // ChkNearMid:
  lda_abs_nf(Player_Pos_ForScroll);
  ...
  ldy_abs_nf(Player_X_Scroll); // otherwise get original value undecremented
...
void ScrollScreen(void) {
  ...
  carry_flag = false;
  adc_abs_nf(ScrollThirtyTwo); // add to value already set here
  ...
  lda_abs_nf(ScreenLeft_PageLoc);
  adc_imm_fc(0x0); // add carry to page location for left
  ram[ScreenLeft_PageLoc] = a; // side of the screen
  and_imm_nf(0x1); // get LSB of page location
  ram[0x0] = a; // save as temp variable for PPU register 1 mirror
  lda_abs_nf(Mirror_PPU_CTRL_REG1); // get PPU register 1 mirror
  and_imm_nf(0b11111110); // save all bits except d0
...
KeepOnscr:
  ...
  lda_absy_nf(ScreenEdge_PageLoc); // get left or right page location based on offset
  sbc_imm_nf(0x0); // subtract borrow
  ram[Player_PageLoc] = a; // save as player's page location
  lda_zp_nf(Left_Right_Buttons); // check saved controller bits
...
void GetScreenPosition(void) {
  lda_abs_nf(ScreenLeft_X_Pos); // get coordinate of screen's left boundary
  carry_flag = false;
  adc_imm_fc(0xff); // add 255 pixels
  ram[ScreenRight_X_Pos] = a; // store as coordinate of screen's right boundary
  lda_abs_nf(ScreenLeft_PageLoc); // get page number where left boundary is
...
void PlayerEntrance(void) {
  lda_abs_nf(AltEntranceControl); // check for mode of alternate entry
  cmp_imm_fcz(0x2);
  if (zero_flag) { goto EntrMode2; } // if found, branch to enter from pipe or with vine
  lda_imm_nf(0x0);
  ldy_zp_nf(Player_Y_Position); // if vertical position above a certain
  ...
  lda_abs_nf(PlayerEntranceCtrl); // check player entry bits from header
  cmp_imm_fcz(0x6);
...
EntrMode2:
  ...
  lda_zp_nf(Player_Y_Position); // check to see if player is at a specific coordinate
...
OffVine:
  ...
  lda_zp_nf(Player_X_Position);
  cmp_imm(0x48); // check player's horizontal position
  if (!carry_flag) { return; } // if not far enough to the right, branch to leave
  
PlayerRdy:
  lda_imm_nf(0x8); // set routine to be executed by game engine next frame
  ram[GameEngineSubroutine] = a;
  lda_imm_nf(0x1); // set to face player to the right
...
void PlayerCtrlRoutine(void) {
  lda_zp_nf(GameEngineSubroutine); // check task here
...
DisJoyp:
  lda_imm_nf(0x0); // disable controller bits
  ram[SavedJoypadBits] = a;
  
SaveJoyp:
  lda_abs_nf(SavedJoypadBits); // otherwise store A and B buttons in $0a
  and_imm_nf(0b11000000);
  ram[A_B_Buttons] = a;
  lda_abs_nf(SavedJoypadBits); // store left and right buttons in $0c
  and_imm_nf(0b00000011);
  ram[Left_Right_Buttons] = a;
  lda_abs_nf(SavedJoypadBits); // store up and down buttons in $0b
  and_imm_nf(0b00001100);
...
PlayerSubs:
  ...
  lda_zp_nf(GameEngineSubroutine);
  ...
  lda_abs_nf(Player_SprAttrib);
  and_imm_nf(0b11011111); // otherwise nullify player's
  ram[Player_SprAttrib] = a; // background priority flag
  
PlayerHole:
  lda_zp_nf(Player_Y_HighPos); // check player's vertical high byte
  ...
  ldx_imm_nf(0x0); // use X as flag, and clear for cloud level
...
HoleDie:
  inx_nf(); // set flag in X for player death
  ldy_zp_nf(GameEngineSubroutine);
...
void Vine_AutoClimb(void) {
  ...
  // AutoClimb:
  lda_imm_nf(0b00001000); // set controller bits override to up
  ram[JoypadOverride] = a;
  ldy_imm_nf(0x3); // set player state to climbing
  ram[Player_State] = y;
  AutoControlPlayer(); return;
}

void SetEntr(void) {
  lda_imm_nf(0x2); // set starting position to override
...
void VerticalPipeEntry(void) {
  ...
  ldy_imm_nf(0x0); // load default mode of entry
  ...
  iny_nf();
  lda_abs_nf(AreaType); // check for castle level type
  cmp_imm_fcz(0x3);
...
void PlayerChangeSize(void) {
  lda_abs_nf(TimerControl); // check master timer control
...
void InitChangeSize(void) {
  ...
  if (zero_flag) {
    ...
    inc_abs_nf(PlayerChangeSizeFlag); // set growing/shrinking flag
    lda_abs_nf(PlayerSize);
...
void CyclePlayerPalette(void) {
  and_imm_nf(0x3); // mask out all but d1-d0 (previously d3-d2)
  ram[0x0] = a; // store result here to use as palette bits
  lda_abs_nf(Player_SprAttrib); // get player attributes
  and_imm_nf(0b11111100); // save any other bits but palette bits
...
void PlayerEndLevel(void) {
  ...
  AutoControlPlayer();
  lda_zp_nf(Player_Y_Position); // check player's vertical position
  cmp_imm_fc(0xae);
  ...
  // ChkStop:
  lda_abs_nf(Player_CollisionBits); // get player collision bits
  ...
  if (!carry_flag) {
    ...
    if (zero_flag) {
      inc_abs_nf(StarFlagTaskControl); // otherwise set task control now (this gets ball rolling!)
    }
    // InCastle:
    lda_imm_nf(0b00100000); // set player's background priority bit to
    ram[Player_SprAttrib] = a; // give illusion of being inside the castle
  }
  // RdyNextA:
  lda_abs_nf(StarFlagTaskControl);
  cmp_imm(0x5); // if star flag task control not yet set
  if (zero_flag) {
    inc_abs_nf(LevelNumber); // increment level number used for game logic
    lda_abs_nf(LevelNumber);
    ...
    ldy_abs_nf(WorldNumber); // get world number as offset
    lda_abs_nf(CoinTallyFor1Ups); // check third area coin tally for bonus 1-ups
    ...
    inc_abs_nf(Hidden1UpFlag); // otherwise set hidden 1-up box control flag
...
void EnterSidePipe(void) {
  lda_imm_nf(0x8); // set player's horizontal speed
  ram[Player_X_Speed] = a;
  ldy_imm_nf(0x1); // set controller right button by default
  lda_zp_nf(Player_X_Position); // mask out higher nybble of player's
  and_imm_fz(0b00001111); // horizontal position
  if (zero_flag) {
    ram[Player_X_Speed] = a; // if lower nybble = 0, set as horizontal speed
    tay_nf(); // and nullify controller bit override here
  }
  // RightPipe:
...
void PlayerMovementSubs(void) {
  lda_imm_nf(0x0); // set A to init crouch flag by default
  ...
  lda_zp_nf(Up_Down_Buttons); // load controller bits for up and down
...
void JumpSwimSub(void) {
  ...
  lda_zp_nf(A_B_Buttons);
  and_imm_nf(A_Button); // check to see if A button is being pressed
  and_zp_fz(PreviousA_B_Buttons); // and was pressed in previous frame
  if (!zero_flag) { goto ProcSwim; } // if so, branch elsewhere
  lda_abs_nf(JumpOrigin_Y_Position); // get vertical position player jumped from
  carry_flag = true;
  sbc_zp_nf(Player_Y_Position); // subtract current from original vertical coordinate
...
DumpFall:
  lda_abs_nf(VerticalForceDown); // otherwise dump falling into main fractional
  ram[VerticalForce] = a;
...
void ClimbingSub(void) {
  ...
  lda_zp_nf(Left_Right_Buttons); // compare left/right controller bits
  ...
  if (!zero_flag) {
    ...
    if (zero_flag) {
      ...
      ldx_imm_nf(0x0); // set default offset here
      ldy_zp_nf(PlayerFacingDir); // get facing direction
      lsr_acc_fc(); // move right button controller bit to carry
      // if controller right pressed, branch ahead
      if (!carry_flag) {
        inx_nf();
        inx_nf(); // otherwise increment offset by 2 bytes
      }
      // ClimbFD:
      dey_fz(); // check to see if facing right
      // if so, branch, do not increment
      if (!zero_flag) {
        inx_nf(); // otherwise increment by 1 byte
      }
      // CSetFDir:
      lda_zp_nf(Player_X_Position);
      ...
      lda_zp_nf(Player_PageLoc); // add or subtract carry or borrow using value here
      adc_absx_fc(ClimbAdderHigh); // from the player's page location
      ram[Player_PageLoc] = a;
      lda_zp_nf(Left_Right_Buttons); // get left/right controller bits again
...
void PlayerPhysicsSub(void) {
  lda_zp_nf(Player_State); // check player state
  cmp_imm_fcz(0x3);
  if (!zero_flag) { goto CheckForJumping; } // if not climbing, branch
  ldy_imm_nf(0x0);
  lda_zp_nf(Up_Down_Buttons); // get controller bits for up/down
...
ProcClimb:
  ldx_absy_nf(Climb_Y_MForceData); // load value here
  ram[Player_Y_MoveForce] = x; // store as vertical movement force
  lda_imm_nf(0x8); // load default animation timing
...
CheckForJumping:
  ...
  lda_zp_nf(A_B_Buttons); // check for A button press
  and_imm_fz(A_Button);
...
InitJS:
  lda_imm_nf(0x20); // set jump/swim timer
  ram[JumpSwimTimer] = a;
  ldy_imm_nf(0x0); // initialize vertical force and dummy variable
  ram[Player_YMF_Dummy] = y;
  ram[Player_Y_MoveForce] = y;
  lda_zp_nf(Player_Y_HighPos); // get vertical high and low bytes of jump origin
  ram[JumpOrigin_Y_HighPos] = a; // and store them next to each other here
  lda_zp_nf(Player_Y_Position);
  ram[JumpOrigin_Y_Position] = a;
  lda_imm_nf(0x1); // set player state to jumping/swimming
  ram[Player_State] = a;
  lda_abs_nf(Player_XSpeedAbsolute); // check value related to walking/running speed
  cmp_imm_fc(0x9);
  if (!carry_flag) { goto ChkWtr; } // branch if below certain values, increment Y
  iny_nf(); // for each amount equal or exceeded
  cmp_imm_fc(0x10);
...
ChkWtr:
  lda_imm_nf(0x1); // set value here (apparently always set to 1)
  ram[DiffToHaltJump] = a;
  lda_abs_fz(SwimmingFlag); // if swimming flag disabled, branch
  if (zero_flag) { goto GetYPhy; }
  ldy_imm_nf(0x5); // otherwise set Y to 5, range is 5-6
  lda_abs_fz(Whirlpool_Flag); // if whirlpool flag not set, branch
  if (zero_flag) { goto GetYPhy; }
  iny_nf(); // otherwise increment to 6
  
GetYPhy:
  lda_absy_nf(JumpMForceData); // store appropriate jump/swim
  ram[VerticalForce] = a; // data here
  lda_absy_nf(FallMForceData);
  ram[VerticalForceDown] = a;
  lda_absy_nf(InitMForceData);
  ram[Player_Y_MoveForce] = a;
  lda_absy_nf(PlayerYSpdData);
  ram[Player_Y_Speed] = a;
  lda_abs_fz(SwimmingFlag); // if swimming flag disabled, branch
  if (zero_flag) { goto PJumpSnd; }
  lda_imm_nf(Sfx_EnemyStomp); // load swim/goomba stomp sound into
  ...
  lda_imm_nf(0x0); // otherwise reset player's vertical speed
  ram[Player_Y_Speed] = a; // and jump to something else to keep player
  goto X_Physics; // from swimming above water level
  
PJumpSnd:
  lda_imm_nf(Sfx_BigJump); // load big mario's jump sound by default
  ldy_abs_fz(PlayerSize); // is mario big?
  if (zero_flag) { goto SJumpSnd; }
  lda_imm_nf(Sfx_SmallJump); // if not, load small mario's jump sound
...
X_Physics:
  ...
  lda_abs_nf(Player_XSpeedAbsolute); // check something that seems to be related
...
ProcPRun:
  iny_nf(); // if mario on the ground, increment Y
  lda_abs_fz(AreaType); // check area type
  if (zero_flag) { goto ChkRFast; } // if water type, branch
  dey_nf(); // decrement Y by default for non-water type area
  lda_zp_nf(Left_Right_Buttons); // get left/right controller bits
  ...
  lda_zp_nf(A_B_Buttons); // check for b button pressed
  and_imm_fz(B_Button);
...
ChkRFast:
  iny_nf(); // if running timer not set or level type is water,
  inc_zp_nf(0x0); // increment Y again and temp variable in memory
  lda_abs_fz(RunningSpeed);
  if (!zero_flag) { goto FastXSp; } // if running speed set here, branch
  lda_abs_nf(Player_XSpeedAbsolute);
...
FastXSp:
  inc_zp_nf(0x0); // if running speed set or speed => $21 increment $00
  goto GetXPhy; // and jump ahead
  
SetRTmr:
  lda_imm_nf(0xa); // if b button pressed, set running timer
  ram[RunningTimer] = a;
  
GetXPhy:
  lda_absy_nf(MaxLeftXSpdData); // get maximum speed to the left
  ram[MaximumLeftSpeed] = a;
  lda_zp_nf(GameEngineSubroutine); // check for specific routine running
  cmp_imm_fz(0x7); // (player entrance)
  if (!zero_flag) { goto GetXPhy2; } // if not running, skip and use old value of Y
  ldy_imm_nf(0x3); // otherwise set Y to 3
  
GetXPhy2:
  lda_absy_nf(MaxRightXSpdData); // get maximum speed to the right
  ram[MaximumRightSpeed] = a;
  ldy_zp_nf(0x0); // get other value in memory
  lda_absy_nf(FrictionData); // get value using value in memory as offset
  ram[FrictionAdderLow] = a;
  lda_imm_nf(0x0);
  ram[FrictionAdderHigh] = a; // init something here
  lda_zp_nf(PlayerFacingDir);
...
void GetPlayerAnimSpeed(void) {
  ldy_imm_nf(0x0); // initialize offset in Y
  lda_abs_nf(Player_XSpeedAbsolute); // check player's walking/running speed
  cmp_imm_fc(0x1c); // against preset amount
  if (carry_flag) { goto SetRunSpd; } // if greater than a certain amount, branch ahead
  iny_nf(); // otherwise increment Y
  ...
  iny_nf(); // otherwise increment Y again
  
ChkSkid:
  lda_abs_nf(SavedJoypadBits); // get controller bits
  and_imm_fz(0b01111111); // mask out A button
  if (zero_flag) { goto SetAnimSpd; } // if no other buttons pressed, branch ahead of all this
  and_imm_nf(0x3); // mask out all others except left and right
  ...
  lda_imm_nf(0x0); // otherwise set zero value here
...
void ImposeFriction(void) {
  and_abs_nf(Player_CollisionBits); // perform AND between left/right controller bits and collision flag
...
LeftFrict:
  lda_abs_nf(Player_X_MoveForce); // load value set here
  carry_flag = false;
  adc_abs_fc(FrictionAdderLow); // add to it another value set here
  ram[Player_X_MoveForce] = a; // store here
  lda_zp_nf(Player_X_Speed);
  adc_abs_nf(FrictionAdderHigh); // add value plus carry to horizontal speed
...
RghtFrict:
  ...
  ram[Player_X_MoveForce] = a; // store here
  lda_zp_nf(Player_X_Speed);
  sbc_abs_nf(FrictionAdderHigh); // subtract value plus borrow from horizontal speed
  ram[Player_X_Speed] = a; // set as new horizontal speed
  ...
  lda_abs_nf(MaximumLeftSpeed); // otherwise set preset value as horizontal speed
...
void ProcFireball_Bubble(void) {
  lda_abs_nf(PlayerStatus); // check player's status
  cmp_imm_fc(0x2);
  // if not fiery, branch
  if (carry_flag) {
    lda_zp_nf(A_B_Buttons);
    ...
    if (!zero_flag) {
      ...
      if (zero_flag) {
        lda_abs_nf(FireballCounter); // load fireball counter
        and_imm_nf(0b00000001); // get LSB and use as offset for buffer
        tax_nf();
        lda_zpx_fz(Fireball_State); // load fireball state
        // if not inactive, branch
        if (zero_flag) {
          ldy_zp_nf(Player_Y_HighPos); // if player too high or too low, branch
          dey_fz();
          if (zero_flag) {
            lda_abs_fz(CrouchingFlag); // if player crouching, branch
            if (zero_flag) {
              lda_zp_nf(Player_State); // if player's state = climbing, branch
              cmp_imm_fcz(0x3);
              if (!zero_flag) {
                lda_imm_nf(Sfx_Fireball); // play fireball sound effect
                ram[Square1SoundQueue] = a;
                lda_imm_nf(0x2); // load state
                ram[Fireball_State + x] = a;
                ldy_abs_nf(PlayerAnimTimerSet); // copy animation frame timer setting
                ...
                inc_abs_nf(FireballCounter); // increment fireball counter
              }
            }
          }
        }
...
void FireballObjCore(void) {
  ...
  lda_zpx_nf(Fireball_State); // check for d7 = 1
  asl_acc();
  ...
  if (!carry_flag) {
    ...
    if (!zero_flag) {
      ...
      if (!zero_flag) {
        lda_zp_nf(Player_X_Position); // get player's horizontal position
        adc_imm_fc(0x4); // add four pixels and store as fireball's horizontal position
        ram[Fireball_X_Position + x] = a;
        lda_zp_nf(Player_PageLoc); // get player's page location
        adc_imm_nf(0x0); // add carry and store as fireball's page location
        ram[Fireball_PageLoc + x] = a;
        lda_zp_nf(Player_Y_Position); // get player's vertical position and store
        ram[Fireball_Y_Position + x] = a;
        lda_imm_nf(0x1); // set high byte of vertical position
        ram[Fireball_Y_HighPos + x] = a;
        ldy_zp_nf(PlayerFacingDir); // get player's facing direction
        dey_nf(); // decrement to use as offset here
        lda_absy_nf(FireballXSpdData); // set horizontal speed of fireball accordingly
        ram[Fireball_X_Speed + x] = a;
        lda_imm_nf(0x4); // set vertical speed of fireball
        ram[Fireball_Y_Speed + x] = a;
        lda_imm_nf(0x7);
        ram[Fireball_BoundBoxCtrl + x] = a; // set bounding box size control for fireball
        dec_zpx_nf(Fireball_State); // decrement state to 1 to skip this part from now on
      }
      // RunFB:
      txa_nf(); // add 7 to offset to use
      carry_flag = false; // as fireball offset for next routines
      adc_imm_fc(0x7);
      tax_nf();
      lda_imm_nf(0x50); // set downward movement force here
      ram[0x0] = a;
      lda_imm_nf(0x3); // set maximum speed here
      ram[0x2] = a;
      lda_imm(0x0);
      ...
      lda_abs_nf(FBall_OffscreenBits); // get fireball offscreen bits
...
void BubbleCheck(void) {
  lda_absx_nf(PseudoRandomBitReg + 1); // get part of LSFR
  and_imm_nf(0x1);
  ram[0x7] = a; // store pseudorandom bit here
  lda_zpx_nf(Bubble_Y_Position); // get vertical coordinate for air bubble
...
void SetupBubble(void) {
  ldy_imm_nf(0x0); // load default value here
  lda_zp_nf(PlayerFacingDir); // get player's facing direction
  lsr_acc_fc(); // move d0 to carry
  // branch to use default value if facing left
  if (carry_flag) {
    ldy_imm_nf(0x8); // otherwise load alternate value here
  }
  // PosBubl:
  tya_nf(); // use value loaded as adder
  ...
  lda_zp_nf(Player_PageLoc);
  adc_imm_nf(0x0); // add carry to player's page location
  ...
  ldy_zp_nf(0x7); // get pseudorandom bit, use as offset
  lda_absy_nf(BubbleTimerData); // get data for air bubble timer
  ram[AirBubbleTimer] = a; // set air bubble timer
  MoveBubl(); // <fallthrough>
}

void MoveBubl(void) {
  ldy_zp_nf(0x7); // get pseudorandom bit again, use as offset
  lda_absx_nf(Bubble_YMF_Dummy);
  ...
  lda_zpx_nf(Bubble_Y_Position);
  sbc_imm_nf(0x0); // subtract borrow from airbubble's vertical coordinate
...
void WarpZoneObject(void) {
  ...
  lda_zp_nf(Player_Y_Position); // check to see if player's vertical coordinate has
...
void ProcessWhirlpools(void) {
  ...
  if (!zero_flag) { return; } // branch to leave
  ldy_imm_nf(0x4); // otherwise start with last whirlpool data
  
WhLoop:
  lda_absy_nf(Whirlpool_LeftExtent); // get left extent of whirlpool
  carry_flag = false;
  ...
  lda_zp_nf(0x2); // otherwise get right extent
  carry_flag = true;
  sbc_zp_fc(Player_X_Position); // subtract player's horizontal coordinate
  lda_zp_nf(0x1); // get right extent's page location
...
WhirlpoolActivate:
  lda_absy_nf(Whirlpool_Length); // get length of whirlpool
  lsr_acc_nf(); // divide by 2
  ram[0x0] = a; // save here
  lda_absy_nf(Whirlpool_LeftExtent); // get left extent of whirlpool
  carry_flag = false;
  adc_zp_fc(0x0); // add length divided by 2
  ram[0x1] = a; // save as center of whirlpool
  lda_absy_nf(Whirlpool_PageLoc); // get page location
  adc_imm_nf(0x0); // add carry
  ...
  lda_zp_nf(0x1); // get center
  carry_flag = true;
  sbc_zp_fc(Player_X_Position); // subtract player's horizontal coordinate
  lda_zp_nf(0x0); // get page location of center
  sbc_zp_fn(Player_PageLoc); // subtract borrow
  if (!neg_flag) { goto LeftWh; } // if player to the left of center, branch
  lda_zp_nf(Player_X_Position); // otherwise slowly pull player left, towards the center
  carry_flag = true;
...
LeftWh:
  lda_abs_nf(Player_CollisionBits); // get player's collision bits
  lsr_acc_fc(); // shift d0 into carry
  if (!carry_flag) { goto WhPull; } // if d0 not set, branch
  lda_zp_nf(Player_X_Position); // otherwise slowly pull player right, towards the center
  carry_flag = false;
...
WhPull:
  ...
  lsr_acc_nf();
  tax_nf(); // set X for player offset
...
void ImposeGravity(void) {
  ...
  lda_absx_nf(SprObject_YMF_Dummy);
  ...
  ldy_imm_nf(0x0); // set Y to zero by default
...
AlterYP:
  ...
  lda_zpx_nf(SprObject_Y_HighPos);
  adc_zp_nf(0x7); // add carry plus contents of $07 to vertical high byte
  ram[SprObject_Y_HighPos + x] = a; // store as new vertical high byte
  lda_absx_nf(SprObject_Y_MoveForce);
  carry_flag = false;
  adc_zp_fc(0x0); // add downward movement amount to contents of $0433
  ram[SprObject_Y_MoveForce + x] = a;
  lda_zpx_nf(SprObject_Y_Speed); // add carry to vertical speed and store
  adc_imm_nf(0x0);
...
ChkUpM:
  ...
  lda_zp_nf(0x2);
  eor_imm_nf(0b11111111); // otherwise get two's compliment of maximum speed
  tay_nf();
  iny_nf();
  ...
  lda_zpx_nf(SprObject_Y_Speed);
  sbc_imm_nf(0x0); // subtract borrow from vertical speed and store
  ram[SprObject_Y_Speed + x] = a;
...
void FlagpoleRoutine(void) {
  ldx_imm_nf(0x5); // set enemy object offset
...
GiveFPScr:
  ldy_abs_nf(FlagpoleScore); // get score offset from earlier (when player touched flagpole)
  lda_absy_nf(FlagpoleScoreMods); // get amount to award player points
...
void JumpspringHandler(void) {
  ...
  tay_nf();
  dey_nf(); // subtract one from frame control,
  tya_nf(); // the only way a poor nmos 6502 can
  ...
  inc_zp_nf(Player_Y_Position);
  inc_zp_nf(Player_Y_Position); // move player's vertical position down two pixels
  goto PosJSpr; // skip to next part
  
DownJSpr:
  dec_zp_nf(Player_Y_Position); // move player's vertical position up two pixels
  dec_zp_nf(Player_Y_Position);
  
PosJSpr:
  lda_zpx_nf(Jumpspring_FixedYPos); // get permanent vertical position
  carry_flag = false;
  adc_absy_nf(Jumpspring_Y_PosData); // add value using frame control as offset
...
void Setup_Vine(void) {
  ...
  // NextVO:
  txa_nf(); // store object offset to next available vine slot
  ram[VineObjOffset + y] = a; // using vine flag as offset
  inc_abs_nf(VineFlagOffset); // increment vine flag offset
...
void VineObjectHandler(void) {
  ...
  if (zero_flag) {
    ldy_abs_nf(VineFlagOffset);
    dey_nf(); // decrement vine flag in Y, use as offset
    lda_abs_nf(VineHeight);
    cmp_absy_fz(VineHeightData); // if vine has reached certain height,
    // branch ahead to skip this part
    if (!zero_flag) {
      lda_zp_nf(FrameCounter); // get frame counter
      lsr_acc_nf(); // shift d1 into carry
      lsr_acc_fc();
      // if d1 not set (2 frames every 4) skip this part
      if (carry_flag) {
        lda_zp_nf(Enemy_Y_Position + 5);
        sbc_imm_nf(0x1); // subtract vertical position of vine
        ram[Enemy_Y_Position + 5] = a; // one pixel every frame it's time
        inc_abs_nf(VineHeight); // increment vine height
      }
    }
    // RunVSubs:
    lda_abs_nf(VineHeight); // if vine still very small,
...
KillVine:
      ...
      // WrCMTile:
      lda_abs_nf(VineHeight); // check vine height
      cmp_imm_fc(0x20); // if vine small (less than 32 pixels tall)
      // then branch ahead to leave
      if (carry_flag) {
        ldx_imm_nf(0x6); // set offset in X to last enemy slot
        lda_imm_nf(0x1); // set A to obtain horizontal in $04, but we don't care
...
ThreeSChk:
    ...
    if (zero_flag) {
      lda_absx_nf(PseudoRandomBitReg + 1); // otherwise get part of LSFR
      ldy_abs_nf(SecondaryHardMode); // get secondary hard mode flag, use as offset
      and_absy_nf(CannonBitmasks); // mask out bits of LSFR as decided by flag
      cmp_imm_fc(0x6); // check to see if lower nybble is above certain value
      // if so, branch to check enemy
      if (!carry_flag) {
        tay_nf(); // transfer masked contents of LSFR to Y as pseudorandom offset
        ...
        if (!zero_flag) {
          ...
          if (!zero_flag) {
            sbc_imm_nf(0x0); // otherwise subtract borrow (note carry will always be clear here)
          ...
          if (zero_flag) {
            lda_imm_nf(0xe); // otherwise we start creating one
            ram[Cannon_Timer + y] = a; // first, reset cannon timer
            lda_absy_nf(Cannon_PageLoc); // get page location of cannon
            ram[Enemy_PageLoc + x] = a; // save as page location of bullet bill
            lda_absy_nf(Cannon_X_Position); // get horizontal coordinate of cannon
            ram[Enemy_X_Position + x] = a; // save as horizontal coordinate of bullet bill
            lda_absy_nf(Cannon_Y_Position); // get vertical coordinate of cannon
            carry_flag = true;
            sbc_imm_nf(0x8); // subtract eight pixels (because enemies are 24 pixels tall)
            ...
            lda_imm_nf(BulletBill_CannonVar);
...
Chk_BB:
    lda_zpx_nf(Enemy_ID); // check enemy identifier for bullet bill (cannon variant)
...
void BulletBillHandler(void) {
  ...
  lda_abs_nf(Enemy_OffscreenBits); // otherwise load offscreen bits
  and_imm_nf(0b00001100); // mask out bits
  ...
  iny_nf(); // otherwise increment to move left
  
SetupBB:
  ram[Enemy_MovingDir + x] = y; // set bullet bill's moving direction
  dey_nf(); // decrement to use as offset
  lda_absy_nf(BulletBillXSpdData); // get horizontal speed based on moving direction
  ram[Enemy_X_Speed + x] = a; // and store it
  lda_zp_nf(0x0); // get horizontal difference
  adc_imm_nf(0x28); // add 40 pixels
  ...
  lda_imm_nf(Sfx_Blast);
  ram[Square2SoundQueue] = a; // play fireworks/gunfire sound
  
ChkDSte:
  lda_zpx_nf(Enemy_State); // check enemy state for d5 set
  and_imm(0b00100000);
...
void EnemyGfxHandler(void) {
  lda_zpx_nf(Enemy_Y_Position); // get enemy object vertical position
  ram[0x2] = a;
  lda_abs_nf(Enemy_Rel_XPos); // get enemy object horizontal position
  ram[0x5] = a; // relative to screen
  ldy_absx_nf(Enemy_SprDataOffset);
  ram[0xeb] = y; // get sprite data offset
  lda_imm_nf(0x0);
  ram[VerticalFlipFlag] = a; // initialize vertical flip flag by default
  lda_zpx_nf(Enemy_MovingDir);
  ram[0x3] = a; // get enemy object moving direction
  lda_absx_nf(Enemy_SprAttrib);
...
CheckForRetainerObj:
  lda_zpx_nf(Enemy_State); // store enemy state
  ram[0xed] = a;
  and_imm_nf(0b00011111); // nullify all but 5 LSB and use as Y
  tay_nf();
  lda_zpx_nf(Enemy_ID); // check for mushroom retainer/princess object
  cmp_imm_fz(RetainerObject);
  if (!zero_flag) { goto CheckForBulletBillCV; } // if not found, branch
  ldy_imm_nf(0x0); // if found, nullify saved state in Y
  lda_imm_nf(0x1); // set value that will not be used
  ram[0x3] = a;
  lda_imm_nf(0x15); // set value $15 as code for mushroom retainer/princess object
...
CheckForBulletBillCV:
  ...
  dec_zp_nf(0x2); // decrement saved vertical position
  lda_imm_nf(0x3);
  ldy_absx_fz(EnemyFrameTimer); // get timer for enemy object
  if (zero_flag) { goto SBBAt; } // if expired, do not set priority bit
  ora_imm_nf(0b00100000); // otherwise do so
  
SBBAt:
  ram[0x4] = a; // set new sprite attributes
  ldy_imm_nf(0x0); // nullify saved enemy state both in Y and in
  ram[0xed] = y; // memory location here
  lda_imm_nf(0x8); // set specific value to unconditionally branch once
  
CheckForJumpspring:
  cmp_imm_fz(JumpspringObject); // check for jumpspring object
  if (!zero_flag) { goto CheckForPodoboo; }
  ldy_imm_nf(0x3); // set enemy state -2 MSB here for jumpspring object
  ldx_abs_nf(JumpspringAnimCtrl); // get current frame number for jumpspring object
  lda_absx_nf(JumpspringFrameOffsets); // load data using frame number as offset
  
CheckForPodoboo:
  ram[0xef] = a; // store saved enemy object value here
  ram[0xec] = y; // and Y here (enemy state -2 MSB if not changed)
  ldx_zp_nf(ObjectOffset); // get enemy object offset
  ...
  inc_abs_nf(VerticalFlipFlag); // otherwise, set flag for vertical flip
  
CheckBowserGfxFlag:
  lda_abs_fz(BowserGfxFlag); // if not drawing bowser at all, skip to something else
  if (zero_flag) { goto CheckForGoomba; }
  ldy_imm_nf(0x16); // if set to 1, draw bowser's front
  cmp_imm_fz(0x1);
  if (zero_flag) { goto SBwsrGfxOfs; }
  iny_nf(); // otherwise draw bowser's rear
  
SBwsrGfxOfs:
  ram[0xef] = y;
  
CheckForGoomba:
  ldy_zp_nf(0xef); // check value for goomba object
  cpy_imm_fz(Goomba);
  ...
  ldx_imm_nf(0x4); // if defeated, write new value here
  ram[0xec] = x;
  
GmbaAnim:
  and_imm_nf(0b00100000); // check for d5 set in enemy object state
  ...
  lda_zp_nf(0x3);
  eor_imm_nf(0b00000011); // invert bits to flip horizontally every eight frames
  ram[0x3] = a; // leave alone otherwise
  
CheckBowserFront:
  lda_absy_nf(EnemyAttributeData); // load sprite attribute using enemy object
  ora_zp_nf(0x4); // as offset, and add to bits already loaded
  ram[0x4] = a;
  lda_absy_nf(EnemyGfxTableOffsets); // load value based on enemy object as offset
  tax_nf(); // save as X
  ldy_zp_nf(0xec); // get previously saved value
  ...
  ldx_imm_nf(0xde); // otherwise load offset for second frame
  
ChkFrontSte:
  lda_zp_nf(0xed); // check saved enemy state
...
CheckBowserRear:
  lda_abs_nf(BowserBodyControls); // check bowser's body control bits
  and_imm_fz(0x1);
  if (zero_flag) { goto ChkRearSte; } // branch if d0 not set (control's bowser's feet)
  ldx_imm_nf(0xe4); // otherwise load offset for second frame
  
ChkRearSte:
  lda_zp_nf(0xed); // check saved enemy state
  and_imm_fz(0b00100000); // if bowser not defeated, do not set flag
  if (zero_flag) { goto DrawBowser; }
  lda_zp_nf(0x2); // subtract 16 pixels from
...
CheckForSpiny:
  ...
  ldx_imm_nf(0x30); // set to spiny egg offset
  lda_imm_nf(0x2);
...
CheckRightSideUpShell:
  ...
  ldx_imm_nf(0x72); // set right-side up buzzy beetle shell by default
  inc_zp_nf(0x2); // increment saved vertical position by one pixel
  ldy_zp_nf(0xef);
  cpy_imm_fz(BuzzyBeetle); // check for buzzy beetle object
  if (zero_flag) { goto CheckForDefdGoomba; } // branch if found
  ldx_imm_nf(0x66); // change to right-side up koopa shell if not found
  inc_zp_nf(0x2); // and increment saved vertical position again
...
CheckForDefdGoomba:
  ...
  ldx_imm_nf(0x54); // load for regular goomba
  lda_zp_nf(0xed); // note that this only gets performed if enemy state => $02
  and_imm_fz(0b00100000); // check saved enemy state for d5 set
  if (!zero_flag) { goto CheckForHammerBro; } // branch if set
  ldx_imm_nf(0x8a); // load offset for defeated goomba
  dec_zp_nf(0x2); // set different value and decrement saved vertical position
  
CheckForHammerBro:
  ldy_zp_nf(ObjectOffset);
  lda_zp_nf(0xef); // check for hammer bro object
  ...
  ldx_imm_fz(0xb4); // otherwise load offset for different frame
...
CheckForBloober:
  ...
  lda_absy_nf(EnemyIntervalTimer);
  cmp_imm_fc(0x5);
  ...
  inc_zp_nf(0x2); // increment saved vertical coordinate three pixels
  inc_zp_nf(0x2);
  inc_zp_nf(0x2);
  goto CheckAnimationStop; // and do something else
  
CheckToAnimateEnemy:
  lda_zp_nf(0xef); // check for specific enemy objects
  cmp_imm_fcz(Goomba);
  ...
  iny_nf(); // residual instruction
  lda_abs_nf(WorldNumber); // are we on world 8?
  cmp_imm_fc(World8);
  if (carry_flag) { goto CheckDefeatedState; } // if so, leave the offset alone (use princess)
  ldx_imm_nf(0xa2); // otherwise, set for mushroom retainer object instead
  lda_imm_fz(0x3); // set alternate state here
  ram[0xec] = a;
  if (!zero_flag) { goto CheckDefeatedState; } // unconditional branch
  
CheckForSecondFrame:
  lda_zp_nf(FrameCounter); // load frame counter
...
CheckAnimationStop:
  lda_zp_nf(0xed); // check saved enemy state
  and_imm_nf(0b10100000); // for d7 or d5, or check for timers stopped
  ...
  tax_nf(); // to animate various enemy objects
...
DrawEnemyObject:
  ...
  DrawEnemyObjRow();
  ldx_zp_nf(ObjectOffset); // get enemy object offset
  ldy_absx_nf(Enemy_SprDataOffset); // get sprite data offset
  lda_zp_nf(0xef);
...
CheckForVerticalFlip:
  ...
  lda_absy_nf(Sprite_Attributes); // get attributes of first sprite we dealt with
  ora_imm_nf(0b10000000); // set bit for vertical flip
  iny_nf();
  iny(); // increment two bytes so that we store the vertical flip
  DumpSixSpr(); // in attribute bytes of enemy obj sprite data
  dey_nf();
  dey_nf(); // now go back to the Y coordinate offset
  tya_nf();
  tax_nf(); // give offset to X
  lda_zp_nf(0xef);
  cmp_imm_fcz(HammerBro); // check saved enemy object for hammer bro
  if (zero_flag) { goto FlipEnemyVertically; }
  cmp_imm_fcz(Lakitu); // check saved enemy object for lakitu
  ...
  txa_nf();
  carry_flag = false;
  adc_imm_fc(0x8); // if not selected objects or => $15, set
  tax_nf(); // offset in X for next row
  
FlipEnemyVertically:
  lda_absx_nf(Sprite_Tilenumber); // load first or second row tiles
  pha(); // and save tiles to the stack
  lda_absx_nf(Sprite_Tilenumber + 4);
  pha();
  lda_absy_nf(Sprite_Tilenumber + 16); // exchange third row tiles
  ram[Sprite_Tilenumber + x] = a; // with first or second row tiles
  lda_absy_nf(Sprite_Tilenumber + 20);
  ram[Sprite_Tilenumber + 4 + x] = a;
  pla_nf(); // pull first or second row tiles from stack
...
CheckForESymmetry:
  ...
  lda_zp_nf(0xef);
  ldx_zp_nf(0xec); // get alternate enemy state
...
ESRtnr:
  ...
  lda_imm_nf(0x42); // set horizontal flip on bottom right sprite
...
MirrorEnemyGfx:
  ...
  lda_absy_nf(Sprite_Attributes); // load attribute bits of first sprite
  and_imm_nf(0b10100011);
...
EggExc:
  ...
  lda_absy_nf(Sprite_Attributes + 8); // get second row left sprite attributes
  ora_imm_nf(0b10000000);
...
CheckToMirrorLakitu:
  lda_zp_nf(0xef); // check for lakitu enemy object
  cmp_imm_fz(Lakitu);
  ...
  lda_absy_nf(Sprite_Attributes + 16); // save vertical flip and palette bits
  and_imm_nf(0b10000001); // in third row left sprite
  ram[Sprite_Attributes + 16 + y] = a;
  lda_absy_nf(Sprite_Attributes + 20); // set horizontal flip and palette bits
  ora_imm_nf(0b01000001); // in third row right sprite
  ram[Sprite_Attributes + 20 + y] = a;
  ldx_abs_nf(FrenzyEnemyTimer); // check timer
  cpx_imm_fc(0x10);
...
NVFLak:
  lda_absy_nf(Sprite_Attributes); // get first row left sprite attributes
  and_imm_nf(0b10000001);
  ram[Sprite_Attributes + y] = a; // save vertical flip and palette bits
  lda_absy_nf(Sprite_Attributes + 4); // get first row right sprite attributes
  ora_imm_nf(0b01000001); // set horizontal flip and palette bits
  ram[Sprite_Attributes + 4 + y] = a; // note that vertical flip is left as-is
  
CheckToMirrorJSpring:
  lda_zp_nf(0xef); // check for jumpspring object (any frame)
  cmp_imm_fc(0x18);
...
void SprObjectOffscrChk(void) {
  ldx_zp_nf(ObjectOffset); // get enemy buffer offset
  lda_abs_nf(Enemy_OffscreenBits); // check offscreen information
  lsr_acc_nf();
  lsr_acc_nf(); // shift three times to the right
...
Row3C:
  pla_nf(); // get from stack again
  lsr_acc_nf(); // move d5 to carry this time
  lsr_acc_fc();
...
AllRowC:
  pla_nf(); // get from stack once more
  ...
  lda_zpx_nf(Enemy_Y_HighPos); // check high byte of vertical position
...
void SpawnHammerObj(void) {
  lda_abs_nf(PseudoRandomBitReg + 1); // get pseudorandom bits from
  and_imm_fz(0b00000111); // second part of LSFR
  // if any bits are set, branch and use as offset
  if (zero_flag) {
    lda_abs_nf(PseudoRandomBitReg + 1);
    and_imm_nf(0b00001000); // get d3 from same part of LSFR
  }
  // SetMOfs:
  tay_nf(); // use either d3 or d2-d0 for offset here
  lda_zpy_fz(Misc_State); // if any values loaded in
  // $2a-$32 where offset is then leave with carry clear
  if (zero_flag) {
    ldx_absy_nf(HammerEnemyOfsData); // get offset of enemy slot to check using Y as offset
    lda_zpx_fz(Enemy_Flag); // check enemy buffer flag at offset
    // if buffer flag set, branch to leave with carry clear
    if (zero_flag) {
      ldx_zp_nf(ObjectOffset); // get original enemy object offset
      txa_nf();
...
void ProcHammerObj(void) {
  ...
  lda_zpx_nf(Misc_State); // otherwise get hammer's state
  and_imm_nf(0b01111111); // mask out d7
  ldy_absx_nf(HammerEnemyOffset); // get enemy object offset that spawned this hammer
...
SetHSpd:
  ...
  lda_zpy_nf(Enemy_State); // get enemy object state
  and_imm_nf(0b11110111); // mask out d3
  ram[Enemy_State + y] = a; // store new state
  ldx_zpy_nf(Enemy_MovingDir); // get enemy's moving direction
  dex_nf(); // decrement to use as offset
  lda_absx_nf(HammerXSpdData); // get proper speed to use based on moving direction
  ldx_zp_nf(ObjectOffset); // reobtain hammer's buffer offset
  ram[Misc_X_Speed + x] = a; // set hammer's horizontal speed
  
SetHPos:
  dec_zpx_nf(Misc_State); // decrement hammer's state
  lda_zpy_nf(Enemy_X_Position); // get enemy's horizontal position
  carry_flag = false;
  adc_imm_fc(0x2); // set position 2 pixels to the right
  ram[Misc_X_Position + x] = a; // store as hammer's horizontal position
  lda_zpy_nf(Enemy_PageLoc); // get enemy's page location
  adc_imm_nf(0x0); // add carry
  ram[Misc_PageLoc + x] = a; // store as hammer's page location
  lda_zpy_nf(Enemy_Y_Position); // get enemy's vertical position
  carry_flag = true;
...
void CoinBlock(void) {
  ...
  lda_zpx_nf(Block_PageLoc); // get page location of block object
  ram[Misc_PageLoc + y] = a; // store as page location of misc object
  lda_zpx_nf(Block_X_Position); // get horizontal coordinate of block object
  ora_imm_nf(0x5); // add 5 pixels
  ram[Misc_X_Position + y] = a; // store as horizontal coordinate of misc object
  lda_zpx_nf(Block_Y_Position); // get vertical coordinate of block object
...
void SetupJumpCoin(void) {
  FindEmptyMiscSlot(); // set offset for empty or last misc object buffer slot
  lda_absx_nf(Block_PageLoc2); // get page location saved earlier
  ram[Misc_PageLoc + y] = a; // and save as page location for misc object
  lda_zp_nf(0x6); // get low byte of block buffer offset
  asl_acc_nf();
  asl_acc_nf(); // multiply by 16 to use lower nybble
  asl_acc_nf();
  asl_acc_fc();
  ora_imm_nf(0x5); // add five pixels
  ram[Misc_X_Position + y] = a; // save as horizontal coordinate for misc object
  lda_zp_nf(0x2); // get vertical high nybble offset from earlier
...
void MiscObjectsCore(void) {
  ldx_imm_nf(0x8); // set at end of misc object buffer
...
ProcJumpCoin:
  ldy_zpx_nf(Misc_State); // check misc object state
  dey_fz(); // decrement to see if it's set to 1
  if (zero_flag) { goto JCoinRun; } // if so, branch to handle jumping coin
  inc_zpx_nf(Misc_State); // otherwise increment state to either start off or as timer
  lda_zpx_nf(Misc_X_Position); // get horizontal coordinate for misc object
  ...
  lda_zpx_nf(Misc_PageLoc); // get page location
  adc_imm_nf(0x0); // add carry
  ram[Misc_PageLoc + x] = a; // store as new page location
  lda_zpx_nf(Misc_State);
...
JCoinRun:
  ...
  adc_imm_nf(0xd);
  tax_nf();
  lda_imm_nf(0x50); // set downward movement amount
  ram[0x0] = a;
  lda_imm_nf(0x6); // set maximum vertical speed
  ram[0x2] = a;
  ...
  ldx_zp_nf(ObjectOffset); // get original misc object offset
  lda_zpx_nf(Misc_Y_Speed); // check vertical speed
  cmp_imm(0x5);
...
void GetSBNybbles(void) {
  ldy_abs_nf(CurrentPlayer); // get current player
...
void UpdateNumber(void) {
  ...
  ldy_abs_nf(VRAM_Buffer1_Offset);
...
void SetupPowerUp(void) {
  lda_imm_nf(PowerUpObject); // load power-up identifier into
  ram[Enemy_ID + 5] = a; // special use slot of enemy object buffer
  lda_zpx_nf(Block_PageLoc); // store page location of block object
  ram[Enemy_PageLoc + 5] = a; // as page location of power-up object
  lda_zpx_nf(Block_X_Position); // store horizontal coordinate of block object
  ...
  lda_zpx_nf(Block_Y_Position); // get vertical coordinate of block object
  carry_flag = true;
  sbc_imm_nf(0x8); // subtract 8 pixels
  ram[Enemy_Y_Position + 5] = a; // and use as vertical coordinate of power-up object
  PwrUpJmp(); // <fallthrough>
}

void PwrUpJmp(void) {
  lda_imm_nf(0x1); // this is a residual jump point in enemy object jump table
  ...
  lda_zp_nf(PowerUpType);
  cmp_imm_fc(0x2); // check currently loaded power-up type
  // if star or 1-up, branch ahead
  if (!carry_flag) {
    lda_abs_nf(PlayerStatus); // otherwise check player's current status
    cmp_imm_fc(0x2);
...
void PowerUpObjHandler(void) {
  ldx_imm_nf(0x5); // set object offset for last slot in enemy object buffer
  ram[ObjectOffset] = x;
...
GrowThePowerUp:
  ...
  dec_zp_nf(Enemy_Y_Position + 5); // otherwise decrement vertical coordinate slowly
  lda_zp_nf(Enemy_State + 5); // load power-up object state
  inc_zp_nf(Enemy_State + 5); // increment state for next frame (to make power-up rise)
  ...
  rol_acc_nf(); // rotate A to set right moving direction
  ram[Enemy_MovingDir + x] = a; // set moving direction
  
ChkPUSte:
  lda_zp_nf(Enemy_State + 5); // check power-up object's state
...
void PlayerHeadCollision(void) {
  ...
  lda_imm_nf(0x11); // load unbreakable block object state by default
  ldx_abs_nf(SprDataOffset_Ctrl); // load offset control bit here
...
DBlockSte:
  ...
  ldx_abs_nf(SprDataOffset_Ctrl); // load offset control bit
  lda_zp_nf(0x2); // get vertical high nybble offset used in block buffer routine
  ram[Block_Orig_YPos + x] = a; // set as vertical coordinate for block object
  tay_nf();
  lda_zp_nf(0x6); // get low byte of block buffer address used in same routine
...
ChkBrick:
  ...
  ldy_imm_nf(0x11); // otherwise load unbreakable state into block object buffer
  ram[Block_State + x] = y; // note this applies to both player sizes
  lda_imm_nf(0xc4); // load empty block metatile into A for now
  ldy_zp_nf(0x0); // get metatile from before
...
StartBTmr:
  ...
  inc_abs_nf(BrickCoinTimerFlag); // and set flag linked to it
...
ContBTmr:
  ...
  ldy_imm_nf(0xc4); // otherwise use empty block metatile
...
SmallBP:
  iny_nf(); // increment for small or big and crouching
  
BigBP:
  lda_zp_nf(Player_Y_Position); // get player's vertical coordinate
  carry_flag = false;
  adc_absy_nf(BlockYPosAdderData); // add value determined by size
  and_imm_nf(0xf0); // mask out low nybble to get 16-pixel correspondence
  ram[Block_Y_Position + x] = a; // save as vertical coordinate for block object
  ldy_zpx_nf(Block_State); // get block object state
  cpy_imm(0x11);
...
InvOBit:
  lda_abs_nf(SprDataOffset_Ctrl); // invert control bit used by block objects
...
void InitBlock_XY_Pos(void) {
  lda_zp_nf(Player_X_Position); // get player's horizontal coordinate
  carry_flag = false;
  adc_imm_fc(0x8); // add eight pixels
  and_imm_nf(0xf0); // mask out low nybble to give 16-pixel correspondence
...
void VineBlock(void) {
  ldx_imm_nf(0x5); // load last slot for enemy object buffer
...
void BlockBumpedChk(void) {
  ldy_imm_nf(0xd); // start at end of metatile data
...
void CheckTopOfBlock(void) {
  ldx_abs_nf(SprDataOffset_Ctrl); // load control bit
  ...
  tya_nf(); // otherwise set to A
  carry_flag = true;
  sbc_imm_nf(0x10); // subtract $10 to move up one row in the block buffer
  ram[0x2] = a; // store as new vertical high nybble offset
  tay_nf();
  lda_indy_nf(0x6); // get contents of block buffer in same column, one row up
...
void SpawnBrickChunks(void) {
  lda_zpx_nf(Block_X_Position); // set horizontal coordinate of block object
...
void BlockObjectsCore(void) {
  ...
  and_imm_nf(0xf); // mask out high nybble
  pha(); // push to stack
  tay_nf(); // put in Y for now
  txa_nf();
  carry_flag = false;
  adc_imm_fc(0x9); // add 9 bytes to offset (note two block objects are created
  tax_nf(); // when using brick chunks, but only one offset for both)
  ...
  pla_nf(); // get lower nybble of saved state
...
ChkTop:
  lda_zpx_nf(Block_Y_Position); // get top block object's vertical coordinate
...
BouncingBlockHandler:
  ...
  lda_zpx_nf(Block_Y_Position); // get vertical coordinate
  and_imm_nf(0xf); // mask out high nybble
...
void BlockObjMT_Updater(void) {
  ldx_imm_nf(0x1); // set offset to start with second block object
...
UpdateLoop:
  ...
  if (zero_flag) {
    ...
    // branch to move onto next block object
    if (!zero_flag) {
      lda_absx_nf(Block_BBuf_Low); // get low byte of block buffer
      ram[0x6] = a; // store into block buffer address
      lda_imm_nf(0x5);
      ram[0x7] = a; // set high byte of block buffer address
      lda_absx_nf(Block_Orig_YPos); // get original vertical coordinate of block object
...
void MovePlayerHorizontally(void) {
  ...
  if (zero_flag) {
    tax_nf(); // otherwise set zero for offset to use player's stuff
    MoveObjectHorizontally(); return;
  }
}

void MoveObjectHorizontally(void) {
  lda_zpx_nf(SprObject_X_Speed); // get currently saved value (horizontal
  asl_acc_nf(); // speed, secondary counter, whatever)
  asl_acc_nf(); // and move low nybble to high
  asl_acc_nf();
  asl_acc_nf();
  ram[0x1] = a; // store result here
  lda_zpx_nf(SprObject_X_Speed); // get saved value again
  lsr_acc_nf(); // move high nybble to low
  lsr_acc_nf();
  lsr_acc_nf();
  lsr_acc_nf();
  ...
  if (carry_flag) {
    ora_imm_nf(0b11110000); // otherwise alter high nybble
  }
  // SaveXSpd:
  ram[0x0] = a; // save result here
  ldy_imm_nf(0x0); // load default Y value here
  ...
  lda_zpx_nf(SprObject_PageLoc);
  adc_zp_nf(0x2); // add carry plus other saved value to the
...
void MovePlayerVertically(void) {
  ldx_imm_nf(0x0); // set X for player offset
...
NoJSChk:
  lda_abs_nf(VerticalForce); // dump vertical force
  ram[0x0] = a;
...
void MoveD_EnemyVertically(void) {
  ldy_imm_nf(0x3d); // set quick movement amount downwards
  lda_zpx_nf(Enemy_State); // then check enemy state
...
void MoveRedPTroopaUp(void) {
  ldy_imm_nf(0x1); // set Y to move upwards
  MoveRedPTroopa(); // <fallthrough>
}

void MoveRedPTroopa(void) {
  inx_nf(); // increment X for enemy offset
  lda_imm_nf(0x3);
...
void MoveDropPlatform(void) {
  ldy_imm_fz(0x7f); // set movement amount for drop platform
...
void MoveEnemySlowVert(void) {
  ldy_imm_nf(0xf); // set movement amount for bowser/other objects
...
void MoveJ_EnemyVertically(void) {
  ldy_imm_nf(0x1c); // set movement amount for podoboo/other objects
  SetHiMax(); // <fallthrough>
}

void SetHiMax(void) {
  lda_imm_nf(0x3); // set maximum speed in A
...
void ImposeGravityBlock(void) {
  ldy_imm_nf(0x1); // set offset for maximum speed
  lda_imm_nf(0x50); // set movement amount here
  ram[0x0] = a;
  lda_absy_nf(MaxSpdBlockData); // get maximum speed
...
void MovePlatformUp(void) {
  lda_imm_nf(0x1); // save value to stack
  MovePlatformUpSkip(); // <fallthrough>
}

void MovePlatformUpSkip(void) {
  pha();
  ldy_zpx_nf(Enemy_ID); // get enemy object identifier
  inx_nf(); // increment offset for enemy object
  lda_imm_nf(0x5); // load default value here
  cpy_imm_fcz(0x29); // residual comparison, object #29 never executes
  // this code, thus unconditional branch here
  if (zero_flag) {
    lda_imm_nf(0x9); // residual code
  }
  // SetDplSpd:
  ram[0x0] = a; // save downward movement amount here
  lda_imm_nf(0xa); // save upward movement amount here
  ram[0x1] = a;
  lda_imm_nf(0x3); // save maximum vertical speed here
  ram[0x2] = a;
  pla_nf(); // get value from stack
...
void EnemiesAndLoopsCore(void) {
  lda_zpx_nf(Enemy_Flag); // check data here for MSB set
  pha(); // save in stack
  asl_acc_fc();
...
ChkAreaTsk:
  lda_abs_nf(AreaParserTaskNum); // check number of tasks to perform
  and_imm_nf(0x7);
...
void ExecGameLoopback(void) {
  lda_zp_nf(Player_PageLoc); // send player back four pages
  carry_flag = true;
  sbc_imm_nf(0x4);
  ram[Player_PageLoc] = a;
  lda_abs_nf(CurrentPageLoc); // send current page back four pages
  carry_flag = true;
  sbc_imm_nf(0x4);
  ram[CurrentPageLoc] = a;
  lda_abs_nf(ScreenLeft_PageLoc); // subtract four from page location
  carry_flag = true; // of screen's left border
  sbc_imm_nf(0x4);
  ram[ScreenLeft_PageLoc] = a;
  lda_abs_nf(ScreenRight_PageLoc); // do the same for the page location
  carry_flag = true; // of screen's right border
  sbc_imm_nf(0x4);
  ram[ScreenRight_PageLoc] = a;
  lda_abs_nf(AreaObjectPageLoc); // subtract four from page control
  carry_flag = true; // for area objects
  sbc_imm_fc(0x4);
  ram[AreaObjectPageLoc] = a;
  lda_imm_nf(0x0); // initialize page select for both
...
FindLoop:
  ...
  lda_abs_nf(WorldNumber); // are we in world 7? (check performed on correct
  ...
  inc_abs_nf(MultiLoopCorrectCntr); // increment counter for correct progression
  
IncMLoop:
  inc_abs_nf(MultiLoopPassCntr); // increment master multi-part counter
  lda_abs_nf(MultiLoopPassCntr); // have we done all three parts?
  cmp_imm_fcz(0x3);
  if (!zero_flag) { goto InitLCmd; } // if not, skip this part
  lda_abs_nf(MultiLoopCorrectCntr); // if so, have we done them all correctly?
  cmp_imm(0x3);
...
WrongChk:
  lda_abs_nf(WorldNumber); // are we in world 7? (check performed on
...
InitMLp:
  lda_imm_nf(0x0); // initialize counters used for multi-part loop commands
  ram[MultiLoopPassCntr] = a;
  ram[MultiLoopCorrectCntr] = a;
  
InitLCmd:
  lda_imm_nf(0x0); // initialize loop command flag
  ram[LoopCommand] = a;
  // --------------------------------
...
ProcessEnemyData:
  ldy_abs_nf(EnemyDataOffset); // get offset of enemy object data
  lda_indy_nf(EnemyData); // load first byte
...
CheckEndofBuffer:
  and_imm_nf(0b00001111); // check for special row $0e
  cmp_imm_fz(0xe);
  ...
  iny_nf();
  lda_indy_nf(EnemyData); // check for specific value here
  and_imm_nf(0b00111111); // not sure what this was intended for, exactly
...
CheckRightBounds:
  lda_abs_nf(ScreenRight_X_Pos); // add 48 to pixel coordinate of right boundary
  carry_flag = false;
  adc_imm_fc(0x30);
  and_imm_nf(0b11110000); // store high nybble
  ram[0x7] = a;
  lda_abs_nf(ScreenRight_PageLoc); // add carry to page location of right boundary
  adc_imm_nf(0x0);
  ram[0x6] = a; // store page location + carry
  ldy_abs_nf(EnemyDataOffset);
  iny_nf();
  lda_indy_nf(EnemyData); // if MSB of enemy object is clear, branch to check for row $0f
  asl_acc_fc();
  ...
  inc_abs_nf(EnemyObjectPageSel); // otherwise, if MSB is set, set page select
  inc_abs_nf(EnemyObjectPageLoc); // and increment page control
  
CheckPageCtrlRow:
  dey_nf();
  lda_indy_nf(EnemyData); // reread first byte
  and_imm_nf(0xf);
  ...
  iny_nf();
  lda_indy_nf(EnemyData); // otherwise, get second byte, mask out 2 MSB
  and_imm_nf(0b00111111);
  ram[EnemyObjectPageLoc] = a; // store as page control for enemy object data
  inc_abs_nf(EnemyDataOffset); // increment enemy object data offset 2 bytes
  inc_abs_nf(EnemyDataOffset);
...
PositionEnemyObj:
  lda_abs_nf(EnemyObjectPageLoc); // store page control as page location
  ram[Enemy_PageLoc + x] = a; // for enemy object
  lda_indy_nf(EnemyData); // get first byte of enemy object
  and_imm_nf(0b11110000);
  ram[Enemy_X_Position + x] = a; // store column position
  cmp_abs_fc(ScreenRight_X_Pos); // check column position against right boundary
  lda_zpx_nf(Enemy_PageLoc); // without subtracting, then subtract borrow
...
CheckRightExtBounds:
  lda_zp_nf(0x7); // check right boundary + 48 against
  cmp_zpx_fc(Enemy_X_Position); // column position without subtracting,
  lda_zp_nf(0x6); // then subtract borrow from page control temp
  sbc_zpx_fc(Enemy_PageLoc); // plus carry
  if (!carry_flag) { goto CheckFrenzyBuffer; } // if enemy object beyond extended boundary, branch
  lda_imm_nf(0x1); // store value in vertical high byte
  ram[Enemy_Y_HighPos + x] = a;
  lda_indy_nf(EnemyData); // get first byte again
  asl_acc_nf(); // multiply by four to get the vertical
  asl_acc_nf(); // coordinate
  asl_acc_nf();
  asl_acc_nf();
  ram[Enemy_Y_Position + x] = a;
  ...
  iny_nf();
  lda_indy_nf(EnemyData); // get second byte of object
...
CheckForEnemyGroup:
  lda_indy_nf(EnemyData); // get second byte and mask out 2 MSB
  and_imm_nf(0b00111111);
...
CheckFrenzyBuffer:
  ...
  lda_abs_nf(VineFlagOffset); // otherwise check vine flag offset
  cmp_imm(0x1);
  if (!zero_flag) { return; } // if other value <> 1, leave
  lda_imm_nf(VineObject); // otherwise put vine in enemy identifier
...
void ParseRow0e(void) {
  iny_nf(); // increment Y to load third byte of object
  iny_nf();
  lda_indy_nf(EnemyData);
  lsr_acc_nf(); // move 3 MSB to the bottom, effectively
  lsr_acc_nf(); // making %xxx00000 into %00000xxx
  lsr_acc_nf();
  lsr_acc_nf();
  lsr_acc_nf();
  ...
  if (zero_flag) {
    dey_nf(); // of the same area, like the underground bonus areas)
    lda_indy_nf(EnemyData); // otherwise, get second byte and use as offset
    ram[AreaPointer] = a; // to addresses for level and enemy object data
    iny_nf();
    lda_indy_nf(EnemyData); // get third byte again, and this time mask out
...
void Inc2B(void) {
  inc_abs_nf(EnemyDataOffset); // otherwise increment two bytes
  inc_abs_nf(EnemyDataOffset);
  lda_imm_nf(0x0); // init page select for enemy objects
  ram[EnemyObjectPageSel] = a;
  ldx_zp(ObjectOffset); // reload current offset in enemy buffers
}

void CheckThreeBytes(void) {
  ldy_abs_nf(EnemyDataOffset); // load current offset for enemy object data
  lda_indy_nf(EnemyData); // get first byte
  and_imm_nf(0b00001111); // check for special row $0e
  cmp_imm_fcz(0xe);
  if (!zero_flag) {
    Inc2B();
...
void Inc3B(void) {
  inc_abs_nf(EnemyDataOffset); // if row = $0e, increment three bytes
  Inc2B(); // <fallthrough>
}
...
void CheckpointEnemyID(void) {
  ...
  if (!carry_flag) {
    tay_nf(); // save identifier in Y register for now
    lda_zpx_nf(Enemy_Y_Position);
...
void InitPodoboo(void) {
  lda_imm_nf(0x2); // set enemy position to below
...
void InitNormalEnemy(void) {
  ldy_imm_nf(0x1); // load offset of 1 by default
  lda_abs_fz(PrimaryHardMode); // check for primary hard mode flag set
  if (zero_flag) {
    dey_nf(); // if not set, decrement offset
  }
  // GetESpd:
...
void InitHammerBro(void) {
  lda_imm_nf(0x0); // init horizontal speed and timer used by hammer bro
  ram[HammerThrowingTimer + x] = a; // apparently to time hammer throwing
  ram[Enemy_X_Speed + x] = a;
  ldy_abs_nf(SecondaryHardMode); // get secondary hard mode flag
  lda_absy_nf(HBroWalkingTimerData);
...
void InitBloober(void) {
  lda_imm_nf(0x0); // initialize horizontal speed
  ram[BlooperMoveSpeed + x] = a;
  SmallBBox(); // <fallthrough>
}

void SmallBBox(void) {
  lda_imm_fz(0x9); // set specific bounding box size control
...
void InitRedPTroopa(void) {
  ldy_imm_nf(0x30); // load central position adder for 48 pixels down
  ...
  if (neg_flag) {
    ldy_imm_nf(0xe0); // if => $80, load position adder for 32 pixels up
  }
  // GetCent:
  tya_nf(); // send central position adder to A
...
void TallBBox(void) {
  lda_imm_nf(0x3); // set specific bounding box size control
  SetBBox(); // <fallthrough>
}

void SetBBox(void) {
  ram[Enemy_BoundBoxCtrl + x] = a; // set bounding box control here
  lda_imm_nf(0x2); // set moving direction for left
...
void InitCheepCheep(void) {
  ...
  lda_absx_nf(PseudoRandomBitReg); // check one portion of LSFR
  and_imm_nf(0b00010000); // get d4 from it
...
void LakituAndSpinyHandler(void) {
  ...
  lda_imm_nf(0x80); // set timer
  ram[FrenzyEnemyTimer] = a;
  ldy_imm_nf(0x4); // start with the last enemy slot
  
ChkLak:
  lda_zpy_nf(Enemy_ID); // check all enemy slots to see
  ...
  inc_abs_nf(LakituReappearTimer); // increment reappearance timer
  lda_abs_nf(LakituReappearTimer);
  cmp_imm(0x7); // check to see if we're up to a certain value yet
  if (!carry_flag) { goto ExLSHand; } // if not, leave
  ldx_imm_nf(0x4); // start with the last enemy slot again
...
CreateL:
  lda_imm_nf(0x0); // initialize enemy state
  ram[Enemy_State + x] = a;
...
CreateSpiny:
  lda_zp_nf(Player_Y_Position); // if player above a certain point, branch to leave
  cmp_imm(0x2c);
  if (!carry_flag) { goto ExLSHand; }
  lda_zpy(Enemy_State); // if lakitu is not in normal state, branch to leave
  if (!zero_flag) { goto ExLSHand; }
  lda_zpy_nf(Enemy_PageLoc); // store horizontal coordinates (high and low) of lakitu
  ram[Enemy_PageLoc + x] = a; // into the coordinates of the spiny we're going to create
  lda_zpy_nf(Enemy_X_Position);
  ram[Enemy_X_Position + x] = a;
  lda_imm_nf(0x1); // put spiny within vertical screen unit
  ram[Enemy_Y_HighPos + x] = a;
  lda_zpy_nf(Enemy_Y_Position); // put spiny eight pixels above where lakitu is
  carry_flag = true;
  sbc_imm_fc(0x8);
  ram[Enemy_Y_Position + x] = a;
  lda_absx_nf(PseudoRandomBitReg); // get 2 LSB of LSFR and save to Y
  and_imm_nf(0b00000011);
  tay_nf();
  ldx_imm_nf(0x2);
  
DifLoop:
  lda_absy_nf(PRDiffAdjustData); // get three values and save them
  ram[0x1 + x] = a; // to $01-$03
  iny_nf();
  iny_nf(); // increment Y four bytes for each value
  iny_nf();
  iny_nf();
  ...
  ldy_zp_nf(Player_X_Speed); // check player's horizontal speed
  cpy_imm(0x8);
  if (carry_flag) { goto SetSpSpd; } // if moving faster than a certain amount, branch elsewhere
  tay_nf(); // otherwise save value in A to Y for now
  lda_absx_nf(PseudoRandomBitReg + 1);
  ...
  tya_nf();
  eor_imm_nf(0b11111111); // otherwise get two's compliment of Y
  tay_nf();
  iny_nf();
...
void InitShortFirebar(void) {
  lda_imm_nf(0x0); // initialize low byte of spin state
  ram[FirebarSpinState_Low + x] = a;
  lda_zpx_nf(Enemy_ID); // subtract $1b from enemy identifier
  carry_flag = true; // to get proper offset for firebar data
  sbc_imm_nf(0x1b);
  tay_nf();
  lda_absy_nf(FirebarSpinSpdData); // get spinning speed of firebar
  ram[FirebarSpinSpeed + x] = a;
  lda_absy_nf(FirebarSpinDirData); // get spinning direction of firebar
  ram[FirebarSpinDirection + x] = a;
  lda_zpx_nf(Enemy_Y_Position);
  carry_flag = false; // add four pixels to vertical coordinate
  adc_imm_nf(0x4);
  ram[Enemy_Y_Position + x] = a;
  lda_zpx_nf(Enemy_X_Position);
  carry_flag = false; // add four pixels to horizontal coordinate
  adc_imm_fc(0x4);
  ram[Enemy_X_Position + x] = a;
  lda_zpx_nf(Enemy_PageLoc);
...
void InitFlyingCheepCheep(void) {
  ...
  lda_absx_nf(PseudoRandomBitReg + 1);
  and_imm_nf(0b00000011); // set pseudorandom offset here
  tay_nf();
  lda_absy_nf(FlyCCTimerData); // load timer with pseudorandom offset
  ram[FrenzyEnemyTimer] = a;
  ldy_imm_nf(0x3); // load Y with default value
  lda_abs_fz(SecondaryHardMode);
  if (zero_flag) { goto MaxCC; } // if secondary hard mode flag not set, do not increment Y
  iny_nf(); // otherwise, increment Y to allow as many as four onscreen
...
MaxCC:
  ...
  lda_absx_nf(PseudoRandomBitReg);
  and_imm_nf(0b00000011); // get last two bits of LSFR, first part
  ram[0x0] = a; // and store in two places
  ram[0x1] = a;
  lda_imm_nf(0xfb); // set vertical speed for cheep-cheep
  ram[Enemy_Y_Speed + x] = a;
  lda_imm_nf(0x0); // load default value
  ...
  asl_acc_nf(); // otherwise, multiply A by 2
  
GSeed:
  pha(); // save to stack
  carry_flag = false;
  adc_zp_nf(0x0); // add to last two bits of LSFR we saved earlier
  ...
  lda_absx_nf(PseudoRandomBitReg + 2);
  and_imm_nf(0b00001111); // otherwise overwrite with lower nybble of
  ram[0x0] = a; // third LSFR part
  
RSeed:
  pla_nf(); // get value from stack we saved earlier
  carry_flag = false;
  adc_zp_nf(0x1); // add to last two bits of LSFR we saved in other place
  tay_nf(); // use as pseudorandom offset here
  lda_absy_nf(FlyCCXSpeedData); // get horizontal speed using pseudorandom offset
  ram[Enemy_X_Speed + x] = a;
  lda_imm_nf(0x1); // set to move towards the right
  ram[Enemy_MovingDir + x] = a;
  lda_zp_fz(Player_X_Speed); // if player moving left or right, branch ahead of this part
  if (!zero_flag) { goto D2XPos1; }
  ldy_zp_nf(0x0); // get first LSFR or third LSFR lower nybble
  tya_nf(); // and check for d1 set
  and_imm_fz(0b00000010);
  if (zero_flag) { goto D2XPos1; } // if d1 not set, branch
  lda_zpx_nf(Enemy_X_Speed);
  eor_imm_nf(0xff); // if d1 set, change horizontal speed
  carry_flag = false; // into two's compliment, thus moving in the opposite
  adc_imm_nf(0x1); // direction
  ram[Enemy_X_Speed + x] = a;
  inc_zpx_nf(Enemy_MovingDir); // increment to move towards the left
  
D2XPos1:
  tya_nf(); // get first LSFR or third LSFR lower nybble again
  and_imm_fz(0b00000010);
...
void InitBowserFlame(void) {
  ...
  if (zero_flag) {
    ...
    lda_zp_nf(NoiseSoundQueue);
    ora_imm_nf(Sfx_BowserFlame); // load bowser's flame sound into queue
    ram[NoiseSoundQueue] = a;
    ldy_abs_nf(BowserFront_Offset); // get bowser's buffer offset
    lda_zpy_nf(Enemy_ID); // check for bowser
    cmp_imm(Bowser);
    // branch if found
    if (zero_flag) {
      SpawnFromMouth();
    ...
    carry_flag = false;
    adc_imm_nf(0x20); // add 32 frames by default
    ldy_abs_fz(SecondaryHardMode);
    // if secondary mode flag not set, use as timer setting
    if (!zero_flag) {
      carry_flag = true;
      sbc_imm_nf(0x10); // otherwise subtract 16 frames for secondary hard mode
    }
    // SetFrT:
    ram[FrenzyEnemyTimer] = a; // set timer accordingly
    lda_absx_nf(PseudoRandomBitReg);
    and_imm_nf(0b00000011); // get 2 LSB from first part of LSFR
    ram[BowserFlamePRandomOfs + x] = a; // set here
    tay_nf(); // use as offset
    lda_absy_nf(FlameYPosData); // load vertical position based on pseudorandom offset
...
void SpawnFromMouth(void) {
  lda_zpy_nf(Enemy_X_Position); // get bowser's horizontal position
  carry_flag = true;
  sbc_imm_nf(0xe); // subtract 14 pixels
  ram[Enemy_X_Position + x] = a; // save as flame's horizontal position
  lda_zpy_nf(Enemy_PageLoc);
  ram[Enemy_PageLoc + x] = a; // copy page location from bowser to flame
  lda_zpy_nf(Enemy_Y_Position);
  ...
  tay_nf(); // use as offset
  lda_absy_nf(FlameYPosData); // get value here using bits as offset
  ldy_imm_nf(0x0); // load default offset
  cmp_zpx_fc(Enemy_Y_Position); // compare value to flame's current vertical position
  // if less, do not increment offset
  if (carry_flag) {
    iny_nf(); // otherwise increment now
  }
  // SetMF:
  lda_absy_nf(FlameYMFAdderData); // get value here and save
...
void FinishFlame(void) {
  lda_imm_nf(0x8); // set $08 for bounding box control
  ram[Enemy_BoundBoxCtrl + x] = a;
  lda_imm_nf(0x1); // set high byte of vertical and
...
void InitFireworks(void) {
  lda_abs(FrenzyEnemyTimer); // if timer not expired yet, branch to leave
  if (zero_flag) {
    lda_imm_nf(0x20); // otherwise reset timer
    ram[FrenzyEnemyTimer] = a;
    dec_abs_nf(FireworksCounter); // decrement for each explosion
    ldy_imm_nf(0x6); // start at last slot
    
StarFChk:
    dey_nf();
    lda_zpy_nf(Enemy_ID); // check for presence of star flag object
    ...
    pha(); // the stack
    lda_zpy_nf(Enemy_PageLoc);
    sbc_imm_nf(0x0); // subtract the carry from the page location
    ram[0x0] = a; // of the star flag object
    lda_abs_nf(FireworksCounter); // get fireworks counter
    carry_flag = false;
    adc_zpy_nf(Enemy_State); // add state of star flag object (possibly not necessary)
    tay_nf(); // use as offset
    pla_nf(); // get saved horizontal coordinate of star flag - 48 pixels
    carry_flag = false;
    ...
    lda_zp_nf(0x0);
    adc_imm_nf(0x0); // add carry and store as page location for
    ram[Enemy_PageLoc + x] = a; // the fireworks object
    lda_absy_nf(FireworksYPosData); // get vertical position using same offset
...
void BulletBillCheepCheep(void) {
  ...
  iny_nf(); // otherwise increment
  
ChkW2:
  lda_abs_nf(WorldNumber); // check world number
  cmp_imm_fz(World2);
...
Get17ID:
  tya_nf();
  and_imm_nf(0b00000001); // mask out all but last bit of offset
  tay_nf();
  lda_absy_nf(SwimCC_IDData); // load identifier for cheep-cheeps
  
Set17ID:
  ram[Enemy_ID + x] = a; // store whatever's in A as enemy identifier
  lda_abs_nf(BitMFilter);
  cmp_imm_fcz(0xff); // if not all bits set, skip init part and compare bits
  if (!zero_flag) { goto GetRBit; }
  lda_imm_nf(0x0); // initialize vertical position filter
  ram[BitMFilter] = a;
  
GetRBit:
  lda_absx_nf(PseudoRandomBitReg); // get first part of LSFR
  and_imm_nf(0b00000111); // mask out all but 3 LSB
  
ChkRBit:
  tay_nf(); // use as offset
  lda_absy_nf(Bitmasks); // load bitmask
  bit_abs_fz(BitMFilter); // perform AND on filter without changing it
  if (zero_flag) { goto AddFBit; }
  iny_nf(); // increment offset
  tya_nf();
  and_imm_nf(0b00000111); // mask out all but 3 LSB thus keeping it 0-7
  goto ChkRBit; // do another check
  
AddFBit:
  ora_abs_nf(BitMFilter); // add bit to already set bits in filter
  ...
  lda_imm_nf(0x20); // set timer
  ram[FrenzyEnemyTimer] = a;
  CheckpointEnemyID(); return; // process our new enemy object
  
DoBulletBills:
  ldy_imm_nf(0xff); // start at beginning of enemy slots
  
BB_SLoop:
  iny_nf(); // move onto the next slot
...
FireBulletBill:
  lda_zp_nf(Square2SoundQueue);
  ora_imm_nf(Sfx_Blast); // play fireworks/gunfire sound
  ram[Square2SoundQueue] = a;
  lda_imm_fz(BulletBill_FrenzyVar); // load identifier for bullet bill object
...
void HandleGroupEnemies(void) {
  ldy_imm_nf(0x0); // load value for green koopa troopa
  carry_flag = true;
  sbc_imm_nf(0x37); // subtract $37 from second byte read
  pha(); // save result in stack for now
  cmp_imm_fc(0x4); // was byte in $3b-$3e range?
  // if so, branch
  if (!carry_flag) {
    pha(); // save another copy to stack
    ldy_imm_nf(Goomba); // load value for goomba enemy
    lda_abs_fz(PrimaryHardMode); // if primary hard mode flag not set,
    // branch, otherwise change to value
    if (!zero_flag) {
      ldy_imm_nf(BuzzyBeetle); // for buzzy beetle
    }
    // PullID:
    pla_nf(); // get second copy from stack
  }
  // SnglID:
  ram[0x1] = y; // save enemy id here
  ldy_imm_nf(0xb0); // load default y coordinate
  and_imm_fz(0x2); // check to see if d1 was set
  // if so, move y coordinate up,
  if (!zero_flag) {
    ldy_imm_nf(0x70); // otherwise branch and use default
  }
  // SetYGp:
  ram[0x0] = y; // save y coordinate here
  lda_abs_nf(ScreenRight_PageLoc); // get page number of right edge of screen
  ram[0x2] = a; // save here
  lda_abs_nf(ScreenRight_X_Pos); // get pixel coordinate of right edge
  ram[0x3] = a; // save here
  ldy_imm_nf(0x2); // load two enemies by default
  pla_nf(); // get first copy from stack
  lsr_acc_fc(); // check to see if d0 was set
  // if not, use default value
  if (carry_flag) {
    iny_nf(); // otherwise increment to three enemies
  }
  // CntGrp:
  ram[NumberofGroupEnemies] = y; // save number of enemies here
  
GrLoop:
  ldx_imm_nf(0xff); // start at beginning of enemy buffers
  
GSltLp:
  inx_nf(); // increment and branch if past
  ...
  if (!carry_flag) {
    ...
    ram[0x3] = a;
    lda_zp_nf(0x2); // add carry to page location for
    adc_imm_fc(0x0); // next enemy
    ram[0x2] = a;
    lda_zp_nf(0x0); // store y coordinate for enemy object
    ram[Enemy_Y_Position + x] = a;
...
void InitPiranhaPlant(void) {
  lda_imm_nf(0x1); // set initial speed
...
void InitEnemyFrenzy(void) {
  lda_zpx_nf(Enemy_ID); // load enemy identifier
...
void EndFrenzy(void) {
  ldy_imm_nf(0x5); // start at last slot
  
LakituChk:
  lda_zpy_nf(Enemy_ID); // check enemy identifiers
  cmp_imm_fcz(Lakitu); // for lakitu
  if (zero_flag) {
    lda_imm_nf(0x1); // if found, set state
    ram[Enemy_State + y] = a;
  }
  // NextFSlot:
...
void InitJumpGPTroopa(void) {
  lda_imm_nf(0x2); // set for movement to the left
  ram[Enemy_MovingDir + x] = a;
  lda_imm_nf(0xf8); // set horizontal speed
  ram[Enemy_X_Speed + x] = a;
...
void InitBalPlatform(void) {
  dec_zpx_nf(Enemy_Y_Position); // raise vertical position by two pixels
  dec_zpx_nf(Enemy_Y_Position);
  ...
  // AlignP:
  ldy_imm_nf(0xff); // set default value here for now
  ...
  if (neg_flag) {
    txa_nf(); // if old contents already $ff, put
    tay_nf(); // object offset as alignment to make next positive
  }
  // SetBPA:
...
void InitVertPlatform(void) {
  ldy_imm_nf(0x40); // set default value here
  ...
  if (neg_flag) {
    ...
    adc_imm_nf(0x1);
    ldy_imm_nf(0xc0); // get alternate value to add to vertical position
  }
  // SetYO:
...
void SPBBox(void) {
  lda_imm_nf(0x5); // set default bounding box size control
  ldy_abs_nf(AreaType);
...
void PlatLiftUp(void) {
  lda_imm_nf(0x10); // set movement amount here
  ram[Enemy_Y_MoveForce + x] = a;
...
void PlatLiftDown(void) {
  lda_imm_nf(0xf0); // set movement amount here
  ram[Enemy_Y_MoveForce + x] = a;
  lda_imm_nf(0x0); // set moving speed for platforms going down
  ram[Enemy_Y_Speed + x] = a;
  // --------------------------------
...
FSLoop:
  ...
  txa_nf(); // transfer original enemy buffer offset
  ora_imm_nf(0b10000000); // store with d7 set as flag in new enemy
  ram[Enemy_Flag + y] = a; // slot as well as enemy offset
  lda_zpx_nf(Enemy_PageLoc);
  ram[Enemy_PageLoc + y] = a; // copy page location and horizontal coordinates
  lda_zpx_nf(Enemy_X_Position); // from original enemy to new enemy
...
void PosPlatform(void) {
  lda_zpx_nf(Enemy_X_Position); // get horizontal coordinate
  carry_flag = false;
...
void RunEnemyObjectsCore(void) {
  ldx_zp_nf(ObjectOffset); // get offset for enemy object buffer
  lda_imm_nf(0x0); // load value 0 for jump engine by default
  ldy_zpx_nf(Enemy_ID);
  cpy_imm(0x15); // if enemy object < $15, use default value
  if (carry_flag) {
    tya_nf(); // otherwise subtract $14 from the value and use
...
void RunBowser(void) {
  lda_zpx_nf(Enemy_State); // if d5 in enemy state is not set
  and_imm_fz(0b00100000); // then branch elsewhere to run bowser
  if (zero_flag) {
    BowserControl();
    return;
  }
  lda_zpx_nf(Enemy_Y_Position); // otherwise check vertical position
...
FeetTmr:
  ...
  lda_abs_nf(BowserBodyControls); // and invert bit used
  eor_imm_nf(0b00000001); // to control bowser's feet
  ram[BowserBodyControls] = a;
  
ResetMDr:
  lda_zp_nf(FrameCounter); // check frame counter
  and_imm_fz(0b00001111); // if not on every sixteenth frame, skip
  if (!zero_flag) { goto B_FaceP; } // ahead to continue code
  lda_imm_nf(0x2); // otherwise reset moving/facing direction every
...
GetPRCmp:
  ...
  lda_absx_nf(PseudoRandomBitReg);
  and_imm_nf(0b00000011); // get pseudorandom offset
  tay_nf();
  lda_absy_nf(PRandomRange); // load value using pseudorandom offset
  ram[MaxRangeFromOrigin] = a; // and store here
  
GetDToO:
  lda_zpx_nf(Enemy_X_Position);
  carry_flag = false; // add movement speed to bowser's horizontal
  adc_abs_nf(BowserMovementSpeed); // coordinate and save as new horizontal position
  ram[Enemy_X_Position + x] = a;
  ldy_zpx_nf(Enemy_MovingDir);
  cpy_imm_fcz(0x1); // if bowser moving and facing to the right, skip ahead
  if (zero_flag) { goto HammerChk; }
  ldy_imm_nf(0xff); // set default movement speed here (move left)
  ...
  adc_imm_nf(0x1);
  ldy_imm_nf(0x1); // set alternate movement speed here (move right)
...
SetHmrTmr:
  lda_zpx_nf(Enemy_Y_Position); // get current vertical position
  cmp_imm_fc(0x80); // if still above a certain point
  if (!carry_flag) { goto ChkFireB; } // then skip to world number check for flames
  lda_absx_nf(PseudoRandomBitReg);
  and_imm_nf(0b00000011); // get pseudorandom offset
  tay_nf();
  lda_absy_nf(PRandomRange); // get value using pseudorandom offset
...
ChkFireB:
  lda_abs_nf(WorldNumber); // check world number here
...
void BowserGfxHandler(void) {
  ...
  ldy_imm_nf(0x10); // load default value here to position bowser's rear
  lda_zpx_nf(Enemy_MovingDir); // check moving direction
  lsr_acc_fc();
  // if moving left, use default
  if (carry_flag) {
    ldy_imm_nf(0xf0); // otherwise load alternate positioning value here
  }
  // CopyFToR:
  tya_nf(); // move bowser's rear object position value to A
  carry_flag = false;
  adc_zpx_nf(Enemy_X_Position); // add to bowser's front object horizontal coordinate
  ldy_abs_nf(DuplicateObj_Offset); // get bowser's rear object offset
  ...
  lda_zp_nf(ObjectOffset); // save enemy object offset of front to stack
  pha();
  ldx_abs_nf(DuplicateObj_Offset); // put enemy object offset of rear as current
  ram[ObjectOffset] = x;
...
void RunFireworks(void) {
  ...
  inc_zpx_nf(ExplosionGfxCounter); // increment explosion graphics counter
  lda_zpx_nf(ExplosionGfxCounter);
...
SetupExpl:
  ...
  lda_abs_nf(Enemy_Rel_YPos); // copy relative coordinates
  ram[Fireball_Rel_YPos] = a; // from the enemy object to the fireball object
  lda_abs_nf(Enemy_Rel_XPos); // first vertical, then horizontal
  ram[Fireball_Rel_XPos] = a;
  ldy_absx_nf(Enemy_SprDataOffset); // get OAM data offset
...
FireworksSoundScore:
  lda_imm_nf(0x0); // disable enemy buffer flag
  ram[Enemy_Flag + x] = a;
  lda_imm_nf(Sfx_Blast); // play fireworks/gunfire sound
  ram[Square2SoundQueue] = a;
...
void RunStarFlagObj(void) {
  lda_imm_nf(0x0); // initialize enemy frenzy buffer
  ram[EnemyFrenzyBuffer] = a;
  lda_abs_nf(StarFlagTaskControl); // check star flag object task number here
...
void GameTimerFireworks(void) {
  ldy_imm_nf(0x5); // set default state for star flag object
  lda_abs_nf(GameTimerDisplay + 2); // get game timer's last digit
  cmp_imm_fcz(0x1);
  // if last digit of game timer set to 1, skip ahead
  if (!zero_flag) {
    ldy_imm_nf(0x3); // otherwise load new value for state
    cmp_imm_fcz(0x3);
    // if last digit of game timer set to 3, skip ahead
    if (!zero_flag) {
      ldy_imm_nf(0x0); // otherwise load one more potential value for state
      cmp_imm_fcz(0x6);
      // if last digit of game timer set to 6, skip ahead
      if (!zero_flag) {
        lda_imm_nf(0xff); // otherwise set value for no fireworks
      }
    }
  }
  // SetFWC:
...
void AwardGameTimerPoints(void) {
  lda_abs_nf(GameTimerDisplay); // check all game timer digits for any intervals left
  ora_abs_nf(GameTimerDisplay + 1);
  ora_abs_fz(GameTimerDisplay + 2);
  ...
  if (!zero_flag) {
    lda_imm_nf(Sfx_TimerTick);
    ram[Square2SoundQueue] = a; // load timer tick sound
  }
  // NoTTick:
  ldy_imm_nf(0x23); // set offset here to subtract from game timer's last digit
...
void EndAreaPoints(void) {
  ...
  lda_abs_nf(CurrentPlayer); // get player on the screen (or 500 points per
  asl_acc_nf(); // fireworks explosion if branched here from there)
  asl_acc_nf(); // shift to high nybble
  asl_acc_nf();
  asl_acc_fc();
...
void RaiseFlagSetoffFWorks(void) {
  lda_zpx_nf(Enemy_Y_Position); // check star flag's vertical position
...
void DrawStarFlag(void) {
  ...
  ldy_absx_nf(Enemy_SprDataOffset); // get OAM data offset
  ldx_imm_nf(0x3); // do four sprites
  
DSFLoop:
  lda_abs_nf(Enemy_Rel_YPos); // get relative vertical coordinate
  carry_flag = false;
  adc_absx_nf(StarFlagYPosAdder); // add Y coordinate adder data
  ram[Sprite_Y_Position + y] = a; // store as Y coordinate
  lda_absx_nf(StarFlagTileData); // get tile number
  ram[Sprite_Tilenumber + y] = a; // store as tile number
  lda_imm_nf(0x22); // set palette and background priority bits
  ram[Sprite_Attributes + y] = a; // store as attributes
  lda_abs_nf(Enemy_Rel_XPos); // get relative horizontal coordinate
  carry_flag = false;
  adc_absx_fc(StarFlagXPosAdder); // add X coordinate adder data
  ram[Sprite_X_Position + y] = a; // store as X coordinate
  iny_nf();
  iny_nf(); // increment OAM data offset four bytes
  iny_nf(); // for next sprite
  iny_nf();
...
void OffscreenBoundsCheck(void) {
  lda_zpx_nf(Enemy_ID); // check for cheep-cheep object
  cmp_imm(FlyingCheepCheep); // branch to leave if found
  if (zero_flag) { return; }
  lda_abs_nf(ScreenLeft_X_Pos); // get horizontal coordinate for left side of screen
  ldy_zpx_nf(Enemy_ID);
...
ExtendLB:
  ...
  lda_abs_nf(ScreenRight_X_Pos); // add 72 pixels to the right side horizontal coordinate
  adc_imm_fc(0x48);
  ram[0x3] = a; // store result here
  lda_abs_nf(ScreenRight_PageLoc);
  adc_imm_nf(0x0); // then add the carry to the page location
  ram[0x2] = a; // and store result here
  lda_zpx_nf(Enemy_X_Position); // compare horizontal coordinate of the enemy object
  ...
  lda_zpx_nf(Enemy_State); // if at this point, enemy is offscreen to the right, so check
...
void MovePodoboo(void) {
  ...
  if (zero_flag) {
    ...
    lda_absx_nf(PseudoRandomBitReg + 1); // get part of LSFR
    ora_imm_nf(0b10000000); // set d7
    ram[Enemy_Y_MoveForce + x] = a; // store as movement force
    and_imm_nf(0b00001111); // mask out high nybble
    ora_imm_nf(0x6); // set for at least six intervals
...
void ProcHammerBro(void) {
  lda_zpx_nf(Enemy_State); // check hammer bro's enemy state for d5 set
  and_imm(0b00100000);
  ...
  if (!zero_flag) {
    dec_zpx_nf(HammerBroJumpTimer); // otherwise decrement jump timer
    lda_abs_nf(Enemy_OffscreenBits);
    ...
    if (zero_flag) {
      ldy_abs_nf(SecondaryHardMode); // otherwise get secondary hard mode flag
  ...
  // HammerBroJumpCode:
  lda_zpx_nf(Enemy_State); // get hammer bro's enemy state
  and_imm_nf(0b00000111); // mask out all but 3 LSB
  ...
  lda_imm_nf(0x0); // load default value here
  ram[0x0] = a; // save into temp variable for now
  ldy_imm_nf(0xfa); // set default vertical speed
  ...
  ldy_imm_nf(0xfd); // otherwise set alternate vertical speed
  cmp_imm_fc(0x70); // check to see if hammer bro is above the middle of screen
  inc_zp_nf(0x0); // increment preset value to $01
  // if above the middle of the screen, use current speed and $01
  if (!carry_flag) {
    SetHJ();
    return;
  }
  dec_zp_nf(0x0); // otherwise return value to $00
  lda_absx_nf(PseudoRandomBitReg + 1); // get part of LSFR, mask out all but LSB
  and_imm_fz(0x1);
...
void SetHJ(void) {
  ...
  lda_zpx_nf(Enemy_State); // set d0 in enemy state for jumping
  ora_imm_nf(0x1);
  ram[Enemy_State + x] = a;
  lda_zp_nf(0x0); // load preset value here to use as bitmask
  and_absx_nf(PseudoRandomBitReg + 2); // and do bit-wise comparison with part of LSFR
  tay_nf(); // then use as offset
  lda_abs_fz(SecondaryHardMode); // check secondary hard mode flag
  if (zero_flag) {
    tay_nf(); // if secondary hard mode flag clear, set offset to 0
  }
  // HJump:
  lda_absy_nf(HammerBroJumpLData); // get jump length timer data using offset from before
  ram[EnemyFrameTimer + x] = a; // save in enemy timer
  lda_absx_nf(PseudoRandomBitReg + 1);
  ora_imm_nf(0b11000000); // get contents of part of LSFR, set d7 and d6, then
  ram[HammerBroJumpTimer + x] = a; // store in jump timer
  MoveHammerBroXDir(); // <fallthrough>
}

void MoveHammerBroXDir(void) {
  ldy_imm_nf(0xfc); // move hammer bro a little to the left
  lda_zp_nf(FrameCounter);
  and_imm_fz(0b01000000); // change hammer bro's direction every 64 frames
  if (zero_flag) {
    ldy_imm_nf(0x4); // if d6 set in counter, move him a little to the right
  }
  // Shimmy:
...
void MoveNormalEnemy(void) {
  ldy_imm_nf(0x0); // init Y to leave horizontal movement as-is
  lda_zpx_nf(Enemy_State);
...
FallE:
  ...
  ldy_imm_nf(0x0);
  lda_zpx_nf(Enemy_State); // check for enemy state $02
  cmp_imm(0x2);
...
SlowM:
  ldy_imm_nf(0x1); // if branched here, increment Y to slow horizontal movement
...
SteadM:
  ...
  iny_nf();
  iny_nf(); // otherwise increment Y to next data
...
void ProcMoveRedPTroopa(void) {
  ...
  lda_zpx_nf(Enemy_Y_Position); // check current vs. original vertical coordinate
...
MoveRedPTUpOrDown:
  lda_zpx_nf(Enemy_Y_Position); // check current vs. central vertical coordinate
...
void MoveFlyGreenPTroopa(void) {
  ...
  ldy_imm_nf(0x1); // set Y to move green paratroopa down
  lda_zp_nf(FrameCounter);
  ...
  if (zero_flag) {
    ...
    if (zero_flag) {
      ldy_imm_nf(0xff); // otherwise set Y to move green paratroopa up
    }
    // YSway:
    ram[0x0] = y; // store adder here
    lda_zpx_nf(Enemy_Y_Position);
...
void MoveBloober(void) {
  ...
  ldy_abs_nf(SecondaryHardMode); // use secondary hard mode flag as offset
  lda_absx_nf(PseudoRandomBitReg + 1); // get LSFR
...
BlooberSwim:
  ...
  lda_zpx_nf(Enemy_Y_Position); // get vertical coordinate
  carry_flag = true;
  sbc_absx_nf(Enemy_Y_MoveForce); // subtract movement force
...
SwimX:
  ldy_zpx_nf(Enemy_MovingDir); // check moving direction
  dey_fz();
...
void MoveBulletBill(void) {
  lda_zpx_nf(Enemy_State); // check bullet bill's enemy object state for d5 set
  and_imm(0b00100000);
...
void MoveSwimmingCheepCheep(void) {
  lda_zpx_nf(Enemy_State); // check cheep-cheep's enemy object state
  and_imm_fz(0b00100000); // for d5 set
...
CCSwim:
  ...
  lda_zpx_nf(Enemy_ID); // get enemy identifier
  carry_flag = true;
  sbc_imm_nf(0xa); // subtract ten for cheep-cheep identifiers
  tay_nf(); // use as offset
  lda_absy_nf(SwimCCXMoveData); // load value here
  ram[0x2] = a;
  lda_absx_nf(Enemy_X_MoveForce); // load horizontal force
  carry_flag = true;
  ...
  lda_zpx_nf(Enemy_PageLoc);
  sbc_imm_nf(0x0); // subtract borrow again, this time from the
  ...
  lda_zpx_nf(CheepCheepMoveMFlag); // check movement flag
  ...
  lda_zpx_nf(Enemy_Y_HighPos);
  adc_imm_nf(0x0); // add carry to page location and
...
CCSwimUpwards:
  ...
  ram[Enemy_Y_Position + x] = a; // save as new vertical coordinate
  lda_zpx_nf(Enemy_Y_HighPos);
  sbc_imm_nf(0x0); // subtract borrow from page location
  
ChkSwimYPos:
  ram[Enemy_Y_HighPos + x] = a; // save new page location here
  ldy_imm_nf(0x0); // load movement speed to upwards by default
  lda_zpx_nf(Enemy_Y_Position); // get vertical coordinate
  carry_flag = true;
  ...
  ldy_imm_nf(0x10); // otherwise load movement speed to downwards
  eor_imm_nf(0xff);
  carry_flag = false; // get two's compliment of result
  adc_imm_nf(0x1); // to obtain total difference of original vs. current
...
void MoveFlyingCheepCheep(void) {
  lda_zpx_nf(Enemy_State); // check cheep-cheep's enemy state
  ...
  ldy_imm_nf(0xd); // set vertical movement amount
  lda_imm(0x5); // set maximum speed
  SetXMoveAmt(); // branch to impose gravity on flying cheep-cheep
  lda_absx_nf(Enemy_Y_MoveForce);
  lsr_acc_nf(); // get vertical movement force and
  lsr_acc_nf(); // move high nybble to low
  lsr_acc_nf();
  lsr_acc_nf();
  tay_nf(); // save as offset (note this tends to go into reach of code)
  lda_zpx_nf(Enemy_Y_Position); // get vertical position
  ...
  if (!carry_flag) {
    lda_absx_nf(Enemy_Y_MoveForce);
    carry_flag = false;
    adc_imm_nf(0x10); // otherwise add to it
    ram[Enemy_Y_MoveForce + x] = a;
    lsr_acc_nf(); // move high nybble to low again
    lsr_acc_nf();
    lsr_acc_nf();
    lsr_acc_fc();
    tay_nf();
...
void MoveLakitu(void) {
  lda_zpx_nf(Enemy_State); // check lakitu's enemy state
  and_imm_fz(0b00100000); // for d5 set
...
LdLDa:
  lda_absy_nf(LakituDiffAdj); // load values
...
SetLSpd:
  ...
  ldy_imm_nf(0x1); // set moving direction to right by default
  lda_zpx_nf(LakituMoveDirection);
  and_imm(0x1); // get LSB of moving direction
  if (!zero_flag) { goto SetLMov; } // if set, branch to the end to use moving direction
  lda_zpx_nf(LakituMoveSpeed);
  eor_imm_nf(0xff); // get two's compliment of moving speed
  carry_flag = false;
  adc_imm_fc(0x1);
...
void MovePiranhaPlant(void) {
  ...
  lda_zp_nf(0x0); // otherwise get saved horizontal difference
  eor_imm_nf(0xff);
...
ChkPlayerNearPipe:
  lda_zp_nf(0x0); // get saved horizontal difference
  cmp_imm_fc(0x21);
  if (!carry_flag) { goto PutinPipe; } // if player within a certain distance, branch to leave
  
ReversePlantSpeed:
  lda_zpx_nf(PiranhaPlant_Y_Speed); // get vertical speed
  eor_imm_nf(0xff);
  carry_flag = false; // change to two's compliment
  adc_imm_nf(0x1);
  ram[PiranhaPlant_Y_Speed + x] = a; // save as new vertical speed
  inc_zpx_nf(PiranhaPlant_MoveFlag); // increment to set movement flag
  
SetupToMovePPlant:
  lda_absx_nf(PiranhaPlantDownYPos); // get original vertical coordinate (lowest point)
  ldy_zpx_fn(PiranhaPlant_Y_Speed); // get vertical speed
  if (!neg_flag) { goto RiseFallPiranhaPlant; } // branch if moving downwards
  lda_absx_nf(PiranhaPlantUpYPos); // otherwise get other vertical coordinate (highest point)
...
RiseFallPiranhaPlant:
  ...
  lda_zpx_nf(Enemy_Y_Position); // get current vertical coordinate
  carry_flag = false;
  adc_zpx_nf(PiranhaPlant_Y_Speed); // add vertical speed to move up or down
  ram[Enemy_Y_Position + x] = a; // save as new vertical coordinate
...
void LargePlatformSubroutines(void) {
  lda_zpx_nf(Enemy_ID); // subtract $24 to get proper offset for jump table
  carry_flag = true;
  sbc_imm(0x24);
  switch (a) {
...
CheckBalPlatform:
  tay_nf(); // save offset from state as Y
  lda_absx_nf(PlatformCollisionFlag); // get collision flag of platform
  ram[0x0] = a; // store here
...
ChkForFall:
  lda_imm_nf(0x2d); // check if platform is above a certain point
...
ChkToMoveBalPlat:
  lda_zpx_nf(Enemy_Y_Position); // save vertical position to stack
  pha();
...
DoOtherPlatform:
  ldy_zpx_nf(Enemy_State); // get offset of other platform
  pla_nf(); // get old vertical coordinate from stack
  carry_flag = true;
  sbc_zpx_nf(Enemy_Y_Position); // get difference of old vs. new coordinate
  carry_flag = false;
...
DrawEraseRope:
  ldy_zp_nf(ObjectOffset); // get enemy object offset
  lda_zpy_nf(Enemy_Y_Speed); // check to see if current platform is
  ora_absy_fz(Enemy_Y_MoveForce); // moving at all
  if (zero_flag) { goto ExitRp; } // if not, skip all of this and branch to leave
  ldx_abs_nf(VRAM_Buffer1_Offset); // get vram buffer offset
  ...
  lda_zp_nf(0x1); // write name table address to vram buffer
  ram[VRAM_Buffer1 + x] = a; // first the high byte, then the low
  lda_zp_nf(0x0);
  ram[VRAM_Buffer1 + 1 + x] = a;
  lda_imm_nf(0x2); // set length for 2 bytes
  ram[VRAM_Buffer1 + 2 + x] = a;
  ...
  lda_imm_nf(0xa3); // and right sides of rope in vram buffer
  ram[VRAM_Buffer1 + 4 + x] = a;
  goto OtherRope; // jump to skip this part
  
EraseR1:
  lda_imm_nf(0x24); // put blank tiles in vram buffer
  ram[VRAM_Buffer1 + 3 + x] = a; // to erase rope
  ram[VRAM_Buffer1 + 4 + x] = a;
  
OtherRope:
  lda_zpy_nf(Enemy_State); // get offset of other platform from state
  tay_nf(); // use as Y here
  pla_nf(); // pull second copy of vertical speed from stack
  ...
  lda_zp_nf(0x0); // the other platform
  ram[VRAM_Buffer1 + 6 + x] = a;
  lda_imm_nf(0x2);
  ...
  lda_imm_nf(0xa3); // and right sides of rope in vram
...
EndRp:
  lda_imm_nf(0x0); // put null terminator at the end
  ram[VRAM_Buffer1 + 10 + x] = a;
  lda_abs_nf(VRAM_Buffer1_Offset); // add ten bytes to the vram buffer offset
...
void YMovingPlatform(void) {
  lda_zpx_nf(Enemy_Y_Speed); // if platform moving up or down, skip ahead to
  ...
  // ChkYCenterPos:
  lda_zpx_nf(Enemy_Y_Position); // if current vertical position < central position, branch
...
void XMoveCntr_GreenPTroopa(void) {
  lda_imm_nf(0x13); // load preset maximum value for secondary counter
...
void XMoveCntr_Platform(void) {
  ...
  ldy_zpx_nf(XMoveSecondaryCounter); // get secondary counter
  lda_zpx_nf(XMovePrimaryCounter); // get primary counter
  lsr_acc_fc();
...
void MoveWithXMCntrs(void) {
  lda_zpx_nf(XMoveSecondaryCounter); // save secondary counter to stack
  pha();
  ldy_imm_nf(0x1); // set value here by default
  lda_zpx_nf(XMovePrimaryCounter);
  and_imm(0b00000010); // if d1 of primary counter is
  // set, branch ahead of this part here
  if (zero_flag) {
    lda_zpx_nf(XMoveSecondaryCounter);
    eor_imm_nf(0xff); // otherwise change secondary
...
void ProcSwimmingB(void) {
  lda_zpx_nf(BlooperMoveCounter); // get enemy's movement counter
  and_imm_fz(0b00000010); // check for d1 set
  // branch if set
  if (zero_flag) {
    lda_zp_nf(FrameCounter);
    and_imm_nf(0b00000111); // get 3 LSB of frame counter
    ...
    if (zero_flag) {
      ...
      if (zero_flag) {
        inc_zpx_nf(BlooperMoveCounter); // otherwise increment movement counter
        lda_imm(0x2);
...
Floatdown:
  ...
  // ChkNearPlayer:
  lda_zpx_nf(Enemy_Y_Position); // get vertical coordinate
  adc_imm_nf(0x10); // add sixteen pixels
...
void ProcFirebar(void) {
  GetEnemyOffscreenBits(); // get offscreen information
  lda_abs_nf(Enemy_OffscreenBits); // check for d3 set
  ...
  and_imm_nf(0b00011111); // mask out all but 5 LSB
  ram[FirebarSpinState_High + x] = a; // and store as new high byte of spinstate
  
SusFbar:
  lda_zpx_nf(FirebarSpinState_High); // get high byte of spinstate
  ldy_zpx_nf(Enemy_ID); // check enemy identifier
  cpy_imm(0x1f);
...
SetupGFB:
  ...
  ldy_imm_nf(0x5); // load value for short firebars by default
  lda_zpx_nf(Enemy_ID);
  cmp_imm_fc(0x1f); // are we doing a long firebar?
  if (!carry_flag) { goto SetMFbar; } // no, branch then
  ldy_imm_nf(0xb); // otherwise load value for long firebars
...
DrawFbar:
  ...
  lda_zp_nf(0x0); // check which firebar part
  cmp_imm_fz(0x4);
  if (!zero_flag) { goto NextFbar; }
  ldy_abs_nf(DuplicateObj_Offset); // if we arrive at fifth firebar part,
  lda_absy_nf(Enemy_SprDataOffset); // get offset from long firebar and load OAM data offset
  ram[0x6] = a; // using long firebar offset, then store as new one here
  
NextFbar:
  inc_zp_nf(0x0); // move onto the next firebar part
  lda_zp_nf(0x0);
...
void DrawFirebar_Collision(void) {
  lda_zp_nf(0x3); // store mirror data elsewhere
  ram[0x5] = a;
  ldy_zp_nf(0x6); // load OAM data offset for firebar
  lda_zp_nf(0x1); // load horizontal adder we got from position loader
  lsr_zp_fc(0x5); // shift LSB of mirror data
  if (carry_flag) { goto AddHA; } // if carry was set, skip this part
  eor_imm_nf(0xff);
  adc_imm_nf(0x1); // otherwise get two's compliment of horizontal adder
  
AddHA:
  carry_flag = false; // add horizontal coordinate relative to screen to
  adc_abs_nf(Enemy_Rel_XPos); // horizontal adder, modified or otherwise
  ...
  lda_abs_nf(Enemy_Rel_XPos);
  carry_flag = true; // otherwise subtract sprite X from the
  sbc_zp_nf(0x6); // original one and skip this part
  goto ChkFOfs;
  
SubtR1:
  carry_flag = true; // subtract original X from the
  sbc_abs_nf(Enemy_Rel_XPos); // current sprite X
...
void FirebarCollision(void) {
  ...
  tya_nf(); // return OAM data offset and save
  pha(); // to the stack for now
  lda_abs_nf(StarInvincibleTimer); // if star mario invincibility timer
  ...
  ldy_zp_nf(Player_Y_Position); // get player's vertical position
...
AdjSm:
  inc_zp_nf(0x5); // if small or big but crouching, execute this part
  inc_zp_nf(0x5); // first increment our counter twice (setting $02 as flag)
  tya_nf();
  carry_flag = false; // then add 24 pixels to the player's
  adc_imm_nf(0x18); // vertical coordinate
  tay_nf();
  
BigJp:
  tya_nf(); // get vertical coordinate, altered or otherwise, from Y
...
FBCLoop:
  ...
  eor_imm_nf(0xff); // skip two's compliment part
  carry_flag = false; // otherwise get two's compliment
  adc_imm_nf(0x1);
...
ChkVFBD:
  ...
  lda_zp_nf(0x6); // if firebar on far right on the screen, skip this,
  cmp_imm_fc(0xf0); // because, really, what's the point?
  if (carry_flag) { goto Chk2Ofs; }
  lda_abs_nf(Sprite_X_Position + 4); // get OAM X coordinate for sprite #1
  carry_flag = false;
  adc_imm_nf(0x4); // add four pixels
...
Chk2Ofs:
  lda_zp_nf(0x5); // if value of $02 was set earlier for whatever reason,
  cmp_imm_fz(0x2); // branch to increment OAM offset and leave, no collision
  if (zero_flag) { goto NoColFB; }
  ldy_zp_nf(0x5); // otherwise get temp here and use as offset
  lda_zp_nf(Player_Y_Position);
  carry_flag = false;
  adc_absy_nf(FirebarYPos); // add value loaded with offset to player's vertical coordinate
  inc_zp_nf(0x5); // then increment temp and jump back
  goto FBCLoop;
  
ChgSDir:
  ldx_imm_nf(0x1); // set movement direction by default
  lda_zp_nf(0x4); // if OAM X coordinate of player's sprite 1
...
void GetFirebarPosition(void) {
  ...
  and_imm_nf(0b00001111); // mask out low nybble
  cmp_imm_fc(0x9);
  // if lower than $09, branch ahead
  if (carry_flag) {
    eor_imm_nf(0b00001111); // otherwise get two's compliment to oscillate
    carry_flag = false;
    adc_imm_nf(0x1);
  }
  // GetHAdder:
  ram[0x1] = a; // store result, modified or not, here
  ldy_zp_nf(0x0); // load number of firebar ball where we're at
  lda_absy_nf(FirebarTblOffsets); // load offset to firebar position data
  carry_flag = false;
  adc_zp_nf(0x1); // add oscillated high byte of spinstate
  tay_nf(); // to offset here and use as new offset
  lda_absy_nf(FirebarPosLookupTbl); // get data here and store as horizontal adder
  ram[0x1] = a;
  pla_nf(); // pull whatever was in A from the stack
  pha(); // save it again because we still need it
  carry_flag = false;
  adc_imm_nf(0x8); // add eight this time, to get vertical adder
  and_imm_nf(0b00001111); // mask out high nybble
  cmp_imm_fc(0x9); // if lower than $09, branch ahead
  if (carry_flag) {
    eor_imm_nf(0b00001111); // otherwise get two's compliment
    carry_flag = false;
    adc_imm_nf(0x1);
  }
  // GetVAdder:
  ...
  ldy_zp_nf(0x0);
  lda_absy_nf(FirebarTblOffsets); // load offset to firebar position data again
  carry_flag = false;
  adc_zp_nf(0x2); // this time add value in $02 to offset here and use as offset
  tay_nf();
  lda_absy_nf(FirebarPosLookupTbl); // get data here and store as vertica adder
  ram[0x2] = a;
  pla_nf(); // pull out whatever was in A one last time
  lsr_acc_nf(); // divide by eight or shift three to the right
  lsr_acc_nf();
  lsr_acc_fc();
  tay_nf(); // use as offset
...
void PlayerLakituDiff(void) {
  ...
  iny_nf(); // increment Y for left of player
  lda_zp_nf(0x0);
  eor_imm_nf(0xff); // get two's compliment of low byte of horizontal difference
  carry_flag = false;
  adc_imm_nf(0x1); // store two's compliment as horizontal difference
  ram[0x0] = a;
  
ChkLakDif:
  lda_zp_nf(0x0); // get low byte of horizontal difference
  cmp_imm_fc(0x3c); // if within a certain distance of player, branch
  if (!carry_flag) { goto ChkPSpeed; }
  lda_imm_nf(0x3c); // otherwise set maximum distance
  ram[0x0] = a;
  lda_zpx_nf(Enemy_ID); // check if lakitu is in our current enemy slot
  cmp_imm_fz(Lakitu);
  if (!zero_flag) { goto ChkPSpeed; } // if not, branch elsewhere
  tya_nf(); // compare contents of Y, now in A
  ...
  dec_zpx_nf(LakituMoveSpeed); // decrement horizontal speed
  lda_zpx(LakituMoveSpeed); // if horizontal speed not yet at zero, branch to leave
  if (!zero_flag) { return; }
  
SetLMovD:
  tya_nf(); // set horizontal direction depending on horizontal
  ram[LakituMoveDirection + x] = a; // difference between enemy and player if necessary
  
ChkPSpeed:
  lda_zp_nf(0x0);
  and_imm_nf(0b00111100); // mask out all but four bits in the middle
  lsr_acc_nf(); // divide masked difference by four
  lsr_acc_nf();
  ram[0x0] = a; // store as new value
  ldy_imm_nf(0x0); // init offset
...
ChkEmySpd:
  ...
  ldy_imm_nf(0x0); // otherwise reinit offset
  
SubDifAdj:
  lda_absy_nf(0x1); // get one of three saved values from earlier
  ldy_zp_nf(0x0); // get saved horizontal difference
...
void SetFlameTimer(void) {
  ldy_abs_nf(BowserFlameTimerCtrl); // load counter as offset
  inc_abs_nf(BowserFlameTimerCtrl); // increment
  lda_abs_nf(BowserFlameTimerCtrl); // mask out all but 3 LSB
  and_imm_nf(0b00000111); // to keep in range of 0-7
...
void ProcBowserFlame(void) {
  ...
  lda_imm_nf(0x40); // load default movement force
  ldy_abs_fz(SecondaryHardMode);
  if (zero_flag) { goto SFlmX; } // if secondary hard mode flag not set, use default
  lda_imm_nf(0x60); // otherwise load alternate movement force to go faster
  
SFlmX:
  ram[0x0] = a; // store value here
  lda_absx_nf(Enemy_X_MoveForce);
  ...
  sbc_imm_nf(0x0); // subtract borrow from page location
  ram[Enemy_PageLoc + x] = a;
  ldy_absx_nf(BowserFlamePRandomOfs); // get some value here and use as offset
  lda_zpx_nf(Enemy_Y_Position); // load vertical coordinate
...
SetGfxF:
  ...
  if (!zero_flag) { return; } // branch to leave
  lda_imm_nf(0x51); // otherwise, continue
  ram[0x0] = a; // write first tile number
  ldy_imm_nf(0x2); // load attributes without vertical flip by default
  lda_zp_nf(FrameCounter);
  and_imm_fz(0b00000010); // invert vertical flip bit every 2 frames
  if (zero_flag) { goto FlmeAt; } // if d1 not set, write default value
  ldy_imm_nf(0x82); // otherwise write value with vertical flip bit set
...
DrawFlameLoop:
  lda_abs_nf(Enemy_Rel_YPos); // get Y relative coordinate of current enemy object
  ...
  inc_zp_nf(0x0); // increment tile number to draw more bowser's flame
  lda_zp_nf(0x1);
...
void FirebarSpin(void) {
  ...
  if (zero_flag) {
    ldy_imm_nf(0x18); // possibly residual ldy
    lda_zpx_nf(FirebarSpinState_Low);
    carry_flag = false; // add spinning speed to what would normally be
    adc_zp_fc(0x7); // the horizontal speed
    ram[FirebarSpinState_Low + x] = a;
    lda_zpx_nf(FirebarSpinState_High); // add carry to what would normally be the vertical speed
    adc_imm(0x0);
    return;
  }
  // SpinCounterClockwise:
  ldy_imm_nf(0x8); // possibly residual ldy
  lda_zpx_nf(FirebarSpinState_Low);
...
void SetupPlatformRope(void) {
  ...
  lda_zpy_nf(Enemy_X_Position); // get horizontal coordinate
  carry_flag = false;
  adc_imm_fc(0x8); // add eight pixels
  ...
  lda_zpy_nf(Enemy_PageLoc);
  adc_imm_nf(0x0); // add carry to page location
  ram[0x2] = a; // and save here
  pla_nf(); // pull modified horizontal coordinate
  and_imm_nf(0b11110000); // from the stack, mask out low nybble
  lsr_acc_nf(); // and shift three bits to the right
  lsr_acc_nf();
  lsr_acc_nf();
  ram[0x0] = a; // store result here as part of name table low byte
  ldx_zpy_nf(Enemy_Y_Position); // get vertical coordinate
  pla_fn(); // get second/third copy of vertical speed from stack
  // skip this part if moving downwards or not at all
  if (neg_flag) {
    txa_nf();
    carry_flag = false;
    adc_imm_nf(0x8); // add eight to vertical coordinate and
    tax_nf(); // save as X
  }
  // GetHRp:
  txa_nf(); // move vertical coordinate to A
  ldx_abs_nf(VRAM_Buffer1_Offset); // get vram buffer offset
  asl_acc_fc();
  rol_acc_fc(); // rotate d7 to d0 and d6 into carry
  pha(); // save modified vertical coordinate to stack
  rol_acc_nf(); // rotate carry to d0, thus d7 and d6 are at 2 LSB
  and_imm_nf(0b00000011); // mask out all bits but d7 and d6, then set
  ora_imm_nf(0b00100000); // d5 to get appropriate high byte of name table
  ram[0x1] = a; // address, then store
  lda_zp_nf(0x2); // get saved page location from earlier
  and_imm_nf(0x1); // mask out all but LSB
  asl_acc_nf();
  asl_acc_nf(); // shift twice to the left and save with the
  ora_zp_nf(0x1); // rest of the bits of the high byte, to get
  ram[0x1] = a; // the proper name table and the right place on it
  pla_nf(); // get modified vertical coordinate from stack
  and_imm_nf(0b11100000); // mask out low nybble and LSB of high nybble
  carry_flag = false;
  adc_zp_nf(0x0); // add to horizontal part saved here
...
void InitPlatformFall(void) {
  tya_nf(); // move offset of other platform from Y to X
  tax();
  GetEnemyOffscreenBits(); // get offscreen bits
  lda_imm(0x6);
  SetupFloateyNumber(); // award 1000 points to player
  lda_abs_nf(Player_Rel_XPos);
...
void MoveLiftPlatforms(void) {
  ...
  if (zero_flag) {
    ...
    lda_zpx_nf(Enemy_Y_Position); // add whatever vertical speed is set to current
...
void FireballEnemyCollision(void) {
  ...
  txa_nf();
  asl_acc_nf(); // multiply fireball offset by four
  asl_acc_nf();
  carry_flag = false;
  adc_imm_fc(0x1c); // then add $1c or 28 bytes to it
  tay_nf(); // to use fireball's bounding box coordinates
  ldx_imm_nf(0x4);
...
FireballEnemyCDLoop:
  ...
  lda_zpx_nf(Enemy_ID); // check enemy identifier
  cmp_imm_fc(0x24);
...
GoombaDie:
  ...
  lda_zpx_nf(Enemy_State); // otherwise check for defeated state
...
NotGoomba:
  ...
  txa_nf();
  asl_acc_nf(); // otherwise multiply enemy offset by four
  asl_acc_nf();
  carry_flag = false;
  ...
  ldx_zp_nf(ObjectOffset); // return fireball's original offset
...
NoFToECol:
  pla_nf(); // pull fireball offset from stack
  tay_nf(); // put it in Y
  ldx_zp_nf(0x1); // get enemy object offset
  dex_fn(); // decrement it
...
HurtBowser:
  ...
  ldy_abs_nf(WorldNumber); // use world number as offset
  lda_absy_nf(BowserIdentities); // get enemy identifier to replace bowser with
  ram[Enemy_ID + x] = a; // set as new enemy identifier
  lda_imm_nf(0x20); // set A to use starting value for state
  cpy_imm_fc(0x3); // check to see if using offset of 3 or more
  if (carry_flag) { goto SetDBSte; } // branch if so
  ora_imm_nf(0x3); // otherwise add 3 to enemy state
  
SetDBSte:
  ram[Enemy_State + x] = a; // set defeated enemy state
  lda_imm_nf(Sfx_BowserFall);
  ram[Square2SoundQueue] = a; // load bowser defeat sound
  ldx_zp_nf(0x1); // get enemy offset
...
void ShellOrBlockDefeat(void) {
  lda_zpx_nf(Enemy_ID); // check for piranha plant
  ...
  lda_zpx_nf(Enemy_State);
  and_imm_nf(0b00011111); // mask out 2 MSB of enemy object's state
  ora_imm_nf(0b00100000); // set d5 to defeat enemy and save as new state
  ram[Enemy_State + x] = a;
  lda_imm_nf(0x2); // award 200 points by default
  ldy_zpx_nf(Enemy_ID); // check for hammer bro
  cpy_imm_fz(HammerBro);
  // branch if not found
  if (zero_flag) {
    lda_imm_nf(0x6); // award 1000 points for hammer bro
  }
  // GoombaPoints:
...
void PlayerHammerCollision(void) {
  ...
  lda_abs_nf(TimerControl); // if either master timer control
  ora_abs(Misc_OffscreenBits); // or any offscreen bits for hammer are set,
  if (!zero_flag) { return; } // branch to leave
  txa_nf();
  asl_acc_nf(); // multiply misc object offset by four
  asl_acc_nf();
  carry_flag = false;
  ...
  ldx_zp_nf(ObjectOffset); // get misc object offset
  ...
  lda_zpx_nf(Misc_X_Speed);
  eor_imm_nf(0xff); // get two's compliment of
...
void HandlePowerUpCollision(void) {
  ...
  lda_imm_nf(Sfx_PowerUpGrab);
  ram[Square2SoundQueue] = a; // play the power-up sound
  lda_zp_nf(PowerUpType); // check power-up type
  cmp_imm_fc(0x2);
  ...
  lda_imm_nf(0x23); // otherwise set star mario invincibility
...
Shroom_Flower_PUp:
  ...
  ldx_zp_nf(ObjectOffset); // get enemy offset, not necessary
  lda_imm(0x2); // set player status to fiery
  ram[PlayerStatus] = a;
  GetPlayerColors(); // run sub to change colors of player
  ldx_zp_nf(ObjectOffset); // get enemy offset again, and again not necessary
...
UpToSuper:
  lda_imm_nf(0x1); // set player status to super
  ram[PlayerStatus] = a;
  lda_imm_nf(0x9); // set value to be used by subroutine tree (super)
...
void PlayerEnemyCollision(void) {
  lda_zp_nf(FrameCounter); // check counter for d0 set
  lsr_acc();
  ...
  ldx_zp_nf(ObjectOffset); // get enemy object buffer offset
  if (carry_flag) { goto CheckForPUpCollision; } // if collision, branch past this part here
  lda_absx_nf(Enemy_CollisionBits);
...
HandlePECollisions:
  lda_absx_nf(Enemy_CollisionBits); // check enemy collision bits for d0 set
  and_imm_nf(0b00000001); // or for being offscreen at all
  ora_absx(EnemyOffscrBitsMasked);
  if (!zero_flag) { goto ExPEC; } // branch to leave if either is true
  lda_imm_nf(0x1);
  ora_absx_nf(Enemy_CollisionBits); // otherwise set d0 now
  ...
  if (zero_flag) { InjurePlayer(); return; }
  lda_zpx_nf(Enemy_State); // branch if d7 of enemy state was set
  asl_acc_fc();
  if (carry_flag) { goto ChkForPlayerInjury; }
  lda_zpx_nf(Enemy_State); // mask out all but 3 LSB of enemy state
  and_imm_nf(0b00000111);
  cmp_imm_fc(0x2); // branch if enemy is in normal or falling state
  if (!carry_flag) { goto ChkForPlayerInjury; }
  lda_zpx_nf(Enemy_ID); // branch to leave if goomba in defeated state
  cmp_imm(Goomba);
  if (zero_flag) { goto ExPEC; }
  lda_imm_nf(Sfx_EnemySmack); // play smack enemy sound
  ram[Square1SoundQueue] = a;
  lda_zpx_nf(Enemy_State); // set d7 in enemy state, thus become moving shell
  ora_imm(0b10000000);
  ram[Enemy_State + x] = a;
  EnemyFacePlayer(); // set moving direction and get offset
  lda_absy_nf(KickedShellXSpdData); // load and set horizontal speed data with offset
  ram[Enemy_X_Speed + x] = a;
  lda_imm_nf(0x3); // add three to whatever the stomp counter contains
  carry_flag = false; // to give points for kicking the shell
  adc_abs_nf(StompChainCounter);
  ldy_absx_nf(EnemyIntervalTimer); // check shell enemy's timer
...
ChkInj:
  lda_zpx_nf(Enemy_ID); // branch if enemy object < $07
  cmp_imm_fc(Bloober);
  if (!carry_flag) { goto ChkETmrs; }
  lda_zp_nf(Player_Y_Position); // add 12 pixels to player's vertical position
  carry_flag = false;
  adc_imm_nf(0xc);
...
TInjE:
  lda_zpx_nf(Enemy_MovingDir); // if enemy moving towards the left,
...
void ForceInjury(void) {
  ...
  lda_imm_nf(0xa); // set subroutine to run on next frame
  SetKRout(); // <fallthrough>
}

void SetKRout(void) {
  ldy_imm_nf(0x1); // set new player state
...
void KillPlayer(void) {
  ...
  lda_imm_fz(0xb); // set subroutine to run on next frame
...
void EnemyStomped(void) {
  lda_zpx_nf(Enemy_ID); // check for spiny, branch to hurt player
  cmp_imm_fcz(Spiny); // if found
  if (zero_flag) { InjurePlayer(); return; }
  lda_imm_nf(Sfx_EnemyStomp); // otherwise play stomp/swim sound
  ram[Square1SoundQueue] = a;
  lda_zpx_nf(Enemy_ID);
  ldy_imm_nf(0x0); // initialize points data offset for stomped enemies
  ...
  iny_nf(); // increment points data offset
...
ChkForDemoteKoopa:
  ...
  and_imm_nf(0b00000001); // demote koopa paratroopas to ordinary troopas
  ram[Enemy_ID + x] = a;
  ldy_imm_nf(0x0); // return enemy to normal state
  ram[Enemy_State + x] = y;
  ...
  lda_absy_nf(DemotedKoopaXSpdData);
  ram[Enemy_X_Speed + x] = a; // set appropriate moving speed based on direction
  goto SBnce; // then move onto something else
  
HandleStompedShellE:
  lda_imm_nf(0x4); // set defeated state for enemy
  ram[Enemy_State + x] = a;
  inc_abs_nf(StompChainCounter); // increment the stomp counter
  lda_abs_nf(StompChainCounter); // add whatever is in the stomp counter
  carry_flag = false; // to whatever is in the stomp timer
  adc_abs(StompTimer);
  SetupFloateyNumber(); // award points accordingly
  inc_abs_nf(StompTimer); // increment stomp timer of some sort
  ldy_abs_nf(PrimaryHardMode); // check primary hard mode flag
  lda_absy_nf(RevivalRateData); // load timer setting according to flag
...
void ChkEnemyFaceRight(void) {
  lda_zpx_nf(Enemy_MovingDir); // check to see if enemy is moving to the right
  cmp_imm(0x1);
  // if not, branch
  if (!zero_flag) {
    LInj();
...
void EnemyFacePlayer(void) {
  ...
  if (neg_flag) {
    iny_nf(); // otherwise, increment to set to move to the left
  }
  // SFcRt:
...
void ECLoop(void) {
  ...
  tya_nf(); // save first enemy's bounding box offset to stack
  pha();
  ...
  txa_nf(); // get second enemy object's bounding box offset
  asl_acc_nf(); // multiply by four, then add four
  asl_acc_nf();
  carry_flag = false;
  adc_imm_fc(0x4);
  tax(); // use as new contents of X
  SprObjectCollisionCore(); // do collision detection using the two enemies here
  ldx_zp_nf(ObjectOffset); // use first enemy offset for X
  ldy_zp_nf(0x1); // use second enemy offset for Y
  // if carry clear, no collision, branch ahead of this
  if (carry_flag) {
    lda_zpx_nf(Enemy_State);
    ora_zpy_nf(Enemy_State); // check both enemy states for d7 set
    and_imm(0b10000000);
    // branch if at least one of them is set
    if (zero_flag) {
      lda_absy_nf(Enemy_CollisionBits); // load first enemy's collision-related bits
      ...
      lda_absy_nf(Enemy_CollisionBits);
  ...
  // NoEnemyCollision:
  lda_absy_nf(Enemy_CollisionBits); // load first enemy's collision-related bits
  and_absx_nf(ClearBitsMask); // clear bit connected to second enemy
  ram[Enemy_CollisionBits + y] = a; // then move onto next enemy slot
  ReadyNextEnemy(); // <fallthrough>
}

void ReadyNextEnemy(void) {
  pla_nf(); // get first enemy's bounding box offset from the stack
  tay_nf(); // use as Y again
  ldx_zp_nf(0x1); // get and decrement second enemy's object buffer offset
  dex_fn();
...
void ProcEnemyCollisions(void) {
  lda_zpy_nf(Enemy_State); // check both enemy states for d5 set
  ora_zpx_nf(Enemy_State);
  ...
  if (carry_flag) {
    lda_zpx_nf(Enemy_ID); // check second enemy identifier for hammer bro
    cmp_imm(HammerBro); // if hammer bro found in alt state, branch to leave
    if (zero_flag) { goto ExitProcessEColl; }
    lda_zpy_nf(Enemy_State); // check first enemy state for d7 set
    asl_acc_fc();
    ...
    if (carry_flag) {
      ...
      ldy_zp_nf(0x1); // original offset of second enemy
    }
    // ShellCollisions:
    tya_nf(); // move Y to X
    tax();
    ...
    ldx_zp_nf(ObjectOffset);
    lda_absx_nf(ShellChainCounter); // get chain counter for shell
    carry_flag = false;
    ...
    ldx_zp_nf(ObjectOffset); // load original offset of first enemy
...
ExitProcessEColl:
  ...
  // ProcSecondEnemyColl:
  lda_zpy_nf(Enemy_State); // if first enemy state < $06, branch elsewhere
  cmp_imm_fc(0x6);
  if (carry_flag) {
    lda_zpy_nf(Enemy_ID); // check first enemy identifier for hammer bro
    cmp_imm(HammerBro); // if hammer bro found in alt state, branch to leave
    if (zero_flag) { goto ExitProcessEColl; }
    ShellOrBlockDefeat(); // otherwise, kill first enemy
    ldy_zp_nf(0x1);
    lda_absy_nf(ShellChainCounter); // get chain counter for shell
    carry_flag = false;
    adc_imm_fc(0x4); // add four to get appropriate point offset
    ldx_zp(ObjectOffset);
    SetupFloateyNumber(); // award appropriate number of points for first enemy
    ldx_zp_nf(0x1); // load original offset of second enemy
    inc_absx(ShellChainCounter); // increment chain counter for additional enemies
    return; // leave!!!
  }
  // MoveEOfs:
  tya_nf(); // move Y ($01) to X
  tax();
  EnemyTurnAround(); // do the sub here using value from $01
  ldx_zp_nf(ObjectOffset); // then do it again using value from $08
  EnemyTurnAround(); // <fallthrough>
}

void EnemyTurnAround(void) {
  lda_zpx_nf(Enemy_ID); // check for specific enemies
  cmp_imm(PiranhaPlant);
...
void RXSpd(void) {
  lda_zpx_nf(Enemy_X_Speed); // load horizontal speed
  eor_imm_nf(0xff); // get two's compliment for horizontal speed
  tay_nf();
  iny_nf();
...
void LargePlatformCollision(void) {
  lda_imm_nf(0xff); // save value here
...
void ChkForPlayerC_LargeP(void) {
  ...
  lda_zpx_nf(Enemy_Y_Position); // store vertical coordinate in
  ...
  pla_nf(); // retrieve offset from the stack
  tax();
...
ChkSmallPlatLoop:
  ...
  lda_absy_nf(BoundingBox_UL_YPos); // check top of platform's bounding box for being
...
MoveBoundBox:
  lda_absy_nf(BoundingBox_UL_YPos); // move bounding box vertical coordinates
  carry_flag = false; // 128 pixels downwards
  adc_imm_nf(0x80);
  ram[BoundingBox_UL_YPos + y] = a;
  lda_absy_nf(BoundingBox_DR_YPos);
  carry_flag = false;
  adc_imm_fc(0x80);
...
ProcSPlatCollisions:
  ldx_zp_nf(ObjectOffset); // return enemy object buffer offset to X, then continue
  ProcLPlatCollisions(); // <fallthrough>
}

void ProcLPlatCollisions(void) {
  lda_absy_nf(BoundingBox_DR_YPos); // get difference by subtracting the top
  carry_flag = true; // of the player's bounding box from the bottom
  sbc_abs_nf(BoundingBox_UL_YPos); // of the platform's bounding box
  ...
  lda_imm_nf(0x1); // otherwise, set vertical
  ram[Player_Y_Speed] = a; // speed of player to kill jump
  
ChkForTopCollision:
  lda_abs_nf(BoundingBox_DR_YPos); // get difference by subtracting the top
  carry_flag = true; // of the platform's bounding box from the bottom
  sbc_absy_nf(BoundingBox_UL_YPos); // of the player's bounding box
  cmp_imm_fc(0x6);
  ...
  lda_zp_nf(0x0); // get saved bounding box counter from earlier
  ldy_zpx_nf(Enemy_ID);
  ...
  txa_nf(); // otherwise use enemy object buffer offset
...
PlatformSideCollisions:
  lda_imm_nf(0x1); // set value here to indicate possible horizontal
  ram[0x0] = a; // collision on left side of platform
  lda_abs_nf(BoundingBox_DR_XPos); // get difference by subtracting platform's left edge
  carry_flag = true; // from player's right edge
  sbc_absy_nf(BoundingBox_UL_XPos);
  cmp_imm(0x8); // if difference close enough, skip all of this
  if (!carry_flag) { goto SideC; }
  inc_zp_nf(0x0); // otherwise increment value set here for right side collision
  lda_absy_nf(BoundingBox_DR_XPos); // get difference by subtracting player's left edge
  carry_flag = false; // from platform's right edge
  sbc_abs_nf(BoundingBox_UL_XPos);
...
void PositionPlayerOnS_Plat(void) {
  tay_nf(); // use bounding box counter saved in collision flag
  lda_zpx_nf(Enemy_Y_Position); // for offset
...
void CheckPlayerVertical(void) {
  lda_abs_nf(Player_OffscreenBits); // if player object is completely offscreen
  cmp_imm(0xf0); // vertically, leave this routine
  if (carry_flag) { return; }
  ldy_zp_nf(Player_Y_HighPos); // if player high vertical byte is not
  dey(); // within the screen, leave this routine
  if (!zero_flag) { return; }
  lda_zp_nf(Player_Y_Position); // if on the screen, check to see how far down
  cmp_imm(0xd0); // the player is vertically
  // -------------------------------------------------------------------------------------
}

void GetEnemyBoundBoxOfs(void) {
  lda_zp_nf(ObjectOffset); // get enemy object buffer offset
  GetEnemyBoundBoxOfsArg(); // <fallthrough>
}

void GetEnemyBoundBoxOfsArg(void) {
  asl_acc_nf(); // multiply A by four, then add four
  asl_acc_nf(); // to skip player's bounding box
  carry_flag = false;
  adc_imm_nf(0x4);
  tay_nf(); // send to Y
  lda_abs_nf(Enemy_OffscreenBits); // get offscreen bits for enemy object
  and_imm_nf(0b00001111); // save low nybble
...
void PlayerBGCollision(void) {
  ...
  lda_imm_nf(0x1); // load default player state for swimming
...
SetFallS:
  lda_imm_nf(0x2); // load default player state for falling
...
ChkCollSize:
  ldy_imm_nf(0x2); // load default offset
  lda_abs_fz(CrouchingFlag);
  if (!zero_flag) { goto GBBAdr; } // if player crouching, skip ahead
  lda_abs_fz(PlayerSize);
  if (!zero_flag) { goto GBBAdr; } // if player small, skip ahead
  dey_nf(); // otherwise decrement offset for big player not crouching
  lda_abs_fz(SwimmingFlag);
  if (!zero_flag) { goto GBBAdr; } // if swimming flag set, skip ahead
  dey_nf(); // otherwise decrement offset
  
GBBAdr:
  lda_absy_nf(BlockBufferAdderData); // get value using offset
  ram[0xeb] = a; // store value here
  tay_nf(); // put value into Y, as offset for block buffer routine
  ldx_abs_nf(PlayerSize); // get player's size as offset
  lda_abs_fz(CrouchingFlag);
  if (zero_flag) { goto HeadChk; } // if player not crouching, branch ahead
  inx_nf(); // otherwise increment size as offset
...
HeadChk:
  ...
  ldy_zp_nf(0x4); // check lower nybble of vertical coordinate returned
...
NYSpd:
  lda_imm_nf(0x1); // set player's vertical speed to nullify
  ram[Player_Y_Speed] = a; // jump or swim
  
DoFootCheck:
  ldy_zp_nf(0xeb); // get block buffer adder offset
  lda_zp_nf(Player_Y_Position);
...
DoPlayerSideCheck:
  ldy_zp_nf(0xeb); // get block buffer adder offset
  iny_nf();
  iny_nf(); // increment offset 2 bytes to use adders for side collisions
  lda_imm_nf(0x2); // set value here to be used as counter
  ram[0x0] = a;
  
SideCheckLoop:
  iny_nf(); // move onto the next one
  ram[0xeb] = y; // store it
  lda_zp_nf(Player_Y_Position);
...
BHalf:
  ldy_zp_nf(0xeb); // load block adder offset
  iny_nf(); // increment it
  lda_zp_nf(Player_Y_Position); // get player's vertical position
  cmp_imm(0x8);
...
ChkPBtm:
  ldy_zp_nf(Player_State); // get player's state
...
PlyrPipe:
  ...
  ldy_imm_nf(0x0); // set default offset for timer setting data
...
SetCATmr:
  lda_absy_nf(AreaChangeTimerData); // set timer for change of area as appropriate
  ram[ChangeAreaTimer] = a;
  
ChkGERtn:
  lda_zp_nf(GameEngineSubroutine); // get number of game engine routine running
  cmp_imm(0x7);
...
HandleCoinMetatile:
  ...
  inc_abs_nf(CoinTallyFor1Ups); // increment coin tally used for 1-up blocks
...
void ErACM(void) {
  ldy_zp_nf(0x2); // load vertical high nybble offset for block buffer
  lda_imm_nf(0x0); // load blank metatile
...
void ImpedePlayerMove(void) {
  lda_imm_nf(0x0); // initialize value here
  ldy_zp_nf(Player_X_Speed); // get player's horizontal speed
  ldx_zp_nf(0x0); // check value set earlier for
  dex_fz(); // left side collision
  if (!zero_flag) { goto RImpd; } // if right side collision, skip this part
  inx_nf(); // return value to X
  cpy_imm_fcn(0x0); // if player moving to the left,
  if (neg_flag) { goto ExIPM; } // branch to invert bit and leave
  lda_imm_nf(0xff); // otherwise load A with value to be used later
  goto NXSpd; // and jump to affect movement
  
RImpd:
  ldx_imm_nf(0x2); // return $02 to X
  cpy_imm_fcn(0x1); // if player moving to the right,
  if (!neg_flag) { goto ExIPM; } // branch to invert bit and leave
  lda_imm_nf(0x1); // otherwise load A with value to be used here
...
NXSpd:
  ...
  dey_nf(); // otherwise decrement Y now
...
void HandleClimbing(void) {
  ldy_zp_nf(0x4); // check low nybble of horizontal coordinate returned from
...
FlagpoleCollision:
  ...
  inc_abs_nf(ScrollLock); // set scroll lock flag
  lda_zp_nf(GameEngineSubroutine);
  ...
  ldx_imm_nf(0x4); // start at end of vertical coordinate data
  lda_zp_nf(Player_Y_Position);
...
PutPlayerOnVine:
  lda_imm_nf(0x3); // set player state to climbing
  ram[Player_State] = a;
  lda_imm_nf(0x0); // nullify player's horizontal speed
  ram[Player_X_Speed] = a; // and fractional horizontal movement force
  ram[Player_X_MoveForce] = a;
  lda_zp_nf(Player_X_Position); // get player's horizontal coordinate
  carry_flag = true;
  sbc_abs_nf(ScreenLeft_X_Pos); // subtract from left side horizontal coordinate
  cmp_imm_fc(0x10);
...
SetVXPl:
  ldy_zp_nf(PlayerFacingDir); // get current facing direction, use as offset
  lda_zp_nf(0x6); // get low byte of block buffer address
  asl_acc_nf();
  asl_acc_nf(); // move low nybble to high
  asl_acc_nf();
  asl_acc_nf();
  carry_flag = false;
  ...
  lda_abs_nf(ScreenRight_PageLoc); // load page location of right side of screen
  carry_flag = false;
...
void HandlePipeEntry(void) {
  lda_zp_nf(Up_Down_Buttons); // check saved controller bits from earlier
  ...
  lda_imm_nf(Sfx_PipeDown_Injury);
  ...
  and_imm_nf(0b00000011); // mask out all but 2 LSB
  asl_acc_nf();
  asl_acc_nf(); // multiply by four
  tax_nf(); // save as offset to warp zone numbers (starts at left pipe)
  lda_zp_nf(Player_X_Position); // get player's horizontal position
  cmp_imm_fc(0x60);
  if (!carry_flag) { goto GetWNum; } // if player at left, not near middle, use offset and skip ahead
  inx_nf(); // otherwise increment for middle pipe
  cmp_imm_fc(0xa0);
  if (!carry_flag) { goto GetWNum; } // if player at middle, but not too far right, use offset and skip
  inx_nf(); // otherwise increment for last pipe
  
GetWNum:
  ldy_absx_nf(WarpZoneNumbers); // get warp zone numbers
  dey_nf(); // decrement for use as world number
  ram[WorldNumber] = y; // store as world number and offset
  ldx_absy_nf(WorldAddrOffsets); // get offset to where this world's area offsets are
  lda_absx_nf(AreaAddrOffsets); // get area offset based on world offset
  ...
  inc_abs_nf(Hidden1UpFlag); // set flag for hidden 1-up blocks
  inc_abs(FetchNewGameTimerFlag); // set flag to load new game timer
}
...
void GetMTileAttrib(void) {
  tay_nf(); // save metatile value into Y
  and_imm_nf(0b11000000); // mask out all but 2 MSB
  asl_acc_fc();
  rol_acc_fc(); // shift and rotate d7-d6 to d1-d0
  rol_acc_fc();
  tax_nf(); // use as offset for metatile data
...
void EnemyToBGCollisionDet(void) {
  lda_zpx_nf(Enemy_State); // check enemy state for d6 set
  and_imm(0b00100000);
...
HandleEToBGCollision:
  ...
  ldy_zp_nf(0x2); // get vertical coordinate used to find block
  lda_imm_nf(0x0); // store default blank metatile in that spot so we won't
...
void ChkToStunEnemies(void) {
  ...
  // Demote:
  and_imm_nf(0b00000001); // erase all but LSB, essentially turning enemy object
  ram[Enemy_ID + x] = a; // into green or red koopa troopa to demote them
  SetStun(); // <fallthrough>
}

void SetStun(void) {
  lda_zpx_nf(Enemy_State); // load enemy state
  and_imm_nf(0b11110000); // save high nybble
  ora_imm_nf(0b00000010);
  ram[Enemy_State + x] = a; // set d1 of enemy state
  dec_zpx_nf(Enemy_Y_Position);
  dec_zpx_nf(Enemy_Y_Position); // subtract two pixels from enemy's vertical position
  lda_zpx_nf(Enemy_ID);
  cmp_imm_fcz(Bloober); // check for bloober object
  if (zero_flag) { goto SetWYSpd; }
  lda_imm_nf(0xfd); // set default vertical speed
  ldy_abs_fz(AreaType);
  if (!zero_flag) { goto SetNotW; } // if area type not water, set as speed, otherwise
  
SetWYSpd:
  lda_imm_nf(0xff); // change the vertical speed
...
NoCDirF:
  dey_nf(); // decrement and use as offset
...
void LandEnemyProperly(void) {
  lda_zp_nf(0x4); // check lower nybble of vertical coordinate saved earlier
  carry_flag = true;
  sbc_imm_nf(0x8); // subtract eight pixels
...
ChkLandedEnemyState:
  ...
  lda_zpx_nf(Enemy_State); // load enemy state again (why?)
  cmp_imm_fz(0x2); // if not in $02 state (used by koopas and buzzy beetles)
  if (!zero_flag) { goto ProcEnemyDirection; } // then branch elsewhere
  lda_imm_nf(0x10); // load default timer here
  ldy_zpx_nf(Enemy_ID); // check enemy identifier for spiny
  cpy_imm_fcz(Spiny);
  if (!zero_flag) { goto SetForStn; } // branch if not found
  lda_imm_nf(0x0); // set timer for $00 if spiny
...
ProcEnemyDirection:
  lda_zpx_nf(Enemy_ID); // check enemy identifier for goomba
...
InvtD:
  ...
  iny_nf(); // if to the left, increment by one for enemy to face right (inverted)
...
NMovShellFallBit:
  lda_zpx_nf(Enemy_State); // nullify d6 of enemy state, save other bits
...
void ChkForRedKoopa(void) {
  lda_zpx_nf(Enemy_ID); // check for red koopa troopa $03
  cmp_imm_fz(RedKoopa);
  // branch if not found
  if (zero_flag) {
    lda_zpx_fz(Enemy_State);
  ...
  // Chk2MSBSt:
  lda_zpx_nf(Enemy_State); // save enemy state into Y
  tay_nf();
...
void DoEnemySideCheck(void) {
  lda_zpx_nf(Enemy_Y_Position); // if enemy within status bar, branch to leave
  cmp_imm(0x20); // because there's nothing there that impedes movement
  if (carry_flag) {
    ldy_imm_nf(0x16); // start by finding block to the left of enemy ($00,$14)
    lda_imm_nf(0x2); // set value here in what is also used as
    ram[0xeb] = a; // OAM data offset
    
SdeCLoop:
    lda_zp_nf(0xeb); // check value
    ...
    // NextSdeC:
    dec_zp_nf(0xeb); // move to the next direction
    iny_nf();
...
void ChkForBump_HammerBroJ(void) {
  ...
  if (!zero_flag) {
    lda_zpx_nf(Enemy_State); // if enemy state d7 not set, branch
    asl_acc_fc(); // ahead and do not play sound
    if (carry_flag) {
      lda_imm_nf(Sfx_Bump); // otherwise, play bump sound
      ram[Square1SoundQueue] = a; // sound will never be played if branching from ChkForRedKoopa
    }
  }
  // NoBump:
  lda_zpx_nf(Enemy_ID); // check for hammer bro
  cmp_imm(0x5);
  // branch if not found
  if (zero_flag) {
    lda_imm_nf(0x0);
...
void PlayerEnemyDiff(void) {
  lda_zpx_nf(Enemy_X_Position); // get distance between enemy object's
...
void EnemyLanding(void) {
  ...
  lda_zpx_nf(Enemy_Y_Position);
  and_imm_nf(0b11110000); // save high nybble of vertical coordinate, and
...
void SubtEnemyYPos(void) {
  lda_zpx_nf(Enemy_Y_Position); // add 62 pixels to enemy object's
...
void NoUnderHammerBro(void) {
  lda_zpx_nf(Enemy_State); // if hammer bro is not standing on anything, set d0
  ora_imm(0x1); // in the enemy state to indicate jumping or falling, then leave
  ram[Enemy_State + x] = a;
}

void ChkUnderEnemy(void) {
  lda_imm_nf(0x0); // set flag in A for save vertical coordinate
  ldy_imm_nf(0x15); // set Y to check the bottom middle (8,18) of enemy object
...
void FireballBGCollision(void) {
  lda_zpx_nf(Fireball_Y_Position); // check fireball's vertical coordinate
  cmp_imm(0x18);
  ...
  lda_zpx_nf(Fireball_Y_Position);
...
void GetFireballBoundBox(void) {
  txa_nf(); // add seven bytes to offset
...
void GetMiscBoundBox(void) {
  txa_nf(); // add nine bytes to offset
...
void GetEnemyBoundBox(void) {
  ldy_imm_nf(0x48); // store bitmask here for now
  ram[0x0] = y;
...
void SmallPlatformBoundBox(void) {
  ldy_imm_nf(0x8); // store bitmask here for now
  ram[0x0] = y;
  ldy_imm_nf(0x4); // store another bitmask here for now
  GetMaskedOffScrBits(); // <fallthrough>
}

void GetMaskedOffScrBits(void) {
  lda_zpx_nf(Enemy_X_Position); // get enemy object position relative
  carry_flag = true; // to the left side of the screen
  sbc_abs_fc(ScreenLeft_X_Pos);
  ram[0x1] = a; // store here
  lda_zpx_nf(Enemy_PageLoc); // subtract borrow from current page location
  sbc_abs_fcn(ScreenLeft_PageLoc); // of left side
  // if enemy object is beyond left edge, branch
  if (!neg_flag) {
    ora_zp_fz(0x1);
    // if precisely at the left edge, branch
    if (!zero_flag) {
      ldy_zp_nf(0x0); // if to the right of left edge, use value in $00 for A
    }
  }
  // CMBits:
  tya_nf(); // otherwise use contents of Y
...
void MoveBoundBoxOffscreen(void) {
  txa_nf(); // multiply offset by 4
  asl_acc_nf();
  asl_acc_fc();
  tay_nf(); // use as offset here
  lda_imm(0xff);
...
void LargePlatformBoundBox(void) {
  ...
  dex_nf(); // decrement to return to original offset
...
void SetupEOffsetFBBox(void) {
  txa_nf(); // add 1 to offset to properly address
...
void CheckRightScreenBBox(void) {
  lda_abs_nf(ScreenLeft_X_Pos); // add 128 pixels to left side of screen
  carry_flag = false; // and store as horizontal coordinate of middle
  adc_imm_fc(0x80);
  ram[0x2] = a;
  lda_abs_nf(ScreenLeft_PageLoc); // add carry to page location of left side of screen
  adc_imm_nf(0x0); // and store as page location of middle
  ram[0x1] = a;
  lda_zpx_nf(SprObject_X_Position); // get horizontal coordinate
  cmp_zp_fc(0x2); // compare against middle horizontal coordinate
  lda_zpx_nf(SprObject_PageLoc); // get page location
  ...
  if (carry_flag) {
    ...
    if (!neg_flag) {
      lda_imm_nf(0xff); // load offscreen value here to use on one or both horizontal sides
...
void BoundingBoxCore(void) {
  ...
  lda_absy_nf(SprObject_Rel_YPos); // store object coordinates relative to screen
  ram[0x2] = a; // vertically and horizontally, respectively
  lda_absy_nf(SprObject_Rel_XPos);
  ram[0x1] = a;
  txa_nf(); // multiply offset by four and save to stack
  asl_acc_nf();
  asl_acc_nf();
  pha();
  tay_nf(); // use as offset for Y, X is left alone
  lda_absx_nf(SprObj_BoundBoxCtrl); // load value here to be used as offset for X
  asl_acc_nf(); // multiply that by four and use as X
  asl_acc_nf();
  tax_nf();
  lda_zp_nf(0x1); // add the first number in the bounding box data to the
  carry_flag = false; // relative horizontal coordinate using enemy object offset
  adc_absx_nf(BoundBoxCtrlData); // and store somewhere using same offset * 4
  ram[BoundingBox_UL_Corner + y] = a; // store here
  lda_zp_nf(0x1);
  carry_flag = false;
  adc_absx_nf(BoundBoxCtrlData + 2); // add the third number in the bounding box data to the
  ram[BoundingBox_LR_Corner + y] = a; // relative horizontal coordinate and store
  inx_nf(); // increment both offsets
  iny_nf();
  lda_zp_nf(0x2); // add the second number to the relative vertical coordinate
  carry_flag = false; // using incremented offset and store using the other
  adc_absx_nf(BoundBoxCtrlData); // incremented offset
  ...
  pla_nf(); // get original offset loaded into $00 * y from stack
  tay_nf(); // use as Y
  ldx_zp(0x0); // get original offset and use as X again
}

void PlayerCollisionCore(void) {
  ldx_imm_nf(0x0); // initialize X to use player's bounding box for comparison
...
CollisionCoreLoop:
  lda_absy_nf(BoundingBox_UL_Corner); // compare left/top coordinates
  ...
  lda_absy_nf(BoundingBox_LR_Corner); // if somehow greater, check to see if bottom of
...
SecondBoxVerticalChk:
  lda_absx_nf(BoundingBox_LR_Corner); // check to see if the vertical bottom of the box
  cmp_absx_fc(BoundingBox_UL_Corner); // is greater than the vertical top
  if (!carry_flag) { goto CollisionFound; } // if somehow less, vertical wrap collision, thus branch
  lda_absy_nf(BoundingBox_LR_Corner); // otherwise compare horizontal right or vertical bottom
...
FirstBoxGreater:
  ...
  lda_absy_nf(BoundingBox_LR_Corner); // otherwise compare bottom of first to top of second
...
CollisionFound:
  inx_nf(); // increment offsets on both objects to check
  iny_nf(); // the vertical coordinates
...
void BlockBufferChk_FBall(void) {
  ldy_imm_nf(0x1a); // set offset for block buffer adder data
  txa_nf();
  carry_flag = false;
...
void BBChk_E(void) {
  ...
  ldx_zp_nf(ObjectOffset); // get object offset
  cmp_imm(0x0); // check to see if object bumped into anything
}

void BlockBufferColli_Feet(void) {
  iny_nf(); // if branched here, increment to next set of adders
...
void BlockBufferColli_Side(void) {
  lda_imm_nf(0x1); // set flag to return horizontal coordinate
  BlockBufferColli_SideSkip(); // <fallthrough>
}

void BlockBufferColli_SideSkip(void) {
  ldx_imm_nf(0x0); // set offset for player object
...
void BlockBufferCollision(void) {
  ...
  lda_absy_nf(BlockBuffer_X_Adder); // add horizontal coordinate
  carry_flag = false; // of object to value obtained using Y as offset
  adc_zpx_fc(SprObject_X_Position);
  ram[0x5] = a; // store here
  lda_zpx_nf(SprObject_PageLoc);
  adc_imm_nf(0x0); // add carry to page location
  and_imm_nf(0x1); // get LSB, mask out all other bits
  lsr_acc_fc(); // move to carry
  ora_zp_nf(0x5); // get stored value
  ror_acc_nf(); // rotate carry to MSB of A
  lsr_acc_nf(); // and effectively move high nybble to
  lsr_acc_nf(); // lower, LSB which became MSB will be
  lsr_acc(); // d4 at this point
  GetBlockBufferAddr(); // get address of block buffer into $06, $07
  ldy_zp_nf(0x4); // get old contents of Y
  lda_zpx_nf(SprObject_Y_Position); // get vertical coordinate of object
  carry_flag = false;
  adc_absy_nf(BlockBuffer_Y_Adder); // add it to value obtained using Y as offset
  and_imm_nf(0b11110000); // mask out low nybble
  carry_flag = true;
  sbc_imm_fc(0x20); // subtract 32 pixels for the status bar
  ram[0x2] = a; // store result here
  tay_nf(); // use as offset for block buffer
  lda_indy_nf(0x6); // check current content of block buffer
  ram[0x3] = a; // and store here
  ldy_zp_nf(0x4); // get old contents of Y again
  pla_fz(); // pull A from stack
  // if A = 1, branch
  if (zero_flag) {
    lda_zpx_nf(SprObject_Y_Position); // if A = 0, load vertical coordinate
    goto RetYC; // and jump
  }
  // RetXC:
  lda_zpx_nf(SprObject_X_Position); // otherwise load horizontal coordinate
  
RetYC:
  and_imm_nf(0b00001111); // and mask out high nybble
...
void DrawVine(void) {
  ...
  ldx_absy_nf(VineObjOffset); // get offset to vine
  ...
  carry_flag = false;
  adc_imm_nf(0x6); // add six pixels to second, fourth and sixth sprites
  ...
  lda_imm_nf(0b00100001); // set bg priority and palette attribute bits
  ram[Sprite_Attributes + y] = a; // set in first, third and fifth sprites
  ram[Sprite_Attributes + 8 + y] = a;
  ram[Sprite_Attributes + 16 + y] = a;
  ora_imm_nf(0b01000000); // additionally, set horizontal flip bit
  ram[Sprite_Attributes + 4 + y] = a; // for second, fourth and sixth sprites
  ram[Sprite_Attributes + 12 + y] = a;
  ram[Sprite_Attributes + 20 + y] = a;
  ldx_imm_nf(0x5); // set tiles for six sprites
  
VineTL:
  lda_imm_nf(0xe1); // set tile number for sprite
  ram[Sprite_Tilenumber + y] = a;
  iny_nf(); // move offset to next sprite data
  iny_nf();
  iny_nf();
  iny_nf();
  dex_fn(); // move onto next sprite
  if (!neg_flag) { goto VineTL; } // loop until all sprites are done
  ldy_zp_nf(0x2); // get original offset
  ...
  // SkpVTop:
  ldx_imm_nf(0x0); // start with the first sprite again
  
ChkFTop:
  lda_abs_nf(VineStart_Y_Position); // get original starting vertical coordinate
  carry_flag = true;
  sbc_absy_nf(Sprite_Y_Position); // subtract top-most sprite's Y coordinate
  ...
  // NextVSp:
  iny_nf(); // move offset to next OAM data
  iny_nf();
  iny_nf();
  iny_nf();
  inx_nf(); // move onto next sprite
...
StkLp:
  ...
  adc_imm_fc(0x8); // add eight pixels
  iny_nf();
  iny_nf(); // move offset four bytes forward
  iny_nf();
  iny_nf();
...
void DrawHammer(void) {
  ldy_absx_nf(Misc_SprDataOffset); // get misc object OAM data offset
  lda_abs_fz(TimerControl);
...
ForceHPose:
  ldx_imm_fz(0x0); // reset offset here
  if (zero_flag) { goto RenderH; } // do unconditional branch to rendering part
  
GetHPose:
  lda_zp_nf(FrameCounter); // get frame counter
  lsr_acc_nf(); // move d3-d2 to d1-d0
  lsr_acc_nf();
  and_imm_nf(0b00000011); // mask out all but d1-d0 (changes every four frames)
  tax_nf(); // use as timing offset
  
RenderH:
  lda_abs_nf(Misc_Rel_YPos); // get relative vertical coordinate
  carry_flag = false;
  adc_absx_nf(FirstSprYPos); // add first sprite vertical adder based on offset
  ram[Sprite_Y_Position + y] = a; // store as sprite Y coordinate for first sprite
  carry_flag = false;
  adc_absx_nf(SecondSprYPos); // add second sprite vertical adder based on offset
  ram[Sprite_Y_Position + 4 + y] = a; // store as sprite Y coordinate for second sprite
  lda_abs_nf(Misc_Rel_XPos); // get relative horizontal coordinate
  carry_flag = false;
  adc_absx_nf(FirstSprXPos); // add first sprite horizontal adder based on offset
...
void FlagpoleGfxHandler(void) {
  ldy_absx_nf(Enemy_SprDataOffset); // get sprite data offset for flagpole flag
  lda_abs_nf(Enemy_Rel_XPos); // get relative horizontal coordinate
  ram[Sprite_X_Position + y] = a; // store as X coordinate for first sprite
  carry_flag = false;
  adc_imm_nf(0x8); // add eight pixels and store
  ...
  lda_abs_nf(FlagpoleFNum_Y_Pos); // get vertical coordinate for floatey number
  ...
  if (!zero_flag) {
    ...
    adc_imm_nf(0xc);
    tay_nf(); // put back in Y
    lda_abs_nf(FlagpoleScore); // get offset used to award points for touching flagpole
    asl_acc_fc(); // multiply by 2 to get proper offset here
    tax_nf();
    lda_absx_nf(FlagpoleScoreNumTiles); // get appropriate tile data
    ram[0x0] = a;
    lda_absx(FlagpoleScoreNumTiles + 1);
    DrawOneSpriteRow(); // use it to render floatey number
  }
  // ChkFlagOffscreen:
  ldx_zp_nf(ObjectOffset); // get object offset for flag
  ldy_absx_nf(Enemy_SprDataOffset); // get OAM data offset
  lda_abs_nf(Enemy_OffscreenBits); // get offscreen bits
...
void DrawLargePlatform(void) {
  ...
  iny_nf(); // add 3 to it for offset
  iny_nf(); // to X coordinate
  iny_nf();
...
ShrinkPlatform:
  lda_imm_nf(0xf8); // load offscreen coordinate if flag set or castle-type level
...
SetPlatformTilenum:
  ...
  lda_imm_nf(0xf8); // if d7 was set, move first sprite offscreen
...
SChk2:
  ...
  lda_imm_nf(0xf8); // if d6 was set, move second sprite offscreen
  ram[Sprite_Y_Position + 4 + y] = a;
...
SChk3:
  ...
  lda_imm_nf(0xf8); // if d5 was set, move third sprite offscreen
  ram[Sprite_Y_Position + 8 + y] = a;
...
SChk4:
  ...
  lda_imm_nf(0xf8); // if d4 was set, move fourth sprite offscreen
...
SChk5:
  ...
  lda_imm_nf(0xf8); // if d3 was set, move fifth sprite offscreen
...
SLChk:
  lda_abs_nf(Enemy_OffscreenBits); // check d7 of offscreen bits
...
void DrawFloateyNumber_Coin(void) {
  ...
  if (!carry_flag) {
    dec_zpx_nf(Misc_Y_Position); // otherwise, decrement vertical coordinate
  }
  // NotRsNum:
...
void JCoinGfxHandler(void) {
  ldy_absx_nf(Misc_SprDataOffset); // get coin/floatey number's OAM data offset
  lda_zpx_nf(Misc_State); // get state of misc object
  ...
  lda_zpx_nf(Misc_Y_Position); // store vertical coordinate as
  ram[Sprite_Y_Position + y] = a; // Y coordinate for first sprite
  carry_flag = false;
  adc_imm_nf(0x8); // add eight pixels
  ...
  and_imm_nf(0b00000011); // mask out d2-d1
  tax_nf(); // use as graphical offset
  lda_absx_nf(JumpingCoinTiles); // load tile number
  iny(); // increment OAM data offset to write tile numbers
  DumpTwoSpr(); // do sub to dump tile number into both sprites
  dey_nf(); // decrement to get old offset
  lda_imm_nf(0x2);
...
void DrawPowerUp(void) {
  ldy_abs_nf(Enemy_SprDataOffset + 5); // get power-up's sprite data offset
  lda_abs_nf(Enemy_Rel_YPos); // get relative vertical coordinate
  carry_flag = false;
  adc_imm_nf(0x8); // add eight pixels
  ram[0x2] = a; // store result here
  lda_abs_nf(Enemy_Rel_XPos); // get relative horizontal coordinate
  ram[0x5] = a; // store here
  ldx_zp_nf(PowerUpType); // get power-up type
  lda_absx_nf(PowerUpAttributes); // get attribute data for power-up type
  ora_abs_nf(Enemy_SprAttrib + 5); // add background priority bit if set
...
PUpDrawLoop:
  lda_absx_nf(PowerUpGfxTable); // load left tile of power-up object
  ram[0x0] = a;
  ...
  ldy_abs_nf(Enemy_SprDataOffset + 5); // get sprite data offset again
  ...
  if (!zero_flag) {
    ...
    if (!zero_flag) {
      ...
      lda_zp_nf(FrameCounter); // get frame counter
      lsr_acc_fc(); // divide by 2 to change colors every two frames
      and_imm_nf(0b00000011); // mask out all but d1 and d0 (previously d2 and d1)
      ora_abs_nf(Enemy_SprAttrib + 5); // add background priority bit if any set
      ...
      // FlipPUpRightSide:
      lda_absy_nf(Sprite_Attributes + 4);
      ora_imm_nf(0b01000000); // set horizontal flip bit for top right sprite
      ram[Sprite_Attributes + 4 + y] = a;
      lda_absy_nf(Sprite_Attributes + 12);
...
void DrawEnemyObjRow(void) {
  lda_absx_nf(EnemyGraphicsTable); // load two tiles of enemy graphics
  ram[0x0] = a;
...
void DrawBlock(void) {
  lda_abs_nf(Block_Rel_YPos); // get relative vertical coordinate of block object
  ram[0x2] = a; // store here
  lda_abs_nf(Block_Rel_XPos); // get relative horizontal coordinate of block object
  ram[0x5] = a; // store here
  lda_imm_nf(0x3);
  ram[0x4] = a; // set attribute byte here
  lsr_acc_fc();
  ram[0x3] = a; // set horizontal flip bit here (will not be used)
  ldy_absx_nf(Block_SprDataOffset); // get sprite data offset
  ldx_imm_nf(0x0); // reset X for use as offset to tile data
  
DBlkLoop:
  lda_absx_nf(DefaultBlockObjTiles); // get left tile number
  ram[0x0] = a; // set here
  ...
  ldx_zp_nf(ObjectOffset); // get block object offset
  ldy_absx_nf(Block_SprDataOffset); // get sprite data offset
  lda_abs_nf(AreaType);
  ...
  if (zero_flag) {
    ...
    // SetBFlip:
    ldx_zp_nf(ObjectOffset); // put block object offset back in X
...
void DrawBrickChunks(void) {
  lda_imm_nf(0x2); // set palette bits here
  ram[0x0] = a;
  lda_imm_nf(0x75); // set tile number for ball (something residual, likely)
  ldy_zp_nf(GameEngineSubroutine);
  cpy_imm_fcz(0x5); // if end-of-level routine running,
  if (zero_flag) { goto DChunks; } // use palette and tile number assigned
  lda_imm_nf(0x3); // otherwise set different palette bits
  ram[0x0] = a;
  lda_imm_nf(0x84); // and set tile number for brick chunks
  
DChunks:
  ldy_absx_nf(Block_SprDataOffset); // get OAM data offset
  ...
  asl_acc_nf(); // move low nybble to high
  asl_acc_fc();
  and_imm_nf(0xc0); // get what was originally d3-d2 of low nybble
  ora_zp_nf(0x0); // add palette bits
  iny(); // increment offset for attribute bytes
  DumpFourSpr(); // do sub to dump attribute data into all four sprites
  dey_nf();
  dey_nf(); // decrement offset to Y coordinate
  ...
  lda_abs_nf(Block_Rel_XPos); // get first block object's relative horizontal coordinate
  ram[Sprite_X_Position + y] = a; // save into X coordinate of first sprite
  lda_absx_nf(Block_Orig_XPos); // get original horizontal coordinate
  carry_flag = true;
  sbc_abs_nf(ScreenLeft_X_Pos); // subtract coordinate of left side from original coordinate
  ...
  adc_imm_nf(0x6); // plus 6 pixels to position second brick chunk correctly
  ram[Sprite_X_Position + 4 + y] = a; // save into X coordinate of second sprite
  lda_abs_nf(Block_Rel_YPos + 1); // get second block object's relative vertical coordinate
  ram[Sprite_Y_Position + 8 + y] = a;
  ram[Sprite_Y_Position + 12 + y] = a; // dump into Y coordinates of third and fourth sprites
  lda_abs_nf(Block_Rel_XPos + 1); // get second block object's relative horizontal coordinate
  ram[Sprite_X_Position + 8 + y] = a; // save into X coordinate of third sprite
  lda_zp_nf(0x0); // use original relative horizontal position
  carry_flag = true;
  ...
  lda_abs_nf(Block_OffscreenBits); // get offscreen bits again
...
ChnkOfs:
  ...
  lda_absy_nf(Sprite_X_Position); // otherwise compare left-side X coordinate
...
void DrawFireball(void) {
  ldy_absx_nf(FBall_SprDataOffset); // get fireball's sprite data offset
  lda_abs_nf(Fireball_Rel_YPos); // get relative vertical coordinate
  ram[Sprite_Y_Position + y] = a; // store as sprite Y coordinate
  lda_abs_nf(Fireball_Rel_XPos); // get relative horizontal coordinate
...
void DrawFirebar(void) {
  lda_zp_nf(FrameCounter); // get frame counter
  lsr_acc_nf(); // divide by four
  lsr_acc_nf();
  pha(); // save result to stack
  and_imm_nf(0x1); // mask out all but last bit
  eor_imm_nf(0x64); // set either tile $64 or $65 as fireball tile
  ram[Sprite_Tilenumber + y] = a; // thus tile changes every four frames
  pla_nf(); // get from stack
  lsr_acc_nf(); // divide by four again
  lsr_acc_fc();
...
void DrawExplosion_Fireball(void) {
  ldy_absx_nf(Alt_SprDataOffset); // get OAM data offset of alternate sort for fireball's explosion
  lda_zpx_nf(Fireball_State); // load fireball state
  inc_zpx_nf(Fireball_State); // increment state for next frame
  lsr_acc_nf(); // divide by 2
  and_imm_nf(0b00000111); // mask out all but d3-d1
...
void DrawExplosion_Fireworks(void) {
  tax_nf(); // use whatever's in A for offset
  lda_absx_nf(ExplosionTiles); // get tile number using offset
  iny(); // increment Y (contains sprite data offset)
  DumpFourSpr(); // and dump into tile number part of sprite data
  dey_nf(); // decrement Y so we have the proper offset again
  ldx_zp_nf(ObjectOffset); // return enemy object buffer offset to X
  lda_abs_nf(Fireball_Rel_YPos); // get relative vertical coordinate
  carry_flag = true; // subtract four pixels vertically
  sbc_imm_nf(0x4); // for first and third sprites
  ram[Sprite_Y_Position + y] = a;
  ram[Sprite_Y_Position + 8 + y] = a;
  carry_flag = false; // add eight pixels vertically
  adc_imm_nf(0x8); // for second and fourth sprites
  ram[Sprite_Y_Position + 4 + y] = a;
  ram[Sprite_Y_Position + 12 + y] = a;
  lda_abs_nf(Fireball_Rel_XPos); // get relative horizontal coordinate
  carry_flag = true; // subtract four pixels horizontally
  sbc_imm_nf(0x4); // for first and second sprites
  ram[Sprite_X_Position + y] = a;
  ...
  lda_imm_nf(0x2); // set palette attributes for all sprites, but
...
void DrawSmallPlatform(void) {
  ldy_absx_nf(Enemy_SprDataOffset); // get OAM data offset
  lda_imm_nf(0x5b); // load tile number for small platforms
  iny(); // increment offset for tile numbers
  DumpSixSpr(); // dump tile number into all six sprites
  iny_nf(); // increment offset for attributes
  lda_imm(0x2); // load palette controls
  DumpSixSpr(); // dump attributes into all six sprites
  dey_nf(); // decrement for original offset
  dey_nf();
  lda_abs_nf(Enemy_Rel_XPos); // get relative horizontal coordinate
  ram[Sprite_X_Position + y] = a;
  ...
  ram[Sprite_X_Position + 16 + y] = a;
  carry_flag = false;
  adc_imm_nf(0x8); // add eight more pixels
  ...
  pla_nf(); // pull from stack
  carry_flag = false;
  adc_imm_nf(0x80); // add 128 pixels
  tax_nf();
  cpx_imm_fc(0x20); // if below status bar (taking wrap into account)
  // then do not change altered coordinate
  if (!carry_flag) {
    lda_imm_nf(0xf8); // otherwise move last three sprites offscreen
  }
  // BotSP:
  ...
  if (!zero_flag) {
    lda_imm_nf(0xf8); // if d3 was set, move first and
    ram[Sprite_Y_Position + y] = a; // fourth sprites offscreen
    ram[Sprite_Y_Position + 12 + y] = a;
  }
  // SOfs:
  pla_nf(); // move out and back into stack
  pha();
  and_imm_fz(0b00000100); // check d2
  if (!zero_flag) {
    lda_imm_nf(0xf8); // if d2 was set, move second and
  ...
  if (!zero_flag) {
    lda_imm_nf(0xf8); // if d1 was set, move third and
...
void DrawBubble(void) {
  ldy_zp_nf(Player_Y_HighPos); // if player's vertical high position
  dey(); // not within screen, skip all of this
  if (!zero_flag) { return; }
  lda_abs_nf(Bubble_OffscreenBits); // check air bubble's offscreen bits
  and_imm(0b00001000);
  if (!zero_flag) { return; } // if bit set, branch to leave
  ldy_absx_nf(Bubble_SprDataOffset); // get air bubble's OAM data offset
  lda_abs_nf(Bubble_Rel_XPos); // get relative horizontal coordinate
  ram[Sprite_X_Position + y] = a; // store as X coordinate here
  lda_abs_nf(Bubble_Rel_YPos); // get relative vertical coordinate
  ram[Sprite_Y_Position + y] = a; // store as Y coordinate here
  lda_imm_nf(0x74);
...
void DrawPlayer_Intermediate(void) {
  ldx_imm_nf(0x5); // store data into zero page memory
  
PIntLoop:
  lda_absx_nf(IntermediatePlayerData); // load data to display player as he always
  ram[0x2 + x] = a; // appears on world/lives display
  dex_fn();
  if (!neg_flag) { goto PIntLoop; } // do this until all data is loaded
  ldx_imm_nf(0xb8); // load offset for small standing
  ldy_imm(0x4); // load sprite data offset
  DrawPlayerLoop(); // draw player accordingly
  lda_abs_nf(Sprite_Attributes + 36); // get empty sprite attributes
  ora_imm(0b01000000); // set horizontal flip bit for bottom-right sprite
...
void RenderPlayerSub(void) {
  ...
  ldx_abs_nf(PlayerGfxOffset); // load graphics table offset
  ldy_abs_nf(Player_SprDataOffset); // get player's sprite data offset
  DrawPlayerLoop(); // <fallthrough>
}

void DrawPlayerLoop(void) {
  lda_absx_nf(PlayerGraphicsTable); // load player's left side
  ram[0x0] = a;
...
void ProcessPlayerAction(void) {
  lda_zp_nf(Player_State); // get player's state
  cmp_imm_fcz(0x3);
  ...
  ldy_imm_nf(0x6); // load offset for crouching
...
ProcOnGroundActs:
  ...
  lda_abs(CrouchingFlag); // get crouching flag
  if (!zero_flag) { goto NonAnimatedActs; } // if set, branch to get offset for graphics table
  ldy_imm_nf(0x2); // load offset for standing
  lda_zp_nf(Player_X_Speed); // check player's horizontal speed
  ...
  lda_abs_nf(Player_XSpeedAbsolute); // load walking/running speed
  cmp_imm_fc(0x9);
  if (!carry_flag) { goto ActionWalkRun; } // if less than a certain amount, branch, too slow to skid
  lda_zp_nf(Player_MovingDir); // otherwise check to see if moving direction
...
ActionClimbing:
  ldy_imm_nf(0x5); // load offset for climbing
  lda_zp(Player_Y_Speed); // check player's vertical speed
...
ActionSwimming:
  ...
  GetGfxOffsetAdder();
  lda_abs_nf(JumpSwimTimer); // check jump/swim timer
...
void AnimationControl(void) {
  ...
  // branch if not expired
  if (zero_flag) {
    lda_abs_nf(PlayerAnimTimerSet); // get animation frame timer amount
    ram[PlayerAnimTimer] = a; // and set timer accordingly
    lda_abs_nf(PlayerAnimCtrl);
    ...
    if (carry_flag) {
      lda_imm_nf(0x0); // otherwise initialize frame control
    }
    // SetAnimC:
...
void HandleChangeSize(void) {
  ...
  lda_absy_nf(ChangeSizeOffsetAdder); // get offset adder based on frame control as offset
  ldy_imm_nf(0xf); // load offset for player growing
  GetOffsetFromAnimCtrl(); // <fallthrough>
}

void GetOffsetFromAnimCtrl(void) {
  asl_acc_nf(); // multiply animation frame control
  asl_acc_nf(); // by eight to get proper amount
  asl_acc_fc(); // to add to our offset
  adc_absy(PlayerGfxTblOffsets); // add to offset to graphics table
}

void ShrinkPlayer(void) {
  tya_nf(); // add ten bytes to frame control as offset
  carry_flag = false;
  adc_imm_fc(0xa); // this thing apparently uses two of the swimming frames
  tax_nf(); // to draw the player shrinking
  ldy_imm_nf(0x9); // load offset for small player swimming
  lda_absx_fz(ChangeSizeOffsetAdder); // get what would normally be offset adder
  // and branch to use offset if nonzero
  if (zero_flag) {
    ldy_imm_nf(0x1); // otherwise load offset for big player swimming
  }
  // ShrPlF:
...
void ChkForPlayerAttrib(void) {
  ldy_abs_nf(Player_SprDataOffset); // get sprite data offset
  lda_zp_nf(GameEngineSubroutine);
  ...
  lda_abs_nf(PlayerGfxOffset); // get graphics table offset
  cmp_imm_fcz(0x50);
...
KilledAtt:
  lda_absy_nf(Sprite_Attributes + 16);
  and_imm_nf(0b00111111); // mask out horizontal and vertical flip bits
  ram[Sprite_Attributes + 16 + y] = a; // for third row sprites and save
  lda_absy_nf(Sprite_Attributes + 20);
  and_imm_nf(0b00111111);
  ora_imm_nf(0b01000000); // set horizontal flip bit for second
  ram[Sprite_Attributes + 20 + y] = a; // sprite in the third row
  
C_S_IGAtt:
  lda_absy_nf(Sprite_Attributes + 24);
  and_imm_nf(0b00111111); // mask out horizontal and vertical flip bits
  ram[Sprite_Attributes + 24 + y] = a; // for fourth row sprites and save
  lda_absy_nf(Sprite_Attributes + 28);
  and_imm_nf(0b00111111);
...
void RelativePlayerPosition(void) {
  ldx_imm_nf(0x0); // set offsets for relative cooordinates
...
void RelativeEnemyPosition(void) {
  lda_imm_nf(0x1); // get coordinates of enemy object
  ldy_imm(0x1); // relative to the screen
  VariableObjOfsRelPos();
}

void RelativeBlockPosition(void) {
  lda_imm_nf(0x9); // get coordinates of one block object
  ldy_imm(0x4); // relative to the screen
  VariableObjOfsRelPos();
  inx_nf(); // adjust offset for other block object if any
  inx_nf();
  lda_imm_nf(0x9);
  iny_nf(); // adjust other and get coordinates for other one
...
void GetObjRelativePosition(void) {
  lda_zpx_nf(SprObject_Y_Position); // load vertical coordinate low
  ram[SprObject_Rel_YPos + y] = a; // store here
  lda_zpx_nf(SprObject_X_Position); // load horizontal coordinate
...
void GetPlayerOffscreenBits(void) {
  ldx_imm_nf(0x0); // set offsets for player-specific variables
...
void GetEnemyOffscreenBits(void) {
  lda_imm_nf(0x1); // set A to add 1 byte in order to get enemy offset
  ldy_imm(0x1); // set Y to put offscreen bits in Enemy_OffscreenBits
  SetOffscrBitsOffset();
}

void GetBlockOffscreenBits(void) {
  lda_imm_nf(0x9); // set A to add 9 bytes in order to get block obj offset
  ldy_imm_nf(0x4); // set Y to put offscreen bits in Block_OffscreenBits
...
void GetOffScreenBitsSet(void) {
  ...
  pha();
  RunOffscrBitsSubs();
  asl_acc_nf(); // move low nybble to high nybble
  asl_acc_nf();
  asl_acc_nf();
  asl_acc_fc();
  ora_zp_nf(0x0); // mask together with previously saved low nybble
  ram[0x0] = a; // store both here
  pla_nf(); // get offscreen bits offset from stack
  tay_nf();
  lda_zp_nf(0x0); // get value here and store elsewhere
...
void GetXOffscreenBits(void) {
  ...
  ldy_imm_nf(0x1); // start with right side of screen
  
XOfsLoop:
  lda_absy_nf(ScreenEdge_X_Pos); // get pixel coordinate of edge
  ...
  lda_absy_nf(ScreenEdge_PageLoc); // get page location of edge
  sbc_zpx_nf(SprObject_PageLoc); // subtract from page location of object position
  ldx_absy_nf(DefaultXOnscreenOfs); // load offset value here
  cmp_imm_fn(0x0);
  // if beyond right edge or in front of left edge, branch
  if (!neg_flag) {
    ldx_absy_nf(DefaultXOnscreenOfs + 1); // if not, load alternate offset value here
    cmp_imm_fcn(0x1);
    // if one page or more to the left of either edge, branch
    if (neg_flag) {
      lda_imm_nf(0x38); // if no branching, load value here and store
      ram[0x6] = a;
  ...
  // XLdBData:
  lda_absx_nf(XOffscreenBitsData); // get bits here
  ldx_zp_nf(0x4); // reobtain position in buffer
...
void GetYOffscreenBits(void) {
  ram[0x4] = x; // save position in buffer to here
  ldy_imm_nf(0x1); // start with top of screen
  
YOfsLoop:
  lda_absy_nf(HighPosUnitData); // load coordinate for edge of vertical unit
  carry_flag = true;
  sbc_zpx_fc(SprObject_Y_Position); // subtract from vertical coordinate of object
  ram[0x7] = a; // store here
  lda_imm_nf(0x1); // subtract one from vertical high byte of object
  sbc_zpx_nf(SprObject_Y_HighPos);
  ldx_absy_nf(DefaultYOnscreenOfs); // load offset value here
  cmp_imm_fn(0x0);
  // if under top of the screen or beyond bottom, branch
  if (!neg_flag) {
    ldx_absy_nf(DefaultYOnscreenOfs + 1); // if not, load alternate offset value here
    cmp_imm_fcn(0x1);
    // if one vertical unit or more above the screen, branch
    if (neg_flag) {
      lda_imm_nf(0x20); // if no branching, load value here and store
      ram[0x6] = a;
  ...
  // YLdBData:
  lda_absx_nf(YOffscreenBitsData); // get offscreen data bits using offset
  ldx_zp_nf(0x4); // reobtain position in buffer
  cmp_imm(0x0);
  if (zero_flag) {
...
void DividePDiff(void) {
  ...
  lda_zp_nf(0x7); // get pixel difference
  cmp_zp(0x6); // compare to preset value
  if (!carry_flag) {
    lsr_acc_nf(); // divide by eight
    lsr_acc_nf();
    lsr_acc_nf();
    and_imm_nf(0x7); // mask out all but 3 LSB
...
void DrawSpriteObject(void) {
  lda_zp_nf(0x3); // get saved flip control bits
  lsr_acc_nf();
  ...
  lda_zp_nf(0x1); // and second into first sprite
  ram[Sprite_Tilenumber + y] = a;
  lda_imm_fz(0x40); // activate horizontal flip OAM attribute
  if (!zero_flag) { goto SetHFAt; } // and unconditionally branch
  
NoHFlip:
  ram[Sprite_Tilenumber + y] = a; // store first tile into first sprite
  lda_zp_nf(0x1); // and second into second sprite
  ram[Sprite_Tilenumber + 4 + y] = a;
  lda_imm_nf(0x0); // clear bit for horizontal flip
  
SetHFAt:
  ora_zp_nf(0x4); // add other OAM attributes if necessary
  ram[Sprite_Attributes + y] = a; // store sprite attributes
  ram[Sprite_Attributes + 4 + y] = a;
  lda_zp_nf(0x2); // now the y coordinates
  ...
  adc_imm_nf(0x8); // put them side by side
  ram[Sprite_X_Position + 4 + y] = a;
  lda_zp_nf(0x2); // add eight pixels to the next y
  carry_flag = false; // coordinate
  adc_imm_nf(0x8);
  ram[0x2] = a;
  tya_nf(); // add eight to the offset in Y to
  carry_flag = false; // move to the next two sprites
  adc_imm_fc(0x8);
  tay_nf();
  inx_nf(); // increment offset to return it to the
...
SndOn:
  ...
  lda_zp_nf(PauseSoundQueue); // if not, check pause sfx queue
  cmp_imm(0x1);
...
InPause:
  ...
  lda_imm_nf(0x0); // disable sound and clear sfx buffers
  ...
  lda_imm_nf(0x2a); // store length of sound in pause counter
  ram[Squ1_SfxLenCounter] = a;
  
PTone1F:
  lda_imm_fz(0x44); // play first tone
  if (!zero_flag) { goto PTRegC; } // unconditional branch
  
ContPau:
  lda_abs_nf(Squ1_SfxLenCounter); // check pause length left
...
PTone2F:
  lda_imm_nf(0x64); // store reg contents and play the pause sfx
...
DecPauC:
  ...
  if (!zero_flag) { goto SkipSoundSubroutines; }
  lda_imm_nf(0x0); // disable sound if in pause mode and
  apu_write(SND_MASTERCTRL_REG, a); // not currently playing the pause sfx
  lda_abs_nf(PauseSoundBuffer); // if no longer playing pause sfx, check to see
  cmp_imm_fcz(0x2); // if we need to be playing sound again
  if (!zero_flag) { goto SkipPIn; }
  lda_imm_nf(0x0); // clear pause mode to allow game sounds again
  ram[PauseModeFlag] = a;
...
RunSoundSubroutines:
  ...
  lda_imm_nf(0x0); // clear the music queues
  ram[AreaMusicQueue] = a;
  ram[EventMusicQueue] = a;
  
SkipSoundSubroutines:
  lda_imm_nf(0x0); // clear the sound effects queues
  ram[Square1SoundQueue] = a;
  ram[Square2SoundQueue] = a;
  ram[NoiseSoundQueue] = a;
  ram[PauseSoundQueue] = a;
  ldy_abs_nf(DAC_Counter); // load some sort of counter
  lda_zp_nf(AreaMusicBuffer);
  and_imm_fz(0b00000011); // check for specific music
  if (zero_flag) { goto NoIncDAC; }
  inc_abs_nf(DAC_Counter); // increment and check counter
  cpy_imm(0x30);
...
void SetFreq_Squ1(void) {
  ldx_imm_nf(0x0); // set frequency reg offset for square 1 sound channel
...
void Dump_Freq_Regs(void) {
  ...
  if (!zero_flag) {
    ...
    lda_absy_nf(FreqRegLookupTbl); // second byte goes into 3 MSB plus extra bit for
...
void SetFreq_Squ2(void) {
  ldx_imm_fz(0x4); // set frequency reg offset for square 2 sound channel
  // unconditional branch
  if (!zero_flag) {
    Dump_Freq_Regs();
    return;
...
void SetFreq_Tri(void) {
  ldx_imm_fz(0x8); // set frequency reg offset for triangle sound channel
  // unconditional branch
  if (!zero_flag) {
    Dump_Freq_Regs();
    return;
...
void PlayFlagpoleSlide(void) {
  lda_imm_nf(0x40); // store length of flagpole sound
  ram[Squ1_SfxLenCounter] = a;
  lda_imm(0x62); // load part of reg contents for flagpole sound
  SetFreq_Squ1();
  ldx_imm_fz(0x99); // now load the rest
  if (!zero_flag) {
    FPS2nd();
    return;
  }
...
void PlaySmallJump(void) {
  lda_imm_fz(0x26); // branch here for small mario jumping sound
  if (!zero_flag) {
    JumpRegContents();
    return;
  }
...
void PlayBigJump(void) {
  lda_imm_nf(0x18); // branch here for big mario jumping sound
  JumpRegContents(); // <fallthrough>
}

void JumpRegContents(void) {
  ldx_imm_nf(0x82); // note that small and big jump borrow each others' reg contents
  ldy_imm(0xa7); // anyway, this loads the first part of mario's jumping sound
  PlaySqu1Sfx();
  lda_imm_nf(0x28); // store length of sfx for both jumping sounds
  ram[Squ1_SfxLenCounter] = a; // then continue on here
  ContinueSndJump(); // <fallthrough>
}

void ContinueSndJump(void) {
  lda_abs_nf(Squ1_SfxLenCounter); // jumping sounds seem to be composed of three parts
  cmp_imm_fcz(0x25); // check for time to play second part yet
  if (zero_flag) {
    ldx_imm_nf(0x5f); // load second part
    ldy_imm(0xf6);
    // unconditional branch
    if (!zero_flag) {
      DmpJpFPS();
  ...
  ldx_imm_nf(0x48); // load third part
  FPS2nd(); // <fallthrough>
}
...
void PlayFireballThrow(void) {
  lda_imm_nf(0x5);
  ldy_imm_fz(0x99); // load reg contents for fireball throw sound
  // unconditional branch
  if (!zero_flag) {
    Fthrow();
    return;
...
void PlayBump(void) {
  lda_imm_nf(0xa); // load length of sfx and reg contents for bump sound
  ldy_imm_nf(0x93);
  Fthrow(); // <fallthrough>
}

void Fthrow(void) {
  ldx_imm_nf(0x9e); // the fireball sound shares reg contents with the bump sound
  ram[Squ1_SfxLenCounter] = a;
...
void ContinueBumpThrow(void) {
  lda_abs_nf(Squ1_SfxLenCounter); // check for second part of bump sound
  cmp_imm_fcz(0x6);
  if (!zero_flag) {
    DecJpFPS();
    return;
...
PlaySwimStomp:
  lda_imm_nf(0xe); // store length of swim/stomp sound
  ram[Squ1_SfxLenCounter] = a;
  ldy_imm_nf(0x9c); // store reg contents for swim/stomp sound
  ldx_imm_nf(0x9e);
  lda_imm(0x26);
  PlaySqu1Sfx();
  
ContinueSwimStomp:
  ldy_abs_nf(Squ1_SfxLenCounter); // look up reg contents in data section based on
  lda_absy_nf(SwimStompEnvelopeData - 1); // length of sound left, used to control sound's
...
void PlaySmackEnemy(void) {
  lda_imm_nf(0xe); // store length of smack enemy sound
  ldy_imm_nf(0xcb);
  ldx_imm_nf(0x9f);
  ram[Squ1_SfxLenCounter] = a;
...
void ContinueSmackEnemy(void) {
  ldy_abs_nf(Squ1_SfxLenCounter); // check about halfway through
  cpy_imm_fcz(0x8);
  if (!zero_flag) { goto SmSpc; }
  lda_imm_nf(0xa0); // if we're at the about-halfway point, make the second tone
  dynamic_ram_write(SND_SQUARE1_REG + 2, a); // in the smack enemy sound
  lda_imm_fz(0x9f);
  if (!zero_flag) { goto SmTick; }
  
SmSpc:
  lda_imm_nf(0x90); // this creates spaces in the sound, giving it its distinct noise
...
void StopSquare1Sfx(void) {
  ldx_imm_nf(0x0); // if end of sfx reached, clear buffer
...
void PlayPipeDownInj(void) {
  lda_imm_nf(0x2f); // load length of pipedown sound
  ram[Squ1_SfxLenCounter] = a;
  ContinuePipeDownInj(); // <fallthrough>
}

void ContinuePipeDownInj(void) {
  lda_abs_nf(Squ1_SfxLenCounter); // some bitwise logic, forces the regs
  ...
  if (!carry_flag) {
    ...
    if (!carry_flag) {
      and_imm(0b00000010);
      if (!zero_flag) {
        ldy_imm_nf(0x91); // and this is where it actually gets written in
        ldx_imm_nf(0x9a);
        lda_imm(0x44);
        PlaySqu1Sfx();
      }
...
void PlayCoinGrab(void) {
  lda_imm_nf(0x35); // load length of coin grab sound
  ldx_imm_fz(0x8d); // and part of reg contents
  if (!zero_flag) {
    CGrab_TTickRegL();
    return;
  }
...
void PlayTimerTick(void) {
  lda_imm_nf(0x6); // load length of timer tick sound
  ldx_imm_nf(0x98); // and part of reg contents
  CGrab_TTickRegL(); // <fallthrough>
}

void CGrab_TTickRegL(void) {
  ram[Squ2_SfxLenCounter] = a;
  ldy_imm_nf(0x7f); // load the rest of reg contents
...
void ContinueCGrabTTick(void) {
  lda_abs_nf(Squ2_SfxLenCounter); // check for time to play second tone yet
...
void EmptySfx2Buffer(void) {
  ldx_imm_nf(0x0); // initialize square 2's sound effects buffer
  ram[Square2SoundBuffer] = x;
  StopSquare2Sfx(); // <fallthrough>
}

void StopSquare2Sfx(void) {
  ldx_imm_nf(0xd); // stop playing the sfx
  apu_write(SND_MASTERCTRL_REG, x);
  ldx_imm(0xf);
  apu_write(SND_MASTERCTRL_REG, x);
}
...
void PlayBlast(void) {
  lda_imm_nf(0x20); // load length of fireworks/gunfire sound
  ram[Squ2_SfxLenCounter] = a;
  ldy_imm_nf(0x94); // load reg contents of fireworks/gunfire sound
  lda_imm_fz(0x5e);
  if (!zero_flag) {
    SBlasJ();
    return;
...
void ContinueBlast(void) {
  lda_abs_nf(Squ2_SfxLenCounter); // check for time to play second part
  cmp_imm_fcz(0x18);
  if (!zero_flag) {
    DecrementSfx2Length();
    return;
  }
  ldy_imm_nf(0x93); // load second part reg contents then
  lda_imm_fz(0x18);
  SBlasJ(); // <fallthrough>
}
...
void PlayPowerUpGrab(void) {
  lda_imm_nf(0x36); // load length of power-up grab sound
  ram[Squ2_SfxLenCounter] = a;
  ContinuePowerUpGrab(); // <fallthrough>
}

void ContinuePowerUpGrab(void) {
  lda_abs_nf(Squ2_SfxLenCounter); // load frequency reg based on length left over
  lsr_acc_fc(); // divide by 2
  ...
  tay_nf();
  lda_absy_nf(PowerUpGrabFreqData - 1); // use length left over / 2 for frequency offset
  ldx_imm_nf(0x5d); // store reg contents of power-up grab sound
  ldy_imm(0x7f);
...
void ContinueBowserFall(void) {
  lda_abs_nf(Squ2_SfxLenCounter); // check for almost near the end
  cmp_imm_fcz(0x8);
  if (!zero_flag) {
    DecrementSfx2Length();
    return;
  }
  ldy_imm_nf(0xa4); // if so, load the rest of reg contents for bowser defeat sound
  lda_imm_nf(0x5a);
  PBFRegs(); // <fallthrough>
}
...
void PlayExtraLife(void) {
  lda_imm_nf(0x30); // load length of 1-up sound
  ram[Squ2_SfxLenCounter] = a;
  ContinueExtraLife(); // <fallthrough>
}

void ContinueExtraLife(void) {
  lda_abs_nf(Squ2_SfxLenCounter);
  ldx_imm_nf(0x3); // load new tones only every eight frames
...
DivLLoop:
  ...
  tay_nf();
  lda_absy_nf(ExtraLifeFreqData - 1); // load our reg contents
  ldx_imm_nf(0x82);
  ldy_imm(0x7f);
  // unconditional branch
  if (!zero_flag) {
...
void PlayGrowPowerUp(void) {
  lda_imm_fz(0x10); // load length of power-up reveal sound
  if (!zero_flag) {
    GrowItemRegs();
    return;
  }
...
void PlayGrowVine(void) {
  lda_imm_nf(0x20); // load length of vine grow sound
  GrowItemRegs(); // <fallthrough>
}

void GrowItemRegs(void) {
  ram[Squ2_SfxLenCounter] = a;
  lda_imm_nf(0x7f); // load contents of reg for both sounds directly
  dynamic_ram_write(SND_SQUARE2_REG + 1, a);
  lda_imm_nf(0x0); // start secondary counter for both sounds
  ram[Sfx_SecondaryCounter] = a;
  ContinueGrowItems(); // <fallthrough>
}

void ContinueGrowItems(void) {
  inc_abs_nf(Sfx_SecondaryCounter); // increment secondary counter for both sounds
  lda_abs_nf(Sfx_SecondaryCounter); // this sound doesn't decrement the usual counter
  lsr_acc_nf(); // divide by 2 to get the offset
  tay_nf();
  cpy_abs(Squ2_SfxLenCounter); // have we reached the end yet?
  // if so, branch to jump, and stop playing sounds
  if (!zero_flag) {
    lda_imm_nf(0x9d); // load contents of other reg directly
...
void Square2SfxHandler(void) {
  lda_zp_nf(Square2SoundBuffer); // special handling for the 1-up sound to keep it
...
void PlayBowserFall(void) {
  lda_imm_nf(0x38); // load length of bowser defeat sound
  ram[Squ2_SfxLenCounter] = a;
  ldy_imm_nf(0xc4); // load contents of reg for bowser defeat sound
  lda_imm_fz(0x18);
  BlstSJp(); // <fallthrough>
}

void PlayBrickShatter(void) {
  lda_imm_nf(0x20); // load length of brick shatter sound
  ram[Noise_SfxLenCounter] = a;
  ContinueBrickShatter(); // <fallthrough>
}

void ContinueBrickShatter(void) {
  lda_abs_nf(Noise_SfxLenCounter);
  ...
  tay_nf();
  ldx_absy_nf(BrickShatterFreqData); // load reg contents of brick shatter sound
  lda_absy_nf(BrickShatterEnvData);
...
void DecrementSfx3Length(void) {
  ...
  if (zero_flag) {
    lda_imm_nf(0xf0); // if done, stop playing the sfx
...
PlayBowserFlame:
  lda_imm_nf(0x40); // load length of bowser flame sound
  ram[Noise_SfxLenCounter] = a;
...
ContinueBowserFlame:
  ...
  lsr_acc_fc();
  tay_nf();
  ldx_imm_nf(0xf); // load reg contents of bowser flame sound
...
void MusicHandler(void) {
  ...
  lda_abs_nf(EventMusicBuffer); // check both buffers
...
void LoadEventMusic(void) {
  ...
  ldx_imm_fz(0x8); // load offset to be added to length byte of header
...
void LoadAreaMusic(void) {
  ...
  // NoStop1:
  ldy_imm_nf(0x10); // start counter used only by ground level music
  GMLoopB(); // <fallthrough>
}
...
void HandleAreaMusicLoopB(void) {
  ldy_imm_nf(0x0); // clear event music buffer
  ...
  if (zero_flag) {
    inc_abs_nf(GroundMusicHeaderOfs); // increment but only if playing ground level music
    ldy_abs_nf(GroundMusicHeaderOfs); // is it time to loopback ground level music?
    cpy_imm_fcz(0x32);
  ...
  // FindAreaMusicHeader:
  ldy_imm_nf(0x8); // load Y for offset of area music
  ram[MusicOffset_Square2] = y; // residual instruction here
  FindEventMusicHeader(); // <fallthrough>
}

void FindEventMusicHeader(void) {
  iny_nf(); // increment Y pointer based on previously loaded queue contents
...
void LoadHeader(void) {
  lda_absy_nf(MusicHeaderOffsetData); // load offset for header
  tay_nf();
  lda_absy_nf(MusicHeaderData); // now load the header
  ram[NoteLenLookupTblOfs] = a;
  lda_absy_nf(MusicHeaderData + 1);
  ram[MusicDataLow] = a;
  lda_absy_nf(MusicHeaderData + 2);
  ram[MusicDataHigh] = a;
  lda_absy_nf(MusicHeaderData + 3);
  ram[MusicOffset_Triangle] = a;
  lda_absy_nf(MusicHeaderData + 4);
  ram[MusicOffset_Square1] = a;
  lda_absy_nf(MusicHeaderData + 5);
  ram[MusicOffset_Noise] = a;
  ram[NoiseDataLoopbackOfs] = a;
  lda_imm_nf(0x1); // initialize music note counters
  ram[Squ2_NoteLenCounter] = a;
  ram[Squ1_NoteLenCounter] = a;
  ram[Tri_NoteLenCounter] = a;
  ram[Noise_BeatLenCounter] = a;
  lda_imm_nf(0x0); // initialize music data offset for square 2
  ram[MusicOffset_Square2] = a;
  ram[AltRegContentFlag] = a; // initialize alternate control reg data used by square 1
  lda_imm_nf(0xb); // disable triangle channel and reenable it
  apu_write(SND_MASTERCTRL_REG, a);
  lda_imm_nf(0xf);
  apu_write(SND_MASTERCTRL_REG, a);
...
void HandleSquare2Music(void) {
  ...
  ldy_zp_nf(MusicOffset_Square2); // increment square 2 music offset and fetch data
  inc_zp_nf(MusicOffset_Square2);
  lda_indy(MusicData);
...
EndOfMusicData:
  lda_abs_nf(EventMusicBuffer); // check secondary buffer for time running out music
...
NotTRO:
  ...
  lda_zp_nf(AreaMusicBuffer); // check primary buffer for any music except pipe intro
  and_imm(0b01011111);
  if (!zero_flag) { goto MusicLoopBack; } // if any area music except pipe intro, music loops
  lda_imm_nf(0x0); // clear primary and secondary buffers and initialize
...
Squ2LengthHandler:
  ...
  ldy_zp_nf(MusicOffset_Square2); // fetch another byte (MUST NOT BE LENGTH BYTE!)
  inc_zp_nf(MusicOffset_Square2);
  lda_indy_nf(MusicData);
...
SkipFqL1:
  lda_abs_nf(Squ2_NoteLenBuffer); // save length in square 2 note counter
  ram[Squ2_NoteLenCounter] = a;
  
MiscSqu2MusicTasks:
  lda_zp_fz(Square2SoundBuffer); // is there a sound playing on square 2?
  if (!zero_flag) { goto HandleSquare1Music; }
  lda_abs_nf(EventMusicBuffer); // check for death music or d4 set on secondary buffer
...
NoDecEnv1:
  ...
  ldx_imm_nf(0x7f); // death music or d4 set on secondary buffer
...
FetchSqu1MusicData:
  ldy_zp_nf(MusicOffset_Square1); // increment square 1 music offset and fetch data
  inc_zp_nf(MusicOffset_Square1);
  lda_indy(MusicData);
...
DeathMAltReg:
  ...
  lda_imm_nf(0x7f); // load this value if zero, the alternate value
  
DoAltLoad:
  dynamic_ram_write(SND_SQUARE1_REG + 1, a); // if nonzero, and let's move on
  
HandleTriangleMusic:
  lda_zp_nf(MusicOffset_Triangle);
  dec_abs_fz(Tri_NoteLenCounter); // decrement triangle note length
  if (!zero_flag) { goto HandleNoiseMusic; } // is it time for more data?
  ldy_zp_nf(MusicOffset_Triangle); // increment square 1 music offset and fetch data
  inc_zp_nf(MusicOffset_Triangle);
  lda_indy(MusicData);
  ...
  ldy_zp_nf(MusicOffset_Triangle); // fetch another byte
  inc_zp_nf(MusicOffset_Triangle);
  lda_indy(MusicData);
...
TriNoteHandler:
  SetFreq_Tri();
  ldx_abs_nf(Tri_NoteLenBuffer); // save length in triangle note counter
  ram[Tri_NoteLenCounter] = x;
  lda_abs_nf(EventMusicBuffer);
  ...
  lda_zp_nf(AreaMusicBuffer); // check primary buffer for water or castle level music
...
NotDOrD4:
  txa_nf(); // if playing water or castle music or any secondary
  cmp_imm_fc(0x12); // besides death music or d4 set, check length of note
  if (carry_flag) { goto LongN; }
  lda_abs_nf(EventMusicBuffer); // check for win castle music again if not playing a long note
  and_imm_fz(EndOfCastleMusic);
  if (zero_flag) { goto MediN; }
  lda_imm_fz(0xf); // load value $0f if playing the win castle music and playing a short
  if (!zero_flag) { goto LoadTriCtrlReg; } // note, load value $1f if playing water or castle level music or any
  
MediN:
  lda_imm_fz(0x1f); // secondary besides death and d4 except win castle or win castle and playing
  if (!zero_flag) { goto LoadTriCtrlReg; } // a short note, and load value $ff if playing a long note on water, castle
  
LongN:
  lda_imm_nf(0xff); // or any secondary (including win castle) except death and d4
  
LoadTriCtrlReg:
  apu_write(SND_TRIANGLE_REG, a); // save final contents of A into control reg for triangle
  
HandleNoiseMusic:
  lda_zp_nf(AreaMusicBuffer); // check if playing underground or castle music
  and_imm(0b11110011);
...
FetchNoiseBeatData:
  ldy_abs_nf(MusicOffset_Noise); // increment noise beat offset and fetch data
  inc_abs_nf(MusicOffset_Noise);
...
NoiseBeatHandler:
  ...
  lda_imm_nf(0x1c); // short beat data
  ldx_imm_nf(0x3);
  ldy_imm(0x18);
  if (!zero_flag) { goto PlayBeat; }
  
StrongBeat:
  lda_imm_nf(0x1c); // strong beat data
  ldx_imm_nf(0xc);
  ldy_imm(0x18);
  if (!zero_flag) { goto PlayBeat; }
  
LongBeat:
  lda_imm_nf(0x1c); // long beat data
  ldx_imm_nf(0x3);
  ldy_imm(0x58);
  if (!zero_flag) { goto PlayBeat; }
...
void AlternateLengthHandler(void) {
  tax_nf(); // save a copy of original byte into X
  ror_acc_fc(); // save LSB from original byte into carry
  txa_nf(); // reload original byte and rotate three times
...
void ProcessLengthData(void) {
  and_imm_nf(0b00000111); // clear all but the three LSBs
  carry_flag = false;
...
void LoadControlRegs(void) {
  lda_abs_nf(EventMusicBuffer); // check secondary buffer for win castle music
  and_imm_fz(EndOfCastleMusic);
  if (zero_flag) { goto NotECstlM; }
  lda_imm_fz(0x4); // this value is only used for win castle music
...
NotECstlM:
  ...
  lda_imm_fz(0x8); // this is the default value for all other music
  if (!zero_flag) { goto AllMus; }
  
WaterMus:
  lda_imm_nf(0x28); // this value is used for water music and all other event music
  
AllMus:
  ldx_imm_nf(0x82); // load contents of other sound regs for square 2
  ldy_imm(0x7f);
}
...
void LoadEnvelopeData(void) {
  ...
  // LoadUsualEnvData:
  lda_zp_nf(AreaMusicBuffer); // check primary buffer for water music
